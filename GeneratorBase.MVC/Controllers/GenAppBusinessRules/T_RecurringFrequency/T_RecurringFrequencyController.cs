...
{	
    ...
    {
		private void LoadViewDataForCount(T_RecurringFrequency t_recurringfrequency, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_RecurringFrequency t_recurringfrequency)
        {
        }
        private void LoadViewDataBeforeOnEdit(T_RecurringFrequency t_recurringfrequency)
        {
		
	 ViewBag.T_RecurringRepeatFrequencyCount = t_recurringfrequency.t_recurringrepeatfrequency.Count();
        }
        private void LoadViewDataAfterOnCreate(T_RecurringFrequency t_recurringfrequency)
        {
                   }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {
        }
		private IQueryable<T_RecurringFrequency> searchRecords(IQueryable<T_RecurringFrequency> lstT_RecurringFrequency, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_RecurringFrequency = lstT_RecurringFrequency.Where(s => (s.T_Name != null && SqlFunctions.StringConvert((double)s.T_Name).Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_RecurringFrequency = lstT_RecurringFrequency.Where(s => (s.T_Name != null && SqlFunctions.StringConvert((double)s.T_Name).Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_RecurringFrequency;
        }
		private IQueryable<T_RecurringFrequency> sortRecords(IQueryable<T_RecurringFrequency> lstT_RecurringFrequency, string sortBy, string isAsc)
        {
            string methodName = "";
            ...
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_RecurringFrequency), "t_recurringfrequency");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_RecurringFrequency>)lstT_RecurringFrequency.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_RecurringFrequency.ElementType, lambda.Body.Type },
                    lstT_RecurringFrequency.Expression,
                    lambda));
        }
        ...
        public ActionResult SetFSearch(string searchString, string HostingEntity )
        {
		 ...
		 // GET: /T_RecurringFrequency/FSearch/
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport  ,string T_NameFrom,string T_NameTo, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype)//, string HostingEntity
        {
            ...
            {
                lstT_RecurringFrequency  = searchRecords(lstT_RecurringFrequency, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_RecurringFrequency = searchRecords(lstT_RecurringFrequency, search,true);
			}
			...
			lstT_RecurringFrequency = lstT_RecurringFrequency;
            var _T_RecurringFrequency = lstT_RecurringFrequency;
		 if(T_NameFrom!=null || T_NameTo !=null)
         {
                try
                {
                    int from = T_NameFrom == null ? 0 : Convert.ToInt32(T_NameFrom);
                    int to =  T_NameTo == null ? int.MaxValue : Convert.ToInt32(T_NameTo);
                    _T_RecurringFrequency =  _T_RecurringFrequency.Where(o => o.T_Name >= from && o.T_Name <= to);
					ViewBag.SearchResult += "\r\n Repeat Every= "+from+"-"+to;
                }
                catch { }
          }
           ...
           // return View("Index", _T_RecurringFrequency.ToPagedList(pageNumber, pageSize));
		        ...
		        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_RecurringFrequencys.FirstOrDefault(p => p.Id == idvalue);
		 ...
		 public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_RecurringFrequencys.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_RecurringFrequencys.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_RecurringFrequency>(_Obj, "T_RecurringFrequency", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_RecurringFrequency> list = db.T_RecurringFrequencys;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_RecurringFrequencys;
                    Type[] exprArgTypes = { query.ElementType };
                    string propToWhere = AssoNameWithParent;
                    ParameterExpression p = Expression.Parameter(typeof(T_RecurringFrequency), "p");
                    MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                    LambdaExpression lambda = Expression.Lambda<Func<T_RecurringFrequency, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                    MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                    IQueryable q = query.Provider.CreateQuery(methodCall);
                    list = ((IQueryable<T_RecurringFrequency>)q);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_RecurringFrequencys;
            ...
            list = _fad.FilterDropdown<T_RecurringFrequency>(User,list, "T_RecurringFrequency",caller);
		   if (key != null && key.Length > 0)
            {
			    if (ExtraVal != null && ExtraVal.Length > 0)
                {
                    long? val = Convert.ToInt64(ExtraVal);
                    var data = from x in list.Where(p => p.DisplayValue.Contains(key) && p.Id != val).Take(9).Union(list.Where(p=>p.Id == val)).OrderBy(q => q.Id).ToList()
                               select new { Id = x.Id, Name = x.DisplayValue };
                    return Json(data, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    var data = from x in list.Where(p => p.DisplayValue.Contains(key)).Take(10).OrderBy(q => q.Id).ToList()
                               select new { Id = x.Id, Name = x.DisplayValue };
                    return Json(data, JsonRequestBehavior.AllowGet);
                }
            }
            else
            {
               if (ExtraVal != null && ExtraVal.Length > 0)
                {
                    long? val = Convert.ToInt64(ExtraVal);
                     var data = from x in list.Where(p=>p.Id != val).Take(9).Union(list.Where(p=>p.Id == val)).Distinct().OrderBy(q => q.Id).ToList()
                               select new { Id = x.Id, Name = x.DisplayValue };
                    return Json(data, JsonRequestBehavior.AllowGet);
                }                
                else
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_RecurringFrequency>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_RecurringFrequency), "p"));
        ...
        {
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_RecurringFrequency")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_RecurringFrequency").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_RecurringFrequency");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_RecurringFrequency" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Recurring Frequencys";
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_RecurringFrequency";
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_RecurringFrequency model = new T_RecurringFrequency();
                        ...
                        for (int j = 0; j < sheetColumns.Count; j++)
                        {
                            ...
                            {
    case "T_Name":
    model.T_Name = Int32.Parse(columnValue);
	break;
    case "T_Description":
	...
	break;
                ...
                break;
					    ...
					    if (ValidateModel(model) && string.IsNullOrEmpty(CheckBeforeSave(model)))
                        {
                            ...
                            {
								db.T_RecurringFrequencys.Add(model);
								db.SaveChanges();
							}
							else
                            {
		...
		public bool ValidateModel(T_RecurringFrequency validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_RecurringFrequencys.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_RecurringFrequency OModel)
        {
			Dictionary<string, string> RequiredProperties = new Dictionary<string, string>();
			if(User.businessrules != null)
			{
				var BR = User.businessrules.Where(p => p.EntityName == "T_RecurringFrequency").ToList();
				if (BR != null && BR.Count > 0)
				{
					var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_RecurringFrequency");
					...
					if (ResultOfBusinessRules.Keys.Select(p => p.TypeNo).Contains(4))
					{
						...
						{
							var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_RecurringFrequency").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
		...
		[AcceptVerbs(HttpVerbs.Get)]
		public JsonResult GetMandatoryProperties(T_RecurringFrequency OModel,string ruleType)
        {
		Dictionary<string, string> RequiredProperties = new Dictionary<string, string>();
		if(User.businessrules != null)
		{
          var BR = User.businessrules.Where(p => p.EntityName == "T_RecurringFrequency").ToList();
            if (BR != null && BR.Count > 0)
            {
                ...
                var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR,"T_RecurringFrequency");
                BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_RecurringFrequency OModel)
        {
            string RulesApplied = "";
			if(User.businessrules != null)
			{
				...
				if (BR != null && BR.Count > 0)
				{
					var ResultOfBusinessRules = db.LockEntityRule(OModel, BR,"T_RecurringFrequency");
					BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_RecurringFrequency OModel)
        {
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_RecurringFrequencys.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_RecurringFrequency t_recurringfrequency)
        {
            Dictionary<string, string> msgThreshold = new Dictionary<string, string>();
            return Json(msgThreshold, JsonRequestBehavior.AllowGet);
        }
		public bool CheckBeforeDelete(T_RecurringFrequency t_recurringfrequency)
        {
            var result = true;
		...
		public string CheckBeforeSave(T_RecurringFrequency t_recurringfrequency)
        {
            var result = true;
            var AlertMsg = "";
            // Write your logic here
		...
		public void OnDeleting(T_RecurringFrequency t_recurringfrequency)
        {
            // Write your logic here
 
		}
        public void OnSaving(T_RecurringFrequency t_recurringfrequency, ApplicationContext db)
        {
            // Write your logic here
 
        }
		public void AfterSave(T_RecurringFrequency t_recurringfrequency)
        {
            // Write your logic here
        ...
        public ActionResult GetCalculationValues(T_RecurringFrequency t_recurringfrequency)
        {
            t_recurringfrequency.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_recurringfrequency.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList()
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_RecurringFrequency"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_RecurringFrequency t_recurringfrequency, string IgnoreEditable)
        {
