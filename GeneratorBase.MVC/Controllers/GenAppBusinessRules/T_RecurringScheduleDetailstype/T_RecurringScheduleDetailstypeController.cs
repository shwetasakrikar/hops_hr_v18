...
{	
    ...
    {
		private void LoadViewDataForCount(T_RecurringScheduleDetailstype t_recurringscheduledetailstype, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_RecurringScheduleDetailstype t_recurringscheduledetailstype)
        {
        }
        private void LoadViewDataBeforeOnEdit(T_RecurringScheduleDetailstype t_recurringscheduledetailstype)
        {
		
	 ViewBag.T_AssociatedRecurringScheduleDetailsTypeCount = t_recurringscheduledetailstype.t_associatedrecurringscheduledetailstype.Count();
        }
        private void LoadViewDataAfterOnCreate(T_RecurringScheduleDetailstype t_recurringscheduledetailstype)
        {
                   }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {
        }
		private IQueryable<T_RecurringScheduleDetailstype> searchRecords(IQueryable<T_RecurringScheduleDetailstype> lstT_RecurringScheduleDetailstype, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_RecurringScheduleDetailstype = lstT_RecurringScheduleDetailstype.Where(s => (!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_RecurringScheduleDetailstype = lstT_RecurringScheduleDetailstype.Where(s => (!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_RecurringScheduleDetailstype;
        }
		private IQueryable<T_RecurringScheduleDetailstype> sortRecords(IQueryable<T_RecurringScheduleDetailstype> lstT_RecurringScheduleDetailstype, string sortBy, string isAsc)
        {
            string methodName = "";
            ...
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_RecurringScheduleDetailstype), "t_recurringscheduledetailstype");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_RecurringScheduleDetailstype>)lstT_RecurringScheduleDetailstype.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_RecurringScheduleDetailstype.ElementType, lambda.Body.Type },
                    lstT_RecurringScheduleDetailstype.Expression,
                    lambda));
        }
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_RecurringScheduleDetailstypes.FirstOrDefault(p => p.Id == idvalue);
		 ...
		 public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_RecurringScheduleDetailstypes.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_RecurringScheduleDetailstypes.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_RecurringScheduleDetailstype>(_Obj, "T_RecurringScheduleDetailstype", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_RecurringScheduleDetailstype> list = db.T_RecurringScheduleDetailstypes;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_RecurringScheduleDetailstypes;
                    Type[] exprArgTypes = { query.ElementType };
                    string propToWhere = AssoNameWithParent;
                    ParameterExpression p = Expression.Parameter(typeof(T_RecurringScheduleDetailstype), "p");
                    MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                    LambdaExpression lambda = Expression.Lambda<Func<T_RecurringScheduleDetailstype, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                    MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                    IQueryable q = query.Provider.CreateQuery(methodCall);
                    list = ((IQueryable<T_RecurringScheduleDetailstype>)q);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_RecurringScheduleDetailstypes;
            ...
            list = _fad.FilterDropdown<T_RecurringScheduleDetailstype>(User,list, "T_RecurringScheduleDetailstype",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_RecurringScheduleDetailstype>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_RecurringScheduleDetailstype), "p"));
        ...
        {
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_RecurringScheduleDetailstype")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_RecurringScheduleDetailstype").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_RecurringScheduleDetailstype");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        string colDisKey = "";
                        string colListInx = "";
                        typeName = "T_RecurringScheduleDetailstype";
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_RecurringScheduleDetailstype" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new RecurringScheduleDetails Types";
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_RecurringScheduleDetailstype";
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_RecurringScheduleDetailstype model = new T_RecurringScheduleDetailstype();
	...
	break;
                ...
                break;
					    ...
					    if (ValidateModel(model) && string.IsNullOrEmpty(CheckBeforeSave(model)))
                        {
                            ...
                            {
								db.T_RecurringScheduleDetailstypes.Add(model);
								db.SaveChanges();
							}
							else
                            {
		...
		public bool ValidateModel(T_RecurringScheduleDetailstype validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_RecurringScheduleDetailstypes.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_RecurringScheduleDetailstype OModel)
        {
			Dictionary<string, string> RequiredProperties = new Dictionary<string, string>();
			if(User.businessrules != null)
			{
				var BR = User.businessrules.Where(p => p.EntityName == "T_RecurringScheduleDetailstype").ToList();
				if (BR != null && BR.Count > 0)
				{
					var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_RecurringScheduleDetailstype");
					...
					if (ResultOfBusinessRules.Keys.Select(p => p.TypeNo).Contains(4))
					{
						...
						{
							var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_RecurringScheduleDetailstype").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
		...
		[AcceptVerbs(HttpVerbs.Get)]
		public JsonResult GetMandatoryProperties(T_RecurringScheduleDetailstype OModel,string ruleType)
        {
		Dictionary<string, string> RequiredProperties = new Dictionary<string, string>();
		if(User.businessrules != null)
		{
          var BR = User.businessrules.Where(p => p.EntityName == "T_RecurringScheduleDetailstype").ToList();
            if (BR != null && BR.Count > 0)
            {
                ...
                var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR,"T_RecurringScheduleDetailstype");
                BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_RecurringScheduleDetailstype OModel)
        {
            string RulesApplied = "";
			if(User.businessrules != null)
			{
				...
				if (BR != null && BR.Count > 0)
				{
					var ResultOfBusinessRules = db.LockEntityRule(OModel, BR,"T_RecurringScheduleDetailstype");
					BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_RecurringScheduleDetailstype OModel)
        {
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_RecurringScheduleDetailstypes.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_RecurringScheduleDetailstype t_recurringscheduledetailstype)
        {
            Dictionary<string, string> msgThreshold = new Dictionary<string, string>();
            return Json(msgThreshold, JsonRequestBehavior.AllowGet);
        }
		public bool CheckBeforeDelete(T_RecurringScheduleDetailstype t_recurringscheduledetailstype)
        {
            var result = true;
		...
		public string CheckBeforeSave(T_RecurringScheduleDetailstype t_recurringscheduledetailstype)
        {
            var result = true;
            var AlertMsg = "";
            // Write your logic here
		...
		public void OnDeleting(T_RecurringScheduleDetailstype t_recurringscheduledetailstype)
        {
            // Write your logic here
 
		}
        public void OnSaving(T_RecurringScheduleDetailstype t_recurringscheduledetailstype, ApplicationContext db)
        {
            // Write your logic here
 
        }
		public void AfterSave(T_RecurringScheduleDetailstype t_recurringscheduledetailstype)
        {
            // Write your logic here
        ...
        public ActionResult GetCalculationValues(T_RecurringScheduleDetailstype t_recurringscheduledetailstype)
        {
            t_recurringscheduledetailstype.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_recurringscheduledetailstype.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList()
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_RecurringScheduleDetailstype"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_RecurringScheduleDetailstype t_recurringscheduledetailstype, string IgnoreEditable)
        {
