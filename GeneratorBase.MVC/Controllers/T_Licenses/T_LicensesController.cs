...
{	
    ...
    {
		private void LoadViewDataForCount(T_Licenses t_licenses, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_Licenses t_licenses)
        {		
			 ViewBag.T_LicenseRecordsID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_licenses.T_LicenseRecordsID);
			 ViewBag.T_AssociatedLicensesTypeID = new SelectList(db.T_Licensestypes, "ID", "DisplayValue", t_licenses.T_AssociatedLicensesTypeID);
			CustomLoadViewDataListAfterEdit(t_licenses);
        }
        private void LoadViewDataBeforeOnEdit(T_Licenses t_licenses)
        {		
               var _objT_LicenseRecords = new List<T_Employee>();
               _objT_LicenseRecords.Add(t_licenses.t_licenserecords);
			   			   ViewBag.T_LicenseRecordsID = new SelectList(_objT_LicenseRecords, "ID", "DisplayValue", t_licenses.T_LicenseRecordsID);
			               var _objT_AssociatedLicensesType = new List<T_Licensestype>();
               _objT_AssociatedLicensesType.Add(t_licenses.t_associatedlicensestype);
			   			   ViewBag.T_AssociatedLicensesTypeID = new SelectList(_objT_AssociatedLicensesType, "ID", "DisplayValue", t_licenses.T_AssociatedLicensesTypeID);
				 ViewBag.T_LicensesCommentsCount = t_licenses.t_licensescomments.Count();
		CustomLoadViewDataListBeforeEdit(t_licenses);
        }
        private void LoadViewDataAfterOnCreate(T_Licenses t_licenses)
        {			
			 ViewBag.T_LicenseRecordsID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_licenses.T_LicenseRecordsID);
			 ViewBag.T_AssociatedLicensesTypeID = new SelectList(db.T_Licensestypes, "ID", "DisplayValue", t_licenses.T_AssociatedLicensesTypeID);
		CustomLoadViewDataListAfterOnCreate(t_licenses);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Employee" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_LicenseRecords")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_LicenseRecordsID = new SelectList(db.T_Employees.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Employees.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_LicenseRecords = new List<T_Employee>();
			 ViewBag.T_LicenseRecordsID = new SelectList(objT_LicenseRecords , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_Licensestype" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_AssociatedLicensesType")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_AssociatedLicensesTypeID = new SelectList(db.T_Licensestypes.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Licensestypes.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_AssociatedLicensesType = new List<T_Licensestype>();
			 ViewBag.T_AssociatedLicensesTypeID = new SelectList(objT_AssociatedLicensesType , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_Licenses> searchRecords(IQueryable<T_Licenses> lstT_Licenses, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_Licenses = lstT_Licenses.Where(s => (s.t_licenserecords!= null && (s.t_licenserecords.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_NameontheLicense) && s.T_NameontheLicense.ToUpper().Contains(searchString)) ||(s.t_associatedlicensestype!= null && (s.t_associatedlicensestype.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_LicenseNumber) && s.T_LicenseNumber.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Notes) && s.T_Notes.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_Licenses = lstT_Licenses.Where(s => (s.t_licenserecords!= null && (s.t_licenserecords.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_NameontheLicense) && s.T_NameontheLicense.ToUpper().Contains(searchString)) ||(s.t_associatedlicensestype!= null && (s.t_associatedlicensestype.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_LicenseNumber) && s.T_LicenseNumber.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
			try
            {
                var datevalue = Convert.ToDateTime(searchString);
                lstT_Licenses = lstT_Licenses.Union(db.T_Licensess.Where(s => (s.T_InitialLicenseDate == datevalue) ||(s.T_ExpiryDate == datevalue) ||(s.T_PrintDate == datevalue) ));
            }
            catch { }
            return lstT_Licenses;
        }
		private IQueryable<T_Licenses> sortRecords(IQueryable<T_Licenses> lstT_Licenses, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_LicenseRecordsID")
                return isAsc.ToLower() == "asc" ? lstT_Licenses.OrderBy(p => p.t_licenserecords.DisplayValue) : lstT_Licenses.OrderByDescending(p => p.t_licenserecords.DisplayValue);
	 if(sortBy == "T_AssociatedLicensesTypeID")
                return isAsc.ToLower() == "asc" ? lstT_Licenses.OrderBy(p => p.t_associatedlicensestype.DisplayValue) : lstT_Licenses.OrderByDescending(p => p.t_associatedlicensestype.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_Licenses.Sort(sortBy,true) : lstT_Licenses.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_Licenses), "t_licenses");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_Licenses>)lstT_Licenses.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_Licenses.ElementType, lambda.Body.Type },
                    lstT_Licenses.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "FileDocument")
            {
                ...
                url += "?search=";
						var T_LicenseRecords  = _Object.T_EmployeeDocumentsID;
						url += "&t_licenserecords=" + T_LicenseRecords;									
            ...
            if (sourceEntity == "T_ServiceRecord")
            {
                ...
                url += "?search=";
						var T_LicenseRecords  = _Object.T_EmployeeEmploymentProfileID;
						url += "&t_licenserecords=" + T_LicenseRecords;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Comment")
            {
                ...
                url += "?search=";
						var T_LicenseRecords  = _Object.T_EmployeeCommentsID;
						url += "&t_licenserecords=" + T_LicenseRecords;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DrugAlcoholTest")
            {
                ...
                url += "?search=";
						var T_LicenseRecords  = _Object.T_EmployeeDrugAlcoholTestID;
						url += "&t_licenserecords=" + T_LicenseRecords;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_LicenseRecords  = _Object.T_EmployeeAdministratorID;
						url += "&t_licenserecords=" + T_LicenseRecords;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_JobAssignment")
            {
                ...
                url += "?search=";
						var T_LicenseRecords  = _Object.T_EmployeeJobAssignmentID;
						url += "&t_licenserecords=" + T_LicenseRecords;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_LeaveProfile")
            {
                ...
                url += "?search=";
						var T_LicenseRecords  = _Object.T_EmployeeLeaveProfileID;
						url += "&t_licenserecords=" + T_LicenseRecords;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_EmployeeInjury")
            {
                ...
                url += "?search=";
						var T_LicenseRecords  = _Object.T_EmployeeEmployeeInjuryID;
						url += "&t_licenserecords=" + T_LicenseRecords;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_BackgroundCheck")
            {
                ...
                url += "?search=";
						var T_LicenseRecords  = _Object.T_EmployeeCriminalBackgroundCheckID;
						url += "&t_licenserecords=" + T_LicenseRecords;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Education")
            {
                ...
                url += "?search=";
						var T_LicenseRecords  = _Object.T_EmployeeEducationID;
						url += "&t_licenserecords=" + T_LicenseRecords;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Accommodation")
            {
                ...
                url += "?search=";
						var T_LicenseRecords  = _Object.T_EmployeeAccomodationID;
						url += "&t_licenserecords=" + T_LicenseRecords;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_PayDetails")
            {
                ...
                url += "?search=";
						var T_LicenseRecords  = _Object.T_EmployeePayDetailsID;
						url += "&t_licenserecords=" + T_LicenseRecords;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_licenserecords,string t_associatedlicensestype, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_LicenseRecordslist = db.T_Employees.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_licenserecords != null)
            {
                var ids = t_licenserecords.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Employee= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_LicenseRecordslist.Union(db.T_Employees.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
					ViewBag.t_licenserecords = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_LicenseRecordslist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
				ViewBag.t_licenserecords = new SelectList(list, "ID", "DisplayValue");
			}
			var T_AssociatedLicensesTypelist = db.T_Licensestypes.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_associatedlicensestype != null)
            {
                var ids = t_associatedlicensestype.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Licenses Type= ";
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            {
                               ids1.Add(Convert.ToInt64(str));
							   ViewBag.SearchResult += db.T_Licensestypes.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_AssociatedLicensesTypelist.Union(db.T_Licensestypes.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Licensestype>(User, list, "T_Licensestype", null);
					ViewBag.t_associatedlicensestype = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_AssociatedLicensesTypelist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Licensestype>(User, list, "T_Licensestype", null);
				ViewBag.t_associatedlicensestype = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_LicenseRecords = new List<T_Employee>();
		    ViewBag.t_licenserecords = new SelectList(objT_LicenseRecords, "ID", "DisplayValue");
			var objT_AssociatedLicensesType = new List<T_Licensestype>();
		    ViewBag.t_associatedlicensestype = new SelectList(objT_AssociatedLicensesType, "ID", "DisplayValue");
			}
			...
			columns.Add("3", "Name on the License");
			columns.Add("4", "Licenses Type");
			columns.Add("5", "License Number");
			columns.Add("6", "Initial License Date");
			columns.Add("7", "Expiry Date");
			columns.Add("8", "License Print Date");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				return View(new T_Licenses());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_Licenses", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_Licenses", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_Licenses()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_Licenses/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_licenserecords,string t_associatedlicensestype ,string T_InitialLicenseDateFrom,string T_InitialLicenseDateTo,string T_ExpiryDateFrom,string T_ExpiryDateTo,string T_PrintDateFrom,string T_PrintDateTo,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_Licenses  = searchRecords(lstT_Licenses, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_Licenses = searchRecords(lstT_Licenses, search,true);
			}
			...
			lstT_Licenses = lstT_Licenses.Include(t=>t.t_licenserecords).Include(t=>t.t_associatedlicensestype);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_Licenses", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_Licenses = lstT_Licenses.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_Licenses = Sorting.Sort<T_Licenses>(lstT_Licenses, DataOrdering);
            var _T_Licenses = lstT_Licenses;
				if(T_InitialLicenseDateFrom!=null || T_InitialLicenseDateTo !=null)
				{
						try
						{
							DateTime from = T_InitialLicenseDateFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_InitialLicenseDateFrom);
							DateTime to = T_InitialLicenseDateTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_InitialLicenseDateTo).AddTicks(-1).AddDays(1);
							                       _T_Licenses =  _T_Licenses.Where(o =>o.T_InitialLicenseDate!=null && o.T_InitialLicenseDate.Value.CompareTo(from) >= 0 && o.T_InitialLicenseDate.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n Initial License Date= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
				if(T_ExpiryDateFrom!=null || T_ExpiryDateTo !=null)
				{
						try
						{
							DateTime from = T_ExpiryDateFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_ExpiryDateFrom);
							DateTime to = T_ExpiryDateTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_ExpiryDateTo).AddTicks(-1).AddDays(1);
							                       _T_Licenses =  _T_Licenses.Where(o =>o.T_ExpiryDate!=null && o.T_ExpiryDate.Value.CompareTo(from) >= 0 && o.T_ExpiryDate.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n Expiry Date= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
				if(T_PrintDateFrom!=null || T_PrintDateTo !=null)
				{
						try
						{
							DateTime from = T_PrintDateFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_PrintDateFrom);
							DateTime to = T_PrintDateTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_PrintDateTo).AddTicks(-1).AddDays(1);
							                       _T_Licenses =  _T_Licenses.Where(o =>o.T_PrintDate!=null && o.T_PrintDate.Value.CompareTo(from) >= 0 && o.T_PrintDate.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n License Print Date= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
			//if (lstT_Licenses.Where(p => p.t_licenserecords != null).Count() <= 50)
		    //ViewBag.t_licenserecords = new SelectList(lstT_Licenses.Where(p => p.t_licenserecords != null).Select(P => P.t_licenserecords).Distinct(), "ID", "DisplayValue");
            if (t_licenserecords != null)
            {
                var ids = t_licenserecords.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_Licenses = _T_Licenses.Where(p => ids1.Contains(p.T_LicenseRecordsID));
            }
				//if (lstT_Licenses.Where(p => p.t_associatedlicensestype != null).Count() <= 50)
		    //ViewBag.t_associatedlicensestype = new SelectList(lstT_Licenses.Where(p => p.t_associatedlicensestype != null).Select(P => P.t_associatedlicensestype).Distinct(), "ID", "DisplayValue");
            if (t_associatedlicensestype != null)
            {
                var ids = t_associatedlicensestype.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_Licensestypes.Find(Convert.ToInt64(str));
                            ViewBag.SearchResult += obj != null ? obj.DisplayValue + ", " : "";
                        }
                    }
                    //
                ...
                ids1 = ids1.ToList();
                _T_Licenses = _T_Licenses.Where(p => ids1.Contains(p.T_AssociatedLicensesTypeID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Licenses");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_Licenses", tagsSplit.ToArray());
                    }
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_Licensess.GroupBy(p => p.t_licenserecords.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Employee", "Licenses"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Licenses", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_Licensess.GroupBy(p => p.t_associatedlicensestype.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "2" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Licenses Type (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Licenses Type"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Licenses Type", "Licenses"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Licenses", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_Licensess.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_licenses = jedb.T_Licensess.Find(id);
			var T_LicensesCommentsIds = jedb.T_Comments.Where(p=>p.T_LicensesCommentsID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_Comment" && T_LicensesCommentsIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_Licensess.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_Licensess.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_Licenses>(_Obj, "T_Licenses", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_Licenses> list = db.T_Licensess;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_Licensess;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_Licenses), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_Licenses, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_Licenses>)q);
				   list = ((IQueryable<T_Licenses>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_Licensess;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_Licenses>(User,list, "T_Licenses",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_Licenses>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_Licenses), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_Licenses).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_Licenses), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Licenses), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Licenses), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Licenses), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Licenses), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Licenses), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_Licenses), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_Licenses), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_Licenses", User) || !User.CanAdd("T_Licenses"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_Licenses")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_Licenses").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_Licenses");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_Licenses" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Licensess";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_LicenseRecordsID":
										 var t_licenserecordsId = db.T_Employees.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_licenserecordsId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_AssociatedLicensesTypeID":
										 var t_associatedlicensestypeId = db.T_Licensestypes.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_associatedlicensestypeId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                        default:
                                            break;
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_LicenseRecordsID":
										 var strPropertyT_LicenseRecords = lstEntityProp.FirstOrDefault(p => p.Key == "T_LicenseRecordsID").Value;
										 ModelReflector.Property propT_LicenseRecords = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Employee").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_LicenseRecords);
										 //var elementTypeT_LicenseRecords = db.T_Employees.ElementType;
										 //var propertyInfoT_LicenseRecords = elementTypeT_LicenseRecords.GetProperty(propT_LicenseRecords.Name);
										 // var t_licenserecordsId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_LicenseRecords.GetValue(p, null)) == assovalue);
										 var t_licenserecordsId = db.T_Employees.Where(propT_LicenseRecords.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_licenserecordsId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_AssociatedLicensesTypeID":
										 var strPropertyT_AssociatedLicensesType = lstEntityProp.FirstOrDefault(p => p.Key == "T_AssociatedLicensesTypeID").Value;
										 ModelReflector.Property propT_AssociatedLicensesType = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Licensestype").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_AssociatedLicensesType);
										 //var elementTypeT_AssociatedLicensesType = db.T_Licensestypes.ElementType;
										 //var propertyInfoT_AssociatedLicensesType = elementTypeT_AssociatedLicensesType.GetProperty(propT_AssociatedLicensesType.Name);
										 // var t_associatedlicensestypeId = db.T_Licensestypes.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_AssociatedLicensesType.GetValue(p, null)) == assovalue);
										 var t_associatedlicensestypeId = db.T_Licensestypes.Where(propT_AssociatedLicensesType.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_associatedlicensestypeId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_Licenses model = new T_Licenses();
                        ...
                        for (int j = 0; j < sheetColumns.Count; j++)
                        {
                            ...
                            switch (columntable)
                            {
    case "T_NameontheLicense":
    model.T_NameontheLicense = columnValue;
	break;
    case "T_LicenseNumber":
    model.T_LicenseNumber = columnValue;
	break;
    case "T_InitialLicenseDate":
    model.T_InitialLicenseDate = DateTime.Parse(columnValue);
	break;
    case "T_ExpiryDate":
    model.T_ExpiryDate = DateTime.Parse(columnValue);
	break;
    case "T_PrintDate":
    model.T_PrintDate = DateTime.Parse(columnValue);
	break;
    case "T_Notes":
    model.T_Notes = columnValue;
	break;
					 case "T_LicenseRecordsID":
		 dynamic t_licenserecordsId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_LicenseRecords = db.T_Employees.ElementType;
			 //var propertyInfoT_LicenseRecords = elementTypeT_LicenseRecords.GetProperty(propT_LicenseRecords.Name);			 
			 //t_licenserecordsId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_LicenseRecords.GetValue(p, null)) == columnValue);
			 t_licenserecordsId = db.T_Employees.Where(propT_LicenseRecords.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_licenserecordsId = db.T_Employees.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_licenserecordsId != null)
			model.T_LicenseRecordsID = t_licenserecordsId.Id;
		  else
			{
				if ((collection["T_LicenseRecordsID"].ToString() == "true,false"))
				{
					try
					{
				 ...
				 try { objT_Employee.T_FirstName =(columnValue); }
						...
						db.SaveChanges();
						model.T_LicenseRecordsID = objT_Employee.Id;
					}
					catch
					{
						continue;
         ...
         break;
		 case "T_AssociatedLicensesTypeID":
		 dynamic t_associatedlicensestypeId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 var strPropertyT_AssociatedLicensesType = lstEntityProp.FirstOrDefault(p => p.Key == "T_AssociatedLicensesTypeID").Value;
			 ModelReflector.Property propT_AssociatedLicensesType = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Licensestype").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_AssociatedLicensesType);
			 //var elementTypeT_AssociatedLicensesType = db.T_Licensestypes.ElementType;
			 //var propertyInfoT_AssociatedLicensesType = elementTypeT_AssociatedLicensesType.GetProperty(propT_AssociatedLicensesType.Name);			 
			 //t_associatedlicensestypeId = db.T_Licensestypes.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_AssociatedLicensesType.GetValue(p, null)) == columnValue);
			 t_associatedlicensestypeId = db.T_Licensestypes.Where(propT_AssociatedLicensesType.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_associatedlicensestypeId = db.T_Licensestypes.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_associatedlicensestypeId != null)
			model.T_AssociatedLicensesTypeID = t_associatedlicensestypeId.Id;
		  else
			{
				if ((collection["T_AssociatedLicensesTypeID"].ToString() == "true,false"))
				{
					try
					{
						T_Licensestype objT_Licensestype = new T_Licensestype();
						objT_Licensestype.T_Name  = (columnValue);
						db.T_Licensestypes.Add(objT_Licensestype);
						db.SaveChanges();
						model.T_AssociatedLicensesTypeID = objT_Licensestype.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_Licenses validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_Licensess.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_Licenses OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_Licenses").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_Licenses");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_Licenses OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_Licenses");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_Licenses OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_Licenses");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_Licenses OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_Licenses",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_Licenses OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_Licenses");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_Licenses OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Licenses").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_Licensess.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_Licensess;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_Licenses, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_Licenses, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_Licenses>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_Licenses t_licenses)
        {
        ...
        {
            string[][] groupsarr = new string[][] { new string[] {"48903","T_Licenses48903"},new string[] {"48885","Employee Information"},new string[] {"48884","License Information"},new string[] {"48886","Notes"} };
            return groupsarr;
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_Licenses t_licenses)
        {
            t_licenses.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_licenses.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_Licenses t_licenses, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_Licenses t_licenses, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
