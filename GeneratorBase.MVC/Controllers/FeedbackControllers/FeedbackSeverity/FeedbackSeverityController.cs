...
{	
    ...
    {
		private void LoadViewDataAfterOnEdit(FeedbackSeverity feedbackseverity)
        {
        }
        private void LoadViewDataBeforeOnEdit(FeedbackSeverity feedbackseverity)
        {
                   }
        private void LoadViewDataAfterOnCreate(FeedbackSeverity feedbackseverity)
        {
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {
        }
		private IQueryable<FeedbackSeverity> searchRecords(IQueryable<FeedbackSeverity> lstFeedbackSeverity, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstFeedbackSeverity = lstFeedbackSeverity.Where(s => (!String.IsNullOrEmpty(s.Name) && s.Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.Description) && s.Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstFeedbackSeverity = lstFeedbackSeverity.Where(s => (!String.IsNullOrEmpty(s.Name) && s.Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.Description) && s.Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstFeedbackSeverity;
        }
		private IQueryable<FeedbackSeverity> sortRecords(IQueryable<FeedbackSeverity> lstFeedbackSeverity, string sortBy, string isAsc)
        {
            string methodName = "";
            ...
            ParameterExpression paramExpression = Expression.Parameter(typeof(FeedbackSeverity), "feedbackseverity");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<FeedbackSeverity>)lstFeedbackSeverity.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstFeedbackSeverity.ElementType, lambda.Body.Type },
                    lstFeedbackSeverity.Expression,
                    lambda));
        }
        public string GetDisplayValue(string id)
        {
			...
			var _Obj = db1.FeedbackSeveritys.Find(Convert.ToInt64(id));
        ...
        {
            IQueryable<FeedbackSeverity> list = db.FeedbackSeveritys;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.FeedbackSeveritys;
                    Type[] exprArgTypes = { query.ElementType };
                    string propToWhere = AssoNameWithParent;
                    ParameterExpression p = Expression.Parameter(typeof(FeedbackSeverity), "p");
                    MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                    LambdaExpression lambda = Expression.Lambda<Func<FeedbackSeverity, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                    MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                    IQueryable q = query.Provider.CreateQuery(methodCall);
                    list = ((IQueryable<FeedbackSeverity>)q);
                }
            }
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<FeedbackSeverity>(User,list, "FeedbackSeverity",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<FeedbackSeverity>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(FeedbackSeverity), "p"));
        ...
        {
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "FeedbackSeverity")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "FeedbackSeverity").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "FeedbackSeverity");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "FeedbackSeverity" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Feedback Severitys";
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "FeedbackSeverity";
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        FeedbackSeverity model = new FeedbackSeverity();
	...
	break;
                ...
                break;
					    ...
					    if (ValidateModel(model))
                        {
                            ...
                            {
								db.FeedbackSeveritys.Add(model);
								db.SaveChanges();
							}
							else
                            {
		...
		public bool ValidateModel(FeedbackSeverity validate)
        {
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(FeedbackSeverity OModel)
        {
			Dictionary<string, string> RequiredProperties = new Dictionary<string, string>();
			if(User.businessrules != null)
			{
				var BR = User.businessrules.Where(p => p.EntityName == "FeedbackSeverity").ToList();
				if (BR != null && BR.Count > 0)
				{
					var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "FeedbackSeverity");
					...
					if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(4))
					{
						...
						{
							var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "FeedbackSeverity").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(FeedbackSeverity OModel)
        {
		Dictionary<string, string> RequiredProperties = new Dictionary<string, string>();
		if(User.businessrules != null)
		{
          var BR = User.businessrules.Where(p => p.EntityName == "FeedbackSeverity").ToList();
            if (BR != null && BR.Count > 0)
            {
                var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR,"FeedbackSeverity");
                BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(FeedbackSeverity OModel)
        {
            string RulesApplied = "";
			if(User.businessrules != null)
			{
				...
				if (BR != null && BR.Count > 0)
				{
					var ResultOfBusinessRules = db.LockEntityRule(OModel, BR,"FeedbackSeverity");
					BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(FeedbackSeverity OModel)
        {
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.FeedbackSeveritys.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            var obj1 = db.FeedbackSeveritys.Find(Id);
