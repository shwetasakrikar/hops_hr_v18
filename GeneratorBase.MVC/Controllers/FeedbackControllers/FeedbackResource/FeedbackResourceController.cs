...
{	
    ...
    {
		private void LoadViewDataAfterOnEdit(FeedbackResource feedbackresource)
        {
        }
        private void LoadViewDataBeforeOnEdit(FeedbackResource feedbackresource)
        {
                   }
        private void LoadViewDataAfterOnCreate(FeedbackResource feedbackresource)
        {
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {
        }
		private IQueryable<FeedbackResource> searchRecords(IQueryable<FeedbackResource> lstFeedbackResource, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstFeedbackResource = lstFeedbackResource.Where(s => (s.ResourceId != null && SqlFunctions.StringConvert((double)s.ResourceId).Contains(searchString)) ||(!String.IsNullOrEmpty(s.FirstName) && s.FirstName.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.LastName) && s.LastName.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.Email) && s.Email.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.PhoneNo) && s.PhoneNo.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstFeedbackResource = lstFeedbackResource.Where(s => (s.ResourceId != null && SqlFunctions.StringConvert((double)s.ResourceId).Contains(searchString)) ||(!String.IsNullOrEmpty(s.FirstName) && s.FirstName.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.LastName) && s.LastName.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.Email) && s.Email.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.PhoneNo) && s.PhoneNo.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstFeedbackResource;
        }
		private IQueryable<FeedbackResource> sortRecords(IQueryable<FeedbackResource> lstFeedbackResource, string sortBy, string isAsc)
        {
            string methodName = "";
            ...
            ParameterExpression paramExpression = Expression.Parameter(typeof(FeedbackResource), "feedbackresource");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<FeedbackResource>)lstFeedbackResource.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstFeedbackResource.ElementType, lambda.Body.Type },
                    lstFeedbackResource.Expression,
                    lambda));
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity )
        {
		 ...
		 // GET: /FeedbackResource/FSearch/
		 public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport  ,string ResourceIdFrom,string ResourceIdTo)//, string HostingEntity
        {
            ...
            {
                lstFeedbackResource  = searchRecords(lstFeedbackResource, searchString.ToUpper(),true);
            }
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstFeedbackResource = searchRecords(lstFeedbackResource, search,true);
			}
			...
			lstFeedbackResource = lstFeedbackResource;
            var _FeedbackResource = lstFeedbackResource;
		 if(ResourceIdFrom!=null || ResourceIdTo !=null)
         {
                try
                {
                    long from = ResourceIdFrom == null ? 0 : Convert.ToInt64(ResourceIdFrom);
                    long to =  ResourceIdTo == null ? long.MaxValue : Convert.ToInt64(ResourceIdTo);
                    _FeedbackResource =  _FeedbackResource.Where(o => o.ResourceId >= from && o.ResourceId <= to);
					ViewBag.SearchResult += "\r\n Resource Id= "+from+"-"+to;
                }
                catch { }
          }
           ...
           // return View("Index", _FeedbackResource.ToPagedList(pageNumber, pageSize));
        ...
        public string GetDisplayValue(string id)
        {
			...
			var _Obj = db1.FeedbackResources.Find(Convert.ToInt64(id));
        ...
        {
            IQueryable<FeedbackResource> list = db.FeedbackResources;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.FeedbackResources;
                    Type[] exprArgTypes = { query.ElementType };
                    string propToWhere = AssoNameWithParent;
                    ParameterExpression p = Expression.Parameter(typeof(FeedbackResource), "p");
                    MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                    LambdaExpression lambda = Expression.Lambda<Func<FeedbackResource, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                    MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                    IQueryable q = query.Provider.CreateQuery(methodCall);
                    list = ((IQueryable<FeedbackResource>)q);
                }
            }
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<FeedbackResource>(User,list, "FeedbackResource",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<FeedbackResource>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(FeedbackResource), "p"));
        ...
        {
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "FeedbackResource")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "FeedbackResource").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "FeedbackResource");
                    if (entList != null)
                    {
                        foreach (var prop in entList.Properties.Where(p => p.Name != "DisplayValue" && p.Name != "ResourceId"))
                        {
                            long selectedVal = 0;
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "FeedbackResource" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Feedback Resources";
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "FeedbackResource";
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        FeedbackResource model = new FeedbackResource();
	...
	break;
                ...
                break;
					    ...
					    if (ValidateModel(model))
                        {
                            ...
                            {
								db.FeedbackResources.Add(model);
								db.SaveChanges();
							}
							else
                            {
		...
		public bool ValidateModel(FeedbackResource validate)
        {
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(FeedbackResource OModel)
        {
			Dictionary<string, string> RequiredProperties = new Dictionary<string, string>();
			if(User.businessrules != null)
			{
				var BR = User.businessrules.Where(p => p.EntityName == "FeedbackResource").ToList();
				if (BR != null && BR.Count > 0)
				{
					var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "FeedbackResource");
					...
					if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(4))
					{
						...
						{
							var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "FeedbackResource").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(FeedbackResource OModel)
        {
		Dictionary<string, string> RequiredProperties = new Dictionary<string, string>();
		if(User.businessrules != null)
		{
          var BR = User.businessrules.Where(p => p.EntityName == "FeedbackResource").ToList();
            if (BR != null && BR.Count > 0)
            {
                var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR,"FeedbackResource");
                BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(FeedbackResource OModel)
        {
            string RulesApplied = "";
			if(User.businessrules != null)
			{
				...
				if (BR != null && BR.Count > 0)
				{
					var ResultOfBusinessRules = db.LockEntityRule(OModel, BR,"FeedbackResource");
					BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(FeedbackResource OModel)
        {
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.FeedbackResources.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            var obj1 = db.FeedbackResources.Find(Id);
