...
{	
    ...
    {
		private void LoadViewDataForCount(T_FacilityUser t_facilityuser, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_FacilityUser t_facilityuser)
        {		
			 ViewBag.T_UserID = new SelectList(UserContext.Users, "Id", "UserName", t_facilityuser.T_UserID);
			 ViewBag.T_FacilityID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_facilityuser.T_FacilityID);
			CustomLoadViewDataListAfterEdit(t_facilityuser);
        }
        private void LoadViewDataBeforeOnEdit(T_FacilityUser t_facilityuser)
        {		
			 ViewBag.T_UserID = new SelectList(UserContext.Users, "Id", "UserName", t_facilityuser.T_UserID);
               var _objT_Facility = new List<T_Facility>();
               _objT_Facility.Add(t_facilityuser.t_facility);
			   			   ViewBag.T_FacilityID = new SelectList(_objT_Facility, "ID", "DisplayValue", t_facilityuser.T_FacilityID);
					CustomLoadViewDataListBeforeEdit(t_facilityuser);
        }
        private void LoadViewDataAfterOnCreate(T_FacilityUser t_facilityuser)
        {			
			 ViewBag.T_UserID = new SelectList(UserContext.Users, "Id", "UserName", t_facilityuser.T_UserID);
			 ViewBag.T_FacilityID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_facilityuser.T_FacilityID);
		CustomLoadViewDataListAfterOnCreate(t_facilityuser);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_User" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_FacilityUser_User")
			{
			    var hostid = Convert.ToString(HostingEntityID);
				ViewBag.T_UserID = new SelectList(db.T_Users.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Users.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_FacilityUsers.Where(p => p.T_FacilityID == hostid).Select(q => q.T_UserID);
		var AvailableList = db.T_Users.Where(p => !Selectedlist.Contains(p.Id));
                ViewBag.IsTop50 = "none";
				if (AvailableList.Count() > 50)
                    ViewBag.IsTop50 = "block";
				ViewBag.T_UserIDAvailable  = new SelectList(AvailableList.ToList().OrderBy(p => p.DisplayValue).Take(50), "ID", "DisplayValue");
                ViewBag.T_UserIDSelected = new MultiSelectList(db.T_Users.Where(p => Selectedlist.Contains(p.Id)).ToList().OrderBy(p => p.DisplayValue), "ID", "DisplayValue", Selectedlist);
		    }
			if(HostingEntityName == "T_Facility" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_FacilityUser_T_Facility")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_FacilityID = new SelectList(db.T_Facilitys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Facilitys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
								//Manytomany associations
								var T_FacilityUserlist = db.T_Users.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
								ViewBag.t_facilityuser = new SelectList(T_FacilityUserlist, "ID", "DisplayValue");
						//ManytoOne association
						var T_FacilityAddresslist = db.T_Addresss.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_facilityaddress = new SelectList(T_FacilityAddresslist, "ID", "DisplayValue");
				var hostid = Convert.ToString(HostingEntityID);
                var Selectedlist = db.T_FacilityUsers.Where(p => p.T_UserID == hostid).Select(q => q.T_FacilityID);
		var AvailableList = db.T_Facilitys.Where(p => !Selectedlist.Contains(p.Id));
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        AvailableList = _fad.FilterDropdown<T_Facility>(User, AvailableList, "T_Facility", "T_FacilityUser.T_FacilityID");
                ViewBag.IsTop50 = "none";
				if (AvailableList.Count() > 50)
                    ViewBag.IsTop50 = "block";
				ViewBag.T_FacilityIDAvailable  = new SelectList(AvailableList.OrderBy(p => p.DisplayValue).Take(50).ToList(), "ID", "DisplayValue");
				var SelectedListComplete = db.T_Facilitys.Where(p => Selectedlist.Contains(p.Id));
				var SelectedListFiltered = db.T_Facilitys.Where(p => Selectedlist.Contains(p.Id));
				SelectedListFiltered = _fad.FilterDropdown<T_Facility>(User, SelectedListFiltered, "T_Facility", "T_FacilityUser.T_FacilityID");
				ViewBag.SelectedListRemoved= string.Join(",", SelectedListComplete.Except(SelectedListFiltered).Select(p=>p.Id).ToList());
                ViewBag.T_FacilityIDSelected = new MultiSelectList(SelectedListComplete.OrderBy(p => p.DisplayValue).ToList(), "ID", "DisplayValue", Selectedlist);
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_FacilityUser> searchRecords(IQueryable<T_FacilityUser> lstT_FacilityUser, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_FacilityUser = lstT_FacilityUser.Where(s => (s.t_user!= null && (s.t_user.UserName.ToUpper().Contains(searchString) )) ||(s.t_facility!= null && (s.t_facility.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_FacilityUser = lstT_FacilityUser.Where(s => (s.t_user!= null && (s.t_user.UserName.ToUpper().Contains(searchString) )) ||(s.t_facility!= null && (s.t_facility.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_FacilityUser;
        }
		private IQueryable<T_FacilityUser> sortRecords(IQueryable<T_FacilityUser> lstT_FacilityUser, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_UserID")
                return isAsc.ToLower() == "asc" ? lstT_FacilityUser.OrderBy(p => p.t_user.UserName) : lstT_FacilityUser.OrderByDescending(p => p.t_user.UserName);
	 if(sortBy == "T_FacilityID")
                return isAsc.ToLower() == "asc" ? lstT_FacilityUser.OrderBy(p => p.t_facility.DisplayValue) : lstT_FacilityUser.OrderByDescending(p => p.t_facility.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_FacilityUser.Sort(sortBy,true) : lstT_FacilityUser.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_FacilityUser), "t_facilityuser");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_FacilityUser>)lstT_FacilityUser.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_FacilityUser.ElementType, lambda.Body.Type },
                    lstT_FacilityUser.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Department")
            {
                ...
                url += "?search=";
						var T_Facility  = _Object.T_DepartmentFacilityAssociationID;
						url += "&t_facility=" + T_Facility;									
            ...
            if (sourceEntity == "T_Position")
            {
                ...
                url += "?search=";
						var T_Facility  = _Object.T_FacilityAssignedToID;
						url += "&t_facility=" + T_Facility;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Employee")
            {
                ...
                url += "?search=";
						var T_Facility  = _Object.T_EmployeeAtFacilityID;
						url += "&t_facility=" + T_Facility;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DepartmentArea")
            {
                ...
                url += "?search=";
						var T_Facility  = _Object.T_DepartmentAreaEmployeeTypeAssociationID;
						url += "&t_facility=" + T_Facility;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_ClaimType")
            {
                ...
                url += "?search=";
						var T_Facility  = _Object.T_ClaimTypeFacilityAssociationID;
						url += "&t_facility=" + T_Facility;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Restrictions")
            {
                ...
                url += "?search=";
						var T_Facility  = _Object.T_RestrictionsFacilityAssociationID;
						url += "&t_facility=" + T_Facility;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_Facility  = _Object.T_FacilityUnitXID;
						url += "&t_facility=" + T_Facility;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Unit")
            {
                ...
                url += "?search=";
						var T_Facility  = _Object.T_FacilityUnitID;
						url += "&t_facility=" + T_Facility;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_SalaryRange")
            {
                ...
                url += "?search=";
						var T_Facility  = _Object.T_FacilitySalaryRangeAssociationID;
						url += "&t_facility=" + T_Facility;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_FacilityConfiguration")
            {
                ...
                url += "?search=";
						var T_Facility  = _Object.T_TenantConfigurationAssociationID;
						url += "&t_facility=" + T_Facility;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_user,string t_facility, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_Facilitylist = db.T_Facilitys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_facility != null)
            {
                var ids = t_facility.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Facility Users= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_Facilitylist.Union(db.T_Facilitys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
					ViewBag.t_facility = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_Facilitylist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
				ViewBag.t_facility = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_Facility = new List<T_Facility>();
		    ViewBag.t_facility = new SelectList(objT_Facility, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_FacilityUser";
			...
			columns.Add("2", "Facility Users");
			columns.Add("3", "Facility Users");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				return View(new T_FacilityUser());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_FacilityUser", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_FacilityUser", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_FacilityUser()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_FacilityUser/FSearch/
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_user,string t_facility ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_FacilityUser  = searchRecords(lstT_FacilityUser, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_FacilityUser = searchRecords(lstT_FacilityUser, search,true);
			}
			...
			lstT_FacilityUser = lstT_FacilityUser.Include(t=>t.t_user).Include(t=>t.t_facility);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_FacilityUser", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_FacilityUser = lstT_FacilityUser.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_FacilityUser = Sorting.Sort<T_FacilityUser>(lstT_FacilityUser, DataOrdering);
            var _T_FacilityUser = lstT_FacilityUser;
			//if (lstT_FacilityUser.Where(p => p.t_facility != null).Count() <= 50)
		    //ViewBag.t_facility = new SelectList(lstT_FacilityUser.Where(p => p.t_facility != null).Select(P => P.t_facility).Distinct(), "ID", "DisplayValue");
            if (t_facility != null)
            {
                var ids = t_facility.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_FacilityUser = _T_FacilityUser.Where(p => ids1.Contains(p.T_FacilityID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_FacilityUser");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_FacilityUser", tagsSplit.ToArray());
                    }
		...
		var entity = "T_FacilityUser";
           ...
           var entitylist = db.T_FacilityUsers;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_FacilityUsers.GroupBy(p => p.t_facility.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Facility Users (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Facility Users"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Facility Users", "Facility User"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_FacilityUser", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_FacilityUsers.FirstOrDefault(p => p.Id == idvalue);
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_FacilityUsers.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_FacilityUsers.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_FacilityUser>(_Obj, "T_FacilityUser", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_FacilityUser> list = db.T_FacilityUsers;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_FacilityUsers;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_FacilityUser), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_FacilityUser, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_FacilityUser>)q);
				   list = ((IQueryable<T_FacilityUser>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_FacilityUsers;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_FacilityUser>(User,list, "T_FacilityUser",caller);
        ...
        {
			int pagesize = 50;
			if (HostingEntityName == "User")
            {
				var hostid = Convert.ToString(HostingEntityID);
                var Selectedlist = db.T_FacilityUsers.Where(p => p.T_UserID == hostid).Select(q => q.T_FacilityID);
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        var Finallist = _fad.FilterDropdown<T_Facility>(User, db.T_Facilitys.Where(p => !Selectedlist.Contains(p.Id)), "T_Facility", "T_FacilityUser.T_FacilityID").ToList();
                if(!string.IsNullOrEmpty(SearchString))
            ...
            if (HostingEntityName == "T_Facility")
            {
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_FacilityUsers.Where(p => p.T_FacilityID == hostid).Select(q => q.T_UserID);
				 var Finallist = db.T_Users.Where(p => !Selectedlist.Contains(p.Id)).ToList();
                if(!string.IsNullOrEmpty(SearchString))
                    Finallist = Finallist.Where(p=>p.DisplayValue.ToUpper().Contains(SearchString.ToUpper())).OrderBy(p => p.DisplayValue).ToList();
                else
                    Finallist = Finallist.OrderBy(p => p.DisplayValue).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValues1(string SearchString, string HostingEntityName, string HostingEntityID, string AssociatedType, string size , string t_userstatus,string t_facilityuser,string t_facilityaddress  )
        {
			int pagesize = 50;
			if (HostingEntityName == "User")
            {
			...
			if (t_facilityuser != null)
            {
               ...
               Finallist = Finallist.Where(p => ids1.Contains(p.Id)).ToList();
				...
				if (t_facilityaddress != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_FacilityAddressID)).ToList();
                }
				var ListCount = Finallist.Count();
				var Results= false;
				 if (!string.IsNullOrEmpty(size))
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_FacilityUser>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_FacilityUser), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_FacilityUser).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_FacilityUser), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_FacilityUser), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_FacilityUser), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_FacilityUser), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_FacilityUser), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_FacilityUser), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_FacilityUser), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_FacilityUser), "p"));
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_FacilityUser>(User, list, "T_FacilityUser", null);
			if (key != null && key.Length > 0)
            {
			   var data = from x in list.Where(p=>p.DisplayValue.Contains(key)).OrderBy(q=>q.DisplayValue).Take(10).ToList()
                           select new { Id = x.Id, Name = x.DisplayValue };
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_FacilityUser", User) || !User.CanAdd("T_FacilityUser"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_FacilityUser")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_FacilityUser").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_FacilityUser");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_FacilityUser" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Facility Users";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_UserID":
										 var t_userId = UserContext.Users.FirstOrDefault(p => p.UserName == assovalue);
										 if (t_userId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_FacilityID":
										 var t_facilityId = db.T_Facilitys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_facilityId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                        default:
                                            break;
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_FacilityUser";
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            switch (association.AssociationProperty)
                            {
										 ...
										 break;
																		 case "T_FacilityID":
										 var strPropertyT_Facility = lstEntityProp.FirstOrDefault(p => p.Key == "T_FacilityID").Value;
										 ModelReflector.Property propT_Facility = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Facility").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_Facility);
										 //var elementTypeT_Facility = db.T_Facilitys.ElementType;
										 //var propertyInfoT_Facility = elementTypeT_Facility.GetProperty(propT_Facility.Name);
										 // var t_facilityId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_Facility.GetValue(p, null)) == assovalue);
										 var t_facilityId = db.T_Facilitys.Where(propT_Facility.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_facilityId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
        ...
        public ActionResult ImportData(FormCollection collection)
        {
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_FacilityUser model = new T_FacilityUser();
                        ...
                        for (int j = 0; j < sheetColumns.Count; j++)
                        {
                            ...
                            {
					 case "T_UserID":
         var t_userId = UserContext.Users.FirstOrDefault(p => p.UserName == columnValue);
         if (t_userId != null)
			model.T_UserID = t_userId.Id;
         break;
		 case "T_FacilityID":
		 dynamic t_facilityId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_Facility = db.T_Facilitys.ElementType;
			 //var propertyInfoT_Facility = elementTypeT_Facility.GetProperty(propT_Facility.Name);			 
			 //t_facilityId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_Facility.GetValue(p, null)) == columnValue);
			 t_facilityId = db.T_Facilitys.Where(propT_Facility.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_facilityId = db.T_Facilitys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_facilityId != null)
			model.T_FacilityID = t_facilityId.Id;
		  else
			{
				if ((collection["T_FacilityID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_FacilityID = objT_Facility.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_FacilityUser validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_FacilityUsers.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_FacilityUser OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_FacilityUser").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_FacilityUser");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_FacilityUser OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_FacilityUser");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_FacilityUser OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_FacilityUser");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_FacilityUser OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_FacilityUser",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_FacilityUser OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_FacilityUser");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_FacilityUser OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_FacilityUser").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_FacilityUsers.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_FacilityUsers;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_FacilityUser, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_FacilityUser, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_FacilityUser>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_FacilityUser t_facilityuser)
        {
...
{
    var result = false;
    var obj = db.T_FacilityUsers.FirstOrDefault(p => p.T_UserID==UserId );
    if (obj != null)
        result = true;
    return result;
}
...
{
    T_FacilityUser newregistration = new T_FacilityUser();
			 newregistration.T_UserID = userid;
    ApplicationContext dbregistration = new ApplicationContext(new SystemUser());
    //set required properties
    var properties = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_FacilityUser").Properties;
    ...
    dbregistration.T_FacilityUsers.Add(newregistration);
    dbregistration.SaveChanges();
    return RedirectToAction("Edit", new { id = newregistration.Id });
}
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_FacilityUser t_facilityuser)
        {
            t_facilityuser.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_facilityuser.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_FacilityUser"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_FacilityUser t_facilityuser, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_FacilityUser t_facilityuser, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
	   [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetDisplayValueByUserName()
        {
            ...
            var userid = usercontext.Users.FirstOrDefault(p => p.UserName == User.Name).Id;
			 IQueryable<T_FacilityUser> list = db.T_FacilityUsers;
            var data = (from x in list.Where(p => p.T_UserID == userid).ToList()
