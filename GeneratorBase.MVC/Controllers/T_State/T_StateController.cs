...
{	
    public partial class T_StateController : BaseController
    {
        ...
        {		
			...
			CustomLoadViewDataListAfterEdit(t_state);
        }
	 ...
	 ViewBag.T_AddressStateCount = t_state.t_addressstate.Count();
		CustomLoadViewDataListBeforeEdit(t_state);
        }
        private void LoadViewDataAfterOnCreate(T_State t_state)
        {			
			 ViewBag.T_StateCountryID = new SelectList(db.T_Countrys, "ID", "DisplayValue", t_state.T_StateCountryID);
		CustomLoadViewDataListAfterOnCreate(t_state);
        }
	 ...
	 if(sortBy == "T_StateCountryID")
		    ...
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_State.Sort(sortBy,true) : lstT_State.Sort(sortBy,false);
		...
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Address")
            {
                ...
                url += "?search=";
						...
						url += "&t_statecountry=" + T_StateCountry;									
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_State()).m_Timezone)).Date);
						}
						else
						{
		...
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_statecountry ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_State = Sorting.Sort<T_State>(lstT_State, DataOrdering);
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_State", tagsSplit.ToArray());
                    }
		...
		var t_state = jedb.T_States.Find(id);
			var T_CityStateIds = jedb.T_Citys.Where(p=>p.T_CityStateID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_City" && T_CityStateIds.Contains(d.RecordId) && d.Type != "View"));
			var T_AddressStateIds = jedb.T_Addresss.Where(p=>p.T_AddressStateID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_Address" && T_AddressStateIds.Contains(d.RecordId) && d.Type != "View"));
...
[AcceptVerbs(HttpVerbs.Get)]
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_State", User) || !User.CanAdd("T_State"))
            {
	...
	break;
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_State OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_State");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        public JsonResult GetValidateBeforeSaveProperties(T_State OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_State",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
