...
{	
    ...
    {
		private void LoadViewDataForCount(T_TypeOfRestrictions t_typeofrestrictions, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_TypeOfRestrictions t_typeofrestrictions)
        {		
			 ViewBag.T_RestrictionsID = new SelectList(db.T_Restrictionss, "ID", "DisplayValue", t_typeofrestrictions.T_RestrictionsID);
			 ViewBag.T_EmployeeInjuryID = new SelectList(db.T_EmployeeInjurys, "ID", "DisplayValue", t_typeofrestrictions.T_EmployeeInjuryID);
			CustomLoadViewDataListAfterEdit(t_typeofrestrictions);
        }
        private void LoadViewDataBeforeOnEdit(T_TypeOfRestrictions t_typeofrestrictions)
        {		
               var _objT_Restrictions = new List<T_Restrictions>();
               _objT_Restrictions.Add(t_typeofrestrictions.t_restrictions);
			   			   ViewBag.T_RestrictionsID = new SelectList(_objT_Restrictions, "ID", "DisplayValue", t_typeofrestrictions.T_RestrictionsID);
			               var _objT_EmployeeInjury = new List<T_EmployeeInjury>();
               _objT_EmployeeInjury.Add(t_typeofrestrictions.t_employeeinjury);
			   			   ViewBag.T_EmployeeInjuryID = new SelectList(_objT_EmployeeInjury, "ID", "DisplayValue", t_typeofrestrictions.T_EmployeeInjuryID);
					CustomLoadViewDataListBeforeEdit(t_typeofrestrictions);
        }
        private void LoadViewDataAfterOnCreate(T_TypeOfRestrictions t_typeofrestrictions)
        {			
			 ViewBag.T_RestrictionsID = new SelectList(db.T_Restrictionss, "ID", "DisplayValue", t_typeofrestrictions.T_RestrictionsID);
			 ViewBag.T_EmployeeInjuryID = new SelectList(db.T_EmployeeInjurys, "ID", "DisplayValue", t_typeofrestrictions.T_EmployeeInjuryID);
		CustomLoadViewDataListAfterOnCreate(t_typeofrestrictions);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Restrictions" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_TypeOfRestrictions_T_Restrictions")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_RestrictionsID = new SelectList(db.T_Restrictionss.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Restrictionss.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
								//Manytomany associations
								var T_TypeOfRestrictionslist = db.T_EmployeeInjurys.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
								ViewBag.t_typeofrestrictions = new SelectList(T_TypeOfRestrictionslist, "ID", "DisplayValue");
						//ManytoOne association
						var T_RestrictionsFacilityAssociationlist = db.T_Facilitys.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_restrictionsfacilityassociation = new SelectList(T_RestrictionsFacilityAssociationlist, "ID", "DisplayValue");
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_TypeOfRestrictionss.Where(p => p.T_EmployeeInjuryID == hostid).Select(q => q.T_RestrictionsID);
		var AvailableList = db.T_Restrictionss.Where(p => !Selectedlist.Contains(p.Id));
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        AvailableList = _fad.FilterDropdown<T_Restrictions>(User, AvailableList, "T_Restrictions", "T_TypeOfRestrictions.T_RestrictionsID");
                ViewBag.IsTop50 = "none";
				if (AvailableList.Count() > 50)
                    ViewBag.IsTop50 = "block";
				ViewBag.T_RestrictionsIDAvailable  = new SelectList(AvailableList.OrderBy(p => p.DisplayValue).Take(50).ToList(), "ID", "DisplayValue");
				var SelectedListComplete = db.T_Restrictionss.Where(p => Selectedlist.Contains(p.Id));
				var SelectedListFiltered = db.T_Restrictionss.Where(p => Selectedlist.Contains(p.Id));
				SelectedListFiltered = _fad.FilterDropdown<T_Restrictions>(User, SelectedListFiltered, "T_Restrictions", "T_TypeOfRestrictions.T_RestrictionsID");
				ViewBag.SelectedListRemoved= string.Join(",", SelectedListComplete.Except(SelectedListFiltered).Select(p=>p.Id).ToList());
                ViewBag.T_RestrictionsIDSelected = new MultiSelectList(SelectedListComplete.OrderBy(p => p.DisplayValue).ToList(), "ID", "DisplayValue", Selectedlist);
		    }
			if(HostingEntityName == "T_EmployeeInjury" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_TypeOfRestrictions_T_EmployeeInjury")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_EmployeeInjuryID = new SelectList(db.T_EmployeeInjurys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_EmployeeInjurys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
						//ManytoOne association
						var T_EmployeeEmployeeInjurylist = db.T_Employees.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_employeeemployeeinjury = new SelectList(T_EmployeeEmployeeInjurylist, "ID", "DisplayValue");
						//ManytoOne association
						var T_TypeofClaimMCIlist = db.T_ClaimTypeMCIs.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_typeofclaimmci = new SelectList(T_TypeofClaimMCIlist, "ID", "DisplayValue");
						//ManytoOne association
						var T_AccidentShiftlist = db.T_ShiftTimes.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_accidentshift = new SelectList(T_AccidentShiftlist, "ID", "DisplayValue");
						//ManytoOne association
						var T_FacilityAccidentOccuredlist = db.T_AllFacilitiess.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_facilityaccidentoccured = new SelectList(T_FacilityAccidentOccuredlist, "ID", "DisplayValue");
						//ManytoOne association
						var T_UnitWhereAccidentOccuredlist = db.T_AllFacilitiesUnits.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_unitwhereaccidentoccured = new SelectList(T_UnitWhereAccidentOccuredlist, "ID", "DisplayValue");
						//ManytoOne association
						var T_EmployeInjuryFloorlist = db.T_AllFacilitiesFloors.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_employeinjuryfloor = new SelectList(T_EmployeInjuryFloorlist, "ID", "DisplayValue");
						//ManytoOne association
						var T_LocationOfAccidentlist = db.T_WCAccidents.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_locationofaccident = new SelectList(T_LocationOfAccidentlist, "ID", "DisplayValue");
						//ManytoOne association
						var T_EmployeeInjuryCauselist = db.T_InjuryCauses.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_employeeinjurycause = new SelectList(T_EmployeeInjuryCauselist, "ID", "DisplayValue");
						//ManytoOne association
						var T_EmployeeInjuryNaturelist = db.T_InjuryNatures.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_employeeinjurynature = new SelectList(T_EmployeeInjuryNaturelist, "ID", "DisplayValue");
						//ManytoOne association
						var T_EmployeeInjuryBodyPartsAffectedlist = db.T_BodyPartsAffecteds.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_employeeinjurybodypartsaffected = new SelectList(T_EmployeeInjuryBodyPartsAffectedlist, "ID", "DisplayValue");
						//ManytoOne association
						var T_EMployeeInjuryMachineToollist = db.T_MachineTools.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_employeeinjurymachinetool = new SelectList(T_EMployeeInjuryMachineToollist, "ID", "DisplayValue");
						//ManytoOne association
						var T_InitialTreatmentListlist = db.T_InitialTreatments.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_initialtreatmentlist = new SelectList(T_InitialTreatmentListlist, "ID", "DisplayValue");
						//ManytoOne association
						var T_EmployeeInjuryMedicalFacilityForTreatmentlist = db.T_MedicalFacilityForTreatments.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_employeeinjurymedicalfacilityfortreatment = new SelectList(T_EmployeeInjuryMedicalFacilityForTreatmentlist, "ID", "DisplayValue");
						//ManytoOne association
						var T_EmployeeInjuryRefusallist = db.T_Refusals.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_employeeinjuryrefusal = new SelectList(T_EmployeeInjuryRefusallist, "ID", "DisplayValue");
								//Manytomany associations
								var T_TypeofClaimlist = db.T_ClaimTypes.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
								ViewBag.t_typeofclaim = new SelectList(T_TypeofClaimlist, "ID", "DisplayValue");
								//Manytomany associations
								var T_TypeOfRestrictionslist = db.T_Restrictionss.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
								ViewBag.t_typeofrestrictions = new SelectList(T_TypeOfRestrictionslist, "ID", "DisplayValue");
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_TypeOfRestrictionss.Where(p => p.T_RestrictionsID == hostid).Select(q => q.T_EmployeeInjuryID);
		var AvailableList = db.T_EmployeeInjurys.Where(p => !Selectedlist.Contains(p.Id));
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        AvailableList = _fad.FilterDropdown<T_EmployeeInjury>(User, AvailableList, "T_EmployeeInjury", "T_TypeOfRestrictions.T_EmployeeInjuryID");
                ViewBag.IsTop50 = "none";
				if (AvailableList.Count() > 50)
                    ViewBag.IsTop50 = "block";
				ViewBag.T_EmployeeInjuryIDAvailable  = new SelectList(AvailableList.OrderBy(p => p.DisplayValue).Take(50).ToList(), "ID", "DisplayValue");
				var SelectedListComplete = db.T_EmployeeInjurys.Where(p => Selectedlist.Contains(p.Id));
				var SelectedListFiltered = db.T_EmployeeInjurys.Where(p => Selectedlist.Contains(p.Id));
				SelectedListFiltered = _fad.FilterDropdown<T_EmployeeInjury>(User, SelectedListFiltered, "T_EmployeeInjury", "T_TypeOfRestrictions.T_EmployeeInjuryID");
				ViewBag.SelectedListRemoved= string.Join(",", SelectedListComplete.Except(SelectedListFiltered).Select(p=>p.Id).ToList());
                ViewBag.T_EmployeeInjuryIDSelected = new MultiSelectList(SelectedListComplete.OrderBy(p => p.DisplayValue).ToList(), "ID", "DisplayValue", Selectedlist);
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_TypeOfRestrictions> searchRecords(IQueryable<T_TypeOfRestrictions> lstT_TypeOfRestrictions, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_TypeOfRestrictions = lstT_TypeOfRestrictions.Where(s => (s.t_restrictions!= null && (s.t_restrictions.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_employeeinjury!= null && (s.t_employeeinjury.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_TypeOfRestrictions = lstT_TypeOfRestrictions.Where(s => (s.t_restrictions!= null && (s.t_restrictions.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_employeeinjury!= null && (s.t_employeeinjury.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_TypeOfRestrictions;
        }
		private IQueryable<T_TypeOfRestrictions> sortRecords(IQueryable<T_TypeOfRestrictions> lstT_TypeOfRestrictions, string sortBy, string isAsc)
        {
            string methodName = "";
		    ...
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_TypeOfRestrictions.Sort(sortBy,true) : lstT_TypeOfRestrictions.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_TypeOfRestrictions), "t_typeofrestrictions");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_TypeOfRestrictions>)lstT_TypeOfRestrictions.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_TypeOfRestrictions.ElementType, lambda.Body.Type },
                    lstT_TypeOfRestrictions.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Comment")
            {
                ...
                url += "?search=";
						var T_EmployeeInjury  = _Object.T_InjuryCommentsID;
						url += "&t_employeeinjury=" + T_EmployeeInjury;									
            ...
            if (sourceEntity == "T_LeaveProfile")
            {
                ...
                url += "?search=";
						var T_EmployeeInjury  = _Object.T_InjuryRelatedLeaveID;
						url += "&t_employeeinjury=" + T_EmployeeInjury;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Accommodation")
            {
                ...
                url += "?search=";
						var T_EmployeeInjury  = _Object.T_AccommodationInjuryID;
						url += "&t_employeeinjury=" + T_EmployeeInjury;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_restrictions,string t_employeeinjury, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_Restrictionslist = db.T_Restrictionss.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_restrictions != null)
            {
                var ids = t_restrictions.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Type Of Restrictions= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_Restrictionslist.Union(db.T_Restrictionss.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Restrictions>(User, list, "T_Restrictions", null);
					ViewBag.t_restrictions = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_Restrictionslist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Restrictions>(User, list, "T_Restrictions", null);
				ViewBag.t_restrictions = new SelectList(list, "ID", "DisplayValue");
			}
			var T_EmployeeInjurylist = db.T_EmployeeInjurys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_employeeinjury != null)
            {
                var ids = t_employeeinjury.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Employees Injuries= ";
				   foreach (var str in ids)
                    {
					...
					ids1 = ids1.ToList();
					var list = T_EmployeeInjurylist.Union(db.T_EmployeeInjurys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_EmployeeInjury>(User, list, "T_EmployeeInjury", null);
					ViewBag.t_employeeinjury = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_EmployeeInjurylist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_EmployeeInjury>(User, list, "T_EmployeeInjury", null);
				ViewBag.t_employeeinjury = new SelectList(list, "ID", "DisplayValue");
			}
		    ...
		    ViewBag.t_restrictions = new SelectList(objT_Restrictions, "ID", "DisplayValue");
			var objT_EmployeeInjury = new List<T_EmployeeInjury>();
		    ViewBag.t_employeeinjury = new SelectList(objT_EmployeeInjury, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_TypeOfRestrictions";
               ...
               = ViewBag.SuggestedPropertyValue = ViewBag.AssociationPropertyList = ViewBag.SuggestedDynamicValueInCondition71 = new SelectList(Dumplist, "key", "value");
				...
				return View(new T_TypeOfRestrictions());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_TypeOfRestrictions", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_TypeOfRestrictions", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_TypeOfRestrictions()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_TypeOfRestrictions/FSearch/
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_restrictions,string t_employeeinjury ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_TypeOfRestrictions  = searchRecords(lstT_TypeOfRestrictions, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_TypeOfRestrictions = searchRecords(lstT_TypeOfRestrictions, search,true);
			}
			...
			lstT_TypeOfRestrictions = lstT_TypeOfRestrictions.Include(t=>t.t_restrictions).Include(t=>t.t_employeeinjury);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_TypeOfRestrictions", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_TypeOfRestrictions = lstT_TypeOfRestrictions.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_TypeOfRestrictions = Sorting.Sort<T_TypeOfRestrictions>(lstT_TypeOfRestrictions, DataOrdering);
            var _T_TypeOfRestrictions = lstT_TypeOfRestrictions;
			//if (lstT_TypeOfRestrictions.Where(p => p.t_restrictions != null).Count() <= 50)
		    //ViewBag.t_restrictions = new SelectList(lstT_TypeOfRestrictions.Where(p => p.t_restrictions != null).Select(P => P.t_restrictions).Distinct(), "ID", "DisplayValue");
            if (t_restrictions != null)
            {
                var ids = t_restrictions.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_Restrictionss.Find(Convert.ToInt64(str));
                ...
                ids1 = ids1.ToList();
                _T_TypeOfRestrictions = _T_TypeOfRestrictions.Where(p => ids1.Contains(p.T_RestrictionsID));
            }
				//if (lstT_TypeOfRestrictions.Where(p => p.t_employeeinjury != null).Count() <= 50)
		    //ViewBag.t_employeeinjury = new SelectList(lstT_TypeOfRestrictions.Where(p => p.t_employeeinjury != null).Select(P => P.t_employeeinjury).Distinct(), "ID", "DisplayValue");
            if (t_employeeinjury != null)
            {
                var ids = t_employeeinjury.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_TypeOfRestrictions = _T_TypeOfRestrictions.Where(p => ids1.Contains(p.T_EmployeeInjuryID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_TypeOfRestrictions");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_TypeOfRestrictions", tagsSplit.ToArray());
                    }
        ...
        {
		string result = "";
		var entity = "T_TypeOfRestrictions";
           ...
           var entitylist = db.T_TypeOfRestrictionss;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_TypeOfRestrictionss.GroupBy(p => p.t_restrictions.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Type Of Restrictions (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Type Of Restrictions"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Type Of Restrictions", "Type Of Restrictions"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_TypeOfRestrictions", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_TypeOfRestrictionss.GroupBy(p => p.t_employeeinjury.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "2" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Employees Injuries (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Employees Injuries"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Employees Injuries", "Type Of Restrictions"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_TypeOfRestrictions", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_TypeOfRestrictionss.FirstOrDefault(p => p.Id == idvalue);
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_TypeOfRestrictionss.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_TypeOfRestrictionss.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_TypeOfRestrictions>(_Obj, "T_TypeOfRestrictions", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_TypeOfRestrictions> list = db.T_TypeOfRestrictionss;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_TypeOfRestrictionss;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_TypeOfRestrictions), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_TypeOfRestrictions, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_TypeOfRestrictions>)q);
				   list = ((IQueryable<T_TypeOfRestrictions>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_TypeOfRestrictionss;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_TypeOfRestrictions>(User,list, "T_TypeOfRestrictions",caller);
        ...
        {
			int pagesize = 50;
			if (HostingEntityName == "T_Restrictions")
            {
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_TypeOfRestrictionss.Where(p => p.T_RestrictionsID == hostid).Select(q => q.T_EmployeeInjuryID);
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        var Finallist = _fad.FilterDropdown<T_EmployeeInjury>(User, db.T_EmployeeInjurys.Where(p => !Selectedlist.Contains(p.Id)), "T_EmployeeInjury", "T_TypeOfRestrictions.T_EmployeeInjuryID").ToList();
                if(!string.IsNullOrEmpty(SearchString))
            ...
            if (HostingEntityName == "T_EmployeeInjury")
            {
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_TypeOfRestrictionss.Where(p => p.T_EmployeeInjuryID == hostid).Select(q => q.T_RestrictionsID);
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
         var Finallist = _fad.FilterDropdown<T_Restrictions>(User, db.T_Restrictionss.Where(p => !Selectedlist.Contains(p.Id)), "T_Restrictions", "T_TypeOfRestrictions.T_RestrictionsID").ToList();
                if(!string.IsNullOrEmpty(SearchString))
                    Finallist = Finallist.Where(p=>p.DisplayValue.ToUpper().Contains(SearchString.ToUpper())).OrderBy(p => p.DisplayValue).ToList();
                else
                    Finallist = Finallist.OrderBy(p => p.DisplayValue).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValues1(string SearchString, string HostingEntityName, string HostingEntityID, string AssociatedType, string size , string t_restrictionsfacilityassociation,string t_typeofrestrictions,string t_typeofclaim,string t_employeeemployeeinjury,string t_typeofclaimmci,string t_accidentshift,string t_facilityaccidentoccured,string t_unitwhereaccidentoccured,string t_employeinjuryfloor,string t_locationofaccident,string t_employeeinjurycause,string t_employeeinjurynature,string t_employeeinjurybodypartsaffected,string t_employeeinjurymachinetool,string t_initialtreatmentlist,string t_employeeinjurymedicalfacilityfortreatment,string t_employeeinjuryrefusal,string T_OSHA  )
        {
			int pagesize = 50;
			if (HostingEntityName == "T_Restrictions")
            {
        ...
        var Finallist = _fad.FilterDropdown<T_EmployeeInjury>(User, db.T_EmployeeInjurys.Where(p => !Selectedlist.Contains(p.Id)), "T_EmployeeInjury", "T_TypeOfRestrictions.T_EmployeeInjuryID").ToList();
			if(T_OSHA!=null)
            {
                try
                {
                    ...
                    Finallist =  Finallist.Where(o => o.T_OSHA == boolvalue).ToList();
                }
                catch { }
            }
				if(!string.IsNullOrEmpty(SearchString))
...
//ManyToMany search in m.m
			...
			if (t_typeofrestrictions != null)
            {
               ...
               Finallist = Finallist.Where(p => ids1.Contains(p.Id)).ToList();
				...
				if (t_employeeemployeeinjury != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_EmployeeEmployeeInjuryID)).ToList();
                }
				if (t_typeofclaimmci != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_TypeofClaimMCIID)).ToList();
                }
				if (t_accidentshift != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_AccidentShiftID)).ToList();
                }
				if (t_facilityaccidentoccured != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_FacilityAccidentOccuredID)).ToList();
                }
				if (t_unitwhereaccidentoccured != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_UnitWhereAccidentOccuredID)).ToList();
                }
				if (t_employeinjuryfloor != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_EmployeInjuryFloorID)).ToList();
                }
				if (t_locationofaccident != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_LocationOfAccidentID)).ToList();
                }
				if (t_employeeinjurycause != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_EmployeeInjuryCauseID)).ToList();
                }
				if (t_employeeinjurynature != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_EmployeeInjuryNatureID)).ToList();
                }
				if (t_employeeinjurybodypartsaffected != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_EmployeeInjuryBodyPartsAffectedID)).ToList();
                }
				if (t_employeeinjurymachinetool != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_EMployeeInjuryMachineToolID)).ToList();
                }
				if (t_initialtreatmentlist != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_InitialTreatmentListID)).ToList();
                }
				if (t_employeeinjurymedicalfacilityfortreatment != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_EmployeeInjuryMedicalFacilityForTreatmentID)).ToList();
                }
				if (t_employeeinjuryrefusal != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_EmployeeInjuryRefusalID)).ToList();
                }
				var ListCount = Finallist.Count();
				var Results= false;
				 if (!string.IsNullOrEmpty(size))
        ...
        var Finallist = _fad.FilterDropdown<T_Restrictions>(User, db.T_Restrictionss.Where(p => !Selectedlist.Contains(p.Id)), "T_Restrictions", "T_TypeOfRestrictions.T_RestrictionsID").ToList();
                if(!string.IsNullOrEmpty(SearchString))
                    ...
                    Finallist = Finallist.OrderBy(p => p.DisplayValue).ToList();
				if (t_restrictionsfacilityassociation != null)
                {
                    var ids = t_restrictionsfacilityassociation.Split(",".ToCharArray());
                    List<long?> ids1 = new List<long?>();
                    foreach (var str in ids)
                    {
                        //Null Search
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_RestrictionsFacilityAssociationID)).ToList();
                }
			...
			if (t_typeofrestrictions != null)
            {
   			 ...
   			 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                        var idvalue= Convert.ToInt64(str);
                        ids1.AddRange(db.T_TypeOfRestrictionss.Where(p=>p.T_EmployeeInjuryID ==idvalue).Select(p=>p.T_RestrictionsID));
                        }
                    }
                    //
                }
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_TypeOfRestrictions>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_TypeOfRestrictions), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_TypeOfRestrictions).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_TypeOfRestrictions), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TypeOfRestrictions), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TypeOfRestrictions), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TypeOfRestrictions), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TypeOfRestrictions), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TypeOfRestrictions), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_TypeOfRestrictions), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_TypeOfRestrictions), "p"));
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_TypeOfRestrictions>(User, list, "T_TypeOfRestrictions", null);
			if (key != null && key.Length > 0)
            {
			   var data = from x in list.Where(p=>p.DisplayValue.Contains(key)).OrderBy(q=>q.DisplayValue).Take(10).ToList()
                           select new { Id = x.Id, Name = x.DisplayValue };
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_TypeOfRestrictions", User) || !User.CanAdd("T_TypeOfRestrictions"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_TypeOfRestrictions")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_TypeOfRestrictions").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_TypeOfRestrictions");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_TypeOfRestrictions" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Type Of Restrictionss";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_RestrictionsID":
										 var t_restrictionsId = db.T_Restrictionss.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_restrictionsId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_EmployeeInjuryID":
										 var t_employeeinjuryId = db.T_EmployeeInjurys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_employeeinjuryId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                        default:
                                            break;
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_TypeOfRestrictions";
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_RestrictionsID":
										 var strPropertyT_Restrictions = lstEntityProp.FirstOrDefault(p => p.Key == "T_RestrictionsID").Value;
										 ModelReflector.Property propT_Restrictions = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Restrictions").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_Restrictions);
										 //var elementTypeT_Restrictions = db.T_Restrictionss.ElementType;
										 //var propertyInfoT_Restrictions = elementTypeT_Restrictions.GetProperty(propT_Restrictions.Name);
										 // var t_restrictionsId = db.T_Restrictionss.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_Restrictions.GetValue(p, null)) == assovalue);
										 var t_restrictionsId = db.T_Restrictionss.Where(propT_Restrictions.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_restrictionsId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_EmployeeInjuryID":
										 var strPropertyT_EmployeeInjury = lstEntityProp.FirstOrDefault(p => p.Key == "T_EmployeeInjuryID").Value;
										 ModelReflector.Property propT_EmployeeInjury = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_EmployeeInjury").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_EmployeeInjury);
										 //var elementTypeT_EmployeeInjury = db.T_EmployeeInjurys.ElementType;
										 //var propertyInfoT_EmployeeInjury = elementTypeT_EmployeeInjury.GetProperty(propT_EmployeeInjury.Name);
										 // var t_employeeinjuryId = db.T_EmployeeInjurys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EmployeeInjury.GetValue(p, null)) == assovalue);
										 var t_employeeinjuryId = db.T_EmployeeInjurys.Where(propT_EmployeeInjury.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_employeeinjuryId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
        ...
        public ActionResult ImportData(FormCollection collection)
        {
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_TypeOfRestrictions model = new T_TypeOfRestrictions();
		 ...
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_Restrictions = db.T_Restrictionss.ElementType;
			 //var propertyInfoT_Restrictions = elementTypeT_Restrictions.GetProperty(propT_Restrictions.Name);			 
			 //t_restrictionsId = db.T_Restrictionss.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_Restrictions.GetValue(p, null)) == columnValue);
			 t_restrictionsId = db.T_Restrictionss.Where(propT_Restrictions.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_restrictionsId = db.T_Restrictionss.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_restrictionsId != null)
			model.T_RestrictionsID = t_restrictionsId.Id;
		  else
			{
				if ((collection["T_RestrictionsID"].ToString() == "true,false"))
				{
					try
					{
						T_Restrictions objT_Restrictions = new T_Restrictions();
						objT_Restrictions.T_Name  = (columnValue);
						db.T_Restrictionss.Add(objT_Restrictions);
						db.SaveChanges();
						model.T_RestrictionsID = objT_Restrictions.Id;
					}
					catch
					{
						continue;
         ...
         break;
		 case "T_EmployeeInjuryID":
		 dynamic t_employeeinjuryId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 var strPropertyT_EmployeeInjury = lstEntityProp.FirstOrDefault(p => p.Key == "T_EmployeeInjuryID").Value;
			 ModelReflector.Property propT_EmployeeInjury = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_EmployeeInjury").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_EmployeeInjury);
			 //var elementTypeT_EmployeeInjury = db.T_EmployeeInjurys.ElementType;
			 //var propertyInfoT_EmployeeInjury = elementTypeT_EmployeeInjury.GetProperty(propT_EmployeeInjury.Name);			 
			 //t_employeeinjuryId = db.T_EmployeeInjurys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EmployeeInjury.GetValue(p, null)) == columnValue);
			 t_employeeinjuryId = db.T_EmployeeInjurys.Where(propT_EmployeeInjury.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_employeeinjuryId = db.T_EmployeeInjurys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_employeeinjuryId != null)
			model.T_EmployeeInjuryID = t_employeeinjuryId.Id;
		  else
			{
				if ((collection["T_EmployeeInjuryID"].ToString() == "true,false"))
				{
					try
					{
				 ...
				 try { objT_EmployeeInjury.T_AccidentDate =Convert.ToDateTime(columnValue); }
						...
						db.SaveChanges();
						model.T_EmployeeInjuryID = objT_EmployeeInjury.Id;
					}
					catch
					{
						continue;
                ...
                break;
					    ...
					    if (ValidateModel(model))
                        {
                            ...
                            if (result == null || result.Count == 0)
                            {
								...
								{
									db.T_TypeOfRestrictionss.Add(model);
									db.SaveChanges();
								}
								error += customerror;
							}
		...
		public bool ValidateModel(T_TypeOfRestrictions validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_TypeOfRestrictionss.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_TypeOfRestrictions OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_TypeOfRestrictions").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_TypeOfRestrictions");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_TypeOfRestrictions OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_TypeOfRestrictions");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_TypeOfRestrictions OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_TypeOfRestrictions");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_TypeOfRestrictions OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_TypeOfRestrictions",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_TypeOfRestrictions OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_TypeOfRestrictions");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_TypeOfRestrictions OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_TypeOfRestrictions").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_TypeOfRestrictionss.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_TypeOfRestrictionss;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_TypeOfRestrictions, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_TypeOfRestrictions, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_TypeOfRestrictions>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_TypeOfRestrictions t_typeofrestrictions)
        {
        ...
        public ActionResult GetCalculationValues(T_TypeOfRestrictions t_typeofrestrictions)
        {
            t_typeofrestrictions.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_typeofrestrictions.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_TypeOfRestrictions"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_TypeOfRestrictions t_typeofrestrictions, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_TypeOfRestrictions t_typeofrestrictions, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
