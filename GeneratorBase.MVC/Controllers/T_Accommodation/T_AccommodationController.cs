...
{	
    ...
    {
		private void LoadViewDataForCount(T_Accommodation t_accommodation, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_Accommodation t_accommodation)
        {		
			 ViewBag.T_EmployeeAccomodationID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_accommodation.T_EmployeeAccomodationID);
			 ViewBag.T_AccommodationInjuryID = new SelectList(db.T_EmployeeInjurys, "ID", "DisplayValue", t_accommodation.T_AccommodationInjuryID);
			CustomLoadViewDataListAfterEdit(t_accommodation);
        }
        private void LoadViewDataBeforeOnEdit(T_Accommodation t_accommodation)
        {		
               var _objT_EmployeeAccomodation = new List<T_Employee>();
               _objT_EmployeeAccomodation.Add(t_accommodation.t_employeeaccomodation);
			   			   ViewBag.T_EmployeeAccomodationID = new SelectList(_objT_EmployeeAccomodation, "ID", "DisplayValue", t_accommodation.T_EmployeeAccomodationID);
			               var _objT_AccommodationInjury = new List<T_EmployeeInjury>();
               _objT_AccommodationInjury.Add(t_accommodation.t_accommodationinjury);
			   			   ViewBag.T_AccommodationInjuryID = new SelectList(_objT_AccommodationInjury, "ID", "DisplayValue", t_accommodation.T_AccommodationInjuryID);
				 ViewBag.T_AccommodationCommentsCount = t_accommodation.t_accommodationcomments.Count();
		CustomLoadViewDataListBeforeEdit(t_accommodation);
        }
        private void LoadViewDataAfterOnCreate(T_Accommodation t_accommodation)
        {			
			 ViewBag.T_EmployeeAccomodationID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_accommodation.T_EmployeeAccomodationID);
			 ViewBag.T_AccommodationInjuryID = new SelectList(db.T_EmployeeInjurys, "ID", "DisplayValue", t_accommodation.T_AccommodationInjuryID);
		CustomLoadViewDataListAfterOnCreate(t_accommodation);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Employee" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_EmployeeAccomodation")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_EmployeeAccomodationID = new SelectList(db.T_Employees.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Employees.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_EmployeeAccomodation = new List<T_Employee>();
			 ViewBag.T_EmployeeAccomodationID = new SelectList(objT_EmployeeAccomodation , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_EmployeeInjury" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_AccommodationInjury")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_AccommodationInjuryID = new SelectList(db.T_EmployeeInjurys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_EmployeeInjurys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_AccommodationInjury = new List<T_EmployeeInjury>();
			 ViewBag.T_AccommodationInjuryID = new SelectList(objT_AccommodationInjury , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_Accommodation> searchRecords(IQueryable<T_Accommodation> lstT_Accommodation, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_Accommodation = lstT_Accommodation.Where(s => (s.t_employeeaccomodation!= null && (s.t_employeeaccomodation.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_accommodationinjury!= null && (s.t_accommodationinjury.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.t_accommodationinjury.T_DetailsOfRestriction) && s.t_accommodationinjury.T_DetailsOfRestriction.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_AccommodationNotes) && s.T_AccommodationNotes.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_Accommodation = lstT_Accommodation.Where(s => (s.t_employeeaccomodation!= null && (s.t_employeeaccomodation.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_accommodationinjury!= null && (s.t_accommodationinjury.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.t_accommodationinjury.T_DetailsOfRestriction) && s.t_accommodationinjury.T_DetailsOfRestriction.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
			try
            {
                var boolvalue = Convert.ToBoolean(searchString);
                lstT_Accommodation = lstT_Accommodation.Union(db.T_Accommodations.Where(s => (s.T_TemporaryAccommodation == boolvalue) ));
            }
            catch { }
			try
            {
                var datevalue = Convert.ToDateTime(searchString);
                lstT_Accommodation = lstT_Accommodation.Union(db.T_Accommodations.Where(s => (s.T_AccommodationStartDate == datevalue) ||(s.T_AccommodationEndDate == datevalue) ||(s.T_NinetyDaysSinceAccommodation == datevalue) ||(s.T_DateLetterGivenToEmployee == datevalue) ));
            }
            catch { }
            return lstT_Accommodation;
        }
		private IQueryable<T_Accommodation> sortRecords(IQueryable<T_Accommodation> lstT_Accommodation, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_RestrictionID")
                return isAsc.ToLower() == "asc" ? lstT_Accommodation.OrderBy(p => p.t_accommodationinjury.T_DetailsOfRestriction) : lstT_Accommodation.OrderByDescending(p => p.t_accommodationinjury.T_DetailsOfRestriction);
	 if(sortBy == "T_EmployeeAccomodationID")
                return isAsc.ToLower() == "asc" ? lstT_Accommodation.OrderBy(p => p.t_employeeaccomodation.DisplayValue) : lstT_Accommodation.OrderByDescending(p => p.t_employeeaccomodation.DisplayValue);
	 if(sortBy == "T_AccommodationInjuryID")
                return isAsc.ToLower() == "asc" ? lstT_Accommodation.OrderBy(p => p.t_accommodationinjury.DisplayValue) : lstT_Accommodation.OrderByDescending(p => p.t_accommodationinjury.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_Accommodation.Sort(sortBy,true) : lstT_Accommodation.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_Accommodation), "t_accommodation");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_Accommodation>)lstT_Accommodation.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_Accommodation.ElementType, lambda.Body.Type },
                    lstT_Accommodation.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "FileDocument")
            {
                ...
                url += "?search=";
						var T_EmployeeAccomodation  = _Object.T_EmployeeDocumentsID;
						url += "&t_employeeaccomodation=" + T_EmployeeAccomodation;									
            ...
            if (sourceEntity == "T_Licenses")
            {
                ...
                url += "?search=";
						var T_EmployeeAccomodation  = _Object.T_LicenseRecordsID;
						url += "&t_employeeaccomodation=" + T_EmployeeAccomodation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_ServiceRecord")
            {
                ...
                url += "?search=";
						var T_EmployeeAccomodation  = _Object.T_EmployeeEmploymentProfileID;
						url += "&t_employeeaccomodation=" + T_EmployeeAccomodation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Comment")
            {
                ...
                url += "?search=";
						var T_EmployeeAccomodation  = _Object.T_EmployeeCommentsID;
						url += "&t_employeeaccomodation=" + T_EmployeeAccomodation;									
						var T_AccommodationInjury  = _Object.T_InjuryCommentsID;
						url += "&t_accommodationinjury=" + T_AccommodationInjury;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DrugAlcoholTest")
            {
                ...
                url += "?search=";
						var T_EmployeeAccomodation  = _Object.T_EmployeeDrugAlcoholTestID;
						url += "&t_employeeaccomodation=" + T_EmployeeAccomodation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_EmployeeAccomodation  = _Object.T_EmployeeAdministratorID;
						url += "&t_employeeaccomodation=" + T_EmployeeAccomodation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_JobAssignment")
            {
                ...
                url += "?search=";
						var T_EmployeeAccomodation  = _Object.T_EmployeeJobAssignmentID;
						url += "&t_employeeaccomodation=" + T_EmployeeAccomodation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_LeaveProfile")
            {
                ...
                url += "?search=";
						var T_EmployeeAccomodation  = _Object.T_EmployeeLeaveProfileID;
						url += "&t_employeeaccomodation=" + T_EmployeeAccomodation;									
						var T_AccommodationInjury  = _Object.T_InjuryRelatedLeaveID;
						url += "&t_accommodationinjury=" + T_AccommodationInjury;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_EmployeeInjury")
            {
                ...
                url += "?search=";
						var T_EmployeeAccomodation  = _Object.T_EmployeeEmployeeInjuryID;
						url += "&t_employeeaccomodation=" + T_EmployeeAccomodation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_BackgroundCheck")
            {
                ...
                url += "?search=";
						var T_EmployeeAccomodation  = _Object.T_EmployeeCriminalBackgroundCheckID;
						url += "&t_employeeaccomodation=" + T_EmployeeAccomodation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Education")
            {
                ...
                url += "?search=";
						var T_EmployeeAccomodation  = _Object.T_EmployeeEducationID;
						url += "&t_employeeaccomodation=" + T_EmployeeAccomodation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_PayDetails")
            {
                ...
                url += "?search=";
						var T_EmployeeAccomodation  = _Object.T_EmployeePayDetailsID;
						url += "&t_employeeaccomodation=" + T_EmployeeAccomodation;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_employeeaccomodation,string t_accommodationinjury, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_EmployeeAccomodationlist = db.T_Employees.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_employeeaccomodation != null)
            {
                var ids = t_employeeaccomodation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Employee= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_EmployeeAccomodationlist.Union(db.T_Employees.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
					ViewBag.t_employeeaccomodation = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_EmployeeAccomodationlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
				ViewBag.t_employeeaccomodation = new SelectList(list, "ID", "DisplayValue");
			}
			var T_AccommodationInjurylist = db.T_EmployeeInjurys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_accommodationinjury != null)
            {
                var ids = t_accommodationinjury.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Employee Injury= ";
				   foreach (var str in ids)
                    {
					...
					ids1 = ids1.ToList();
					var list = T_AccommodationInjurylist.Union(db.T_EmployeeInjurys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_EmployeeInjury>(User, list, "T_EmployeeInjury", null);
					ViewBag.t_accommodationinjury = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_AccommodationInjurylist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_EmployeeInjury>(User, list, "T_EmployeeInjury", null);
				ViewBag.t_accommodationinjury = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_EmployeeAccomodation = new List<T_Employee>();
		    ViewBag.t_employeeaccomodation = new SelectList(objT_EmployeeAccomodation, "ID", "DisplayValue");
			var objT_AccommodationInjury = new List<T_EmployeeInjury>();
		    ViewBag.t_accommodationinjury = new SelectList(objT_AccommodationInjury, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_Accommodation";
			...
			columns.Add("3", "Employee Injury");
			columns.Add("4", "Accommodation Start Date");
			columns.Add("5", "Ninety Days Since Accommodation");
			columns.Add("6", "Temporary Accommodation");
			columns.Add("7", "Date Letter Given To Employee");
			columns.Add("8", "Restriction");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				return View(new T_Accommodation());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_Accommodation", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_Accommodation", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_Accommodation()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_Accommodation/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_employeeaccomodation,string t_accommodationinjury ,string T_AccommodationStartDateFrom,string T_AccommodationStartDateTo,string T_AccommodationEndDateFrom,string T_AccommodationEndDateTo,string T_NinetyDaysSinceAccommodationFrom,string T_NinetyDaysSinceAccommodationTo,string T_TemporaryAccommodation,string T_DateLetterGivenToEmployeeFrom,string T_DateLetterGivenToEmployeeTo,string T_NinetyDaysSinceAccommodationFromhdn,string T_NinetyDaysSinceAccommodationTohdn,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            if (!string.IsNullOrEmpty(searchString) && FSFilter == null)
                ViewBag.FSFilter = "Fsearch";
				 var lstT_Accommodation  = from s in db.T_Accommodations
                           select s;
            if (!String.IsNullOrEmpty(searchString))
            {
                lstT_Accommodation  = searchRecords(lstT_Accommodation, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_Accommodation = searchRecords(lstT_Accommodation, search,true);
			}
            if (!String.IsNullOrEmpty(sortBy) && !String.IsNullOrEmpty(isAsc))
            {
                lstT_Accommodation  = sortRecords(lstT_Accommodation, sortBy, isAsc);
            }
            else   lstT_Accommodation  = lstT_Accommodation.OrderByDescending(c => c.Id);
			lstT_Accommodation = CustomSorting(lstT_Accommodation);
			lstT_Accommodation = lstT_Accommodation.Include(t=>t.t_employeeaccomodation).Include(t=>t.t_accommodationinjury);
					if (!string.IsNullOrEmpty(SortOrder) && SortOrder.Contains("T_Restriction"))
                    {
                      SortOrder = SortOrder.Replace("T_Restriction", "t_accommodationinjury.T_DetailsOfRestriction");  
                    }
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_Accommodation", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
					if (PropertyName == "T_Restriction")
                    {
                        PropertyName = "[T_AccommodationInjuryID.T_DetailsOfRestriction]";
                    }
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_Accommodation = lstT_Accommodation.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(GroupByColumn))
            {
                DataOrdering = GroupByColumn;
									if (DataOrdering.Contains("T_Restriction"))
                    {
                      DataOrdering = DataOrdering.Replace("T_Restriction", "t_accommodationinjury.T_DetailsOfRestriction");  
                    }
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_Accommodation = Sorting.Sort<T_Accommodation>(lstT_Accommodation, DataOrdering);
            var _T_Accommodation = lstT_Accommodation;
				if(T_AccommodationStartDateFrom!=null || T_AccommodationStartDateTo !=null)
				{
						try
						{
							DateTime from = T_AccommodationStartDateFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_AccommodationStartDateFrom);
							DateTime to = T_AccommodationStartDateTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_AccommodationStartDateTo).AddTicks(-1).AddDays(1);
							                       _T_Accommodation =  _T_Accommodation.Where(o =>o.T_AccommodationStartDate!=null && o.T_AccommodationStartDate.CompareTo(from) >= 0 && o.T_AccommodationStartDate.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n Accommodation Start Date= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
				if(T_AccommodationEndDateFrom!=null || T_AccommodationEndDateTo !=null)
				{
						try
						{
							DateTime from = T_AccommodationEndDateFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_AccommodationEndDateFrom);
							DateTime to = T_AccommodationEndDateTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_AccommodationEndDateTo).AddTicks(-1).AddDays(1);
							                       _T_Accommodation =  _T_Accommodation.Where(o =>o.T_AccommodationEndDate!=null && o.T_AccommodationEndDate.Value.CompareTo(from) >= 0 && o.T_AccommodationEndDate.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n Accommodation End Date= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
				if(T_NinetyDaysSinceAccommodationFrom!=null || T_NinetyDaysSinceAccommodationTo !=null)
				{
						try
						{
							TimeSpan from = T_NinetyDaysSinceAccommodationFrom == null ? Convert.ToDateTime("01/01/1900").TimeOfDay : Convert.ToDateTime(T_NinetyDaysSinceAccommodationFrom).TimeOfDay;
							TimeSpan to = T_NinetyDaysSinceAccommodationTo == null ? DateTime.MaxValue.TimeOfDay : Convert.ToDateTime(T_NinetyDaysSinceAccommodationTo).TimeOfDay;
												   _T_Accommodation =  _T_Accommodation.Where(o => o.T_NinetyDaysSinceAccommodation.Value.Hour * 60 + o.T_NinetyDaysSinceAccommodation.Value.Minute >= from.TotalMinutes && o.T_NinetyDaysSinceAccommodation.Value.Hour * 60 + o.T_NinetyDaysSinceAccommodation.Value.Minute <= to.TotalMinutes);
                   							ViewBag.SearchResult += "\r\n Ninety Days Since Accommodation= " + T_NinetyDaysSinceAccommodationFromhdn + "-" + T_NinetyDaysSinceAccommodationTohdn;
						}
						catch { }
				}
		 if(T_TemporaryAccommodation!=null)
            {
                try
                {
                    bool boolvalue = Convert.ToBoolean(T_TemporaryAccommodation);
                    _T_Accommodation =  _T_Accommodation.Where(o => o.T_TemporaryAccommodation == boolvalue);
					ViewBag.SearchResult += "\r\n Temporary Accommodation= "+T_TemporaryAccommodation;
                }
                catch { }
            }
				if(T_DateLetterGivenToEmployeeFrom!=null || T_DateLetterGivenToEmployeeTo !=null)
				{
						try
						{
							DateTime from = T_DateLetterGivenToEmployeeFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_DateLetterGivenToEmployeeFrom);
							DateTime to = T_DateLetterGivenToEmployeeTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_DateLetterGivenToEmployeeTo).AddTicks(-1).AddDays(1);
							                       _T_Accommodation =  _T_Accommodation.Where(o =>o.T_DateLetterGivenToEmployee!=null && o.T_DateLetterGivenToEmployee.Value.CompareTo(from) >= 0 && o.T_DateLetterGivenToEmployee.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n Date Letter Given To Employee= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
			//if (lstT_Accommodation.Where(p => p.t_employeeaccomodation != null).Count() <= 50)
		    //ViewBag.t_employeeaccomodation = new SelectList(lstT_Accommodation.Where(p => p.t_employeeaccomodation != null).Select(P => P.t_employeeaccomodation).Distinct(), "ID", "DisplayValue");
            if (t_employeeaccomodation != null)
            {
                var ids = t_employeeaccomodation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_Accommodation = _T_Accommodation.Where(p => ids1.Contains(p.T_EmployeeAccomodationID));
            }
				//if (lstT_Accommodation.Where(p => p.t_accommodationinjury != null).Count() <= 50)
		    //ViewBag.t_accommodationinjury = new SelectList(lstT_Accommodation.Where(p => p.t_accommodationinjury != null).Select(P => P.t_accommodationinjury).Distinct(), "ID", "DisplayValue");
            if (t_accommodationinjury != null)
            {
                var ids = t_accommodationinjury.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_Accommodation = _T_Accommodation.Where(p => ids1.Contains(p.T_AccommodationInjuryID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Accommodation");
		   ...
		   if (itemsPerPage != null)
            ...
            if (Request.Cookies["pageSize" + HttpUtility.UrlEncode(User.Name) + "T_Accommodation"] != null)
            {
                pageSize = Convert.ToInt32(Request.Cookies["pageSize" + HttpUtility.UrlEncode(User.Name) + "T_Accommodation"].Value);
                ViewBag.CurrentItemsPerPage = itemsPerPage;
            }
            ...
            {
                pageNumber = 1;
                if (_T_Accommodation.Count() > 0)
                    pageSize = _T_Accommodation.Count();
                return View("ExcelExport", _T_Accommodation.ToPagedList(pageNumber, pageSize));
            }
			else
            {
			 if (pageNumber > 1)
			 {
                var totalListCount = _T_Accommodation.Count();
			...
			if (!Request.IsAjaxRequest())
			{
				...
				var list = _T_Accommodation.ToPagedList(pageNumber, pageSize);
				ViewBag.EntityT_AccommodationDisplayValue = new SelectList(list.Select(z => new { ID = z.Id, DisplayValue = z.DisplayValue }), "ID", "DisplayValue");
				TempData["T_Accommodationlist"] = list.Select(z => new { ID = z.Id, DisplayValue = z.DisplayValue });
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        var tagsSplit = GroupByColumn.Split(',').Select(tag => tag.Trim()).Where(tag => !string.IsNullOrEmpty(tag));
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_Accommodation", tagsSplit.ToArray());
                    }
		...
		var entity = "T_Accommodation";
           ...
           var entitylist = db.T_Accommodations;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_Accommodations.GroupBy(p => p.t_employeeaccomodation.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Employee", "Accommodation"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Accommodation", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_Accommodations.GroupBy(p => p.t_accommodationinjury.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "2" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Employee Injury (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Employee Injury"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Employee Injury", "Accommodation"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Accommodation", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_Accommodations.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_accommodation = jedb.T_Accommodations.Find(id);
			var T_AccommodationCommentsIds = jedb.T_Comments.Where(p=>p.T_AccommodationCommentsID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_Comment" && T_AccommodationCommentsIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_Accommodations.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_Accommodations.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_Accommodation>(_Obj, "T_Accommodation", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_Accommodation> list = db.T_Accommodations;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_Accommodations;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_Accommodation), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_Accommodation, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_Accommodation>)q);
				   list = ((IQueryable<T_Accommodation>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_Accommodations;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_Accommodation>(User,list, "T_Accommodation",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_Accommodation>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_Accommodation), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_Accommodation).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_Accommodation), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Accommodation), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Accommodation), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Accommodation), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Accommodation), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Accommodation), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_Accommodation), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_Accommodation), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_Accommodation", User) || !User.CanAdd("T_Accommodation"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_Accommodation")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_Accommodation").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_Accommodation");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_Accommodation" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Accommodations";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_EmployeeAccomodationID":
										 var t_employeeaccomodationId = db.T_Employees.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_employeeaccomodationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_AccommodationInjuryID":
										 var t_accommodationinjuryId = db.T_EmployeeInjurys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_accommodationinjuryId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                        default:
                                            break;
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_Accommodation";
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_EmployeeAccomodationID":
										 var strPropertyT_EmployeeAccomodation = lstEntityProp.FirstOrDefault(p => p.Key == "T_EmployeeAccomodationID").Value;
										 ModelReflector.Property propT_EmployeeAccomodation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Employee").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_EmployeeAccomodation);
										 //var elementTypeT_EmployeeAccomodation = db.T_Employees.ElementType;
										 //var propertyInfoT_EmployeeAccomodation = elementTypeT_EmployeeAccomodation.GetProperty(propT_EmployeeAccomodation.Name);
										 // var t_employeeaccomodationId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EmployeeAccomodation.GetValue(p, null)) == assovalue);
										 var t_employeeaccomodationId = db.T_Employees.Where(propT_EmployeeAccomodation.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_employeeaccomodationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_AccommodationInjuryID":
										 var strPropertyT_AccommodationInjury = lstEntityProp.FirstOrDefault(p => p.Key == "T_AccommodationInjuryID").Value;
										 ModelReflector.Property propT_AccommodationInjury = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_EmployeeInjury").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_AccommodationInjury);
										 //var elementTypeT_AccommodationInjury = db.T_EmployeeInjurys.ElementType;
										 //var propertyInfoT_AccommodationInjury = elementTypeT_AccommodationInjury.GetProperty(propT_AccommodationInjury.Name);
										 // var t_accommodationinjuryId = db.T_EmployeeInjurys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_AccommodationInjury.GetValue(p, null)) == assovalue);
										 var t_accommodationinjuryId = db.T_EmployeeInjurys.Where(propT_AccommodationInjury.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_accommodationinjuryId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_Accommodation model = new T_Accommodation();
                        ...
                        for (int j = 0; j < sheetColumns.Count; j++)
                        {
                            ...
                            switch (columntable)
                            {
    case "T_AccommodationStartDate":
    model.T_AccommodationStartDate = DateTime.Parse(columnValue);
	break;
    case "T_AccommodationEndDate":
    model.T_AccommodationEndDate = DateTime.Parse(columnValue);
	break;
    case "T_NinetyDaysSinceAccommodation":
    model.T_NinetyDaysSinceAccommodation = DateTime.Parse(columnValue);
	break;
    case "T_TemporaryAccommodation":
    model.T_TemporaryAccommodation = Boolean.Parse(columnValue);
	break;
    case "T_DateLetterGivenToEmployee":
    model.T_DateLetterGivenToEmployee = DateTime.Parse(columnValue);
	break;
    case "T_Restriction":
    model.T_Restriction = columnValue;
	break;
    case "T_AccommodationNotes":
    model.T_AccommodationNotes = columnValue;
	break;
					 case "T_EmployeeAccomodationID":
		 dynamic t_employeeaccomodationId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_EmployeeAccomodation = db.T_Employees.ElementType;
			 //var propertyInfoT_EmployeeAccomodation = elementTypeT_EmployeeAccomodation.GetProperty(propT_EmployeeAccomodation.Name);			 
			 //t_employeeaccomodationId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EmployeeAccomodation.GetValue(p, null)) == columnValue);
			 t_employeeaccomodationId = db.T_Employees.Where(propT_EmployeeAccomodation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_employeeaccomodationId = db.T_Employees.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_employeeaccomodationId != null)
			model.T_EmployeeAccomodationID = t_employeeaccomodationId.Id;
		  else
			{
				if ((collection["T_EmployeeAccomodationID"].ToString() == "true,false"))
				{
					try
					{
				 ...
				 try { objT_Employee.T_FirstName =(columnValue); }
						...
						db.SaveChanges();
						model.T_EmployeeAccomodationID = objT_Employee.Id;
					}
					catch
					{
						continue;
         ...
         break;
		 case "T_AccommodationInjuryID":
		 dynamic t_accommodationinjuryId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 var strPropertyT_AccommodationInjury = lstEntityProp.FirstOrDefault(p => p.Key == "T_AccommodationInjuryID").Value;
			 ModelReflector.Property propT_AccommodationInjury = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_EmployeeInjury").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_AccommodationInjury);
			 //var elementTypeT_AccommodationInjury = db.T_EmployeeInjurys.ElementType;
			 //var propertyInfoT_AccommodationInjury = elementTypeT_AccommodationInjury.GetProperty(propT_AccommodationInjury.Name);			 
			 //t_accommodationinjuryId = db.T_EmployeeInjurys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_AccommodationInjury.GetValue(p, null)) == columnValue);
			 t_accommodationinjuryId = db.T_EmployeeInjurys.Where(propT_AccommodationInjury.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_accommodationinjuryId = db.T_EmployeeInjurys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_accommodationinjuryId != null)
			model.T_AccommodationInjuryID = t_accommodationinjuryId.Id;
		  else
			{
				if ((collection["T_AccommodationInjuryID"].ToString() == "true,false"))
				{
					try
					{
				 ...
				 try { objT_EmployeeInjury.T_AccidentDate =Convert.ToDateTime(columnValue); }
						...
						db.SaveChanges();
						model.T_AccommodationInjuryID = objT_EmployeeInjury.Id;
					}
					catch
					{
						continue;
         ...
         break;
            default:
			 ...
			 if (model.T_AccommodationStartDate == DateTime.MinValue)
                            model.T_AccommodationStartDate =  DateTime.UtcNow;
		...
		public bool ValidateModel(T_Accommodation validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_Accommodations.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_Accommodation OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_Accommodation").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_Accommodation");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_Accommodation OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_Accommodation");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_Accommodation OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_Accommodation");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_Accommodation OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_Accommodation",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_Accommodation OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_Accommodation");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_Accommodation OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Accommodation").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_Accommodations.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_Accommodations;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_Accommodation, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_Accommodation, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_Accommodation>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_Accommodation t_accommodation)
        {
        ...
        {
            string[][] groupsarr = new string[][] { new string[] {"48903","T_Accommodation48903"},new string[] {"48929","Employee Information"},new string[] {"48930","Accommodation Information"},new string[] {"48931","Notes"} };
            return groupsarr;
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_Accommodation t_accommodation)
        {
            t_accommodation.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_accommodation.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
		    var T_NinetyDaysSinceAccommodationProperty = properties.FirstOrDefault(p => p.Name == "T_NinetyDaysSinceAccommodation");
            if (T_NinetyDaysSinceAccommodationProperty != null)
            {
			  object PropValue = T_NinetyDaysSinceAccommodationProperty.GetValue(t_accommodation, null);
				       Calculations.Add("T_NinetyDaysSinceAccommodation", Convert.ToString(PropValue));
		            }
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_Accommodation"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_Accommodation t_accommodation, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_accommodation.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
 
			var T_RestrictionsourceProp = properties.FirstOrDefault(p => p.Name == "T_AccommodationInjuryID");
            if (T_RestrictionsourceProp != null)
            {
                object sourcePropValue = T_RestrictionsourceProp.GetValue(t_accommodation, null);
                var Property = properties.FirstOrDefault(p => p.Name == "T_Restriction");
                if (Property != null && sourcePropValue != null)
                {
                   T_EmployeeInjuryController objController = new T_EmployeeInjuryController();
                    object PropValue = objController.GetPropertyValueByEntityId(Convert.ToInt64(sourcePropValue), "T_DetailsOfRestriction");
                    if (PropValue != null && Convert.ToString(((System.Web.Mvc.JsonResult)(PropValue)).Data) != "0" && !derivedlist.ContainsKey("T_Restriction"))
                        derivedlist.Add("T_Restriction", Convert.ToString(((System.Web.Mvc.JsonResult)(PropValue)).Data));
                }
				if (!derivedlist.ContainsKey("T_Restriction"))
                   derivedlist.Add("T_Restriction", "");
            }
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_Accommodation t_accommodation, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
		if (host == "T_AccommodationInjuryID")
			{
			var T_RestrictionsourceProp = host;
            if (T_RestrictionsourceProp != null)
            {
