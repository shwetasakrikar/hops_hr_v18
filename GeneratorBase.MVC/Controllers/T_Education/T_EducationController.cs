...
{	
    public partial class T_EducationController : BaseController
    {
		private void LoadViewDataForCount(T_Education t_education, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_Education t_education)
        {		
			 ViewBag.T_EmployeeEducationID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_education.T_EmployeeEducationID);
			 ViewBag.T_LevelOfEducationID = new SelectList(db.T_EducationLevels, "ID", "DisplayValue", t_education.T_LevelOfEducationID);
			 ViewBag.T_MajorInMultiCheckBoxID = new SelectList(db.T_Majors, "ID", "DisplayValue", t_education.T_MajorInMultiCheckBoxID);
			 ViewBag.T_EducationVerificationVendorID = new SelectList(db.T_Vendors, "ID", "DisplayValue", t_education.T_EducationVerificationVendorID);
			CustomLoadViewDataListAfterEdit(t_education);
        }
        private void LoadViewDataBeforeOnEdit(T_Education t_education)
        {		
               var _objT_EmployeeEducation = new List<T_Employee>();
               _objT_EmployeeEducation.Add(t_education.t_employeeeducation);
			   			   ViewBag.T_EmployeeEducationID = new SelectList(_objT_EmployeeEducation, "ID", "DisplayValue", t_education.T_EmployeeEducationID);
			               var _objT_LevelOfEducation = new List<T_EducationLevel>();
               _objT_LevelOfEducation.Add(t_education.t_levelofeducation);
			   			   ViewBag.T_LevelOfEducationID = new SelectList(_objT_LevelOfEducation, "ID", "DisplayValue", t_education.T_LevelOfEducationID);
			               var _objT_MajorInMultiCheckBox = new List<T_Major>();
               _objT_MajorInMultiCheckBox.Add(t_education.t_majorinmulticheckbox);
			   			   ViewBag.T_MajorInMultiCheckBoxID = new SelectList(_objT_MajorInMultiCheckBox, "ID", "DisplayValue", t_education.T_MajorInMultiCheckBoxID);
			               var _objT_EducationVerificationVendor = new List<T_Vendor>();
               _objT_EducationVerificationVendor.Add(t_education.t_educationverificationvendor);
			   			   ViewBag.T_EducationVerificationVendorID = new SelectList(_objT_EducationVerificationVendor, "ID", "DisplayValue", t_education.T_EducationVerificationVendorID);
				 ViewBag.T_EducationCommentsCount = t_education.t_educationcomments.Count();
		CustomLoadViewDataListBeforeEdit(t_education);
        }
        private void LoadViewDataAfterOnCreate(T_Education t_education)
        {			
			 ViewBag.T_EmployeeEducationID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_education.T_EmployeeEducationID);
			 ViewBag.T_LevelOfEducationID = new SelectList(db.T_EducationLevels, "ID", "DisplayValue", t_education.T_LevelOfEducationID);
			 ViewBag.T_MajorInMultiCheckBoxID = new SelectList(db.T_Majors, "ID", "DisplayValue", t_education.T_MajorInMultiCheckBoxID);
			 ViewBag.T_EducationVerificationVendorID = new SelectList(db.T_Vendors, "ID", "DisplayValue", t_education.T_EducationVerificationVendorID);
		CustomLoadViewDataListAfterOnCreate(t_education);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Employee" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_EmployeeEducation")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_EmployeeEducationID = new SelectList(db.T_Employees.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Employees.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_EmployeeEducation = new List<T_Employee>();
			 ViewBag.T_EmployeeEducationID = new SelectList(objT_EmployeeEducation , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_EducationLevel" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_LevelOfEducation")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_LevelOfEducationID = new SelectList(db.T_EducationLevels.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_EducationLevels.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_LevelOfEducation = new List<T_EducationLevel>();
			 ViewBag.T_LevelOfEducationID = new SelectList(objT_LevelOfEducation , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_Major" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_MajorInMultiCheckBox")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_MajorInMultiCheckBoxID = new SelectList(db.T_Majors.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Majors.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_MajorInMultiCheckBox = new List<T_Major>();
			 ViewBag.T_MajorInMultiCheckBoxID = new SelectList(objT_MajorInMultiCheckBox , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_Vendor" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_EducationVerificationVendor")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_EducationVerificationVendorID = new SelectList(db.T_Vendors.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Vendors.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_EducationVerificationVendor = new List<T_Vendor>();
			 ViewBag.T_EducationVerificationVendorID = new SelectList(objT_EducationVerificationVendor , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_Education> searchRecords(IQueryable<T_Education> lstT_Education, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_Education = lstT_Education.Where(s => (s.t_employeeeducation!= null && (s.t_employeeeducation.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_levelofeducation!= null && (s.t_levelofeducation.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_majorinmulticheckbox!= null && (s.t_majorinmulticheckbox.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_educationverificationvendor!= null && (s.t_educationverificationvendor.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_DetailsofEducation) && s.T_DetailsofEducation.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_EducationNotes) && s.T_EducationNotes.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_Education = lstT_Education.Where(s => (s.t_employeeeducation!= null && (s.t_employeeeducation.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_levelofeducation!= null && (s.t_levelofeducation.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_majorinmulticheckbox!= null && (s.t_majorinmulticheckbox.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_educationverificationvendor!= null && (s.t_educationverificationvendor.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_DetailsofEducation) && s.T_DetailsofEducation.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_EducationNotes) && s.T_EducationNotes.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
			try
            {
                var datevalue = Convert.ToDateTime(searchString);
                lstT_Education = lstT_Education.Union(db.T_Educations.Where(s => (s.T_DateVerified == datevalue) ));
            }
            catch { }
            return lstT_Education;
        }
		private IQueryable<T_Education> sortRecords(IQueryable<T_Education> lstT_Education, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_EmployeeEducationID")
                return isAsc.ToLower() == "asc" ? lstT_Education.OrderBy(p => p.t_employeeeducation.DisplayValue) : lstT_Education.OrderByDescending(p => p.t_employeeeducation.DisplayValue);
	 if(sortBy == "T_LevelOfEducationID")
                return isAsc.ToLower() == "asc" ? lstT_Education.OrderBy(p => p.t_levelofeducation.DisplayValue) : lstT_Education.OrderByDescending(p => p.t_levelofeducation.DisplayValue);
	 if(sortBy == "T_MajorInMultiCheckBoxID")
                return isAsc.ToLower() == "asc" ? lstT_Education.OrderBy(p => p.t_majorinmulticheckbox.DisplayValue) : lstT_Education.OrderByDescending(p => p.t_majorinmulticheckbox.DisplayValue);
	 if(sortBy == "T_EducationVerificationVendorID")
                return isAsc.ToLower() == "asc" ? lstT_Education.OrderBy(p => p.t_educationverificationvendor.DisplayValue) : lstT_Education.OrderByDescending(p => p.t_educationverificationvendor.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_Education.Sort(sortBy,true) : lstT_Education.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_Education), "t_education");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_Education>)lstT_Education.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_Education.ElementType, lambda.Body.Type },
                    lstT_Education.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "FileDocument")
            {
                ...
                url += "?search=";
						var T_EmployeeEducation  = _Object.T_EmployeeDocumentsID;
						url += "&t_employeeeducation=" + T_EmployeeEducation;									
            ...
            if (sourceEntity == "T_Licenses")
            {
                ...
                url += "?search=";
						var T_EmployeeEducation  = _Object.T_LicenseRecordsID;
						url += "&t_employeeeducation=" + T_EmployeeEducation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_ServiceRecord")
            {
                ...
                url += "?search=";
						var T_EmployeeEducation  = _Object.T_EmployeeEmploymentProfileID;
						url += "&t_employeeeducation=" + T_EmployeeEducation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Comment")
            {
                ...
                url += "?search=";
						var T_EmployeeEducation  = _Object.T_EmployeeCommentsID;
						url += "&t_employeeeducation=" + T_EmployeeEducation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DrugAlcoholTest")
            {
                ...
                url += "?search=";
						var T_EmployeeEducation  = _Object.T_EmployeeDrugAlcoholTestID;
						url += "&t_employeeeducation=" + T_EmployeeEducation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_EmployeeEducation  = _Object.T_EmployeeAdministratorID;
						url += "&t_employeeeducation=" + T_EmployeeEducation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_JobAssignment")
            {
                ...
                url += "?search=";
						var T_EmployeeEducation  = _Object.T_EmployeeJobAssignmentID;
						url += "&t_employeeeducation=" + T_EmployeeEducation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_LeaveProfile")
            {
                ...
                url += "?search=";
						var T_EmployeeEducation  = _Object.T_EmployeeLeaveProfileID;
						url += "&t_employeeeducation=" + T_EmployeeEducation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_EmployeeInjury")
            {
                ...
                url += "?search=";
						var T_EmployeeEducation  = _Object.T_EmployeeEmployeeInjuryID;
						url += "&t_employeeeducation=" + T_EmployeeEducation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_BackgroundCheck")
            {
                ...
                url += "?search=";
						var T_EmployeeEducation  = _Object.T_EmployeeCriminalBackgroundCheckID;
						url += "&t_employeeeducation=" + T_EmployeeEducation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Accommodation")
            {
                ...
                url += "?search=";
						var T_EmployeeEducation  = _Object.T_EmployeeAccomodationID;
						url += "&t_employeeeducation=" + T_EmployeeEducation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_RequiredEducation")
            {
                ...
                url += "?search=";
						var T_LevelOfEducation  = _Object.T_RequiredEducationEducationLevelID;
						url += "&t_levelofeducation=" + T_LevelOfEducation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_PayDetails")
            {
                ...
                url += "?search=";
						var T_EmployeeEducation  = _Object.T_EmployeePayDetailsID;
						url += "&t_employeeeducation=" + T_EmployeeEducation;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_employeeeducation,string t_levelofeducation,string t_majorinmulticheckbox,string t_educationverificationvendor, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_EmployeeEducationlist = db.T_Employees.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_employeeeducation != null)
            {
                var ids = t_employeeeducation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Employee= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_EmployeeEducationlist.Union(db.T_Employees.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
					ViewBag.t_employeeeducation = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_EmployeeEducationlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
				ViewBag.t_employeeeducation = new SelectList(list, "ID", "DisplayValue");
			}
			var T_LevelOfEducationlist = db.T_EducationLevels.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_levelofeducation != null)
            {
                var ids = t_levelofeducation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Level Of Education= ";
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            {
                               ids1.Add(Convert.ToInt64(str));
							   ViewBag.SearchResult += db.T_EducationLevels.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_LevelOfEducationlist.Union(db.T_EducationLevels.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_EducationLevel>(User, list, "T_EducationLevel", null);
					ViewBag.t_levelofeducation = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_LevelOfEducationlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_EducationLevel>(User, list, "T_EducationLevel", null);
				ViewBag.t_levelofeducation = new SelectList(list, "ID", "DisplayValue");
			}
			var T_MajorInMultiCheckBoxlist = db.T_Majors.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_majorinmulticheckbox != null)
            {
                var ids = t_majorinmulticheckbox.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Major In= ";
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            {
                               ids1.Add(Convert.ToInt64(str));
							   ViewBag.SearchResult += db.T_Majors.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_MajorInMultiCheckBoxlist.Union(db.T_Majors.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Major>(User, list, "T_Major", null);
					ViewBag.t_majorinmulticheckbox = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_MajorInMultiCheckBoxlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Major>(User, list, "T_Major", null);
				ViewBag.t_majorinmulticheckbox = new SelectList(list, "ID", "DisplayValue");
			}
			var T_EducationVerificationVendorlist = db.T_Vendors.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_educationverificationvendor != null)
            {
                var ids = t_educationverificationvendor.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Education Verification Vendor= ";
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            {
                               ids1.Add(Convert.ToInt64(str));
							   ViewBag.SearchResult += db.T_Vendors.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_EducationVerificationVendorlist.Union(db.T_Vendors.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Vendor>(User, list, "T_Vendor", null);
					ViewBag.t_educationverificationvendor = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_EducationVerificationVendorlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Vendor>(User, list, "T_Vendor", null);
				ViewBag.t_educationverificationvendor = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_EmployeeEducation = new List<T_Employee>();
		    ViewBag.t_employeeeducation = new SelectList(objT_EmployeeEducation, "ID", "DisplayValue");
			var objT_LevelOfEducation = new List<T_EducationLevel>();
		    ViewBag.t_levelofeducation = new SelectList(objT_LevelOfEducation, "ID", "DisplayValue");
			var objT_MajorInMultiCheckBox = new List<T_Major>();
		    ViewBag.t_majorinmulticheckbox = new SelectList(objT_MajorInMultiCheckBox, "ID", "DisplayValue");
			var objT_EducationVerificationVendor = new List<T_Vendor>();
		    ViewBag.t_educationverificationvendor = new SelectList(objT_EducationVerificationVendor, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_Education";
			...
			columns.Add("3", "Level Of Education");
			columns.Add("4", "Major In");
			columns.Add("5", "Education Verification Vendor");
			columns.Add("6", "Details of Education");
			columns.Add("7", "Date Verified");
			columns.Add("8", "Education Notes");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				return View(new T_Education());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_Education", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_Education", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_Education()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_Education/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_employeeeducation,string t_levelofeducation,string t_majorinmulticheckbox,string t_educationverificationvendor ,string T_DateVerifiedFrom,string T_DateVerifiedTo,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            if (!string.IsNullOrEmpty(searchString) && FSFilter == null)
                ViewBag.FSFilter = "Fsearch";
				 var lstT_Education  = from s in db.T_Educations
                           select s;
            if (!String.IsNullOrEmpty(searchString))
            {
                lstT_Education  = searchRecords(lstT_Education, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_Education = searchRecords(lstT_Education, search,true);
			}
            if (!String.IsNullOrEmpty(sortBy) && !String.IsNullOrEmpty(isAsc))
            {
                lstT_Education  = sortRecords(lstT_Education, sortBy, isAsc);
            }
            else   lstT_Education  = lstT_Education.OrderByDescending(c => c.Id);
			lstT_Education = CustomSorting(lstT_Education);
			lstT_Education = lstT_Education.Include(t=>t.t_employeeeducation).Include(t=>t.t_levelofeducation).Include(t=>t.t_majorinmulticheckbox).Include(t=>t.t_educationverificationvendor);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_Education", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_Education = lstT_Education.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_Education = Sorting.Sort<T_Education>(lstT_Education, DataOrdering);
            var _T_Education = lstT_Education;
				if(T_DateVerifiedFrom!=null || T_DateVerifiedTo !=null)
				{
						try
						{
							DateTime from = T_DateVerifiedFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_DateVerifiedFrom);
							DateTime to = T_DateVerifiedTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_DateVerifiedTo).AddTicks(-1).AddDays(1);
							                       _T_Education =  _T_Education.Where(o =>o.T_DateVerified!=null && o.T_DateVerified.Value.CompareTo(from) >= 0 && o.T_DateVerified.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n Date Verified= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
			//if (lstT_Education.Where(p => p.t_employeeeducation != null).Count() <= 50)
		    //ViewBag.t_employeeeducation = new SelectList(lstT_Education.Where(p => p.t_employeeeducation != null).Select(P => P.t_employeeeducation).Distinct(), "ID", "DisplayValue");
            if (t_employeeeducation != null)
            {
                var ids = t_employeeeducation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_Education = _T_Education.Where(p => ids1.Contains(p.T_EmployeeEducationID));
            }
				//if (lstT_Education.Where(p => p.t_levelofeducation != null).Count() <= 50)
		    //ViewBag.t_levelofeducation = new SelectList(lstT_Education.Where(p => p.t_levelofeducation != null).Select(P => P.t_levelofeducation).Distinct(), "ID", "DisplayValue");
            if (t_levelofeducation != null)
            {
                var ids = t_levelofeducation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_EducationLevels.Find(Convert.ToInt64(str));
                            ViewBag.SearchResult += obj != null ? obj.DisplayValue + ", " : "";
                        }
                    }
                    //
                ...
                ids1 = ids1.ToList();
                _T_Education = _T_Education.Where(p => ids1.Contains(p.T_LevelOfEducationID));
            }
				//if (lstT_Education.Where(p => p.t_majorinmulticheckbox != null).Count() <= 50)
		    //ViewBag.t_majorinmulticheckbox = new SelectList(lstT_Education.Where(p => p.t_majorinmulticheckbox != null).Select(P => P.t_majorinmulticheckbox).Distinct(), "ID", "DisplayValue");
            if (t_majorinmulticheckbox != null)
            {
                var ids = t_majorinmulticheckbox.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_Majors.Find(Convert.ToInt64(str));
                            ViewBag.SearchResult += obj != null ? obj.DisplayValue + ", " : "";
                        }
                    }
                    //
                ...
                ids1 = ids1.ToList();
                _T_Education = _T_Education.Where(p => ids1.Contains(p.T_MajorInMultiCheckBoxID));
            }
				//if (lstT_Education.Where(p => p.t_educationverificationvendor != null).Count() <= 50)
		    //ViewBag.t_educationverificationvendor = new SelectList(lstT_Education.Where(p => p.t_educationverificationvendor != null).Select(P => P.t_educationverificationvendor).Distinct(), "ID", "DisplayValue");
            if (t_educationverificationvendor != null)
            {
                var ids = t_educationverificationvendor.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_Vendors.Find(Convert.ToInt64(str));
                            ViewBag.SearchResult += obj != null ? obj.DisplayValue + ", " : "";
                        }
                    }
                    //
                ...
                ids1 = ids1.ToList();
                _T_Education = _T_Education.Where(p => ids1.Contains(p.T_EducationVerificationVendorID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Education");
		   ...
		   if (itemsPerPage != null)
            ...
            if (Request.Cookies["pageSize" + HttpUtility.UrlEncode(User.Name) + "T_Education"] != null)
            {
                pageSize = Convert.ToInt32(Request.Cookies["pageSize" + HttpUtility.UrlEncode(User.Name) + "T_Education"].Value);
                ViewBag.CurrentItemsPerPage = itemsPerPage;
            }
            ...
            {
                pageNumber = 1;
                if (_T_Education.Count() > 0)
                    pageSize = _T_Education.Count();
                return View("ExcelExport", _T_Education.ToPagedList(pageNumber, pageSize));
            }
			else
            {
			 if (pageNumber > 1)
			 {
                var totalListCount = _T_Education.Count();
			...
			if (!Request.IsAjaxRequest())
			{
				...
				var list = _T_Education.ToPagedList(pageNumber, pageSize);
				ViewBag.EntityT_EducationDisplayValue = new SelectList(list.Select(z => new { ID = z.Id, DisplayValue = z.DisplayValue }), "ID", "DisplayValue");
				TempData["T_Educationlist"] = list.Select(z => new { ID = z.Id, DisplayValue = z.DisplayValue });
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        var tagsSplit = GroupByColumn.Split(',').Select(tag => tag.Trim()).Where(tag => !string.IsNullOrEmpty(tag));
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_Education", tagsSplit.ToArray());
                    }
		...
		var entity = "T_Education";
           ...
           var entitylist = db.T_Educations;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_Educations.GroupBy(p => p.t_employeeeducation.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Employee", "Education"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Education", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_Educations.GroupBy(p => p.t_levelofeducation.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "2" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Level Of Education (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Level Of Education"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Level Of Education", "Education"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Education", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_Educations.GroupBy(p => p.t_majorinmulticheckbox.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "3" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Major In (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Major In"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Major In", "Education"));
                chart.Series.Add(CreateSeries(SeriesChartType.Column, ""));
                chart.Series[0].Points.DataBindXY(_xval, _yval);
                chart.Legends.Add(CreateLegend(""));
				 if (inlarge != null)
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Education", new { type = "3", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_Educations.GroupBy(p => p.t_educationverificationvendor.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "4" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Education Verification Vendor (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Education Verification Vendor"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Education Verification Vendor", "Education"));
                chart.Series.Add(CreateSeries(SeriesChartType.Pie, ""));
                chart.Series[0].Points.DataBindXY(_xval, _yval);
                chart.Legends.Add(CreateLegend(""));
				 if (inlarge != null)
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Education", new { type = "4", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_Educations.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_education = jedb.T_Educations.Find(id);
			var T_EducationCommentsIds = jedb.T_Comments.Where(p=>p.T_EducationCommentsID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_Comment" && T_EducationCommentsIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_Educations.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_Educations.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_Education>(_Obj, "T_Education", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_Education> list = db.T_Educations;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_Educations;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_Education), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_Education, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_Education>)q);
				   list = ((IQueryable<T_Education>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_Educations;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_Education>(User,list, "T_Education",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_Education>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_Education), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_Education).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_Education), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Education), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Education), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Education), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Education), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Education), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_Education), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_Education), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_Education", User) || !User.CanAdd("T_Education"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_Education")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_Education").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_Education");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_Education" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Educations";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_EmployeeEducationID":
										 var t_employeeeducationId = db.T_Employees.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_employeeeducationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_LevelOfEducationID":
										 var t_levelofeducationId = db.T_EducationLevels.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_levelofeducationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_MajorInMultiCheckBoxID":
										 var t_majorinmulticheckboxId = db.T_Majors.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_majorinmulticheckboxId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_EducationVerificationVendorID":
										 var t_educationverificationvendorId = db.T_Vendors.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_educationverificationvendorId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                        default:
                                            break;
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_EmployeeEducationID":
										 var strPropertyT_EmployeeEducation = lstEntityProp.FirstOrDefault(p => p.Key == "T_EmployeeEducationID").Value;
										 ModelReflector.Property propT_EmployeeEducation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Employee").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_EmployeeEducation);
										 //var elementTypeT_EmployeeEducation = db.T_Employees.ElementType;
										 //var propertyInfoT_EmployeeEducation = elementTypeT_EmployeeEducation.GetProperty(propT_EmployeeEducation.Name);
										 // var t_employeeeducationId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EmployeeEducation.GetValue(p, null)) == assovalue);
										 var t_employeeeducationId = db.T_Employees.Where(propT_EmployeeEducation.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_employeeeducationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_LevelOfEducationID":
										 var strPropertyT_LevelOfEducation = lstEntityProp.FirstOrDefault(p => p.Key == "T_LevelOfEducationID").Value;
										 ModelReflector.Property propT_LevelOfEducation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_EducationLevel").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_LevelOfEducation);
										 //var elementTypeT_LevelOfEducation = db.T_EducationLevels.ElementType;
										 //var propertyInfoT_LevelOfEducation = elementTypeT_LevelOfEducation.GetProperty(propT_LevelOfEducation.Name);
										 // var t_levelofeducationId = db.T_EducationLevels.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_LevelOfEducation.GetValue(p, null)) == assovalue);
										 var t_levelofeducationId = db.T_EducationLevels.Where(propT_LevelOfEducation.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_levelofeducationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_MajorInMultiCheckBoxID":
										 var strPropertyT_MajorInMultiCheckBox = lstEntityProp.FirstOrDefault(p => p.Key == "T_MajorInMultiCheckBoxID").Value;
										 ModelReflector.Property propT_MajorInMultiCheckBox = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Major").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_MajorInMultiCheckBox);
										 //var elementTypeT_MajorInMultiCheckBox = db.T_Majors.ElementType;
										 //var propertyInfoT_MajorInMultiCheckBox = elementTypeT_MajorInMultiCheckBox.GetProperty(propT_MajorInMultiCheckBox.Name);
										 // var t_majorinmulticheckboxId = db.T_Majors.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_MajorInMultiCheckBox.GetValue(p, null)) == assovalue);
										 var t_majorinmulticheckboxId = db.T_Majors.Where(propT_MajorInMultiCheckBox.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_majorinmulticheckboxId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_EducationVerificationVendorID":
										 var strPropertyT_EducationVerificationVendor = lstEntityProp.FirstOrDefault(p => p.Key == "T_EducationVerificationVendorID").Value;
										 ModelReflector.Property propT_EducationVerificationVendor = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Vendor").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_EducationVerificationVendor);
										 //var elementTypeT_EducationVerificationVendor = db.T_Vendors.ElementType;
										 //var propertyInfoT_EducationVerificationVendor = elementTypeT_EducationVerificationVendor.GetProperty(propT_EducationVerificationVendor.Name);
										 // var t_educationverificationvendorId = db.T_Vendors.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EducationVerificationVendor.GetValue(p, null)) == assovalue);
										 var t_educationverificationvendorId = db.T_Vendors.Where(propT_EducationVerificationVendor.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_educationverificationvendorId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_Education model = new T_Education();
                        ...
                        for (int j = 0; j < sheetColumns.Count; j++)
                        {
                            ...
                            switch (columntable)
                            {
    case "T_DetailsofEducation":
    model.T_DetailsofEducation = columnValue;
	break;
    case "T_DateVerified":
    model.T_DateVerified = DateTime.Parse(columnValue);
	break;
    case "T_EducationNotes":
    model.T_EducationNotes = columnValue;
	break;
					 case "T_EmployeeEducationID":
		 dynamic t_employeeeducationId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_EmployeeEducation = db.T_Employees.ElementType;
			 //var propertyInfoT_EmployeeEducation = elementTypeT_EmployeeEducation.GetProperty(propT_EmployeeEducation.Name);			 
			 //t_employeeeducationId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EmployeeEducation.GetValue(p, null)) == columnValue);
			 t_employeeeducationId = db.T_Employees.Where(propT_EmployeeEducation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_employeeeducationId = db.T_Employees.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_employeeeducationId != null)
			model.T_EmployeeEducationID = t_employeeeducationId.Id;
		  else
			{
				if ((collection["T_EmployeeEducationID"].ToString() == "true,false"))
				{
					try
					{
				 ...
				 try { objT_Employee.T_FirstName =(columnValue); }
						...
						db.SaveChanges();
						model.T_EmployeeEducationID = objT_Employee.Id;
					}
					catch
					{
						continue;
         ...
         break;
		 case "T_LevelOfEducationID":
		 dynamic t_levelofeducationId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 var strPropertyT_LevelOfEducation = lstEntityProp.FirstOrDefault(p => p.Key == "T_LevelOfEducationID").Value;
			 ModelReflector.Property propT_LevelOfEducation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_EducationLevel").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_LevelOfEducation);
			 //var elementTypeT_LevelOfEducation = db.T_EducationLevels.ElementType;
			 //var propertyInfoT_LevelOfEducation = elementTypeT_LevelOfEducation.GetProperty(propT_LevelOfEducation.Name);			 
			 //t_levelofeducationId = db.T_EducationLevels.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_LevelOfEducation.GetValue(p, null)) == columnValue);
			 t_levelofeducationId = db.T_EducationLevels.Where(propT_LevelOfEducation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_levelofeducationId = db.T_EducationLevels.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_levelofeducationId != null)
			model.T_LevelOfEducationID = t_levelofeducationId.Id;
		  else
			{
				if ((collection["T_LevelOfEducationID"].ToString() == "true,false"))
				{
					try
					{
						T_EducationLevel objT_EducationLevel = new T_EducationLevel();
						objT_EducationLevel.T_Name  = (columnValue);
						db.T_EducationLevels.Add(objT_EducationLevel);
						db.SaveChanges();
						model.T_LevelOfEducationID = objT_EducationLevel.Id;
					}
					catch
					{
						continue;
         ...
         break;
		 case "T_MajorInMultiCheckBoxID":
		 dynamic t_majorinmulticheckboxId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 var strPropertyT_MajorInMultiCheckBox = lstEntityProp.FirstOrDefault(p => p.Key == "T_MajorInMultiCheckBoxID").Value;
			 ModelReflector.Property propT_MajorInMultiCheckBox = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Major").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_MajorInMultiCheckBox);
			 //var elementTypeT_MajorInMultiCheckBox = db.T_Majors.ElementType;
			 //var propertyInfoT_MajorInMultiCheckBox = elementTypeT_MajorInMultiCheckBox.GetProperty(propT_MajorInMultiCheckBox.Name);			 
			 //t_majorinmulticheckboxId = db.T_Majors.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_MajorInMultiCheckBox.GetValue(p, null)) == columnValue);
			 t_majorinmulticheckboxId = db.T_Majors.Where(propT_MajorInMultiCheckBox.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_majorinmulticheckboxId = db.T_Majors.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_majorinmulticheckboxId != null)
			model.T_MajorInMultiCheckBoxID = t_majorinmulticheckboxId.Id;
		  else
			{
				if ((collection["T_MajorInMultiCheckBoxID"].ToString() == "true,false"))
				{
					try
					{
						T_Major objT_Major = new T_Major();
						objT_Major.T_Name  = (columnValue);
						db.T_Majors.Add(objT_Major);
						db.SaveChanges();
						model.T_MajorInMultiCheckBoxID = objT_Major.Id;
					}
					catch
					{
						continue;
         ...
         break;
		 case "T_EducationVerificationVendorID":
		 dynamic t_educationverificationvendorId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 var strPropertyT_EducationVerificationVendor = lstEntityProp.FirstOrDefault(p => p.Key == "T_EducationVerificationVendorID").Value;
			 ModelReflector.Property propT_EducationVerificationVendor = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Vendor").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_EducationVerificationVendor);
			 //var elementTypeT_EducationVerificationVendor = db.T_Vendors.ElementType;
			 //var propertyInfoT_EducationVerificationVendor = elementTypeT_EducationVerificationVendor.GetProperty(propT_EducationVerificationVendor.Name);			 
			 //t_educationverificationvendorId = db.T_Vendors.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EducationVerificationVendor.GetValue(p, null)) == columnValue);
			 t_educationverificationvendorId = db.T_Vendors.Where(propT_EducationVerificationVendor.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_educationverificationvendorId = db.T_Vendors.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_educationverificationvendorId != null)
			model.T_EducationVerificationVendorID = t_educationverificationvendorId.Id;
		  else
			{
				if ((collection["T_EducationVerificationVendorID"].ToString() == "true,false"))
				{
					try
					{
						T_Vendor objT_Vendor = new T_Vendor();
						objT_Vendor.T_FirstName  = (columnValue);
						db.T_Vendors.Add(objT_Vendor);
						db.SaveChanges();
						model.T_EducationVerificationVendorID = objT_Vendor.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_Education validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_Educations.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_Education OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_Education").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_Education");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_Education OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_Education");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_Education OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_Education");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_Education OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_Education",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_Education OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_Education");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_Education OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Education").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_Educations.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_Educations;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_Education, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_Education, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_Education>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_Education t_education)
        {
        ...
        {
            string[][] groupsarr = new string[][] { new string[] {"48903","T_Education48903"},new string[] {"48926","Employee Information"},new string[] {"48927","Education Information"},new string[] {"48928","Notes"} };
            return groupsarr;
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_Education t_education)
        {
            t_education.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_education.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_Education"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_Education t_education, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_Education t_education, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
