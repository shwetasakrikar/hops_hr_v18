...
{	
    public partial class T_RoleCodeController : BaseController
    {
		private void LoadViewDataForCount(T_RoleCode t_rolecode, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_RoleCode t_rolecode)
        {		
			 ViewBag.T_RoleCodeSalaryBandID = new SelectList(db.T_SalaryBands, "ID", "DisplayValue", t_rolecode.T_RoleCodeSalaryBandID);
			CustomLoadViewDataListAfterEdit(t_rolecode);
        }
        private void LoadViewDataBeforeOnEdit(T_RoleCode t_rolecode)
        {		
               var _objT_RoleCodeSalaryBand = new List<T_SalaryBand>();
               _objT_RoleCodeSalaryBand.Add(t_rolecode.t_rolecodesalaryband);
			   			   ViewBag.T_RoleCodeSalaryBandID = new SelectList(_objT_RoleCodeSalaryBand, "ID", "DisplayValue", t_rolecode.T_RoleCodeSalaryBandID);
				 ViewBag.T_PositionRoleCodeCount = t_rolecode.t_positionrolecode.Count();
	 ViewBag.T_RoleCodeRequiredEducationCount = t_rolecode.t_rolecoderequirededucation.Count();
		CustomLoadViewDataListBeforeEdit(t_rolecode);
        }
        private void LoadViewDataAfterOnCreate(T_RoleCode t_rolecode)
        {			
			 ViewBag.T_RoleCodeSalaryBandID = new SelectList(db.T_SalaryBands, "ID", "DisplayValue", t_rolecode.T_RoleCodeSalaryBandID);
		CustomLoadViewDataListAfterOnCreate(t_rolecode);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_SalaryBand" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_RoleCodeSalaryBand")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_RoleCodeSalaryBandID = new SelectList(db.T_SalaryBands.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_SalaryBands.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_RoleCodeSalaryBand = new List<T_SalaryBand>();
			 ViewBag.T_RoleCodeSalaryBandID = new SelectList(objT_RoleCodeSalaryBand , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_RoleCode> searchRecords(IQueryable<T_RoleCode> lstT_RoleCode, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_RoleCode = lstT_RoleCode.Where(s => (!String.IsNullOrEmpty(s.T_Code) && s.T_Code.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Title) && s.T_Title.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Role_TitleA) && s.T_Role_TitleA.ToUpper().Contains(searchString)) ||(s.t_rolecodesalaryband!= null && (s.t_rolecodesalaryband.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_RoleCode = lstT_RoleCode.Where(s => (!String.IsNullOrEmpty(s.T_Code) && s.T_Code.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Title) && s.T_Title.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Role_TitleA) && s.T_Role_TitleA.ToUpper().Contains(searchString)) ||(s.t_rolecodesalaryband!= null && (s.t_rolecodesalaryband.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_RoleCode;
        }
		private IQueryable<T_RoleCode> sortRecords(IQueryable<T_RoleCode> lstT_RoleCode, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_RoleCodeSalaryBandID")
                return isAsc.ToLower() == "asc" ? lstT_RoleCode.OrderBy(p => p.t_rolecodesalaryband.DisplayValue) : lstT_RoleCode.OrderByDescending(p => p.t_rolecodesalaryband.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_RoleCode.Sort(sortBy,true) : lstT_RoleCode.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_RoleCode), "t_rolecode");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_RoleCode>)lstT_RoleCode.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_RoleCode.ElementType, lambda.Body.Type },
                    lstT_RoleCode.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_SalaryRange")
            {
                ...
                url += "?search=";
						var T_RoleCodeSalaryBand  = _Object.T_SalaryBandSalaryRangeAssociationID;
						url += "&t_rolecodesalaryband=" + T_RoleCodeSalaryBand;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_rolecodesalaryband, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_RoleCodeSalaryBandlist = db.T_SalaryBands.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_rolecodesalaryband != null)
            {
                var ids = t_rolecodesalaryband.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Salary Band= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_RoleCodeSalaryBandlist.Union(db.T_SalaryBands.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_SalaryBand>(User, list, "T_SalaryBand", null);
					ViewBag.t_rolecodesalaryband = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_RoleCodeSalaryBandlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_SalaryBand>(User, list, "T_SalaryBand", null);
				ViewBag.t_rolecodesalaryband = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_RoleCodeSalaryBand = new List<T_SalaryBand>();
		    ViewBag.t_rolecodesalaryband = new SelectList(objT_RoleCodeSalaryBand, "ID", "DisplayValue");
			}
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_RoleCode()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_RoleCode/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_rolecodesalaryband ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            if (!string.IsNullOrEmpty(searchString) && FSFilter == null)
                ViewBag.FSFilter = "Fsearch";
				 var lstT_RoleCode  = from s in db.T_RoleCodes
                           select s;
            if (!String.IsNullOrEmpty(searchString))
            {
                lstT_RoleCode  = searchRecords(lstT_RoleCode, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_RoleCode = searchRecords(lstT_RoleCode, search,true);
			}
            if (!String.IsNullOrEmpty(sortBy) && !String.IsNullOrEmpty(isAsc))
            {
                lstT_RoleCode  = sortRecords(lstT_RoleCode, sortBy, isAsc);
            }
            else   lstT_RoleCode  = lstT_RoleCode.OrderBy(c=>c.T_Code);
			lstT_RoleCode = CustomSorting(lstT_RoleCode);
			lstT_RoleCode = lstT_RoleCode.Include(t=>t.t_rolecodesalaryband);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_RoleCode", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_RoleCode = lstT_RoleCode.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_RoleCode = Sorting.Sort<T_RoleCode>(lstT_RoleCode, DataOrdering);
            var _T_RoleCode = lstT_RoleCode;
			//if (lstT_RoleCode.Where(p => p.t_rolecodesalaryband != null).Count() <= 50)
		    //ViewBag.t_rolecodesalaryband = new SelectList(lstT_RoleCode.Where(p => p.t_rolecodesalaryband != null).Select(P => P.t_rolecodesalaryband).Distinct(), "ID", "DisplayValue");
            if (t_rolecodesalaryband != null)
            {
                var ids = t_rolecodesalaryband.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_RoleCode = _T_RoleCode.Where(p => ids1.Contains(p.T_RoleCodeSalaryBandID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_RoleCode");
		   ...
		   if (itemsPerPage != null)
            ...
            if (Request.Cookies["pageSize" + HttpUtility.UrlEncode(User.Name) + "T_RoleCode"] != null)
            {
                pageSize = Convert.ToInt32(Request.Cookies["pageSize" + HttpUtility.UrlEncode(User.Name) + "T_RoleCode"].Value);
                ViewBag.CurrentItemsPerPage = itemsPerPage;
            }
            ...
            {
                pageNumber = 1;
                if (_T_RoleCode.Count() > 0)
                    pageSize = _T_RoleCode.Count();
                return View("ExcelExport", _T_RoleCode.ToPagedList(pageNumber, pageSize));
            }
			else
            {
			 if (pageNumber > 1)
			 {
                var totalListCount = _T_RoleCode.Count();
			...
			if (!Request.IsAjaxRequest())
			{
				...
				var list = _T_RoleCode.ToPagedList(pageNumber, pageSize);
				ViewBag.EntityT_RoleCodeDisplayValue = new SelectList(list.Select(z => new { ID = z.Id, DisplayValue = z.DisplayValue }), "ID", "DisplayValue");
				TempData["T_RoleCodelist"] = list.Select(z => new { ID = z.Id, DisplayValue = z.DisplayValue });
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        var tagsSplit = GroupByColumn.Split(',').Select(tag => tag.Trim()).Where(tag => !string.IsNullOrEmpty(tag));
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_RoleCode", tagsSplit.ToArray());
                    }
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_RoleCodes.GroupBy(p => p.t_rolecodesalaryband.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Salary Band", "Role Code"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_RoleCode", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_RoleCodes.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_rolecode = jedb.T_RoleCodes.Find(id);
			var T_PositionRoleCodeIds = jedb.T_Positions.Where(p=>p.T_PositionRoleCodeID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_Position" && T_PositionRoleCodeIds.Contains(d.RecordId) && d.Type != "View"));
			var T_RoleCodeRequiredEducationIds = jedb.T_RequiredEducations.Where(p=>p.T_RoleCodeRequiredEducationID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_RequiredEducation" && T_RoleCodeRequiredEducationIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_RoleCodes.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_RoleCodes.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_RoleCode>(_Obj, "T_RoleCode", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_RoleCode> list = db.T_RoleCodes;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_RoleCodes;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_RoleCode), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_RoleCode, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_RoleCode>)q);
				   list = ((IQueryable<T_RoleCode>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_RoleCodes;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_RoleCode>(User,list, "T_RoleCode",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_RoleCode>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_RoleCode), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_RoleCode).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_RoleCode), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_RoleCode), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_RoleCode), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_RoleCode), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_RoleCode), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_RoleCode), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_RoleCode), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_RoleCode), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_RoleCode", User) || !User.CanAdd("T_RoleCode"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_RoleCode")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_RoleCode").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_RoleCode");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_RoleCode" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Role Codes";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_RoleCodeSalaryBandID":
										 var t_rolecodesalarybandId = db.T_SalaryBands.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_rolecodesalarybandId == null)
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_RoleCodeSalaryBandID":
										 var strPropertyT_RoleCodeSalaryBand = lstEntityProp.FirstOrDefault(p => p.Key == "T_RoleCodeSalaryBandID").Value;
										 ModelReflector.Property propT_RoleCodeSalaryBand = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_SalaryBand").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_RoleCodeSalaryBand);
										 //var elementTypeT_RoleCodeSalaryBand = db.T_SalaryBands.ElementType;
										 //var propertyInfoT_RoleCodeSalaryBand = elementTypeT_RoleCodeSalaryBand.GetProperty(propT_RoleCodeSalaryBand.Name);
										 // var t_rolecodesalarybandId = db.T_SalaryBands.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_RoleCodeSalaryBand.GetValue(p, null)) == assovalue);
										 var t_rolecodesalarybandId = db.T_SalaryBands.Where(propT_RoleCodeSalaryBand.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_rolecodesalarybandId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_RoleCode model = new T_RoleCode();
	...
	break;
    case "T_Role_TitleA":
    model.T_Role_TitleA = columnValue;
	break;
					 case "T_RoleCodeSalaryBandID":
		 dynamic t_rolecodesalarybandId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_RoleCodeSalaryBand = db.T_SalaryBands.ElementType;
			 //var propertyInfoT_RoleCodeSalaryBand = elementTypeT_RoleCodeSalaryBand.GetProperty(propT_RoleCodeSalaryBand.Name);			 
			 //t_rolecodesalarybandId = db.T_SalaryBands.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_RoleCodeSalaryBand.GetValue(p, null)) == columnValue);
			 t_rolecodesalarybandId = db.T_SalaryBands.Where(propT_RoleCodeSalaryBand.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_rolecodesalarybandId = db.T_SalaryBands.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_rolecodesalarybandId != null)
			model.T_RoleCodeSalaryBandID = t_rolecodesalarybandId.Id;
		  else
			{
				if ((collection["T_RoleCodeSalaryBandID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_RoleCodeSalaryBandID = objT_SalaryBand.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_RoleCode validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_RoleCodes.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_RoleCode OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_RoleCode").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_RoleCode");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_RoleCode OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_RoleCode");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_RoleCode OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_RoleCode");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_RoleCode OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_RoleCode",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_RoleCode OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_RoleCode");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_RoleCode OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_RoleCode").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_RoleCodes.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_RoleCodes;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_RoleCode, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_RoleCode, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_RoleCode>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_RoleCode t_rolecode)
        {
        ...
        {
            string[][] groupsarr = new string[][] { new string[] {"48890","T_RoleCode48890"} };
            return groupsarr;
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_RoleCode t_rolecode)
        {
            t_rolecode.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_rolecode.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_RoleCode t_rolecode, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_RoleCode t_rolecode, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
