...
{	
    ...
    {
		private void LoadViewDataForCount(T_PayDetails t_paydetails, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_PayDetails t_paydetails)
        {		
			 ViewBag.T_EmployeePayDetailsID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_paydetails.T_EmployeePayDetailsID);
			 ViewBag.T_PayDetailsJobAssignmentID = new SelectList(db.T_JobAssignments, "ID", "DisplayValue", t_paydetails.T_PayDetailsJobAssignmentID);
			 ViewBag.T_OtherPayDetailsTypeID = new SelectList(db.T_PayTypes, "ID", "DisplayValue", t_paydetails.T_OtherPayDetailsTypeID);
			CustomLoadViewDataListAfterEdit(t_paydetails);
        }
        private void LoadViewDataBeforeOnEdit(T_PayDetails t_paydetails)
        {		
               var _objT_EmployeePayDetails = new List<T_Employee>();
               _objT_EmployeePayDetails.Add(t_paydetails.t_employeepaydetails);
			   			   ViewBag.T_EmployeePayDetailsID = new SelectList(_objT_EmployeePayDetails, "ID", "DisplayValue", t_paydetails.T_EmployeePayDetailsID);
			               var _objT_PayDetailsJobAssignment = new List<T_JobAssignment>();
               _objT_PayDetailsJobAssignment.Add(t_paydetails.t_paydetailsjobassignment);
			   			   ViewBag.T_PayDetailsJobAssignmentID = new SelectList(_objT_PayDetailsJobAssignment, "ID", "DisplayValue", t_paydetails.T_PayDetailsJobAssignmentID);
			               var _objT_OtherPayDetailsType = new List<T_PayType>();
               _objT_OtherPayDetailsType.Add(t_paydetails.t_otherpaydetailstype);
			   			   ViewBag.T_OtherPayDetailsTypeID = new SelectList(_objT_OtherPayDetailsType, "ID", "DisplayValue", t_paydetails.T_OtherPayDetailsTypeID);
				 ViewBag.T_SalaryCommentsCount = t_paydetails.t_salarycomments.Count();
		CustomLoadViewDataListBeforeEdit(t_paydetails);
        }
        private void LoadViewDataAfterOnCreate(T_PayDetails t_paydetails)
        {			
			 ViewBag.T_EmployeePayDetailsID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_paydetails.T_EmployeePayDetailsID);
			 ViewBag.T_PayDetailsJobAssignmentID = new SelectList(db.T_JobAssignments, "ID", "DisplayValue", t_paydetails.T_PayDetailsJobAssignmentID);
			 ViewBag.T_OtherPayDetailsTypeID = new SelectList(db.T_PayTypes, "ID", "DisplayValue", t_paydetails.T_OtherPayDetailsTypeID);
		CustomLoadViewDataListAfterOnCreate(t_paydetails);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Employee" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_EmployeePayDetails")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_EmployeePayDetailsID = new SelectList(db.T_Employees.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Employees.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_EmployeePayDetails = new List<T_Employee>();
			 ViewBag.T_EmployeePayDetailsID = new SelectList(objT_EmployeePayDetails , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_JobAssignment" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_PayDetailsJobAssignment")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_PayDetailsJobAssignmentID = new SelectList(db.T_JobAssignments.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_JobAssignments.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_PayDetailsJobAssignment = new List<T_JobAssignment>();
			 ViewBag.T_PayDetailsJobAssignmentID = new SelectList(objT_PayDetailsJobAssignment , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_PayType" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_OtherPayDetailsType")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_OtherPayDetailsTypeID = new SelectList(db.T_PayTypes.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_PayTypes.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_OtherPayDetailsType = new List<T_PayType>();
			 ViewBag.T_OtherPayDetailsTypeID = new SelectList(objT_OtherPayDetailsType , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_PayDetails> searchRecords(IQueryable<T_PayDetails> lstT_PayDetails, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_PayDetails = lstT_PayDetails.Where(s => (s.t_employeepaydetails!= null && (s.t_employeepaydetails.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_paydetailsjobassignment!= null && (s.t_paydetailsjobassignment.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_otherpaydetailstype!= null && (s.t_otherpaydetailstype.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_Notes) && s.T_Notes.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_PayDetails = lstT_PayDetails.Where(s => (s.t_employeepaydetails!= null && (s.t_employeepaydetails.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_paydetailsjobassignment!= null && (s.t_paydetailsjobassignment.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_otherpaydetailstype!= null && (s.t_otherpaydetailstype.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
			try
            {
                var datevalue = Convert.ToDateTime(searchString);
                lstT_PayDetails = lstT_PayDetails.Union(db.T_PayDetailss.Where(s => (s.T_StartDate == datevalue) ||(s.T_EndDate == datevalue) ));
            }
            catch { }
			try
            {
                var decimalvalue = Convert.ToDecimal(searchString);
                lstT_PayDetails = lstT_PayDetails.Union(db.T_PayDetailss.Where(s => (s.T_Amount == decimalvalue) ));
            }
            catch { }
            return lstT_PayDetails;
        }
		private IQueryable<T_PayDetails> sortRecords(IQueryable<T_PayDetails> lstT_PayDetails, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_EmployeePayDetailsID")
                return isAsc.ToLower() == "asc" ? lstT_PayDetails.OrderBy(p => p.t_employeepaydetails.DisplayValue) : lstT_PayDetails.OrderByDescending(p => p.t_employeepaydetails.DisplayValue);
	 if(sortBy == "T_PayDetailsJobAssignmentID")
                return isAsc.ToLower() == "asc" ? lstT_PayDetails.OrderBy(p => p.t_paydetailsjobassignment.DisplayValue) : lstT_PayDetails.OrderByDescending(p => p.t_paydetailsjobassignment.DisplayValue);
	 if(sortBy == "T_OtherPayDetailsTypeID")
                return isAsc.ToLower() == "asc" ? lstT_PayDetails.OrderBy(p => p.t_otherpaydetailstype.DisplayValue) : lstT_PayDetails.OrderByDescending(p => p.t_otherpaydetailstype.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_PayDetails.Sort(sortBy,true) : lstT_PayDetails.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_PayDetails), "t_paydetails");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_PayDetails>)lstT_PayDetails.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_PayDetails.ElementType, lambda.Body.Type },
                    lstT_PayDetails.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "FileDocument")
            {
                ...
                url += "?search=";
						var T_EmployeePayDetails  = _Object.T_EmployeeDocumentsID;
						url += "&t_employeepaydetails=" + T_EmployeePayDetails;									
            ...
            if (sourceEntity == "T_Licenses")
            {
                ...
                url += "?search=";
						var T_EmployeePayDetails  = _Object.T_LicenseRecordsID;
						url += "&t_employeepaydetails=" + T_EmployeePayDetails;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_ServiceRecord")
            {
                ...
                url += "?search=";
						var T_EmployeePayDetails  = _Object.T_EmployeeEmploymentProfileID;
						url += "&t_employeepaydetails=" + T_EmployeePayDetails;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Comment")
            {
                ...
                url += "?search=";
						var T_EmployeePayDetails  = _Object.T_EmployeeCommentsID;
						url += "&t_employeepaydetails=" + T_EmployeePayDetails;									
						var T_PayDetailsJobAssignment  = _Object.T_JobAssignmentCommentsID;
						url += "&t_paydetailsjobassignment=" + T_PayDetailsJobAssignment;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DrugAlcoholTest")
            {
                ...
                url += "?search=";
						var T_EmployeePayDetails  = _Object.T_EmployeeDrugAlcoholTestID;
						url += "&t_employeepaydetails=" + T_EmployeePayDetails;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_EmployeePayDetails  = _Object.T_EmployeeAdministratorID;
						url += "&t_employeepaydetails=" + T_EmployeePayDetails;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_JobAssignment")
            {
                ...
                url += "?search=";
						var T_EmployeePayDetails  = _Object.T_EmployeeJobAssignmentID;
						url += "&t_employeepaydetails=" + T_EmployeePayDetails;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_LeaveProfile")
            {
                ...
                url += "?search=";
						var T_EmployeePayDetails  = _Object.T_EmployeeLeaveProfileID;
						url += "&t_employeepaydetails=" + T_EmployeePayDetails;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_EmployeeInjury")
            {
                ...
                url += "?search=";
						var T_EmployeePayDetails  = _Object.T_EmployeeEmployeeInjuryID;
						url += "&t_employeepaydetails=" + T_EmployeePayDetails;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_BackgroundCheck")
            {
                ...
                url += "?search=";
						var T_EmployeePayDetails  = _Object.T_EmployeeCriminalBackgroundCheckID;
						url += "&t_employeepaydetails=" + T_EmployeePayDetails;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Education")
            {
                ...
                url += "?search=";
						var T_EmployeePayDetails  = _Object.T_EmployeeEducationID;
						url += "&t_employeepaydetails=" + T_EmployeePayDetails;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Accommodation")
            {
                ...
                url += "?search=";
						var T_EmployeePayDetails  = _Object.T_EmployeeAccomodationID;
						url += "&t_employeepaydetails=" + T_EmployeePayDetails;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_employeepaydetails,string t_paydetailsjobassignment,string t_otherpaydetailstype, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_EmployeePayDetailslist = db.T_Employees.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_employeepaydetails != null)
            {
                var ids = t_employeepaydetails.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Employee= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_EmployeePayDetailslist.Union(db.T_Employees.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
					ViewBag.t_employeepaydetails = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_EmployeePayDetailslist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
				ViewBag.t_employeepaydetails = new SelectList(list, "ID", "DisplayValue");
			}
			var T_PayDetailsJobAssignmentlist = db.T_JobAssignments.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_paydetailsjobassignment != null)
            {
                var ids = t_paydetailsjobassignment.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Job Assignment= ";
				   foreach (var str in ids)
                    {
					...
					ids1 = ids1.ToList();
					var list = T_PayDetailsJobAssignmentlist.Union(db.T_JobAssignments.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_JobAssignment>(User, list, "T_JobAssignment", null);
					ViewBag.t_paydetailsjobassignment = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_PayDetailsJobAssignmentlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_JobAssignment>(User, list, "T_JobAssignment", null);
				ViewBag.t_paydetailsjobassignment = new SelectList(list, "ID", "DisplayValue");
			}
			var T_OtherPayDetailsTypelist = db.T_PayTypes.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_otherpaydetailstype != null)
            {
                var ids = t_otherpaydetailstype.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Pay Type= ";
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            {
                               ids1.Add(Convert.ToInt64(str));
							   ViewBag.SearchResult += db.T_PayTypes.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_OtherPayDetailsTypelist.Union(db.T_PayTypes.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_PayType>(User, list, "T_PayType", null);
					ViewBag.t_otherpaydetailstype = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_OtherPayDetailsTypelist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_PayType>(User, list, "T_PayType", null);
				ViewBag.t_otherpaydetailstype = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_EmployeePayDetails = new List<T_Employee>();
		    ViewBag.t_employeepaydetails = new SelectList(objT_EmployeePayDetails, "ID", "DisplayValue");
			var objT_PayDetailsJobAssignment = new List<T_JobAssignment>();
		    ViewBag.t_paydetailsjobassignment = new SelectList(objT_PayDetailsJobAssignment, "ID", "DisplayValue");
			var objT_OtherPayDetailsType = new List<T_PayType>();
		    ViewBag.t_otherpaydetailstype = new SelectList(objT_OtherPayDetailsType, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_PayDetails";
			...
			columns.Add("3", "Job Assignment");
			columns.Add("4", "Pay Type");
			columns.Add("5", "Amount");
			columns.Add("6", "Start Date");
			columns.Add("7", "End Date");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				return View(new T_PayDetails());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_PayDetails", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_PayDetails", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_PayDetails()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_PayDetails/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_employeepaydetails,string t_paydetailsjobassignment,string t_otherpaydetailstype ,string T_AmountFrom,string T_AmountTo,string T_StartDateFrom,string T_StartDateTo,string T_EndDateFrom,string T_EndDateTo,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_PayDetails  = searchRecords(lstT_PayDetails, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_PayDetails = searchRecords(lstT_PayDetails, search,true);
			}
			...
			lstT_PayDetails = lstT_PayDetails.Include(t=>t.t_employeepaydetails).Include(t=>t.t_paydetailsjobassignment).Include(t=>t.t_otherpaydetailstype);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_PayDetails", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_PayDetails = lstT_PayDetails.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_PayDetails = Sorting.Sort<T_PayDetails>(lstT_PayDetails, DataOrdering);
            var _T_PayDetails = lstT_PayDetails;
		 if(T_AmountFrom!=null || T_AmountTo !=null)
		 {
                try
                {
                    decimal from = T_AmountFrom == null ? 0 : Convert.ToDecimal(T_AmountFrom);
                    decimal to =  T_AmountTo == null ? decimal.MaxValue : Convert.ToDecimal(T_AmountTo);
									     _T_PayDetails =  _T_PayDetails.Where(o => o.T_Amount >= from && o.T_Amount <= to);
                   					ViewBag.SearchResult += "\r\n Amount= "+from+"-"+to;
                }
                catch { }
          }
				if(T_StartDateFrom!=null || T_StartDateTo !=null)
				{
						try
						{
							...
							DateTime to = T_StartDateTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_StartDateTo).AddTicks(-1).AddDays(1);
							                       _T_PayDetails =  _T_PayDetails.Where(o =>o.T_StartDate!=null && o.T_StartDate.CompareTo(from) >= 0 && o.T_StartDate.CompareTo(to) <= 0);
				...
				if(T_EndDateFrom!=null || T_EndDateTo !=null)
				{
						try
						{
							...
							DateTime to = T_EndDateTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_EndDateTo).AddTicks(-1).AddDays(1);
							                       _T_PayDetails =  _T_PayDetails.Where(o =>o.T_EndDate!=null && o.T_EndDate.Value.CompareTo(from) >= 0 && o.T_EndDate.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n End Date= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
			//if (lstT_PayDetails.Where(p => p.t_employeepaydetails != null).Count() <= 50)
		    //ViewBag.t_employeepaydetails = new SelectList(lstT_PayDetails.Where(p => p.t_employeepaydetails != null).Select(P => P.t_employeepaydetails).Distinct(), "ID", "DisplayValue");
            if (t_employeepaydetails != null)
            {
                var ids = t_employeepaydetails.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_PayDetails = _T_PayDetails.Where(p => ids1.Contains(p.T_EmployeePayDetailsID));
            }
				//if (lstT_PayDetails.Where(p => p.t_paydetailsjobassignment != null).Count() <= 50)
		    //ViewBag.t_paydetailsjobassignment = new SelectList(lstT_PayDetails.Where(p => p.t_paydetailsjobassignment != null).Select(P => P.t_paydetailsjobassignment).Distinct(), "ID", "DisplayValue");
            if (t_paydetailsjobassignment != null)
            {
                var ids = t_paydetailsjobassignment.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_PayDetails = _T_PayDetails.Where(p => ids1.Contains(p.T_PayDetailsJobAssignmentID));
            }
				//if (lstT_PayDetails.Where(p => p.t_otherpaydetailstype != null).Count() <= 50)
		    //ViewBag.t_otherpaydetailstype = new SelectList(lstT_PayDetails.Where(p => p.t_otherpaydetailstype != null).Select(P => P.t_otherpaydetailstype).Distinct(), "ID", "DisplayValue");
            if (t_otherpaydetailstype != null)
            {
                var ids = t_otherpaydetailstype.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_PayTypes.Find(Convert.ToInt64(str));
                            ViewBag.SearchResult += obj != null ? obj.DisplayValue + ", " : "";
                        }
                    }
                    //
                ...
                ids1 = ids1.ToList();
                _T_PayDetails = _T_PayDetails.Where(p => ids1.Contains(p.T_OtherPayDetailsTypeID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_PayDetails");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_PayDetails", tagsSplit.ToArray());
                    }
		...
		var entity = "T_PayDetails";
           ...
           var entitylist = db.T_PayDetailss;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_PayDetailss.GroupBy(p => p.t_employeepaydetails.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Employee", "Pay Details "));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_PayDetails", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
			...
			if (type == "2" || type == "all")
                {
			var cntgrt10 = false;
                    var _yvalT_Amount = gd.Select(k => k.Sum(p=>p.T_Amount)).ToList();
                    if (_yvalT_Amount .Count > 10 && inlarge == null)
                    {
                        _yvalT_Amount = gd.Select(k => k.Sum(p=>p.T_Amount)).Take(10).ToList();
                        cntgrt10 = true;
                    }
			 var chartT_Amount  = new System.Web.UI.DataVisualization.Charting.Chart
                {
                    //BackColor = System.Drawing.Color.FromArgb(211, 223, 240),
                    BorderlineDashStyle = ChartDashStyle.Solid,
                    BackSecondaryColor = System.Drawing.Color.White,
                ...
                chartT_Amount.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chartT_Amount.Titles.Add(CreateTitle("Total of Amount by Employee (Top 10)"));
				else
				chartT_Amount.Titles.Add(CreateTitle("Total of Amount by Employee"));
                chartT_Amount.ChartAreas.Add(CreateChartArea(SeriesChartType.Column,"", "Employee", "T_Amount"));
                chartT_Amount.Series.Add(CreateSeries(SeriesChartType.Column, ""));
                chartT_Amount.Series[0].Points.DataBindXY(_xval, _yvalT_Amount );
                chartT_Amount.Legends.Add(CreateLegend(""));
				if (inlarge != null)
                    {
                        chartT_Amount.Width = 800;
                        chartT_Amount.Height = 800;
                    }
                byte[] bytesT_Amount;
                using (var chartimage = new MemoryStream())
                {
                    chartT_Amount.SaveImage(chartimage, ChartImageFormat.Png);
                    bytesT_Amount = chartimage.GetBuffer();
                }
				 if (cntgrt10)
                    {
                        string imgT_Amount = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_PayDetails", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encodedT_Amount = Convert.ToBase64String(bytesT_Amount.ToArray());
                        result += String.Format(imgT_Amount, encodedT_Amount);
                    }
                    else
                    {
                        string imgT_Amount = "<div class=" + (inlarge == null ? "col-sm-4" : "col-sm-12") + " style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%;' /></div>";
                        string encodedT_Amount = Convert.ToBase64String(bytesT_Amount.ToArray());
                        result += String.Format(imgT_Amount, encodedT_Amount);
                    }
				}
           ...
           {
                var gd = db.T_PayDetailss.GroupBy(p => p.t_paydetailsjobassignment.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "3" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Job Assignment (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Job Assignment"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Job Assignment", "Pay Details "));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_PayDetails", new { type = "3", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
			...
			var cntgrt10 = false;
			 ...
			 var chartT_Amount  = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chartT_Amount.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chartT_Amount.Titles.Add(CreateTitle("Total of Amount by Job Assignment (Top 10)"));
				else
				chartT_Amount.Titles.Add(CreateTitle("Total of Amount by Job Assignment"));
                chartT_Amount.ChartAreas.Add(CreateChartArea(SeriesChartType.Column,"", "Job Assignment", "T_Amount"));
                chartT_Amount.Series.Add(CreateSeries(SeriesChartType.Column, ""));
                chartT_Amount.Series[0].Points.DataBindXY(_xval, _yvalT_Amount );
                chartT_Amount.Legends.Add(CreateLegend(""));
				if (inlarge != null)
                ...
                using (var chartimage = new MemoryStream())
                {
				 ...
				 if (cntgrt10)
                    {
                        string imgT_Amount = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_PayDetails", new { type = "4", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encodedT_Amount = Convert.ToBase64String(bytesT_Amount.ToArray());
                        result += String.Format(imgT_Amount, encodedT_Amount);
                    }
                    else
           ...
           {
                var gd = db.T_PayDetailss.GroupBy(p => p.t_otherpaydetailstype.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "5" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Pay Type (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Pay Type"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Pay Type", "Pay Details "));
                chart.Series.Add(CreateSeries(SeriesChartType.Column, ""));
                chart.Series[0].Points.DataBindXY(_xval, _yval);
                chart.Legends.Add(CreateLegend(""));
				 if (inlarge != null)
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_PayDetails", new { type = "5", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
			...
			var cntgrt10 = false;
			 ...
			 var chartT_Amount  = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chartT_Amount.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chartT_Amount.Titles.Add(CreateTitle("Total of Amount by Pay Type (Top 10)"));
				else
				chartT_Amount.Titles.Add(CreateTitle("Total of Amount by Pay Type"));
                chartT_Amount.ChartAreas.Add(CreateChartArea(SeriesChartType.Column,"", "Pay Type", "T_Amount"));
                chartT_Amount.Series.Add(CreateSeries(SeriesChartType.Column, ""));
                chartT_Amount.Series[0].Points.DataBindXY(_xval, _yvalT_Amount );
                chartT_Amount.Legends.Add(CreateLegend(""));
				if (inlarge != null)
                ...
                using (var chartimage = new MemoryStream())
                {
				 ...
				 if (cntgrt10)
                    {
                        string imgT_Amount = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_PayDetails", new { type = "6", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encodedT_Amount = Convert.ToBase64String(bytesT_Amount.ToArray());
                        result += String.Format(imgT_Amount, encodedT_Amount);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_PayDetailss.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_paydetails = jedb.T_PayDetailss.Find(id);
			var T_SalaryCommentsIds = jedb.T_Comments.Where(p=>p.T_SalaryCommentsID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_Comment" && T_SalaryCommentsIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_PayDetailss.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_PayDetailss.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_PayDetails>(_Obj, "T_PayDetails", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_PayDetails> list = db.T_PayDetailss;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_PayDetailss;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_PayDetails), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_PayDetails, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_PayDetails>)q);
				   list = ((IQueryable<T_PayDetails>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_PayDetailss;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_PayDetails>(User,list, "T_PayDetails",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_PayDetails>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_PayDetails), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_PayDetails).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_PayDetails), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_PayDetails), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_PayDetails), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_PayDetails), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_PayDetails), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_PayDetails), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_PayDetails), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_PayDetails), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_PayDetails", User) || !User.CanAdd("T_PayDetails"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_PayDetails")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_PayDetails").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_PayDetails");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_PayDetails" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Pay Details s";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_EmployeePayDetailsID":
										 var t_employeepaydetailsId = db.T_Employees.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_employeepaydetailsId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_PayDetailsJobAssignmentID":
										 var t_paydetailsjobassignmentId = db.T_JobAssignments.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_paydetailsjobassignmentId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_OtherPayDetailsTypeID":
										 var t_otherpaydetailstypeId = db.T_PayTypes.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_otherpaydetailstypeId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                        default:
                                            break;
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_EmployeePayDetailsID":
										 var strPropertyT_EmployeePayDetails = lstEntityProp.FirstOrDefault(p => p.Key == "T_EmployeePayDetailsID").Value;
										 ModelReflector.Property propT_EmployeePayDetails = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Employee").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_EmployeePayDetails);
										 //var elementTypeT_EmployeePayDetails = db.T_Employees.ElementType;
										 //var propertyInfoT_EmployeePayDetails = elementTypeT_EmployeePayDetails.GetProperty(propT_EmployeePayDetails.Name);
										 // var t_employeepaydetailsId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EmployeePayDetails.GetValue(p, null)) == assovalue);
										 var t_employeepaydetailsId = db.T_Employees.Where(propT_EmployeePayDetails.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_employeepaydetailsId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_PayDetailsJobAssignmentID":
										 var strPropertyT_PayDetailsJobAssignment = lstEntityProp.FirstOrDefault(p => p.Key == "T_PayDetailsJobAssignmentID").Value;
										 ModelReflector.Property propT_PayDetailsJobAssignment = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_JobAssignment").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_PayDetailsJobAssignment);
										 //var elementTypeT_PayDetailsJobAssignment = db.T_JobAssignments.ElementType;
										 //var propertyInfoT_PayDetailsJobAssignment = elementTypeT_PayDetailsJobAssignment.GetProperty(propT_PayDetailsJobAssignment.Name);
										 // var t_paydetailsjobassignmentId = db.T_JobAssignments.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_PayDetailsJobAssignment.GetValue(p, null)) == assovalue);
										 var t_paydetailsjobassignmentId = db.T_JobAssignments.Where(propT_PayDetailsJobAssignment.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_paydetailsjobassignmentId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_OtherPayDetailsTypeID":
										 var strPropertyT_OtherPayDetailsType = lstEntityProp.FirstOrDefault(p => p.Key == "T_OtherPayDetailsTypeID").Value;
										 ModelReflector.Property propT_OtherPayDetailsType = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_PayType").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_OtherPayDetailsType);
										 //var elementTypeT_OtherPayDetailsType = db.T_PayTypes.ElementType;
										 //var propertyInfoT_OtherPayDetailsType = elementTypeT_OtherPayDetailsType.GetProperty(propT_OtherPayDetailsType.Name);
										 // var t_otherpaydetailstypeId = db.T_PayTypes.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_OtherPayDetailsType.GetValue(p, null)) == assovalue);
										 var t_otherpaydetailstypeId = db.T_PayTypes.Where(propT_OtherPayDetailsType.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_otherpaydetailstypeId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_PayDetails model = new T_PayDetails();
                        ...
                        for (int j = 0; j < sheetColumns.Count; j++)
                        {
                            ...
                            {
    case "T_Amount":
    model.T_Amount = Decimal.Parse(columnValue);
	break;
    case "T_StartDate":
	...
	break;
					 case "T_EmployeePayDetailsID":
		 dynamic t_employeepaydetailsId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_EmployeePayDetails = db.T_Employees.ElementType;
			 //var propertyInfoT_EmployeePayDetails = elementTypeT_EmployeePayDetails.GetProperty(propT_EmployeePayDetails.Name);			 
			 //t_employeepaydetailsId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EmployeePayDetails.GetValue(p, null)) == columnValue);
			 t_employeepaydetailsId = db.T_Employees.Where(propT_EmployeePayDetails.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_employeepaydetailsId = db.T_Employees.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_employeepaydetailsId != null)
			model.T_EmployeePayDetailsID = t_employeepaydetailsId.Id;
		  else
			{
				if ((collection["T_EmployeePayDetailsID"].ToString() == "true,false"))
				{
					try
					{
				 ...
				 try { objT_Employee.T_FirstName =(columnValue); }
						...
						db.SaveChanges();
						model.T_EmployeePayDetailsID = objT_Employee.Id;
					}
					catch
					{
						continue;
         ...
         break;
		 case "T_PayDetailsJobAssignmentID":
		 dynamic t_paydetailsjobassignmentId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 var strPropertyT_PayDetailsJobAssignment = lstEntityProp.FirstOrDefault(p => p.Key == "T_PayDetailsJobAssignmentID").Value;
			 ModelReflector.Property propT_PayDetailsJobAssignment = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_JobAssignment").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_PayDetailsJobAssignment);
			 //var elementTypeT_PayDetailsJobAssignment = db.T_JobAssignments.ElementType;
			 //var propertyInfoT_PayDetailsJobAssignment = elementTypeT_PayDetailsJobAssignment.GetProperty(propT_PayDetailsJobAssignment.Name);			 
			 //t_paydetailsjobassignmentId = db.T_JobAssignments.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_PayDetailsJobAssignment.GetValue(p, null)) == columnValue);
			 t_paydetailsjobassignmentId = db.T_JobAssignments.Where(propT_PayDetailsJobAssignment.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_paydetailsjobassignmentId = db.T_JobAssignments.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_paydetailsjobassignmentId != null)
			model.T_PayDetailsJobAssignmentID = t_paydetailsjobassignmentId.Id;
         break;
		 case "T_OtherPayDetailsTypeID":
		 dynamic t_otherpaydetailstypeId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 var strPropertyT_OtherPayDetailsType = lstEntityProp.FirstOrDefault(p => p.Key == "T_OtherPayDetailsTypeID").Value;
			 ModelReflector.Property propT_OtherPayDetailsType = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_PayType").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_OtherPayDetailsType);
			 //var elementTypeT_OtherPayDetailsType = db.T_PayTypes.ElementType;
			 //var propertyInfoT_OtherPayDetailsType = elementTypeT_OtherPayDetailsType.GetProperty(propT_OtherPayDetailsType.Name);			 
			 //t_otherpaydetailstypeId = db.T_PayTypes.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_OtherPayDetailsType.GetValue(p, null)) == columnValue);
			 t_otherpaydetailstypeId = db.T_PayTypes.Where(propT_OtherPayDetailsType.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_otherpaydetailstypeId = db.T_PayTypes.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_otherpaydetailstypeId != null)
			model.T_OtherPayDetailsTypeID = t_otherpaydetailstypeId.Id;
		  else
			{
				if ((collection["T_OtherPayDetailsTypeID"].ToString() == "true,false"))
				{
					try
					{
						T_PayType objT_PayType = new T_PayType();
						objT_PayType.T_Name  = (columnValue);
						db.T_PayTypes.Add(objT_PayType);
						db.SaveChanges();
						model.T_OtherPayDetailsTypeID = objT_PayType.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_PayDetails validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_PayDetailss.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_PayDetails OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_PayDetails").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_PayDetails");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_PayDetails OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_PayDetails");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_PayDetails OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_PayDetails");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_PayDetails OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_PayDetails",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_PayDetails OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_PayDetails");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_PayDetails OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_PayDetails").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_PayDetailss.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_PayDetailss;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_PayDetails, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_PayDetails, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_PayDetails>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_PayDetails t_paydetails)
        {
        ...
        {
            string[][] groupsarr = new string[][] { new string[] {"48903","T_PayDetails48903"},new string[] {"48932","Employee Information"},new string[] {"48933","Pay Details"},new string[] {"48934","Notes"} };
            return groupsarr;
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_PayDetails t_paydetails)
        {
            t_paydetails.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_paydetails.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_PayDetails"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_PayDetails t_paydetails, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_PayDetails t_paydetails, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
