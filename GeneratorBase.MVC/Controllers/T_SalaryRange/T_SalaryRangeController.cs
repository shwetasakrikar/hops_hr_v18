...
{	
    ...
    {
		private void LoadViewDataForCount(T_SalaryRange t_salaryrange, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_SalaryRange t_salaryrange)
        {		
			 ViewBag.T_SalaryBandSalaryRangeAssociationID = new SelectList(db.T_SalaryBands, "ID", "DisplayValue", t_salaryrange.T_SalaryBandSalaryRangeAssociationID);
			 ViewBag.T_FacilitySalaryRangeAssociationID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_salaryrange.T_FacilitySalaryRangeAssociationID);
			CustomLoadViewDataListAfterEdit(t_salaryrange);
        }
        private void LoadViewDataBeforeOnEdit(T_SalaryRange t_salaryrange)
        {		
               var _objT_SalaryBandSalaryRangeAssociation = new List<T_SalaryBand>();
               _objT_SalaryBandSalaryRangeAssociation.Add(t_salaryrange.t_salarybandsalaryrangeassociation);
			   			   ViewBag.T_SalaryBandSalaryRangeAssociationID = new SelectList(_objT_SalaryBandSalaryRangeAssociation, "ID", "DisplayValue", t_salaryrange.T_SalaryBandSalaryRangeAssociationID);
			               var _objT_FacilitySalaryRangeAssociation = new List<T_Facility>();
               _objT_FacilitySalaryRangeAssociation.Add(t_salaryrange.t_facilitysalaryrangeassociation);
			   			   ViewBag.T_FacilitySalaryRangeAssociationID = new SelectList(_objT_FacilitySalaryRangeAssociation, "ID", "DisplayValue", t_salaryrange.T_FacilitySalaryRangeAssociationID);
					CustomLoadViewDataListBeforeEdit(t_salaryrange);
        }
        private void LoadViewDataAfterOnCreate(T_SalaryRange t_salaryrange)
        {			
			 ViewBag.T_SalaryBandSalaryRangeAssociationID = new SelectList(db.T_SalaryBands, "ID", "DisplayValue", t_salaryrange.T_SalaryBandSalaryRangeAssociationID);
			 ViewBag.T_FacilitySalaryRangeAssociationID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_salaryrange.T_FacilitySalaryRangeAssociationID);
		CustomLoadViewDataListAfterOnCreate(t_salaryrange);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_SalaryBand" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_SalaryBandSalaryRangeAssociation")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_SalaryBandSalaryRangeAssociationID = new SelectList(db.T_SalaryBands.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_SalaryBands.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_SalaryBandSalaryRangeAssociation = new List<T_SalaryBand>();
			 ViewBag.T_SalaryBandSalaryRangeAssociationID = new SelectList(objT_SalaryBandSalaryRangeAssociation , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_Facility" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_FacilitySalaryRangeAssociation")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_FacilitySalaryRangeAssociationID = new SelectList(db.T_Facilitys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Facilitys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_FacilitySalaryRangeAssociation = new List<T_Facility>();
			 ViewBag.T_FacilitySalaryRangeAssociationID = new SelectList(objT_FacilitySalaryRangeAssociation , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_SalaryRange> searchRecords(IQueryable<T_SalaryRange> lstT_SalaryRange, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_SalaryRange = lstT_SalaryRange.Where(s => (s.t_salarybandsalaryrangeassociation!= null && (s.t_salarybandsalaryrangeassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_facilitysalaryrangeassociation!= null && (s.t_facilitysalaryrangeassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_SalaryRange = lstT_SalaryRange.Where(s => (s.t_salarybandsalaryrangeassociation!= null && (s.t_salarybandsalaryrangeassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_facilitysalaryrangeassociation!= null && (s.t_facilitysalaryrangeassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
			try
            {
                var decimalvalue = Convert.ToDecimal(searchString);
                lstT_SalaryRange = lstT_SalaryRange.Union(db.T_SalaryRanges.Where(s => (s.T_Minimum == decimalvalue) ||(s.T_Maximum == decimalvalue) ));
            }
            catch { }
            return lstT_SalaryRange;
        }
		private IQueryable<T_SalaryRange> sortRecords(IQueryable<T_SalaryRange> lstT_SalaryRange, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_SalaryBandSalaryRangeAssociationID")
                return isAsc.ToLower() == "asc" ? lstT_SalaryRange.OrderBy(p => p.t_salarybandsalaryrangeassociation.DisplayValue) : lstT_SalaryRange.OrderByDescending(p => p.t_salarybandsalaryrangeassociation.DisplayValue);
	 if(sortBy == "T_FacilitySalaryRangeAssociationID")
                return isAsc.ToLower() == "asc" ? lstT_SalaryRange.OrderBy(p => p.t_facilitysalaryrangeassociation.DisplayValue) : lstT_SalaryRange.OrderByDescending(p => p.t_facilitysalaryrangeassociation.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_SalaryRange.Sort(sortBy,true) : lstT_SalaryRange.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_SalaryRange), "t_salaryrange");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_SalaryRange>)lstT_SalaryRange.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_SalaryRange.ElementType, lambda.Body.Type },
                    lstT_SalaryRange.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Department")
            {
                ...
                url += "?search=";
						var T_FacilitySalaryRangeAssociation  = _Object.T_DepartmentFacilityAssociationID;
						url += "&t_facilitysalaryrangeassociation=" + T_FacilitySalaryRangeAssociation;									
            ...
            if (sourceEntity == "T_Position")
            {
                ...
                url += "?search=";
						var T_FacilitySalaryRangeAssociation  = _Object.T_FacilityAssignedToID;
						url += "&t_facilitysalaryrangeassociation=" + T_FacilitySalaryRangeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Employee")
            {
                ...
                url += "?search=";
						var T_FacilitySalaryRangeAssociation  = _Object.T_EmployeeAtFacilityID;
						url += "&t_facilitysalaryrangeassociation=" + T_FacilitySalaryRangeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DepartmentArea")
            {
                ...
                url += "?search=";
						var T_FacilitySalaryRangeAssociation  = _Object.T_DepartmentAreaEmployeeTypeAssociationID;
						url += "&t_facilitysalaryrangeassociation=" + T_FacilitySalaryRangeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_ClaimType")
            {
                ...
                url += "?search=";
						var T_FacilitySalaryRangeAssociation  = _Object.T_ClaimTypeFacilityAssociationID;
						url += "&t_facilitysalaryrangeassociation=" + T_FacilitySalaryRangeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Restrictions")
            {
                ...
                url += "?search=";
						var T_FacilitySalaryRangeAssociation  = _Object.T_RestrictionsFacilityAssociationID;
						url += "&t_facilitysalaryrangeassociation=" + T_FacilitySalaryRangeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_RoleCode")
            {
                var url = (Url.Action("FSearch"));
				var _Object = db.T_RoleCodes.Find(Convert.ToInt64(id));
                url += "?search=";
						var T_SalaryBandSalaryRangeAssociation  = _Object.T_RoleCodeSalaryBandID;
						url += "&t_salarybandsalaryrangeassociation=" + T_SalaryBandSalaryRangeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_FacilitySalaryRangeAssociation  = _Object.T_FacilityUnitXID;
						url += "&t_facilitysalaryrangeassociation=" + T_FacilitySalaryRangeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Unit")
            {
                ...
                url += "?search=";
						var T_FacilitySalaryRangeAssociation  = _Object.T_FacilityUnitID;
						url += "&t_facilitysalaryrangeassociation=" + T_FacilitySalaryRangeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_FacilityConfiguration")
            {
                ...
                url += "?search=";
						var T_FacilitySalaryRangeAssociation  = _Object.T_TenantConfigurationAssociationID;
						url += "&t_facilitysalaryrangeassociation=" + T_FacilitySalaryRangeAssociation;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_salarybandsalaryrangeassociation,string t_facilitysalaryrangeassociation, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_SalaryBandSalaryRangeAssociationlist = db.T_SalaryBands.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_salarybandsalaryrangeassociation != null)
            {
                var ids = t_salarybandsalaryrangeassociation.Split(",".ToCharArray());
				   ...
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            else
                            {
							   ...
							   ViewBag.SearchResult += db.T_SalaryBands.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_SalaryBandSalaryRangeAssociationlist.Union(db.T_SalaryBands.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_SalaryBand>(User, list, "T_SalaryBand", null);
					ViewBag.t_salarybandsalaryrangeassociation = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_SalaryBandSalaryRangeAssociationlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_SalaryBand>(User, list, "T_SalaryBand", null);
				ViewBag.t_salarybandsalaryrangeassociation = new SelectList(list, "ID", "DisplayValue");
			}
			var T_FacilitySalaryRangeAssociationlist = db.T_Facilitys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_facilitysalaryrangeassociation != null)
            {
                var ids = t_facilitysalaryrangeassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Facility= ";
				   foreach (var str in ids)
                    {
					...
					ids1 = ids1.ToList();
					var list = T_FacilitySalaryRangeAssociationlist.Union(db.T_Facilitys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
					ViewBag.t_facilitysalaryrangeassociation = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_FacilitySalaryRangeAssociationlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
				ViewBag.t_facilitysalaryrangeassociation = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_SalaryBandSalaryRangeAssociation = new List<T_SalaryBand>();
		    ViewBag.t_salarybandsalaryrangeassociation = new SelectList(objT_SalaryBandSalaryRangeAssociation, "ID", "DisplayValue");
			var objT_FacilitySalaryRangeAssociation = new List<T_Facility>();
		    ViewBag.t_facilitysalaryrangeassociation = new SelectList(objT_FacilitySalaryRangeAssociation, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_SalaryRange";
			...
			columns.Add("2", "Salary Band");
			columns.Add("3", "Facility");
			columns.Add("4", "Minimum");
			columns.Add("5", "Maximum");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				return View(new T_SalaryRange());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_SalaryRange", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_SalaryRange", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_SalaryRange()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_SalaryRange/FSearch/
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_salarybandsalaryrangeassociation,string t_facilitysalaryrangeassociation ,string T_MinimumFrom,string T_MinimumTo,string T_MaximumFrom,string T_MaximumTo,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_SalaryRange  = searchRecords(lstT_SalaryRange, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_SalaryRange = searchRecords(lstT_SalaryRange, search,true);
			}
			...
			lstT_SalaryRange = lstT_SalaryRange.Include(t=>t.t_salarybandsalaryrangeassociation).Include(t=>t.t_facilitysalaryrangeassociation);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_SalaryRange", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_SalaryRange = lstT_SalaryRange.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_SalaryRange = Sorting.Sort<T_SalaryRange>(lstT_SalaryRange, DataOrdering);
            var _T_SalaryRange = lstT_SalaryRange;
		 if(T_MinimumFrom!=null || T_MinimumTo !=null)
		 {
                try
                {
                    decimal from = T_MinimumFrom == null ? 0 : Convert.ToDecimal(T_MinimumFrom);
                    decimal to =  T_MinimumTo == null ? decimal.MaxValue : Convert.ToDecimal(T_MinimumTo);
									     _T_SalaryRange =  _T_SalaryRange.Where(o => o.T_Minimum >= from && o.T_Minimum <= to);
                   					ViewBag.SearchResult += "\r\n Minimum= "+from+"-"+to;
                }
                catch { }
          }
		 if(T_MaximumFrom!=null || T_MaximumTo !=null)
		 {
                try
                {
                    decimal from = T_MaximumFrom == null ? 0 : Convert.ToDecimal(T_MaximumFrom);
                    decimal to =  T_MaximumTo == null ? decimal.MaxValue : Convert.ToDecimal(T_MaximumTo);
									     _T_SalaryRange =  _T_SalaryRange.Where(o => o.T_Maximum >= from && o.T_Maximum <= to);
                   					ViewBag.SearchResult += "\r\n Maximum= "+from+"-"+to;
                }
                catch { }
          }
			//if (lstT_SalaryRange.Where(p => p.t_salarybandsalaryrangeassociation != null).Count() <= 50)
		    //ViewBag.t_salarybandsalaryrangeassociation = new SelectList(lstT_SalaryRange.Where(p => p.t_salarybandsalaryrangeassociation != null).Select(P => P.t_salarybandsalaryrangeassociation).Distinct(), "ID", "DisplayValue");
            if (t_salarybandsalaryrangeassociation != null)
            {
                var ids = t_salarybandsalaryrangeassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_SalaryBands.Find(Convert.ToInt64(str));
                ...
                ids1 = ids1.ToList();
                _T_SalaryRange = _T_SalaryRange.Where(p => ids1.Contains(p.T_SalaryBandSalaryRangeAssociationID));
            }
				//if (lstT_SalaryRange.Where(p => p.t_facilitysalaryrangeassociation != null).Count() <= 50)
		    //ViewBag.t_facilitysalaryrangeassociation = new SelectList(lstT_SalaryRange.Where(p => p.t_facilitysalaryrangeassociation != null).Select(P => P.t_facilitysalaryrangeassociation).Distinct(), "ID", "DisplayValue");
            if (t_facilitysalaryrangeassociation != null)
            {
                var ids = t_facilitysalaryrangeassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_SalaryRange = _T_SalaryRange.Where(p => ids1.Contains(p.T_FacilitySalaryRangeAssociationID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_SalaryRange");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_SalaryRange", tagsSplit.ToArray());
                    }
		...
		var entity = "T_SalaryRange";
           ...
           var entitylist = db.T_SalaryRanges;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_SalaryRanges.GroupBy(p => p.t_salarybandsalaryrangeassociation.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Salary Band (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Salary Band"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Salary Band", "Salary Range"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_SalaryRange", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
			...
			if (type == "2" || type == "all")
                {
			var cntgrt10 = false;
                    var _yvalT_Minimum = gd.Select(k => k.Sum(p=>p.T_Minimum)).ToList();
                    if (_yvalT_Minimum .Count > 10 && inlarge == null)
                    {
                        _yvalT_Minimum = gd.Select(k => k.Sum(p=>p.T_Minimum)).Take(10).ToList();
                        cntgrt10 = true;
                    }
			 var chartT_Minimum  = new System.Web.UI.DataVisualization.Charting.Chart
                {
                    //BackColor = System.Drawing.Color.FromArgb(211, 223, 240),
                    BorderlineDashStyle = ChartDashStyle.Solid,
                    BackSecondaryColor = System.Drawing.Color.White,
                ...
                chartT_Minimum.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chartT_Minimum.Titles.Add(CreateTitle("Total of Minimum by Salary Band (Top 10)"));
				else
				chartT_Minimum.Titles.Add(CreateTitle("Total of Minimum by Salary Band"));
                chartT_Minimum.ChartAreas.Add(CreateChartArea(SeriesChartType.Column,"", "Salary Band", "T_Minimum"));
                chartT_Minimum.Series.Add(CreateSeries(SeriesChartType.Column, ""));
                chartT_Minimum.Series[0].Points.DataBindXY(_xval, _yvalT_Minimum );
                chartT_Minimum.Legends.Add(CreateLegend(""));
				if (inlarge != null)
                    {
                        chartT_Minimum.Width = 800;
                        chartT_Minimum.Height = 800;
                    }
                byte[] bytesT_Minimum;
                using (var chartimage = new MemoryStream())
                {
                    chartT_Minimum.SaveImage(chartimage, ChartImageFormat.Png);
                    bytesT_Minimum = chartimage.GetBuffer();
                }
				 if (cntgrt10)
                    {
                        string imgT_Minimum = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_SalaryRange", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encodedT_Minimum = Convert.ToBase64String(bytesT_Minimum.ToArray());
                        result += String.Format(imgT_Minimum, encodedT_Minimum);
                    }
                    else
                    {
                        string imgT_Minimum = "<div class=" + (inlarge == null ? "col-sm-4" : "col-sm-12") + " style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%;' /></div>";
                        string encodedT_Minimum = Convert.ToBase64String(bytesT_Minimum.ToArray());
                        result += String.Format(imgT_Minimum, encodedT_Minimum);
                    }
				}
			if (type == "3" || type == "all")
                {
			var cntgrt10 = false;
                    var _yvalT_Maximum = gd.Select(k => k.Sum(p=>p.T_Maximum)).ToList();
                    if (_yvalT_Maximum .Count > 10 && inlarge == null)
                    {
                        _yvalT_Maximum = gd.Select(k => k.Sum(p=>p.T_Maximum)).Take(10).ToList();
                        cntgrt10 = true;
                    }
			 var chartT_Maximum  = new System.Web.UI.DataVisualization.Charting.Chart
                {
                    //BackColor = System.Drawing.Color.FromArgb(211, 223, 240),
                    BorderlineDashStyle = ChartDashStyle.Solid,
                    BackSecondaryColor = System.Drawing.Color.White,
                ...
                chartT_Maximum.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chartT_Maximum.Titles.Add(CreateTitle("Total of Maximum by Salary Band (Top 10)"));
				else
				chartT_Maximum.Titles.Add(CreateTitle("Total of Maximum by Salary Band"));
                chartT_Maximum.ChartAreas.Add(CreateChartArea(SeriesChartType.Column,"", "Salary Band", "T_Maximum"));
                chartT_Maximum.Series.Add(CreateSeries(SeriesChartType.Column, ""));
                chartT_Maximum.Series[0].Points.DataBindXY(_xval, _yvalT_Maximum );
                chartT_Maximum.Legends.Add(CreateLegend(""));
				if (inlarge != null)
                    {
                        chartT_Maximum.Width = 800;
                        chartT_Maximum.Height = 800;
                    }
                byte[] bytesT_Maximum;
                using (var chartimage = new MemoryStream())
                {
                    chartT_Maximum.SaveImage(chartimage, ChartImageFormat.Png);
                    bytesT_Maximum = chartimage.GetBuffer();
                }
				 if (cntgrt10)
                    {
                        string imgT_Maximum = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_SalaryRange", new { type = "3", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encodedT_Maximum = Convert.ToBase64String(bytesT_Maximum.ToArray());
                        result += String.Format(imgT_Maximum, encodedT_Maximum);
                    }
                    else
                    {
                        string imgT_Maximum = "<div class=" + (inlarge == null ? "col-sm-4" : "col-sm-12") + " style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%;' /></div>";
                        string encodedT_Maximum = Convert.ToBase64String(bytesT_Maximum.ToArray());
                        result += String.Format(imgT_Maximum, encodedT_Maximum);
                    }
				}
           ...
           {
                var gd = db.T_SalaryRanges.GroupBy(p => p.t_facilitysalaryrangeassociation.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "4" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Facility", "Salary Range"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_SalaryRange", new { type = "4", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
			...
			var cntgrt10 = false;
			 ...
			 var chartT_Minimum  = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chartT_Minimum.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chartT_Minimum.Titles.Add(CreateTitle("Total of Minimum by Facility (Top 10)"));
				else
				chartT_Minimum.Titles.Add(CreateTitle("Total of Minimum by Facility"));
                chartT_Minimum.ChartAreas.Add(CreateChartArea(SeriesChartType.Column,"", "Facility", "T_Minimum"));
                chartT_Minimum.Series.Add(CreateSeries(SeriesChartType.Column, ""));
                chartT_Minimum.Series[0].Points.DataBindXY(_xval, _yvalT_Minimum );
                chartT_Minimum.Legends.Add(CreateLegend(""));
				if (inlarge != null)
                ...
                using (var chartimage = new MemoryStream())
                {
				 ...
				 if (cntgrt10)
                    {
                        string imgT_Minimum = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_SalaryRange", new { type = "5", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encodedT_Minimum = Convert.ToBase64String(bytesT_Minimum.ToArray());
                        result += String.Format(imgT_Minimum, encodedT_Minimum);
                    }
                    else
			...
			var cntgrt10 = false;
			 ...
			 var chartT_Maximum  = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chartT_Maximum.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chartT_Maximum.Titles.Add(CreateTitle("Total of Maximum by Facility (Top 10)"));
				else
				chartT_Maximum.Titles.Add(CreateTitle("Total of Maximum by Facility"));
                chartT_Maximum.ChartAreas.Add(CreateChartArea(SeriesChartType.Column,"", "Facility", "T_Maximum"));
                chartT_Maximum.Series.Add(CreateSeries(SeriesChartType.Column, ""));
                chartT_Maximum.Series[0].Points.DataBindXY(_xval, _yvalT_Maximum );
                chartT_Maximum.Legends.Add(CreateLegend(""));
				if (inlarge != null)
                ...
                using (var chartimage = new MemoryStream())
                {
				 ...
				 if (cntgrt10)
                    {
                        string imgT_Maximum = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_SalaryRange", new { type = "6", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encodedT_Maximum = Convert.ToBase64String(bytesT_Maximum.ToArray());
                        result += String.Format(imgT_Maximum, encodedT_Maximum);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_SalaryRanges.FirstOrDefault(p => p.Id == idvalue);
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_SalaryRanges.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_SalaryRanges.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_SalaryRange>(_Obj, "T_SalaryRange", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_SalaryRange> list = db.T_SalaryRanges;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_SalaryRanges;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_SalaryRange), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_SalaryRange, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_SalaryRange>)q);
				   list = ((IQueryable<T_SalaryRange>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_SalaryRanges;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_SalaryRange>(User,list, "T_SalaryRange",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_SalaryRange>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_SalaryRange), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_SalaryRange).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_SalaryRange), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_SalaryRange), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_SalaryRange), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_SalaryRange), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_SalaryRange), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_SalaryRange), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_SalaryRange), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_SalaryRange), "p"));
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_SalaryRange>(User, list, "T_SalaryRange", null);
			if (key != null && key.Length > 0)
            {
			   var data = from x in list.Where(p=>p.DisplayValue.Contains(key)).OrderBy(q=>q.DisplayValue).Take(10).ToList()
                           select new { Id = x.Id, Name = x.DisplayValue };
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_SalaryRange", User) || !User.CanAdd("T_SalaryRange"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_SalaryRange")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_SalaryRange").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_SalaryRange");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_SalaryRange" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Salary Ranges";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_SalaryBandSalaryRangeAssociationID":
										 var t_salarybandsalaryrangeassociationId = db.T_SalaryBands.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_salarybandsalaryrangeassociationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_FacilitySalaryRangeAssociationID":
										 var t_facilitysalaryrangeassociationId = db.T_Facilitys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_facilitysalaryrangeassociationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                        default:
                                            break;
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_SalaryBandSalaryRangeAssociationID":
										 var strPropertyT_SalaryBandSalaryRangeAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_SalaryBandSalaryRangeAssociationID").Value;
										 ModelReflector.Property propT_SalaryBandSalaryRangeAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_SalaryBand").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_SalaryBandSalaryRangeAssociation);
										 //var elementTypeT_SalaryBandSalaryRangeAssociation = db.T_SalaryBands.ElementType;
										 //var propertyInfoT_SalaryBandSalaryRangeAssociation = elementTypeT_SalaryBandSalaryRangeAssociation.GetProperty(propT_SalaryBandSalaryRangeAssociation.Name);
										 // var t_salarybandsalaryrangeassociationId = db.T_SalaryBands.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_SalaryBandSalaryRangeAssociation.GetValue(p, null)) == assovalue);
										 var t_salarybandsalaryrangeassociationId = db.T_SalaryBands.Where(propT_SalaryBandSalaryRangeAssociation.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_salarybandsalaryrangeassociationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_FacilitySalaryRangeAssociationID":
										 var strPropertyT_FacilitySalaryRangeAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_FacilitySalaryRangeAssociationID").Value;
										 ModelReflector.Property propT_FacilitySalaryRangeAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Facility").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_FacilitySalaryRangeAssociation);
										 //var elementTypeT_FacilitySalaryRangeAssociation = db.T_Facilitys.ElementType;
										 //var propertyInfoT_FacilitySalaryRangeAssociation = elementTypeT_FacilitySalaryRangeAssociation.GetProperty(propT_FacilitySalaryRangeAssociation.Name);
										 // var t_facilitysalaryrangeassociationId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_FacilitySalaryRangeAssociation.GetValue(p, null)) == assovalue);
										 var t_facilitysalaryrangeassociationId = db.T_Facilitys.Where(propT_FacilitySalaryRangeAssociation.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_facilitysalaryrangeassociationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_SalaryRange model = new T_SalaryRange();
                        ...
                        for (int j = 0; j < sheetColumns.Count; j++)
                        {
                            ...
                            {
    case "T_Minimum":
    model.T_Minimum = Decimal.Parse(columnValue);
	break;
    case "T_Maximum":
    model.T_Maximum = Decimal.Parse(columnValue);
	break;
					 case "T_SalaryBandSalaryRangeAssociationID":
		 dynamic t_salarybandsalaryrangeassociationId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_SalaryBandSalaryRangeAssociation = db.T_SalaryBands.ElementType;
			 //var propertyInfoT_SalaryBandSalaryRangeAssociation = elementTypeT_SalaryBandSalaryRangeAssociation.GetProperty(propT_SalaryBandSalaryRangeAssociation.Name);			 
			 //t_salarybandsalaryrangeassociationId = db.T_SalaryBands.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_SalaryBandSalaryRangeAssociation.GetValue(p, null)) == columnValue);
			 t_salarybandsalaryrangeassociationId = db.T_SalaryBands.Where(propT_SalaryBandSalaryRangeAssociation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_salarybandsalaryrangeassociationId = db.T_SalaryBands.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_salarybandsalaryrangeassociationId != null)
			model.T_SalaryBandSalaryRangeAssociationID = t_salarybandsalaryrangeassociationId.Id;
		  else
			{
				if ((collection["T_SalaryBandSalaryRangeAssociationID"].ToString() == "true,false"))
				{
					try
					{
						T_SalaryBand objT_SalaryBand = new T_SalaryBand();
						objT_SalaryBand.T_Name  = (columnValue);
						db.T_SalaryBands.Add(objT_SalaryBand);
						db.SaveChanges();
						model.T_SalaryBandSalaryRangeAssociationID = objT_SalaryBand.Id;
					}
					catch
					{
						continue;
         ...
         break;
		 case "T_FacilitySalaryRangeAssociationID":
		 dynamic t_facilitysalaryrangeassociationId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 var strPropertyT_FacilitySalaryRangeAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_FacilitySalaryRangeAssociationID").Value;
			 ModelReflector.Property propT_FacilitySalaryRangeAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Facility").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_FacilitySalaryRangeAssociation);
			 //var elementTypeT_FacilitySalaryRangeAssociation = db.T_Facilitys.ElementType;
			 //var propertyInfoT_FacilitySalaryRangeAssociation = elementTypeT_FacilitySalaryRangeAssociation.GetProperty(propT_FacilitySalaryRangeAssociation.Name);			 
			 //t_facilitysalaryrangeassociationId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_FacilitySalaryRangeAssociation.GetValue(p, null)) == columnValue);
			 t_facilitysalaryrangeassociationId = db.T_Facilitys.Where(propT_FacilitySalaryRangeAssociation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_facilitysalaryrangeassociationId = db.T_Facilitys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_facilitysalaryrangeassociationId != null)
			model.T_FacilitySalaryRangeAssociationID = t_facilitysalaryrangeassociationId.Id;
		  else
			{
				if ((collection["T_FacilitySalaryRangeAssociationID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_FacilitySalaryRangeAssociationID = objT_Facility.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_SalaryRange validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_SalaryRanges.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_SalaryRange OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_SalaryRange").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_SalaryRange");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_SalaryRange OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_SalaryRange");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_SalaryRange OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_SalaryRange");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_SalaryRange OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_SalaryRange",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_SalaryRange OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_SalaryRange");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_SalaryRange OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_SalaryRange").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_SalaryRanges.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_SalaryRanges;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_SalaryRange, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_SalaryRange, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_SalaryRange>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_SalaryRange t_salaryrange)
        {
        ...
        public ActionResult GetCalculationValues(T_SalaryRange t_salaryrange)
        {
            t_salaryrange.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_salaryrange.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_SalaryRange"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_SalaryRange t_salaryrange, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_SalaryRange t_salaryrange, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
