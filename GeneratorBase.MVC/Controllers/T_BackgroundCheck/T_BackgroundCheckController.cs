...
{	
    ...
    {
		private void LoadViewDataForCount(T_BackgroundCheck t_backgroundcheck, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_BackgroundCheck t_backgroundcheck)
        {		
			 ViewBag.T_EmployeeCriminalBackgroundCheckID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_backgroundcheck.T_EmployeeCriminalBackgroundCheckID);
			 ViewBag.T_RetainTablePreEmploymentCheckID = new SelectList(db.T_RetainTables, "ID", "DisplayValue", t_backgroundcheck.T_RetainTablePreEmploymentCheckID);
			 ViewBag.T_PreEmploymentCheckResultsVAStateID = new SelectList(db.T_ResultsTables, "ID", "DisplayValue", t_backgroundcheck.T_PreEmploymentCheckResultsVAStateID);
			 ViewBag.T_ReviewerID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_backgroundcheck.T_ReviewerID);
			 ViewBag.T_CPSResultID = new SelectList(db.T_CPSResultss, "ID", "DisplayValue", t_backgroundcheck.T_CPSResultID);
			CustomLoadViewDataListAfterEdit(t_backgroundcheck);
        }
        private void LoadViewDataBeforeOnEdit(T_BackgroundCheck t_backgroundcheck)
        {		
               var _objT_EmployeeCriminalBackgroundCheck = new List<T_Employee>();
               _objT_EmployeeCriminalBackgroundCheck.Add(t_backgroundcheck.t_employeecriminalbackgroundcheck);
			   			   ViewBag.T_EmployeeCriminalBackgroundCheckID = new SelectList(_objT_EmployeeCriminalBackgroundCheck, "ID", "DisplayValue", t_backgroundcheck.T_EmployeeCriminalBackgroundCheckID);
			               var _objT_RetainTablePreEmploymentCheck = new List<T_RetainTable>();
               _objT_RetainTablePreEmploymentCheck.Add(t_backgroundcheck.t_retaintablepreemploymentcheck);
			   			   ViewBag.T_RetainTablePreEmploymentCheckID = new SelectList(_objT_RetainTablePreEmploymentCheck, "ID", "DisplayValue", t_backgroundcheck.T_RetainTablePreEmploymentCheckID);
			               var _objT_PreEmploymentCheckResultsVAState = new List<T_ResultsTable>();
               _objT_PreEmploymentCheckResultsVAState.Add(t_backgroundcheck.t_preemploymentcheckresultsvastate);
			   			   ViewBag.T_PreEmploymentCheckResultsVAStateID = new SelectList(_objT_PreEmploymentCheckResultsVAState, "ID", "DisplayValue", t_backgroundcheck.T_PreEmploymentCheckResultsVAStateID);
			               var _objT_Reviewer = new List<T_Employee>();
               _objT_Reviewer.Add(t_backgroundcheck.t_reviewer);
			   			   ViewBag.T_ReviewerID = new SelectList(_objT_Reviewer, "ID", "DisplayValue", t_backgroundcheck.T_ReviewerID);
			               var _objT_CPSResult = new List<T_CPSResults>();
               _objT_CPSResult.Add(t_backgroundcheck.t_cpsresult);
			   			   ViewBag.T_CPSResultID = new SelectList(_objT_CPSResult, "ID", "DisplayValue", t_backgroundcheck.T_CPSResultID);
				 ViewBag.T_PreemploymentCommentsCount = t_backgroundcheck.t_preemploymentcomments.Count();
		CustomLoadViewDataListBeforeEdit(t_backgroundcheck);
        }
        private void LoadViewDataAfterOnCreate(T_BackgroundCheck t_backgroundcheck)
        {			
			 ViewBag.T_EmployeeCriminalBackgroundCheckID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_backgroundcheck.T_EmployeeCriminalBackgroundCheckID);
			 ViewBag.T_RetainTablePreEmploymentCheckID = new SelectList(db.T_RetainTables, "ID", "DisplayValue", t_backgroundcheck.T_RetainTablePreEmploymentCheckID);
			 ViewBag.T_PreEmploymentCheckResultsVAStateID = new SelectList(db.T_ResultsTables, "ID", "DisplayValue", t_backgroundcheck.T_PreEmploymentCheckResultsVAStateID);
			 ViewBag.T_ReviewerID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_backgroundcheck.T_ReviewerID);
			 ViewBag.T_CPSResultID = new SelectList(db.T_CPSResultss, "ID", "DisplayValue", t_backgroundcheck.T_CPSResultID);
		CustomLoadViewDataListAfterOnCreate(t_backgroundcheck);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Employee" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_EmployeeCriminalBackgroundCheck")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_EmployeeCriminalBackgroundCheckID = new SelectList(db.T_Employees.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Employees.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_EmployeeCriminalBackgroundCheck = new List<T_Employee>();
			 ViewBag.T_EmployeeCriminalBackgroundCheckID = new SelectList(objT_EmployeeCriminalBackgroundCheck , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_RetainTable" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_RetainTablePreEmploymentCheck")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_RetainTablePreEmploymentCheckID = new SelectList(db.T_RetainTables.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_RetainTables.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_RetainTablePreEmploymentCheck = new List<T_RetainTable>();
			 ViewBag.T_RetainTablePreEmploymentCheckID = new SelectList(objT_RetainTablePreEmploymentCheck , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_ResultsTable" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_PreEmploymentCheckResultsVAState")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_PreEmploymentCheckResultsVAStateID = new SelectList(db.T_ResultsTables.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_ResultsTables.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_PreEmploymentCheckResultsVAState = new List<T_ResultsTable>();
			 ViewBag.T_PreEmploymentCheckResultsVAStateID = new SelectList(objT_PreEmploymentCheckResultsVAState , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_Employee" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_Reviewer")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_ReviewerID = new SelectList(db.T_Employees.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Employees.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_Reviewer = new List<T_Employee>();
			 ViewBag.T_ReviewerID = new SelectList(objT_Reviewer , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_CPSResults" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_CPSResult")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_CPSResultID = new SelectList(db.T_CPSResultss.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_CPSResultss.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_CPSResult = new List<T_CPSResults>();
			 ViewBag.T_CPSResultID = new SelectList(objT_CPSResult , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_BackgroundCheck> searchRecords(IQueryable<T_BackgroundCheck> lstT_BackgroundCheck, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_BackgroundCheck = lstT_BackgroundCheck.Where(s => (s.t_employeecriminalbackgroundcheck!= null && (s.t_employeecriminalbackgroundcheck.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_retaintablepreemploymentcheck!= null && (s.t_retaintablepreemploymentcheck.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_preemploymentcheckresultsvastate!= null && (s.t_preemploymentcheckresultsvastate.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_reviewer!= null && (s.t_reviewer.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_cpsresult!= null && (s.t_cpsresult.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_Notes) && s.T_Notes.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_BackgroundCheck = lstT_BackgroundCheck.Where(s => (s.t_employeecriminalbackgroundcheck!= null && (s.t_employeecriminalbackgroundcheck.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_retaintablepreemploymentcheck!= null && (s.t_retaintablepreemploymentcheck.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_preemploymentcheckresultsvastate!= null && (s.t_preemploymentcheckresultsvastate.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_cpsresult!= null && (s.t_cpsresult.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
			try
            {
                var datevalue = Convert.ToDateTime(searchString);
                lstT_BackgroundCheck = lstT_BackgroundCheck.Union(db.T_BackgroundChecks.Where(s => (s.T_DateFingerPrintTaken == datevalue) ||(s.T_DateInformationReceivedFromCBC == datevalue) ||(s.T_BackgroundDispositionDate == datevalue) ||(s.T_ReviewDate == datevalue) ||(s.T_DateCheckInitiated == datevalue) ||(s.T_DateCPSResultReceived == datevalue) ||(s.T_CPSReviewDate == datevalue) ));
            }
            catch { }
            return lstT_BackgroundCheck;
        }
		private IQueryable<T_BackgroundCheck> sortRecords(IQueryable<T_BackgroundCheck> lstT_BackgroundCheck, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_EmployeeCriminalBackgroundCheckID")
                return isAsc.ToLower() == "asc" ? lstT_BackgroundCheck.OrderBy(p => p.t_employeecriminalbackgroundcheck.DisplayValue) : lstT_BackgroundCheck.OrderByDescending(p => p.t_employeecriminalbackgroundcheck.DisplayValue);
	 if(sortBy == "T_RetainTablePreEmploymentCheckID")
                return isAsc.ToLower() == "asc" ? lstT_BackgroundCheck.OrderBy(p => p.t_retaintablepreemploymentcheck.DisplayValue) : lstT_BackgroundCheck.OrderByDescending(p => p.t_retaintablepreemploymentcheck.DisplayValue);
	 if(sortBy == "T_PreEmploymentCheckResultsVAStateID")
                return isAsc.ToLower() == "asc" ? lstT_BackgroundCheck.OrderBy(p => p.t_preemploymentcheckresultsvastate.DisplayValue) : lstT_BackgroundCheck.OrderByDescending(p => p.t_preemploymentcheckresultsvastate.DisplayValue);
	 if(sortBy == "T_CPSResultID")
                return isAsc.ToLower() == "asc" ? lstT_BackgroundCheck.OrderBy(p => p.t_cpsresult.DisplayValue) : lstT_BackgroundCheck.OrderByDescending(p => p.t_cpsresult.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_BackgroundCheck.Sort(sortBy,true) : lstT_BackgroundCheck.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_BackgroundCheck), "t_backgroundcheck");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_BackgroundCheck>)lstT_BackgroundCheck.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_BackgroundCheck.ElementType, lambda.Body.Type },
                    lstT_BackgroundCheck.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "FileDocument")
            {
                ...
                url += "?search=";
						var T_EmployeeCriminalBackgroundCheck  = _Object.T_EmployeeDocumentsID;
						url += "&t_employeecriminalbackgroundcheck=" + T_EmployeeCriminalBackgroundCheck;									
            ...
            if (sourceEntity == "T_Licenses")
            {
                ...
                url += "?search=";
						var T_EmployeeCriminalBackgroundCheck  = _Object.T_LicenseRecordsID;
						url += "&t_employeecriminalbackgroundcheck=" + T_EmployeeCriminalBackgroundCheck;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_ServiceRecord")
            {
                ...
                url += "?search=";
						var T_EmployeeCriminalBackgroundCheck  = _Object.T_EmployeeEmploymentProfileID;
						url += "&t_employeecriminalbackgroundcheck=" + T_EmployeeCriminalBackgroundCheck;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Comment")
            {
                ...
                url += "?search=";
						var T_EmployeeCriminalBackgroundCheck  = _Object.T_EmployeeCommentsID;
						url += "&t_employeecriminalbackgroundcheck=" + T_EmployeeCriminalBackgroundCheck;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DrugAlcoholTest")
            {
                ...
                url += "?search=";
						var T_EmployeeCriminalBackgroundCheck  = _Object.T_EmployeeDrugAlcoholTestID;
						url += "&t_employeecriminalbackgroundcheck=" + T_EmployeeCriminalBackgroundCheck;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_EmployeeCriminalBackgroundCheck  = _Object.T_EmployeeAdministratorID;
						url += "&t_employeecriminalbackgroundcheck=" + T_EmployeeCriminalBackgroundCheck;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_JobAssignment")
            {
                ...
                url += "?search=";
						var T_EmployeeCriminalBackgroundCheck  = _Object.T_EmployeeJobAssignmentID;
						url += "&t_employeecriminalbackgroundcheck=" + T_EmployeeCriminalBackgroundCheck;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_LeaveProfile")
            {
                ...
                url += "?search=";
						var T_EmployeeCriminalBackgroundCheck  = _Object.T_EmployeeLeaveProfileID;
						url += "&t_employeecriminalbackgroundcheck=" + T_EmployeeCriminalBackgroundCheck;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_EmployeeInjury")
            {
                ...
                url += "?search=";
						var T_EmployeeCriminalBackgroundCheck  = _Object.T_EmployeeEmployeeInjuryID;
						url += "&t_employeecriminalbackgroundcheck=" + T_EmployeeCriminalBackgroundCheck;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Education")
            {
                ...
                url += "?search=";
						var T_EmployeeCriminalBackgroundCheck  = _Object.T_EmployeeEducationID;
						url += "&t_employeecriminalbackgroundcheck=" + T_EmployeeCriminalBackgroundCheck;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Accommodation")
            {
                ...
                url += "?search=";
						var T_EmployeeCriminalBackgroundCheck  = _Object.T_EmployeeAccomodationID;
						url += "&t_employeecriminalbackgroundcheck=" + T_EmployeeCriminalBackgroundCheck;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_PayDetails")
            {
                ...
                url += "?search=";
						var T_EmployeeCriminalBackgroundCheck  = _Object.T_EmployeePayDetailsID;
						url += "&t_employeecriminalbackgroundcheck=" + T_EmployeeCriminalBackgroundCheck;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_employeecriminalbackgroundcheck,string t_retaintablepreemploymentcheck,string t_preemploymentcheckresultsvastate,string t_reviewer,string t_cpsresult, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_EmployeeCriminalBackgroundChecklist = db.T_Employees.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_employeecriminalbackgroundcheck != null)
            {
                var ids = t_employeecriminalbackgroundcheck.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Employee= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_EmployeeCriminalBackgroundChecklist.Union(db.T_Employees.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
					ViewBag.t_employeecriminalbackgroundcheck = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_EmployeeCriminalBackgroundChecklist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
				ViewBag.t_employeecriminalbackgroundcheck = new SelectList(list, "ID", "DisplayValue");
			}
			var T_RetainTablePreEmploymentChecklist = db.T_RetainTables.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_retaintablepreemploymentcheck != null)
            {
                var ids = t_retaintablepreemploymentcheck.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Employment Decision= ";
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            {
                               ids1.Add(Convert.ToInt64(str));
							   ViewBag.SearchResult += db.T_RetainTables.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_RetainTablePreEmploymentChecklist.Union(db.T_RetainTables.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_RetainTable>(User, list, "T_RetainTable", null);
					ViewBag.t_retaintablepreemploymentcheck = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_RetainTablePreEmploymentChecklist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_RetainTable>(User, list, "T_RetainTable", null);
				ViewBag.t_retaintablepreemploymentcheck = new SelectList(list, "ID", "DisplayValue");
			}
			var T_PreEmploymentCheckResultsVAStatelist = db.T_ResultsTables.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_preemploymentcheckresultsvastate != null)
            {
                var ids = t_preemploymentcheckresultsvastate.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n VSP Initial Results= ";
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            {
                               ids1.Add(Convert.ToInt64(str));
							   ViewBag.SearchResult += db.T_ResultsTables.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_PreEmploymentCheckResultsVAStatelist.Union(db.T_ResultsTables.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_ResultsTable>(User, list, "T_ResultsTable", null);
					ViewBag.t_preemploymentcheckresultsvastate = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_PreEmploymentCheckResultsVAStatelist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_ResultsTable>(User, list, "T_ResultsTable", null);
				ViewBag.t_preemploymentcheckresultsvastate = new SelectList(list, "ID", "DisplayValue");
			}
			var T_Reviewerlist = db.T_Employees.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_reviewer != null)
            {
                var ids = t_reviewer.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Reviewer = ";
				   foreach (var str in ids)
                    {
					...
					ids1 = ids1.ToList();
					var list = T_Reviewerlist.Union(db.T_Employees.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
					ViewBag.t_reviewer = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_Reviewerlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
				ViewBag.t_reviewer = new SelectList(list, "ID", "DisplayValue");
			}
			var T_CPSResultlist = db.T_CPSResultss.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_cpsresult != null)
            {
                var ids = t_cpsresult.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n CPS Result = ";
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            {
                               ids1.Add(Convert.ToInt64(str));
							   ViewBag.SearchResult += db.T_CPSResultss.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_CPSResultlist.Union(db.T_CPSResultss.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_CPSResults>(User, list, "T_CPSResults", null);
					ViewBag.t_cpsresult = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_CPSResultlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_CPSResults>(User, list, "T_CPSResults", null);
				ViewBag.t_cpsresult = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_EmployeeCriminalBackgroundCheck = new List<T_Employee>();
		    ViewBag.t_employeecriminalbackgroundcheck = new SelectList(objT_EmployeeCriminalBackgroundCheck, "ID", "DisplayValue");
			var objT_RetainTablePreEmploymentCheck = new List<T_RetainTable>();
		    ViewBag.t_retaintablepreemploymentcheck = new SelectList(objT_RetainTablePreEmploymentCheck, "ID", "DisplayValue");
			var objT_PreEmploymentCheckResultsVAState = new List<T_ResultsTable>();
		    ViewBag.t_preemploymentcheckresultsvastate = new SelectList(objT_PreEmploymentCheckResultsVAState, "ID", "DisplayValue");
			var objT_Reviewer = new List<T_Employee>();
		    ViewBag.t_reviewer = new SelectList(objT_Reviewer, "ID", "DisplayValue");
			var objT_CPSResult = new List<T_CPSResults>();
		    ViewBag.t_cpsresult = new SelectList(objT_CPSResult, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_BackgroundCheck";
			...
			columns.Add("3", "Employment Decision");
			columns.Add("4", "Date Finger Print Taken");
			columns.Add("5", "VSP Response Rcvd Date");
			columns.Add("6", "VSP Initial Results");
			columns.Add("7", "Date CPS Result Received");
			columns.Add("8", "CPS Result ");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				return View(new T_BackgroundCheck());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_BackgroundCheck", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_BackgroundCheck", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_BackgroundCheck()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_BackgroundCheck/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_employeecriminalbackgroundcheck,string t_retaintablepreemploymentcheck,string t_preemploymentcheckresultsvastate,string t_reviewer,string t_cpsresult ,string T_DateFingerPrintTakenFrom,string T_DateFingerPrintTakenTo,string T_DateInformationReceivedFromCBCFrom,string T_DateInformationReceivedFromCBCTo,string T_BackgroundDispositionDateFrom,string T_BackgroundDispositionDateTo,string T_ReviewDateFrom,string T_ReviewDateTo,string T_DateCheckInitiatedFrom,string T_DateCheckInitiatedTo,string T_DateCPSResultReceivedFrom,string T_DateCPSResultReceivedTo,string T_CPSReviewDateFrom,string T_CPSReviewDateTo,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_BackgroundCheck  = searchRecords(lstT_BackgroundCheck, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_BackgroundCheck = searchRecords(lstT_BackgroundCheck, search,true);
			}
			...
			lstT_BackgroundCheck = lstT_BackgroundCheck.Include(t=>t.t_employeecriminalbackgroundcheck).Include(t=>t.t_retaintablepreemploymentcheck).Include(t=>t.t_preemploymentcheckresultsvastate).Include(t=>t.t_reviewer).Include(t=>t.t_cpsresult);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_BackgroundCheck", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_BackgroundCheck = lstT_BackgroundCheck.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_BackgroundCheck = Sorting.Sort<T_BackgroundCheck>(lstT_BackgroundCheck, DataOrdering);
            var _T_BackgroundCheck = lstT_BackgroundCheck;
				if(T_DateFingerPrintTakenFrom!=null || T_DateFingerPrintTakenTo !=null)
				{
						try
						{
							DateTime from = T_DateFingerPrintTakenFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_DateFingerPrintTakenFrom);
							DateTime to = T_DateFingerPrintTakenTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_DateFingerPrintTakenTo).AddTicks(-1).AddDays(1);
							                       _T_BackgroundCheck =  _T_BackgroundCheck.Where(o =>o.T_DateFingerPrintTaken!=null && o.T_DateFingerPrintTaken.CompareTo(from) >= 0 && o.T_DateFingerPrintTaken.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n Date Finger Print Taken= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
				if(T_DateInformationReceivedFromCBCFrom!=null || T_DateInformationReceivedFromCBCTo !=null)
				{
						try
						{
							DateTime from = T_DateInformationReceivedFromCBCFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_DateInformationReceivedFromCBCFrom);
							DateTime to = T_DateInformationReceivedFromCBCTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_DateInformationReceivedFromCBCTo).AddTicks(-1).AddDays(1);
							                       _T_BackgroundCheck =  _T_BackgroundCheck.Where(o =>o.T_DateInformationReceivedFromCBC!=null && o.T_DateInformationReceivedFromCBC.Value.CompareTo(from) >= 0 && o.T_DateInformationReceivedFromCBC.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n VSP Response Rcvd Date= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
				if(T_BackgroundDispositionDateFrom!=null || T_BackgroundDispositionDateTo !=null)
				{
						try
						{
							DateTime from = T_BackgroundDispositionDateFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_BackgroundDispositionDateFrom);
							DateTime to = T_BackgroundDispositionDateTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_BackgroundDispositionDateTo).AddTicks(-1).AddDays(1);
							                       _T_BackgroundCheck =  _T_BackgroundCheck.Where(o =>o.T_BackgroundDispositionDate!=null && o.T_BackgroundDispositionDate.Value.CompareTo(from) >= 0 && o.T_BackgroundDispositionDate.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n FBI Record Received Date = "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
				if(T_ReviewDateFrom!=null || T_ReviewDateTo !=null)
				{
						try
						{
							DateTime from = T_ReviewDateFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_ReviewDateFrom);
							DateTime to = T_ReviewDateTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_ReviewDateTo).AddTicks(-1).AddDays(1);
							                       _T_BackgroundCheck =  _T_BackgroundCheck.Where(o =>o.T_ReviewDate!=null && o.T_ReviewDate.Value.CompareTo(from) >= 0 && o.T_ReviewDate.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n Review Date= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
				if(T_DateCheckInitiatedFrom!=null || T_DateCheckInitiatedTo !=null)
				{
						try
						{
							DateTime from = T_DateCheckInitiatedFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_DateCheckInitiatedFrom);
							DateTime to = T_DateCheckInitiatedTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_DateCheckInitiatedTo).AddTicks(-1).AddDays(1);
							                       _T_BackgroundCheck =  _T_BackgroundCheck.Where(o =>o.T_DateCheckInitiated!=null && o.T_DateCheckInitiated.Value.CompareTo(from) >= 0 && o.T_DateCheckInitiated.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n Date Check Initiated= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
				if(T_DateCPSResultReceivedFrom!=null || T_DateCPSResultReceivedTo !=null)
				{
						try
						{
							DateTime from = T_DateCPSResultReceivedFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_DateCPSResultReceivedFrom);
							DateTime to = T_DateCPSResultReceivedTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_DateCPSResultReceivedTo).AddTicks(-1).AddDays(1);
							                       _T_BackgroundCheck =  _T_BackgroundCheck.Where(o =>o.T_DateCPSResultReceived!=null && o.T_DateCPSResultReceived.Value.CompareTo(from) >= 0 && o.T_DateCPSResultReceived.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n Date CPS Result Received= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
				if(T_CPSReviewDateFrom!=null || T_CPSReviewDateTo !=null)
				{
						try
						{
							DateTime from = T_CPSReviewDateFrom == null ? Convert.ToDateTime("01/01/1900") : Convert.ToDateTime(T_CPSReviewDateFrom);
							DateTime to = T_CPSReviewDateTo == null ? DateTime.MaxValue : Convert.ToDateTime(T_CPSReviewDateTo).AddTicks(-1).AddDays(1);
							                       _T_BackgroundCheck =  _T_BackgroundCheck.Where(o =>o.T_CPSReviewDate!=null && o.T_CPSReviewDate.Value.CompareTo(from) >= 0 && o.T_CPSReviewDate.Value.CompareTo(to) <= 0);
                   							ViewBag.SearchResult += "\r\n CPS Review Date= "+from.ToShortDateString()+"-"+to.ToShortDateString();
						}
						catch { }
				}
			//if (lstT_BackgroundCheck.Where(p => p.t_employeecriminalbackgroundcheck != null).Count() <= 50)
		    //ViewBag.t_employeecriminalbackgroundcheck = new SelectList(lstT_BackgroundCheck.Where(p => p.t_employeecriminalbackgroundcheck != null).Select(P => P.t_employeecriminalbackgroundcheck).Distinct(), "ID", "DisplayValue");
            if (t_employeecriminalbackgroundcheck != null)
            {
                var ids = t_employeecriminalbackgroundcheck.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_BackgroundCheck = _T_BackgroundCheck.Where(p => ids1.Contains(p.T_EmployeeCriminalBackgroundCheckID));
            }
				//if (lstT_BackgroundCheck.Where(p => p.t_retaintablepreemploymentcheck != null).Count() <= 50)
		    //ViewBag.t_retaintablepreemploymentcheck = new SelectList(lstT_BackgroundCheck.Where(p => p.t_retaintablepreemploymentcheck != null).Select(P => P.t_retaintablepreemploymentcheck).Distinct(), "ID", "DisplayValue");
            if (t_retaintablepreemploymentcheck != null)
            {
                var ids = t_retaintablepreemploymentcheck.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_RetainTables.Find(Convert.ToInt64(str));
                            ViewBag.SearchResult += obj != null ? obj.DisplayValue + ", " : "";
                        }
                    }
                    //
                ...
                ids1 = ids1.ToList();
                _T_BackgroundCheck = _T_BackgroundCheck.Where(p => ids1.Contains(p.T_RetainTablePreEmploymentCheckID));
            }
				//if (lstT_BackgroundCheck.Where(p => p.t_preemploymentcheckresultsvastate != null).Count() <= 50)
		    //ViewBag.t_preemploymentcheckresultsvastate = new SelectList(lstT_BackgroundCheck.Where(p => p.t_preemploymentcheckresultsvastate != null).Select(P => P.t_preemploymentcheckresultsvastate).Distinct(), "ID", "DisplayValue");
            if (t_preemploymentcheckresultsvastate != null)
            {
                var ids = t_preemploymentcheckresultsvastate.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_ResultsTables.Find(Convert.ToInt64(str));
                            ViewBag.SearchResult += obj != null ? obj.DisplayValue + ", " : "";
                        }
                    }
                    //
                ...
                ids1 = ids1.ToList();
                _T_BackgroundCheck = _T_BackgroundCheck.Where(p => ids1.Contains(p.T_PreEmploymentCheckResultsVAStateID));
            }
				//if (lstT_BackgroundCheck.Where(p => p.t_reviewer != null).Count() <= 50)
		    //ViewBag.t_reviewer = new SelectList(lstT_BackgroundCheck.Where(p => p.t_reviewer != null).Select(P => P.t_reviewer).Distinct(), "ID", "DisplayValue");
            if (t_reviewer != null)
            {
                var ids = t_reviewer.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_BackgroundCheck = _T_BackgroundCheck.Where(p => ids1.Contains(p.T_ReviewerID));
            }
				//if (lstT_BackgroundCheck.Where(p => p.t_cpsresult != null).Count() <= 50)
		    //ViewBag.t_cpsresult = new SelectList(lstT_BackgroundCheck.Where(p => p.t_cpsresult != null).Select(P => P.t_cpsresult).Distinct(), "ID", "DisplayValue");
            if (t_cpsresult != null)
            {
                var ids = t_cpsresult.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_CPSResultss.Find(Convert.ToInt64(str));
                            ViewBag.SearchResult += obj != null ? obj.DisplayValue + ", " : "";
                        }
                    }
                    //
                ...
                ids1 = ids1.ToList();
                _T_BackgroundCheck = _T_BackgroundCheck.Where(p => ids1.Contains(p.T_CPSResultID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_BackgroundCheck");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_BackgroundCheck", tagsSplit.ToArray());
                    }
		...
		var entity = "T_BackgroundCheck";
           ...
           var entitylist = db.T_BackgroundChecks;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_BackgroundChecks.GroupBy(p => p.t_employeecriminalbackgroundcheck.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Employee", "Background Check"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_BackgroundCheck", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_BackgroundChecks.GroupBy(p => p.t_retaintablepreemploymentcheck.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "2" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Employment Decision (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Employment Decision"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Employment Decision", "Background Check"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_BackgroundCheck", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_BackgroundChecks.GroupBy(p => p.t_preemploymentcheckresultsvastate.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "3" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by VSP Initial Results (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by VSP Initial Results"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "VSP Initial Results", "Background Check"));
                chart.Series.Add(CreateSeries(SeriesChartType.Column, ""));
                chart.Series[0].Points.DataBindXY(_xval, _yval);
                chart.Legends.Add(CreateLegend(""));
				 if (inlarge != null)
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_BackgroundCheck", new { type = "3", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_BackgroundChecks.GroupBy(p => p.t_reviewer.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "4" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Reviewer  (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Reviewer "));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Reviewer ", "Background Check"));
                chart.Series.Add(CreateSeries(SeriesChartType.Pie, ""));
                chart.Series[0].Points.DataBindXY(_xval, _yval);
                chart.Legends.Add(CreateLegend(""));
				 if (inlarge != null)
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_BackgroundCheck", new { type = "4", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_BackgroundChecks.GroupBy(p => p.t_cpsresult.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "5" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by CPS Result  (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by CPS Result "));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "CPS Result ", "Background Check"));
                chart.Series.Add(CreateSeries(SeriesChartType.Column, ""));
                chart.Series[0].Points.DataBindXY(_xval, _yval);
                chart.Legends.Add(CreateLegend(""));
				 if (inlarge != null)
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_BackgroundCheck", new { type = "5", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_BackgroundChecks.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_backgroundcheck = jedb.T_BackgroundChecks.Find(id);
			var T_PreemploymentCommentsIds = jedb.T_Comments.Where(p=>p.T_PreemploymentCommentsID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_Comment" && T_PreemploymentCommentsIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_BackgroundChecks.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_BackgroundChecks.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_BackgroundCheck>(_Obj, "T_BackgroundCheck", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_BackgroundCheck> list = db.T_BackgroundChecks;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_BackgroundChecks;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_BackgroundCheck), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_BackgroundCheck, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_BackgroundCheck>)q);
				   list = ((IQueryable<T_BackgroundCheck>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_BackgroundChecks;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_BackgroundCheck>(User,list, "T_BackgroundCheck",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_BackgroundCheck>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_BackgroundCheck), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_BackgroundCheck).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_BackgroundCheck), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_BackgroundCheck), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_BackgroundCheck), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_BackgroundCheck), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_BackgroundCheck), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_BackgroundCheck), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_BackgroundCheck), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_BackgroundCheck), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_BackgroundCheck", User) || !User.CanAdd("T_BackgroundCheck"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_BackgroundCheck")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_BackgroundCheck").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_BackgroundCheck");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_BackgroundCheck" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Background Checks";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_EmployeeCriminalBackgroundCheckID":
										 var t_employeecriminalbackgroundcheckId = db.T_Employees.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_employeecriminalbackgroundcheckId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_RetainTablePreEmploymentCheckID":
										 var t_retaintablepreemploymentcheckId = db.T_RetainTables.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_retaintablepreemploymentcheckId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_PreEmploymentCheckResultsVAStateID":
										 var t_preemploymentcheckresultsvastateId = db.T_ResultsTables.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_preemploymentcheckresultsvastateId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_ReviewerID":
										 var t_reviewerId = db.T_Employees.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_reviewerId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_CPSResultID":
										 var t_cpsresultId = db.T_CPSResultss.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_cpsresultId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                        default:
                                            break;
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_BackgroundCheck";
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_EmployeeCriminalBackgroundCheckID":
										 var strPropertyT_EmployeeCriminalBackgroundCheck = lstEntityProp.FirstOrDefault(p => p.Key == "T_EmployeeCriminalBackgroundCheckID").Value;
										 ModelReflector.Property propT_EmployeeCriminalBackgroundCheck = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Employee").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_EmployeeCriminalBackgroundCheck);
										 //var elementTypeT_EmployeeCriminalBackgroundCheck = db.T_Employees.ElementType;
										 //var propertyInfoT_EmployeeCriminalBackgroundCheck = elementTypeT_EmployeeCriminalBackgroundCheck.GetProperty(propT_EmployeeCriminalBackgroundCheck.Name);
										 // var t_employeecriminalbackgroundcheckId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EmployeeCriminalBackgroundCheck.GetValue(p, null)) == assovalue);
										 var t_employeecriminalbackgroundcheckId = db.T_Employees.Where(propT_EmployeeCriminalBackgroundCheck.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_employeecriminalbackgroundcheckId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_RetainTablePreEmploymentCheckID":
										 var strPropertyT_RetainTablePreEmploymentCheck = lstEntityProp.FirstOrDefault(p => p.Key == "T_RetainTablePreEmploymentCheckID").Value;
										 ModelReflector.Property propT_RetainTablePreEmploymentCheck = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_RetainTable").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_RetainTablePreEmploymentCheck);
										 //var elementTypeT_RetainTablePreEmploymentCheck = db.T_RetainTables.ElementType;
										 //var propertyInfoT_RetainTablePreEmploymentCheck = elementTypeT_RetainTablePreEmploymentCheck.GetProperty(propT_RetainTablePreEmploymentCheck.Name);
										 // var t_retaintablepreemploymentcheckId = db.T_RetainTables.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_RetainTablePreEmploymentCheck.GetValue(p, null)) == assovalue);
										 var t_retaintablepreemploymentcheckId = db.T_RetainTables.Where(propT_RetainTablePreEmploymentCheck.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_retaintablepreemploymentcheckId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_PreEmploymentCheckResultsVAStateID":
										 var strPropertyT_PreEmploymentCheckResultsVAState = lstEntityProp.FirstOrDefault(p => p.Key == "T_PreEmploymentCheckResultsVAStateID").Value;
										 ModelReflector.Property propT_PreEmploymentCheckResultsVAState = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ResultsTable").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_PreEmploymentCheckResultsVAState);
										 //var elementTypeT_PreEmploymentCheckResultsVAState = db.T_ResultsTables.ElementType;
										 //var propertyInfoT_PreEmploymentCheckResultsVAState = elementTypeT_PreEmploymentCheckResultsVAState.GetProperty(propT_PreEmploymentCheckResultsVAState.Name);
										 // var t_preemploymentcheckresultsvastateId = db.T_ResultsTables.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_PreEmploymentCheckResultsVAState.GetValue(p, null)) == assovalue);
										 var t_preemploymentcheckresultsvastateId = db.T_ResultsTables.Where(propT_PreEmploymentCheckResultsVAState.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_preemploymentcheckresultsvastateId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_ReviewerID":
										 var strPropertyT_Reviewer = lstEntityProp.FirstOrDefault(p => p.Key == "T_ReviewerID").Value;
										 ModelReflector.Property propT_Reviewer = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Employee").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_Reviewer);
										 //var elementTypeT_Reviewer = db.T_Employees.ElementType;
										 //var propertyInfoT_Reviewer = elementTypeT_Reviewer.GetProperty(propT_Reviewer.Name);
										 // var t_reviewerId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_Reviewer.GetValue(p, null)) == assovalue);
										 var t_reviewerId = db.T_Employees.Where(propT_Reviewer.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_reviewerId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_CPSResultID":
										 var strPropertyT_CPSResult = lstEntityProp.FirstOrDefault(p => p.Key == "T_CPSResultID").Value;
										 ModelReflector.Property propT_CPSResult = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_CPSResults").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_CPSResult);
										 //var elementTypeT_CPSResult = db.T_CPSResultss.ElementType;
										 //var propertyInfoT_CPSResult = elementTypeT_CPSResult.GetProperty(propT_CPSResult.Name);
										 // var t_cpsresultId = db.T_CPSResultss.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_CPSResult.GetValue(p, null)) == assovalue);
										 var t_cpsresultId = db.T_CPSResultss.Where(propT_CPSResult.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_cpsresultId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_BackgroundCheck model = new T_BackgroundCheck();
                        ...
                        for (int j = 0; j < sheetColumns.Count; j++)
                        {
                            ...
                            switch (columntable)
                            {
    case "T_DateFingerPrintTaken":
    model.T_DateFingerPrintTaken = DateTime.Parse(columnValue);
	break;
    case "T_DateInformationReceivedFromCBC":
    model.T_DateInformationReceivedFromCBC = DateTime.Parse(columnValue);
	break;
    case "T_BackgroundDispositionDate":
    model.T_BackgroundDispositionDate = DateTime.Parse(columnValue);
	break;
    case "T_ReviewDate":
    model.T_ReviewDate = DateTime.Parse(columnValue);
	break;
    case "T_DateCheckInitiated":
    model.T_DateCheckInitiated = DateTime.Parse(columnValue);
	break;
    case "T_DateCPSResultReceived":
    model.T_DateCPSResultReceived = DateTime.Parse(columnValue);
	break;
    case "T_CPSReviewDate":
    model.T_CPSReviewDate = DateTime.Parse(columnValue);
	break;
    case "T_Notes":
    model.T_Notes = columnValue;
	break;
					 case "T_EmployeeCriminalBackgroundCheckID":
		 dynamic t_employeecriminalbackgroundcheckId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_EmployeeCriminalBackgroundCheck = db.T_Employees.ElementType;
			 //var propertyInfoT_EmployeeCriminalBackgroundCheck = elementTypeT_EmployeeCriminalBackgroundCheck.GetProperty(propT_EmployeeCriminalBackgroundCheck.Name);			 
			 //t_employeecriminalbackgroundcheckId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EmployeeCriminalBackgroundCheck.GetValue(p, null)) == columnValue);
			 t_employeecriminalbackgroundcheckId = db.T_Employees.Where(propT_EmployeeCriminalBackgroundCheck.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_employeecriminalbackgroundcheckId = db.T_Employees.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_employeecriminalbackgroundcheckId != null)
			model.T_EmployeeCriminalBackgroundCheckID = t_employeecriminalbackgroundcheckId.Id;
		  else
			{
				if ((collection["T_EmployeeCriminalBackgroundCheckID"].ToString() == "true,false"))
				{
					try
					{
				 ...
				 try { objT_Employee.T_FirstName =(columnValue); }
						...
						db.SaveChanges();
						model.T_EmployeeCriminalBackgroundCheckID = objT_Employee.Id;
					}
					catch
					{
						continue;
         ...
         break;
		 case "T_RetainTablePreEmploymentCheckID":
		 dynamic t_retaintablepreemploymentcheckId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 var strPropertyT_RetainTablePreEmploymentCheck = lstEntityProp.FirstOrDefault(p => p.Key == "T_RetainTablePreEmploymentCheckID").Value;
			 ModelReflector.Property propT_RetainTablePreEmploymentCheck = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_RetainTable").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_RetainTablePreEmploymentCheck);
			 //var elementTypeT_RetainTablePreEmploymentCheck = db.T_RetainTables.ElementType;
			 //var propertyInfoT_RetainTablePreEmploymentCheck = elementTypeT_RetainTablePreEmploymentCheck.GetProperty(propT_RetainTablePreEmploymentCheck.Name);			 
			 //t_retaintablepreemploymentcheckId = db.T_RetainTables.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_RetainTablePreEmploymentCheck.GetValue(p, null)) == columnValue);
			 t_retaintablepreemploymentcheckId = db.T_RetainTables.Where(propT_RetainTablePreEmploymentCheck.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_retaintablepreemploymentcheckId = db.T_RetainTables.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_retaintablepreemploymentcheckId != null)
			model.T_RetainTablePreEmploymentCheckID = t_retaintablepreemploymentcheckId.Id;
		  else
			{
				if ((collection["T_RetainTablePreEmploymentCheckID"].ToString() == "true,false"))
				{
					try
					{
						T_RetainTable objT_RetainTable = new T_RetainTable();
						objT_RetainTable.T_Name  = (columnValue);
						db.T_RetainTables.Add(objT_RetainTable);
						db.SaveChanges();
						model.T_RetainTablePreEmploymentCheckID = objT_RetainTable.Id;
					}
					catch
					{
						continue;
         ...
         break;
		 case "T_PreEmploymentCheckResultsVAStateID":
		 dynamic t_preemploymentcheckresultsvastateId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 var strPropertyT_PreEmploymentCheckResultsVAState = lstEntityProp.FirstOrDefault(p => p.Key == "T_PreEmploymentCheckResultsVAStateID").Value;
			 ModelReflector.Property propT_PreEmploymentCheckResultsVAState = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ResultsTable").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_PreEmploymentCheckResultsVAState);
			 //var elementTypeT_PreEmploymentCheckResultsVAState = db.T_ResultsTables.ElementType;
			 //var propertyInfoT_PreEmploymentCheckResultsVAState = elementTypeT_PreEmploymentCheckResultsVAState.GetProperty(propT_PreEmploymentCheckResultsVAState.Name);			 
			 //t_preemploymentcheckresultsvastateId = db.T_ResultsTables.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_PreEmploymentCheckResultsVAState.GetValue(p, null)) == columnValue);
			 t_preemploymentcheckresultsvastateId = db.T_ResultsTables.Where(propT_PreEmploymentCheckResultsVAState.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_preemploymentcheckresultsvastateId = db.T_ResultsTables.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_preemploymentcheckresultsvastateId != null)
			model.T_PreEmploymentCheckResultsVAStateID = t_preemploymentcheckresultsvastateId.Id;
		  else
			{
				if ((collection["T_PreEmploymentCheckResultsVAStateID"].ToString() == "true,false"))
				{
					try
					{
						T_ResultsTable objT_ResultsTable = new T_ResultsTable();
						objT_ResultsTable.T_Name  = (columnValue);
						db.T_ResultsTables.Add(objT_ResultsTable);
						db.SaveChanges();
						model.T_PreEmploymentCheckResultsVAStateID = objT_ResultsTable.Id;
					}
					catch
					{
						continue;
		 ...
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_Reviewer = db.T_Employees.ElementType;
			 //var propertyInfoT_Reviewer = elementTypeT_Reviewer.GetProperty(propT_Reviewer.Name);			 
			 //t_reviewerId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_Reviewer.GetValue(p, null)) == columnValue);
			 t_reviewerId = db.T_Employees.Where(propT_Reviewer.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_reviewerId = db.T_Employees.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_reviewerId != null)
			model.T_ReviewerID = t_reviewerId.Id;
		  else
			{
				if ((collection["T_ReviewerID"].ToString() == "true,false"))
				{
					try
					{
						T_Employee objT_Employee = new T_Employee();
				 ...
				 try { objT_Employee.T_FirstName =(columnValue); }
						...
						db.T_Employees.Add(objT_Employee);
						db.SaveChanges();
						model.T_ReviewerID = objT_Employee.Id;
					}
					catch
					{
						continue;
		 ...
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_CPSResult = db.T_CPSResultss.ElementType;
			 //var propertyInfoT_CPSResult = elementTypeT_CPSResult.GetProperty(propT_CPSResult.Name);			 
			 //t_cpsresultId = db.T_CPSResultss.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_CPSResult.GetValue(p, null)) == columnValue);
			 t_cpsresultId = db.T_CPSResultss.Where(propT_CPSResult.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_cpsresultId = db.T_CPSResultss.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_cpsresultId != null)
			model.T_CPSResultID = t_cpsresultId.Id;
		  else
			{
				if ((collection["T_CPSResultID"].ToString() == "true,false"))
				{
					try
					{
						T_CPSResults objT_CPSResults = new T_CPSResults();
						objT_CPSResults.T_Name  = (columnValue);
						db.T_CPSResultss.Add(objT_CPSResults);
						db.SaveChanges();
						model.T_CPSResultID = objT_CPSResults.Id;
					}
					catch
					{
						continue;
         ...
         break;
            default:
			 ...
			 if (model.T_DateFingerPrintTaken == DateTime.MinValue)
                            model.T_DateFingerPrintTaken =  DateTime.UtcNow;
					    ...
					    if (ValidateModel(model))
                        {
                            ...
                            if (result == null || result.Count == 0)
                            {
								...
								{
									db.T_BackgroundChecks.Add(model);
									db.SaveChanges();
								}
								error += customerror;
							}
		...
		public bool ValidateModel(T_BackgroundCheck validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_BackgroundChecks.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_BackgroundCheck OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_BackgroundCheck").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_BackgroundCheck");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_BackgroundCheck OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_BackgroundCheck");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_BackgroundCheck OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_BackgroundCheck");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_BackgroundCheck OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_BackgroundCheck",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_BackgroundCheck OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_BackgroundCheck");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_BackgroundCheck OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_BackgroundCheck").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_BackgroundChecks.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_BackgroundChecks;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_BackgroundCheck, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_BackgroundCheck, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_BackgroundCheck>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_BackgroundCheck t_backgroundcheck)
        {
        ...
        {
            string[][] groupsarr = new string[][] { new string[] {"48903","T_BackgroundCheck48903"},new string[] {"48923","Employee Information"},new string[] {"48922","Background Check"},new string[] {"48924","Child Protective Services"},new string[] {"48925","Notes"} };
            return groupsarr;
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_BackgroundCheck t_backgroundcheck)
        {
            t_backgroundcheck.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_backgroundcheck.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_BackgroundCheck"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_BackgroundCheck t_backgroundcheck, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_BackgroundCheck t_backgroundcheck, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
