...
{	
    ...
    {
		private void LoadViewDataForCount(T_ReasonForDrugTest t_reasonfordrugtest, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_ReasonForDrugTest t_reasonfordrugtest)
        {		
			CustomLoadViewDataListAfterEdit(t_reasonfordrugtest);
        }
        private void LoadViewDataBeforeOnEdit(T_ReasonForDrugTest t_reasonfordrugtest)
        {		
	 ViewBag.T_TestReasonCount = t_reasonfordrugtest.t_testreason.Count();
		CustomLoadViewDataListBeforeEdit(t_reasonfordrugtest);
        }
        private void LoadViewDataAfterOnCreate(T_ReasonForDrugTest t_reasonfordrugtest)
        {			
		CustomLoadViewDataListAfterOnCreate(t_reasonfordrugtest);
        }
		...
		private IQueryable<T_ReasonForDrugTest> searchRecords(IQueryable<T_ReasonForDrugTest> lstT_ReasonForDrugTest, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_ReasonForDrugTest = lstT_ReasonForDrugTest.Where(s => (!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_ReasonForDrugTest = lstT_ReasonForDrugTest.Where(s => (!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_ReasonForDrugTest;
        }
		private IQueryable<T_ReasonForDrugTest> sortRecords(IQueryable<T_ReasonForDrugTest> lstT_ReasonForDrugTest, string sortBy, string isAsc)
        {
            string methodName = "";
		    ...
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_ReasonForDrugTest.Sort(sortBy,true) : lstT_ReasonForDrugTest.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_ReasonForDrugTest), "t_reasonfordrugtest");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_ReasonForDrugTest>)lstT_ReasonForDrugTest.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_ReasonForDrugTest.ElementType, lambda.Body.Type },
                    lstT_ReasonForDrugTest.Expression,
                    lambda));
        }
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_ReasonForDrugTests.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_reasonfordrugtest = jedb.T_ReasonForDrugTests.Find(id);
			var T_TestReasonIds = jedb.T_DrugAlcoholTests.Where(p=>p.T_TestReasonID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_DrugAlcoholTest" && T_TestReasonIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_ReasonForDrugTests.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_ReasonForDrugTests.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_ReasonForDrugTest>(_Obj, "T_ReasonForDrugTest", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_ReasonForDrugTest> list = db.T_ReasonForDrugTests;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_ReasonForDrugTests;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_ReasonForDrugTest), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_ReasonForDrugTest, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_ReasonForDrugTest>)q);
				   list = ((IQueryable<T_ReasonForDrugTest>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_ReasonForDrugTests;
            ...
            list = _fad.FilterDropdown<T_ReasonForDrugTest>(User,list, "T_ReasonForDrugTest",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_ReasonForDrugTest>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_ReasonForDrugTest), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_ReasonForDrugTest).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_ReasonForDrugTest), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_ReasonForDrugTest), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_ReasonForDrugTest), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_ReasonForDrugTest), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_ReasonForDrugTest), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_ReasonForDrugTest), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_ReasonForDrugTest), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_ReasonForDrugTest), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_ReasonForDrugTest", User) || !User.CanAdd("T_ReasonForDrugTest"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_ReasonForDrugTest")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_ReasonForDrugTest").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_ReasonForDrugTest");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_ReasonForDrugTest" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Reason For Drug Tests";
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_ReasonForDrugTest";
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_ReasonForDrugTest model = new T_ReasonForDrugTest();
	...
	break;
                ...
                break;
					    ...
					    if (ValidateModel(model))
                        {
                            ...
                            if (result == null || result.Count == 0)
                            {
								...
								{
									db.T_ReasonForDrugTests.Add(model);
									db.SaveChanges();
								}
								error += customerror;
							}
		...
		public bool ValidateModel(T_ReasonForDrugTest validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_ReasonForDrugTests.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_ReasonForDrugTest OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_ReasonForDrugTest").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_ReasonForDrugTest");
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p => p.TypeNo).Contains(4))
                    {
                        ...
                        foreach (var parametersArgs in Args)
                        {
                            ...
                            var entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ReasonForDrugTest");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_ReasonForDrugTest OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_ReasonForDrugTest");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_ReasonForDrugTest OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_ReasonForDrugTest");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_ReasonForDrugTest OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_ReasonForDrugTest",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_ReasonForDrugTest OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_ReasonForDrugTest");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_ReasonForDrugTest OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ReasonForDrugTest").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_ReasonForDrugTests.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_ReasonForDrugTests;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_ReasonForDrugTest, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_ReasonForDrugTest, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_ReasonForDrugTest>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_ReasonForDrugTest t_reasonfordrugtest)
        {
        ...
        {
            string[][] groupsarr = new string[][] { new string[] {"48905","T_ReasonForDrugTest48905"} };
            return groupsarr;
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_ReasonForDrugTest t_reasonfordrugtest)
        {
            t_reasonfordrugtest.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_reasonfordrugtest.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_ReasonForDrugTest"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_ReasonForDrugTest t_reasonfordrugtest, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_ReasonForDrugTest t_reasonfordrugtest, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
