...
{	
    ...
    {
		private void LoadViewDataForCount(T_AllFacilitiesFloor t_allfacilitiesfloor, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_AllFacilitiesFloor t_allfacilitiesfloor)
        {		
			 ViewBag.T_AllUnitsFloorID = new SelectList(db.T_AllFacilitiesUnits, "ID", "DisplayValue", t_allfacilitiesfloor.T_AllUnitsFloorID);
			CustomLoadViewDataListAfterEdit(t_allfacilitiesfloor);
        }
        private void LoadViewDataBeforeOnEdit(T_AllFacilitiesFloor t_allfacilitiesfloor)
        {		
               var _objT_AllUnitsFloor = new List<T_AllFacilitiesUnit>();
               _objT_AllUnitsFloor.Add(t_allfacilitiesfloor.t_allunitsfloor);
			   			   ViewBag.T_AllUnitsFloorID = new SelectList(_objT_AllUnitsFloor, "ID", "DisplayValue", t_allfacilitiesfloor.T_AllUnitsFloorID);
				 ViewBag.T_EmployeInjuryFloorCount = t_allfacilitiesfloor.t_employeinjuryfloor.Count();
		CustomLoadViewDataListBeforeEdit(t_allfacilitiesfloor);
        }
        private void LoadViewDataAfterOnCreate(T_AllFacilitiesFloor t_allfacilitiesfloor)
        {			
			 ViewBag.T_AllUnitsFloorID = new SelectList(db.T_AllFacilitiesUnits, "ID", "DisplayValue", t_allfacilitiesfloor.T_AllUnitsFloorID);
		CustomLoadViewDataListAfterOnCreate(t_allfacilitiesfloor);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_AllFacilitiesUnit" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_AllUnitsFloor")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_AllUnitsFloorID = new SelectList(db.T_AllFacilitiesUnits.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_AllFacilitiesUnits.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_AllUnitsFloor = new List<T_AllFacilitiesUnit>();
			 ViewBag.T_AllUnitsFloorID = new SelectList(objT_AllUnitsFloor , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_AllFacilitiesFloor> searchRecords(IQueryable<T_AllFacilitiesFloor> lstT_AllFacilitiesFloor, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_AllFacilitiesFloor = lstT_AllFacilitiesFloor.Where(s => (!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(s.t_allunitsfloor!= null && (s.t_allunitsfloor.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_AllFacilitiesFloor = lstT_AllFacilitiesFloor.Where(s => (!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(s.t_allunitsfloor!= null && (s.t_allunitsfloor.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_AllFacilitiesFloor;
        }
		private IQueryable<T_AllFacilitiesFloor> sortRecords(IQueryable<T_AllFacilitiesFloor> lstT_AllFacilitiesFloor, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_AllUnitsFloorID")
                return isAsc.ToLower() == "asc" ? lstT_AllFacilitiesFloor.OrderBy(p => p.t_allunitsfloor.DisplayValue) : lstT_AllFacilitiesFloor.OrderByDescending(p => p.t_allunitsfloor.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_AllFacilitiesFloor.Sort(sortBy,true) : lstT_AllFacilitiesFloor.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_AllFacilitiesFloor), "t_allfacilitiesfloor");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_AllFacilitiesFloor>)lstT_AllFacilitiesFloor.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_AllFacilitiesFloor.ElementType, lambda.Body.Type },
                    lstT_AllFacilitiesFloor.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_EmployeeInjury")
            {
                ...
                url += "?search=";
						var T_AllUnitsFloor  = _Object.T_UnitWhereAccidentOccuredID;
						url += "&t_allunitsfloor=" + T_AllUnitsFloor;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_allunitsfloor, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_AllUnitsFloorlist = db.T_AllFacilitiesUnits.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_allunitsfloor != null)
            {
                var ids = t_allunitsfloor.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Unit= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_AllUnitsFloorlist.Union(db.T_AllFacilitiesUnits.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_AllFacilitiesUnit>(User, list, "T_AllFacilitiesUnit", null);
					ViewBag.t_allunitsfloor = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_AllUnitsFloorlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_AllFacilitiesUnit>(User, list, "T_AllFacilitiesUnit", null);
				ViewBag.t_allunitsfloor = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_AllUnitsFloor = new List<T_AllFacilitiesUnit>();
		    ViewBag.t_allunitsfloor = new SelectList(objT_AllUnitsFloor, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_AllFacilitiesFloor";
			...
			columns.Add("4", "Unit");
				...
				return View(new T_AllFacilitiesFloor());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_AllFacilitiesFloor", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_AllFacilitiesFloor", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_AllFacilitiesFloor()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_AllFacilitiesFloor/FSearch/
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_allunitsfloor ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            if (!string.IsNullOrEmpty(searchString) && FSFilter == null)
                ViewBag.FSFilter = "Fsearch";
				 var lstT_AllFacilitiesFloor  = from s in db.T_AllFacilitiesFloors
                           select s;
            if (!String.IsNullOrEmpty(searchString))
            {
                lstT_AllFacilitiesFloor  = searchRecords(lstT_AllFacilitiesFloor, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_AllFacilitiesFloor = searchRecords(lstT_AllFacilitiesFloor, search,true);
			}
            if (!String.IsNullOrEmpty(sortBy) && !String.IsNullOrEmpty(isAsc))
            {
                lstT_AllFacilitiesFloor  = sortRecords(lstT_AllFacilitiesFloor, sortBy, isAsc);
            }
            else   lstT_AllFacilitiesFloor  = lstT_AllFacilitiesFloor.OrderByDescending(c => c.Id);
			lstT_AllFacilitiesFloor = CustomSorting(lstT_AllFacilitiesFloor);
			lstT_AllFacilitiesFloor = lstT_AllFacilitiesFloor.Include(t=>t.t_allunitsfloor);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_AllFacilitiesFloor", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_AllFacilitiesFloor = lstT_AllFacilitiesFloor.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_AllFacilitiesFloor = Sorting.Sort<T_AllFacilitiesFloor>(lstT_AllFacilitiesFloor, DataOrdering);
            var _T_AllFacilitiesFloor = lstT_AllFacilitiesFloor;
			//if (lstT_AllFacilitiesFloor.Where(p => p.t_allunitsfloor != null).Count() <= 50)
		    //ViewBag.t_allunitsfloor = new SelectList(lstT_AllFacilitiesFloor.Where(p => p.t_allunitsfloor != null).Select(P => P.t_allunitsfloor).Distinct(), "ID", "DisplayValue");
            if (t_allunitsfloor != null)
            {
                var ids = t_allunitsfloor.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_AllFacilitiesFloor = _T_AllFacilitiesFloor.Where(p => ids1.Contains(p.T_AllUnitsFloorID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_AllFacilitiesFloor");
		   ...
		   if (itemsPerPage != null)
            ...
            if (Request.Cookies["pageSize" + HttpUtility.UrlEncode(User.Name) + "T_AllFacilitiesFloor"] != null)
            {
                pageSize = Convert.ToInt32(Request.Cookies["pageSize" + HttpUtility.UrlEncode(User.Name) + "T_AllFacilitiesFloor"].Value);
                ViewBag.CurrentItemsPerPage = itemsPerPage;
            }
            ...
            {
                pageNumber = 1;
                if (_T_AllFacilitiesFloor.Count() > 0)
                    pageSize = _T_AllFacilitiesFloor.Count();
                return View("ExcelExport", _T_AllFacilitiesFloor.ToPagedList(pageNumber, pageSize));
            }
			else
            {
			 if (pageNumber > 1)
			 {
                var totalListCount = _T_AllFacilitiesFloor.Count();
			...
			if (!Request.IsAjaxRequest())
			{
				...
				var list = _T_AllFacilitiesFloor.ToPagedList(pageNumber, pageSize);
				ViewBag.EntityT_AllFacilitiesFloorDisplayValue = new SelectList(list.Select(z => new { ID = z.Id, DisplayValue = z.DisplayValue }), "ID", "DisplayValue");
				TempData["T_AllFacilitiesFloorlist"] = list.Select(z => new { ID = z.Id, DisplayValue = z.DisplayValue });
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        var tagsSplit = GroupByColumn.Split(',').Select(tag => tag.Trim()).Where(tag => !string.IsNullOrEmpty(tag));
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_AllFacilitiesFloor", tagsSplit.ToArray());
                    }
        ...
        {
		string result = "";
		var entity = "T_AllFacilitiesFloor";
           ...
           var entitylist = db.T_AllFacilitiesFloors;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_AllFacilitiesFloors.GroupBy(p => p.t_allunitsfloor.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Unit (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Unit"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Unit", "All Facilities Floor"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_AllFacilitiesFloor", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_AllFacilitiesFloors.FirstOrDefault(p => p.Id == idvalue);
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_AllFacilitiesFloors.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_AllFacilitiesFloors.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_AllFacilitiesFloor>(_Obj, "T_AllFacilitiesFloor", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_AllFacilitiesFloor> list = db.T_AllFacilitiesFloors;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_AllFacilitiesFloors;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_AllFacilitiesFloor), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_AllFacilitiesFloor, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_AllFacilitiesFloor>)q);
				   list = ((IQueryable<T_AllFacilitiesFloor>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_AllFacilitiesFloors;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_AllFacilitiesFloor>(User,list, "T_AllFacilitiesFloor",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_AllFacilitiesFloor>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_AllFacilitiesFloor), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_AllFacilitiesFloor).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_AllFacilitiesFloor), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_AllFacilitiesFloor), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_AllFacilitiesFloor), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_AllFacilitiesFloor), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_AllFacilitiesFloor), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_AllFacilitiesFloor), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_AllFacilitiesFloor), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_AllFacilitiesFloor), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_AllFacilitiesFloor", User) || !User.CanAdd("T_AllFacilitiesFloor"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_AllFacilitiesFloor")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_AllFacilitiesFloor").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_AllFacilitiesFloor");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_AllFacilitiesFloor" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new All Facilities Floors";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_AllUnitsFloorID":
										 var t_allunitsfloorId = db.T_AllFacilitiesUnits.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_allunitsfloorId == null)
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_AllFacilitiesFloor";
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_AllUnitsFloorID":
										 var strPropertyT_AllUnitsFloor = lstEntityProp.FirstOrDefault(p => p.Key == "T_AllUnitsFloorID").Value;
										 ModelReflector.Property propT_AllUnitsFloor = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_AllFacilitiesUnit").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_AllUnitsFloor);
										 //var elementTypeT_AllUnitsFloor = db.T_AllFacilitiesUnits.ElementType;
										 //var propertyInfoT_AllUnitsFloor = elementTypeT_AllUnitsFloor.GetProperty(propT_AllUnitsFloor.Name);
										 // var t_allunitsfloorId = db.T_AllFacilitiesUnits.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_AllUnitsFloor.GetValue(p, null)) == assovalue);
										 var t_allunitsfloorId = db.T_AllFacilitiesUnits.Where(propT_AllUnitsFloor.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_allunitsfloorId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_AllFacilitiesFloor model = new T_AllFacilitiesFloor();
	...
	break;
					 case "T_AllUnitsFloorID":
		 dynamic t_allunitsfloorId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_AllUnitsFloor = db.T_AllFacilitiesUnits.ElementType;
			 //var propertyInfoT_AllUnitsFloor = elementTypeT_AllUnitsFloor.GetProperty(propT_AllUnitsFloor.Name);			 
			 //t_allunitsfloorId = db.T_AllFacilitiesUnits.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_AllUnitsFloor.GetValue(p, null)) == columnValue);
			 t_allunitsfloorId = db.T_AllFacilitiesUnits.Where(propT_AllUnitsFloor.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_allunitsfloorId = db.T_AllFacilitiesUnits.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_allunitsfloorId != null)
			model.T_AllUnitsFloorID = t_allunitsfloorId.Id;
		  else
			{
				if ((collection["T_AllUnitsFloorID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_AllUnitsFloorID = objT_AllFacilitiesUnit.Id;
					}
					catch
					{
						continue;
                ...
                break;
					    ...
					    if (ValidateModel(model))
                        {
                            ...
                            if (result == null || result.Count == 0)
                            {
								...
								{
									db.T_AllFacilitiesFloors.Add(model);
									db.SaveChanges();
								}
								error += customerror;
							}
		...
		public bool ValidateModel(T_AllFacilitiesFloor validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_AllFacilitiesFloors.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_AllFacilitiesFloor OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_AllFacilitiesFloor").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_AllFacilitiesFloor");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_AllFacilitiesFloor OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_AllFacilitiesFloor");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_AllFacilitiesFloor OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_AllFacilitiesFloor");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_AllFacilitiesFloor OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_AllFacilitiesFloor",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_AllFacilitiesFloor OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_AllFacilitiesFloor");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_AllFacilitiesFloor OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_AllFacilitiesFloor").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_AllFacilitiesFloors.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_AllFacilitiesFloors;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_AllFacilitiesFloor, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_AllFacilitiesFloor, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_AllFacilitiesFloor>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_AllFacilitiesFloor t_allfacilitiesfloor)
        {
        ...
        {
            string[][] groupsarr = new string[][] { new string[] {"48919","T_AllFacilitiesFloor48919"} };
            return groupsarr;
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_AllFacilitiesFloor t_allfacilitiesfloor)
        {
            t_allfacilitiesfloor.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_allfacilitiesfloor.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_AllFacilitiesFloor"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_AllFacilitiesFloor t_allfacilitiesfloor, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_AllFacilitiesFloor t_allfacilitiesfloor, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
