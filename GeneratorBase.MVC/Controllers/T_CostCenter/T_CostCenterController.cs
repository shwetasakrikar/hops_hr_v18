...
{	
    ...
    {
		private void LoadViewDataForCount(T_CostCenter t_costcenter, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_CostCenter t_costcenter)
        {		
			 ViewBag.T_CostProgramID = new SelectList(db.T_Programs, "ID", "DisplayValue", t_costcenter.T_CostProgramID);
			 ViewBag.T_CostFundID = new SelectList(db.T_Funds, "ID", "DisplayValue", t_costcenter.T_CostFundID);
			CustomLoadViewDataListAfterEdit(t_costcenter);
        }
        private void LoadViewDataBeforeOnEdit(T_CostCenter t_costcenter)
        {		
               var _objT_CostProgram = new List<T_Program>();
               _objT_CostProgram.Add(t_costcenter.t_costprogram);
			   			   ViewBag.T_CostProgramID = new SelectList(_objT_CostProgram, "ID", "DisplayValue", t_costcenter.T_CostProgramID);
			               var _objT_CostFund = new List<T_Fund>();
               _objT_CostFund.Add(t_costcenter.t_costfund);
			   			   ViewBag.T_CostFundID = new SelectList(_objT_CostFund, "ID", "DisplayValue", t_costcenter.T_CostFundID);
				 ViewBag.T_PositionCostCenterAssociationCount = t_costcenter.t_positioncostcenterassociation.Count();
	 ViewBag.T_WardCostCenterCount = t_costcenter.t_wardcostcenter.Count();
		CustomLoadViewDataListBeforeEdit(t_costcenter);
        }
        private void LoadViewDataAfterOnCreate(T_CostCenter t_costcenter)
        {			
			 ViewBag.T_CostProgramID = new SelectList(db.T_Programs, "ID", "DisplayValue", t_costcenter.T_CostProgramID);
			 ViewBag.T_CostFundID = new SelectList(db.T_Funds, "ID", "DisplayValue", t_costcenter.T_CostFundID);
		CustomLoadViewDataListAfterOnCreate(t_costcenter);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Program" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_CostProgram")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_CostProgramID = new SelectList(db.T_Programs.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Programs.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_CostProgram = new List<T_Program>();
			 ViewBag.T_CostProgramID = new SelectList(objT_CostProgram , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_Fund" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_CostFund")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_CostFundID = new SelectList(db.T_Funds.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Funds.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_CostFund = new List<T_Fund>();
			 ViewBag.T_CostFundID = new SelectList(objT_CostFund , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_CostCenter> searchRecords(IQueryable<T_CostCenter> lstT_CostCenter, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_CostCenter = lstT_CostCenter.Where(s => (!String.IsNullOrEmpty(s.T_CC) && s.T_CC.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_CCDescription) && s.T_CCDescription.ToUpper().Contains(searchString)) ||(s.t_costprogram!= null && (s.t_costprogram.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_costfund!= null && (s.t_costfund.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_CostCenter = lstT_CostCenter.Where(s => (!String.IsNullOrEmpty(s.T_CC) && s.T_CC.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_CCDescription) && s.T_CCDescription.ToUpper().Contains(searchString)) ||(s.t_costprogram!= null && (s.t_costprogram.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_costfund!= null && (s.t_costfund.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_CostCenter;
        }
		private IQueryable<T_CostCenter> sortRecords(IQueryable<T_CostCenter> lstT_CostCenter, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_CostProgramID")
                return isAsc.ToLower() == "asc" ? lstT_CostCenter.OrderBy(p => p.t_costprogram.DisplayValue) : lstT_CostCenter.OrderByDescending(p => p.t_costprogram.DisplayValue);
	 if(sortBy == "T_CostFundID")
                return isAsc.ToLower() == "asc" ? lstT_CostCenter.OrderBy(p => p.t_costfund.DisplayValue) : lstT_CostCenter.OrderByDescending(p => p.t_costfund.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_CostCenter.Sort(sortBy,true) : lstT_CostCenter.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_CostCenter), "t_costcenter");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_CostCenter>)lstT_CostCenter.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_CostCenter.ElementType, lambda.Body.Type },
                    lstT_CostCenter.Expression,
                    lambda));
        }
        ...
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_costprogram,string t_costfund, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_CostProgramlist = db.T_Programs.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_costprogram != null)
            {
                var ids = t_costprogram.Split(",".ToCharArray());
				   ...
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            else
                            {
							   ...
							   ViewBag.SearchResult += db.T_Programs.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_CostProgramlist.Union(db.T_Programs.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Program>(User, list, "T_Program", null);
					ViewBag.t_costprogram = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_CostProgramlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Program>(User, list, "T_Program", null);
				ViewBag.t_costprogram = new SelectList(list, "ID", "DisplayValue");
			}
			var T_CostFundlist = db.T_Funds.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_costfund != null)
            {
                var ids = t_costfund.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Fund= ";
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            {
                               ids1.Add(Convert.ToInt64(str));
							   ViewBag.SearchResult += db.T_Funds.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_CostFundlist.Union(db.T_Funds.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Fund>(User, list, "T_Fund", null);
					ViewBag.t_costfund = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_CostFundlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Fund>(User, list, "T_Fund", null);
				ViewBag.t_costfund = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_CostProgram = new List<T_Program>();
		    ViewBag.t_costprogram = new SelectList(objT_CostProgram, "ID", "DisplayValue");
			var objT_CostFund = new List<T_Fund>();
		    ViewBag.t_costfund = new SelectList(objT_CostFund, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_CostCenter";
			...
			columns.Add("2", "CC");
			columns.Add("3", "CC Description");
			columns.Add("4", "Program");
			columns.Add("5", "Fund");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				return View(new T_CostCenter());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_CostCenter", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_CostCenter", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_CostCenter()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_CostCenter/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_costprogram,string t_costfund ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_CostCenter  = searchRecords(lstT_CostCenter, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_CostCenter = searchRecords(lstT_CostCenter, search,true);
			}
			...
			lstT_CostCenter = lstT_CostCenter.Include(t=>t.t_costprogram).Include(t=>t.t_costfund);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_CostCenter", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_CostCenter = lstT_CostCenter.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_CostCenter = Sorting.Sort<T_CostCenter>(lstT_CostCenter, DataOrdering);
            var _T_CostCenter = lstT_CostCenter;
			//if (lstT_CostCenter.Where(p => p.t_costprogram != null).Count() <= 50)
		    //ViewBag.t_costprogram = new SelectList(lstT_CostCenter.Where(p => p.t_costprogram != null).Select(P => P.t_costprogram).Distinct(), "ID", "DisplayValue");
            if (t_costprogram != null)
            {
                var ids = t_costprogram.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_Programs.Find(Convert.ToInt64(str));
                ...
                ids1 = ids1.ToList();
                _T_CostCenter = _T_CostCenter.Where(p => ids1.Contains(p.T_CostProgramID));
            }
				//if (lstT_CostCenter.Where(p => p.t_costfund != null).Count() <= 50)
		    //ViewBag.t_costfund = new SelectList(lstT_CostCenter.Where(p => p.t_costfund != null).Select(P => P.t_costfund).Distinct(), "ID", "DisplayValue");
            if (t_costfund != null)
            {
                var ids = t_costfund.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_Funds.Find(Convert.ToInt64(str));
                            ViewBag.SearchResult += obj != null ? obj.DisplayValue + ", " : "";
                        }
                    }
                    //
                ...
                ids1 = ids1.ToList();
                _T_CostCenter = _T_CostCenter.Where(p => ids1.Contains(p.T_CostFundID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_CostCenter");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_CostCenter", tagsSplit.ToArray());
                    }
		...
		var entity = "T_CostCenter";
           ...
           var entitylist = db.T_CostCenters;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_CostCenters.GroupBy(p => p.t_costprogram.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Program (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Program"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Program", "Cost Center"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_CostCenter", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_CostCenters.GroupBy(p => p.t_costfund.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "2" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Fund (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Fund"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Fund", "Cost Center"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_CostCenter", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_CostCenters.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_costcenter = jedb.T_CostCenters.Find(id);
			var T_PositionCostCenterAssociationIds = jedb.T_Positions.Where(p=>p.T_PositionCostCenterAssociationID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_Position" && T_PositionCostCenterAssociationIds.Contains(d.RecordId) && d.Type != "View"));
			var T_WardCostCenterIds = jedb.T_UnitXs.Where(p=>p.T_WardCostCenterID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_UnitX" && T_WardCostCenterIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_CostCenters.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_CostCenters.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_CostCenter>(_Obj, "T_CostCenter", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_CostCenter> list = db.T_CostCenters;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_CostCenters;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_CostCenter), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_CostCenter, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_CostCenter>)q);
				   list = ((IQueryable<T_CostCenter>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_CostCenters;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_CostCenter>(User,list, "T_CostCenter",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_CostCenter>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_CostCenter), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_CostCenter).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_CostCenter), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_CostCenter), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_CostCenter), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_CostCenter), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_CostCenter), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_CostCenter), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_CostCenter), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_CostCenter), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_CostCenter", User) || !User.CanAdd("T_CostCenter"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_CostCenter")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_CostCenter").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_CostCenter");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_CostCenter" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Cost Centers";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_CostProgramID":
										 var t_costprogramId = db.T_Programs.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_costprogramId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_CostFundID":
										 var t_costfundId = db.T_Funds.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_costfundId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                        default:
                                            break;
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_CostProgramID":
										 var strPropertyT_CostProgram = lstEntityProp.FirstOrDefault(p => p.Key == "T_CostProgramID").Value;
										 ModelReflector.Property propT_CostProgram = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Program").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_CostProgram);
										 //var elementTypeT_CostProgram = db.T_Programs.ElementType;
										 //var propertyInfoT_CostProgram = elementTypeT_CostProgram.GetProperty(propT_CostProgram.Name);
										 // var t_costprogramId = db.T_Programs.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_CostProgram.GetValue(p, null)) == assovalue);
										 var t_costprogramId = db.T_Programs.Where(propT_CostProgram.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_costprogramId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_CostFundID":
										 var strPropertyT_CostFund = lstEntityProp.FirstOrDefault(p => p.Key == "T_CostFundID").Value;
										 ModelReflector.Property propT_CostFund = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Fund").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_CostFund);
										 //var elementTypeT_CostFund = db.T_Funds.ElementType;
										 //var propertyInfoT_CostFund = elementTypeT_CostFund.GetProperty(propT_CostFund.Name);
										 // var t_costfundId = db.T_Funds.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_CostFund.GetValue(p, null)) == assovalue);
										 var t_costfundId = db.T_Funds.Where(propT_CostFund.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_costfundId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_CostCenter model = new T_CostCenter();
                        ...
                        for (int j = 0; j < sheetColumns.Count; j++)
                        {
                            ...
                            {
    case "T_CC":
    model.T_CC = columnValue;
	break;
    case "T_CCDescription":
    model.T_CCDescription = columnValue;
	break;
					 case "T_CostProgramID":
		 dynamic t_costprogramId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_CostProgram = db.T_Programs.ElementType;
			 //var propertyInfoT_CostProgram = elementTypeT_CostProgram.GetProperty(propT_CostProgram.Name);			 
			 //t_costprogramId = db.T_Programs.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_CostProgram.GetValue(p, null)) == columnValue);
			 t_costprogramId = db.T_Programs.Where(propT_CostProgram.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_costprogramId = db.T_Programs.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_costprogramId != null)
			model.T_CostProgramID = t_costprogramId.Id;
		  else
			{
				if ((collection["T_CostProgramID"].ToString() == "true,false"))
				{
					try
					{
						T_Program objT_Program = new T_Program();
						objT_Program.T_ProgramNumber  = Convert.ToInt32(columnValue);
						db.T_Programs.Add(objT_Program);
						db.SaveChanges();
						model.T_CostProgramID = objT_Program.Id;
					}
					catch
					{
						continue;
		 ...
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_CostFund = db.T_Funds.ElementType;
			 //var propertyInfoT_CostFund = elementTypeT_CostFund.GetProperty(propT_CostFund.Name);			 
			 //t_costfundId = db.T_Funds.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_CostFund.GetValue(p, null)) == columnValue);
			 t_costfundId = db.T_Funds.Where(propT_CostFund.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_costfundId = db.T_Funds.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_costfundId != null)
			model.T_CostFundID = t_costfundId.Id;
		  else
			{
				if ((collection["T_CostFundID"].ToString() == "true,false"))
				{
					try
					{
						T_Fund objT_Fund = new T_Fund();
						objT_Fund.T_Code  = (columnValue);
						db.T_Funds.Add(objT_Fund);
						db.SaveChanges();
						model.T_CostFundID = objT_Fund.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_CostCenter validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_CostCenters.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_CostCenter OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_CostCenter").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_CostCenter");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_CostCenter OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_CostCenter");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_CostCenter OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_CostCenter");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_CostCenter OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_CostCenter",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_CostCenter OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_CostCenter");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_CostCenter OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_CostCenter").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_CostCenters.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_CostCenters;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_CostCenter, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_CostCenter, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_CostCenter>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_CostCenter t_costcenter)
        {
        ...
        {
            string[][] groupsarr = new string[][] { new string[] {"48890","T_CostCenter48890"},new string[] {"48908","T_CostCenter48908"} };
            return groupsarr;
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_CostCenter t_costcenter)
        {
            t_costcenter.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_costcenter.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_CostCenter"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_CostCenter t_costcenter, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_CostCenter t_costcenter, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
