...
{	
    ...
    {
		private void LoadViewDataForCount(T_FacilityConfiguration t_facilityconfiguration, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_FacilityConfiguration t_facilityconfiguration)
        {		
			 ViewBag.T_TenantConfigurationAssociationID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_facilityconfiguration.T_TenantConfigurationAssociationID);
			CustomLoadViewDataListAfterEdit(t_facilityconfiguration);
        }
        private void LoadViewDataBeforeOnEdit(T_FacilityConfiguration t_facilityconfiguration)
        {		
               var _objT_TenantConfigurationAssociation = new List<T_Facility>();
               _objT_TenantConfigurationAssociation.Add(t_facilityconfiguration.t_tenantconfigurationassociation);
			   			   ViewBag.T_TenantConfigurationAssociationID = new SelectList(_objT_TenantConfigurationAssociation, "ID", "DisplayValue", t_facilityconfiguration.T_TenantConfigurationAssociationID);
					CustomLoadViewDataListBeforeEdit(t_facilityconfiguration);
        }
        private void LoadViewDataAfterOnCreate(T_FacilityConfiguration t_facilityconfiguration)
        {			
			 ViewBag.T_TenantConfigurationAssociationID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_facilityconfiguration.T_TenantConfigurationAssociationID);
		CustomLoadViewDataListAfterOnCreate(t_facilityconfiguration);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Facility" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_TenantConfigurationAssociation")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_TenantConfigurationAssociationID = new SelectList(db.T_Facilitys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Facilitys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_TenantConfigurationAssociation = new List<T_Facility>();
			 ViewBag.T_TenantConfigurationAssociationID = new SelectList(objT_TenantConfigurationAssociation , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_FacilityConfiguration> searchRecords(IQueryable<T_FacilityConfiguration> lstT_FacilityConfiguration, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_FacilityConfiguration = lstT_FacilityConfiguration.Where(s => (!String.IsNullOrEmpty(s.T_Key) && s.T_Key.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Value) && s.T_Value.ToUpper().Contains(searchString)) ||(s.t_tenantconfigurationassociation!= null && (s.t_tenantconfigurationassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_FacilityConfiguration = lstT_FacilityConfiguration.Where(s => (!String.IsNullOrEmpty(s.T_Key) && s.T_Key.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Value) && s.T_Value.ToUpper().Contains(searchString)) ||(s.t_tenantconfigurationassociation!= null && (s.t_tenantconfigurationassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_FacilityConfiguration;
        }
		private IQueryable<T_FacilityConfiguration> sortRecords(IQueryable<T_FacilityConfiguration> lstT_FacilityConfiguration, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_TenantConfigurationAssociationID")
                return isAsc.ToLower() == "asc" ? lstT_FacilityConfiguration.OrderBy(p => p.t_tenantconfigurationassociation.DisplayValue) : lstT_FacilityConfiguration.OrderByDescending(p => p.t_tenantconfigurationassociation.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_FacilityConfiguration.Sort(sortBy,true) : lstT_FacilityConfiguration.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_FacilityConfiguration), "t_facilityconfiguration");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_FacilityConfiguration>)lstT_FacilityConfiguration.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_FacilityConfiguration.ElementType, lambda.Body.Type },
                    lstT_FacilityConfiguration.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Department")
            {
                ...
                url += "?search=";
						var T_TenantConfigurationAssociation  = _Object.T_DepartmentFacilityAssociationID;
						url += "&t_tenantconfigurationassociation=" + T_TenantConfigurationAssociation;									
            ...
            if (sourceEntity == "T_Position")
            {
                ...
                url += "?search=";
						var T_TenantConfigurationAssociation  = _Object.T_FacilityAssignedToID;
						url += "&t_tenantconfigurationassociation=" + T_TenantConfigurationAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Employee")
            {
                ...
                url += "?search=";
						var T_TenantConfigurationAssociation  = _Object.T_EmployeeAtFacilityID;
						url += "&t_tenantconfigurationassociation=" + T_TenantConfigurationAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DepartmentArea")
            {
                ...
                url += "?search=";
						var T_TenantConfigurationAssociation  = _Object.T_DepartmentAreaEmployeeTypeAssociationID;
						url += "&t_tenantconfigurationassociation=" + T_TenantConfigurationAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_ClaimType")
            {
                ...
                url += "?search=";
						var T_TenantConfigurationAssociation  = _Object.T_ClaimTypeFacilityAssociationID;
						url += "&t_tenantconfigurationassociation=" + T_TenantConfigurationAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Restrictions")
            {
                ...
                url += "?search=";
						var T_TenantConfigurationAssociation  = _Object.T_RestrictionsFacilityAssociationID;
						url += "&t_tenantconfigurationassociation=" + T_TenantConfigurationAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_TenantConfigurationAssociation  = _Object.T_FacilityUnitXID;
						url += "&t_tenantconfigurationassociation=" + T_TenantConfigurationAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Unit")
            {
                ...
                url += "?search=";
						var T_TenantConfigurationAssociation  = _Object.T_FacilityUnitID;
						url += "&t_tenantconfigurationassociation=" + T_TenantConfigurationAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_SalaryRange")
            {
                ...
                url += "?search=";
						var T_TenantConfigurationAssociation  = _Object.T_FacilitySalaryRangeAssociationID;
						url += "&t_tenantconfigurationassociation=" + T_TenantConfigurationAssociation;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_tenantconfigurationassociation, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_TenantConfigurationAssociationlist = db.T_Facilitys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_tenantconfigurationassociation != null)
            {
                var ids = t_tenantconfigurationassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Tenant= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_TenantConfigurationAssociationlist.Union(db.T_Facilitys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
					ViewBag.t_tenantconfigurationassociation = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_TenantConfigurationAssociationlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
				ViewBag.t_tenantconfigurationassociation = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_TenantConfigurationAssociation = new List<T_Facility>();
		    ViewBag.t_tenantconfigurationassociation = new SelectList(objT_TenantConfigurationAssociation, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_FacilityConfiguration";
			...
			columns.Add("2", "Key");
			columns.Add("3", "Value");
			columns.Add("4", "Tenant");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				return View(new T_FacilityConfiguration());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_FacilityConfiguration", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_FacilityConfiguration", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_FacilityConfiguration()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_FacilityConfiguration/FSearch/
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_tenantconfigurationassociation ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_FacilityConfiguration  = searchRecords(lstT_FacilityConfiguration, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_FacilityConfiguration = searchRecords(lstT_FacilityConfiguration, search,true);
			}
			...
			lstT_FacilityConfiguration = lstT_FacilityConfiguration.Include(t=>t.t_tenantconfigurationassociation);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_FacilityConfiguration", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_FacilityConfiguration = lstT_FacilityConfiguration.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_FacilityConfiguration = Sorting.Sort<T_FacilityConfiguration>(lstT_FacilityConfiguration, DataOrdering);
            var _T_FacilityConfiguration = lstT_FacilityConfiguration;
			//if (lstT_FacilityConfiguration.Where(p => p.t_tenantconfigurationassociation != null).Count() <= 50)
		    //ViewBag.t_tenantconfigurationassociation = new SelectList(lstT_FacilityConfiguration.Where(p => p.t_tenantconfigurationassociation != null).Select(P => P.t_tenantconfigurationassociation).Distinct(), "ID", "DisplayValue");
            if (t_tenantconfigurationassociation != null)
            {
                var ids = t_tenantconfigurationassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_FacilityConfiguration = _T_FacilityConfiguration.Where(p => ids1.Contains(p.T_TenantConfigurationAssociationID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_FacilityConfiguration");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_FacilityConfiguration", tagsSplit.ToArray());
                    }
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_FacilityConfigurations.FirstOrDefault(p => p.Id == idvalue);
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_FacilityConfigurations.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_FacilityConfigurations.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_FacilityConfiguration>(_Obj, "T_FacilityConfiguration", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_FacilityConfiguration> list = db.T_FacilityConfigurations;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_FacilityConfigurations;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_FacilityConfiguration), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_FacilityConfiguration, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_FacilityConfiguration>)q);
				   list = ((IQueryable<T_FacilityConfiguration>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_FacilityConfigurations;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_FacilityConfiguration>(User,list, "T_FacilityConfiguration",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_FacilityConfiguration>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_FacilityConfiguration), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_FacilityConfiguration).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_FacilityConfiguration), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_FacilityConfiguration), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_FacilityConfiguration), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_FacilityConfiguration), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_FacilityConfiguration), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_FacilityConfiguration), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_FacilityConfiguration), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_FacilityConfiguration), "p"));
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_FacilityConfiguration>(User, list, "T_FacilityConfiguration", null);
			if (key != null && key.Length > 0)
            {
			   var data = from x in list.Where(p=>p.DisplayValue.Contains(key)).OrderBy(q=>q.DisplayValue).Take(10).ToList()
                           select new { Id = x.Id, Name = x.DisplayValue };
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_FacilityConfiguration", User) || !User.CanAdd("T_FacilityConfiguration"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_FacilityConfiguration")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_FacilityConfiguration").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_FacilityConfiguration");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        string colDisKey = "";
                        string colListInx = "";
                        typeName = "T_FacilityConfiguration";
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_FacilityConfiguration" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Facility Configurations";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_TenantConfigurationAssociationID":
										 var t_tenantconfigurationassociationId = db.T_Facilitys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_tenantconfigurationassociationId == null)
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_FacilityConfiguration";
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_TenantConfigurationAssociationID":
										 var strPropertyT_TenantConfigurationAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_TenantConfigurationAssociationID").Value;
										 ModelReflector.Property propT_TenantConfigurationAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Facility").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_TenantConfigurationAssociation);
										 //var elementTypeT_TenantConfigurationAssociation = db.T_Facilitys.ElementType;
										 //var propertyInfoT_TenantConfigurationAssociation = elementTypeT_TenantConfigurationAssociation.GetProperty(propT_TenantConfigurationAssociation.Name);
										 // var t_tenantconfigurationassociationId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_TenantConfigurationAssociation.GetValue(p, null)) == assovalue);
										 var t_tenantconfigurationassociationId = db.T_Facilitys.Where(propT_TenantConfigurationAssociation.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_tenantconfigurationassociationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_FacilityConfiguration model = new T_FacilityConfiguration();
                        ...
                        for (int j = 0; j < sheetColumns.Count; j++)
                        {
                            ...
                            {
    case "T_Key":
    model.T_Key = columnValue;
	break;
    case "T_Value":
    model.T_Value = columnValue;
	break;
					 case "T_TenantConfigurationAssociationID":
		 dynamic t_tenantconfigurationassociationId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_TenantConfigurationAssociation = db.T_Facilitys.ElementType;
			 //var propertyInfoT_TenantConfigurationAssociation = elementTypeT_TenantConfigurationAssociation.GetProperty(propT_TenantConfigurationAssociation.Name);			 
			 //t_tenantconfigurationassociationId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_TenantConfigurationAssociation.GetValue(p, null)) == columnValue);
			 t_tenantconfigurationassociationId = db.T_Facilitys.Where(propT_TenantConfigurationAssociation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_tenantconfigurationassociationId = db.T_Facilitys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_tenantconfigurationassociationId != null)
			model.T_TenantConfigurationAssociationID = t_tenantconfigurationassociationId.Id;
		  else
			{
				if ((collection["T_TenantConfigurationAssociationID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_TenantConfigurationAssociationID = objT_Facility.Id;
					}
					catch
					{
						continue;
                ...
                break;
					    ...
					    if (ValidateModel(model))
                        {
                            ...
                            if (result == null || result.Count == 0)
                            {
								...
								{
									db.T_FacilityConfigurations.Add(model);
									db.SaveChanges();
								}
								error += customerror;
							}
		...
		public bool ValidateModel(T_FacilityConfiguration validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_FacilityConfigurations.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_FacilityConfiguration OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_FacilityConfiguration").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_FacilityConfiguration");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_FacilityConfiguration OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_FacilityConfiguration");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_FacilityConfiguration OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_FacilityConfiguration");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_FacilityConfiguration OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_FacilityConfiguration",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_FacilityConfiguration OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_FacilityConfiguration");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_FacilityConfiguration OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_FacilityConfiguration").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_FacilityConfigurations.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_FacilityConfigurations;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_FacilityConfiguration, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_FacilityConfiguration, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_FacilityConfiguration>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_FacilityConfiguration t_facilityconfiguration)
        {
        ...
        public ActionResult GetCalculationValues(T_FacilityConfiguration t_facilityconfiguration)
        {
            t_facilityconfiguration.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_facilityconfiguration.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_FacilityConfiguration"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_FacilityConfiguration t_facilityconfiguration, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_FacilityConfiguration t_facilityconfiguration, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
