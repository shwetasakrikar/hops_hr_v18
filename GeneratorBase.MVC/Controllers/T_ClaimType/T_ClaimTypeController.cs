...
{	
    ...
    {
		private void LoadViewDataForCount(T_ClaimType t_claimtype, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_ClaimType t_claimtype)
        {		
			var T_EmployeeInjury_T_TypeofClaimlist = db.T_EmployeeInjurys.OrderBy(p => p.DisplayValue).Take(10).Distinct();
            if (t_claimtype.SelectedT_EmployeeInjury_T_TypeofClaim != null)
            {
                var list = T_EmployeeInjury_T_TypeofClaimlist.Union(db.T_EmployeeInjurys.Where(p => t_claimtype.SelectedT_EmployeeInjury_T_TypeofClaim.ToList().Contains(p.Id))).Distinct();
                ViewBag.SelectedT_EmployeeInjury_T_TypeofClaim = new MultiSelectList(list, "ID", "DisplayValue",t_claimtype.SelectedT_EmployeeInjury_T_TypeofClaim);
            }
            else
            {
                ViewBag.SelectedT_EmployeeInjury_T_TypeofClaim = new MultiSelectList(T_EmployeeInjury_T_TypeofClaimlist, "ID", "DisplayValue");
            }
			 ViewBag.T_ClaimTypeFacilityAssociationID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_claimtype.T_ClaimTypeFacilityAssociationID);
			CustomLoadViewDataListAfterEdit(t_claimtype);
        }
        private void LoadViewDataBeforeOnEdit(T_ClaimType t_claimtype)
        {		
			var T_EmployeeInjury_T_TypeofClaimlist = db.T_EmployeeInjurys.OrderBy(p => p.DisplayValue).Take(10).Distinct();
            if (t_claimtype.SelectedT_EmployeeInjury_T_TypeofClaim != null)
            {
            ...
            else
            {
               ...
               var _objT_ClaimTypeFacilityAssociation = new List<T_Facility>();
               _objT_ClaimTypeFacilityAssociation.Add(t_claimtype.t_claimtypefacilityassociation);
			   			   ViewBag.T_ClaimTypeFacilityAssociationID = new SelectList(_objT_ClaimTypeFacilityAssociation, "ID", "DisplayValue", t_claimtype.T_ClaimTypeFacilityAssociationID);
					CustomLoadViewDataListBeforeEdit(t_claimtype);
        }
        private void LoadViewDataAfterOnCreate(T_ClaimType t_claimtype)
        {			
			var T_EmployeeInjury_T_TypeofClaimlist = db.T_EmployeeInjurys.OrderBy(p => p.DisplayValue).Take(10).Distinct();
            ViewBag.SelectedT_EmployeeInjury_T_TypeofClaim = new MultiSelectList(T_EmployeeInjury_T_TypeofClaimlist, "ID", "DisplayValue");
			 ViewBag.T_ClaimTypeFacilityAssociationID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_claimtype.T_ClaimTypeFacilityAssociationID);
		CustomLoadViewDataListAfterOnCreate(t_claimtype);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			var T_EmployeeInjury_T_TypeofClaimlist = db.T_EmployeeInjurys.OrderBy(p => p.DisplayValue).Take(10).Distinct();
            ViewBag.SelectedT_EmployeeInjury_T_TypeofClaim = new MultiSelectList(T_EmployeeInjury_T_TypeofClaimlist, "ID", "DisplayValue");
			if(HostingEntityName == "T_Facility" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_ClaimTypeFacilityAssociation")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_ClaimTypeFacilityAssociationID = new SelectList(db.T_Facilitys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Facilitys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_ClaimTypeFacilityAssociation = new List<T_Facility>();
			 ViewBag.T_ClaimTypeFacilityAssociationID = new SelectList(objT_ClaimTypeFacilityAssociation , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_ClaimType> searchRecords(IQueryable<T_ClaimType> lstT_ClaimType, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_ClaimType = lstT_ClaimType.Where(s => (s.t_claimtypefacilityassociation!= null && (s.t_claimtypefacilityassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||s.T_TypeofClaim_t_claimtype.Any(c => c.t_employeeinjury.DisplayValue.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_ClaimType = lstT_ClaimType.Where(s => (s.t_claimtypefacilityassociation!= null && (s.t_claimtypefacilityassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||s.T_TypeofClaim_t_claimtype.Any(c => c.t_employeeinjury.DisplayValue.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_ClaimType;
        }
		private IQueryable<T_ClaimType> sortRecords(IQueryable<T_ClaimType> lstT_ClaimType, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_ClaimTypeFacilityAssociationID")
                return isAsc.ToLower() == "asc" ? lstT_ClaimType.OrderBy(p => p.t_claimtypefacilityassociation.DisplayValue) : lstT_ClaimType.OrderByDescending(p => p.t_claimtypefacilityassociation.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_ClaimType.Sort(sortBy,true) : lstT_ClaimType.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_ClaimType), "t_claimtype");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_ClaimType>)lstT_ClaimType.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_ClaimType.ElementType, lambda.Body.Type },
                    lstT_ClaimType.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Department")
            {
                ...
                url += "?search=";
						var T_ClaimTypeFacilityAssociation  = _Object.T_DepartmentFacilityAssociationID;
						url += "&t_claimtypefacilityassociation=" + T_ClaimTypeFacilityAssociation;									
            ...
            if (sourceEntity == "T_Position")
            {
                ...
                url += "?search=";
						var T_ClaimTypeFacilityAssociation  = _Object.T_FacilityAssignedToID;
						url += "&t_claimtypefacilityassociation=" + T_ClaimTypeFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Employee")
            {
                ...
                url += "?search=";
						var T_ClaimTypeFacilityAssociation  = _Object.T_EmployeeAtFacilityID;
						url += "&t_claimtypefacilityassociation=" + T_ClaimTypeFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DepartmentArea")
            {
                ...
                url += "?search=";
						var T_ClaimTypeFacilityAssociation  = _Object.T_DepartmentAreaEmployeeTypeAssociationID;
						url += "&t_claimtypefacilityassociation=" + T_ClaimTypeFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Restrictions")
            {
                ...
                url += "?search=";
						var T_ClaimTypeFacilityAssociation  = _Object.T_RestrictionsFacilityAssociationID;
						url += "&t_claimtypefacilityassociation=" + T_ClaimTypeFacilityAssociation;									
							var T_TypeOfRestrictions_t_restrictions_t_typeofclaim  = _Object.T_TypeOfRestrictions_t_restrictions.Select(p => p.T_EmployeeInjuryID);
							if (T_TypeOfRestrictions_t_restrictions_t_typeofclaim != null)
							{
								var str = String.Join(",", T_TypeOfRestrictions_t_restrictions_t_typeofclaim);
								url += "&t_typeofclaim=" + str;
							}
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_ClaimTypeFacilityAssociation  = _Object.T_FacilityUnitXID;
						url += "&t_claimtypefacilityassociation=" + T_ClaimTypeFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Unit")
            {
                ...
                url += "?search=";
						var T_ClaimTypeFacilityAssociation  = _Object.T_FacilityUnitID;
						url += "&t_claimtypefacilityassociation=" + T_ClaimTypeFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_SalaryRange")
            {
                ...
                url += "?search=";
						var T_ClaimTypeFacilityAssociation  = _Object.T_FacilitySalaryRangeAssociationID;
						url += "&t_claimtypefacilityassociation=" + T_ClaimTypeFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_FacilityConfiguration")
            {
                ...
                url += "?search=";
						var T_ClaimTypeFacilityAssociation  = _Object.T_TenantConfigurationAssociationID;
						url += "&t_claimtypefacilityassociation=" + T_ClaimTypeFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_claimtypefacilityassociation,string t_typeofclaim, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_ClaimTypeFacilityAssociationlist = db.T_Facilitys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
                ...
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Facility= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_ClaimTypeFacilityAssociationlist.Union(db.T_Facilitys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
					ViewBag.t_claimtypefacilityassociation = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_ClaimTypeFacilityAssociationlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
				ViewBag.t_claimtypefacilityassociation = new SelectList(list, "ID", "DisplayValue");
			}
			var T_TypeofClaimlist = db.T_EmployeeInjurys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
                ...
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Employee Injury= ";
				  foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_TypeofClaimlist.Union(db.T_EmployeeInjurys.Where(p=>ids1.Contains(p.Id))).Distinct().Select(p=>new { ID=p.Id,DisplayValue=p.DisplayValue }).ToList();
			...
			{
			var objT_ClaimTypeFacilityAssociation = new List<T_Facility>();
		    ViewBag.t_claimtypefacilityassociation = new SelectList(objT_ClaimTypeFacilityAssociation, "ID", "DisplayValue");
			var objT_TypeofClaim = new List<T_EmployeeInjury>();
		    ViewBag.t_typeofclaim = new SelectList(objT_TypeofClaim, "ID", "DisplayValue");
				...
				ViewBag.EntityName = "T_ClaimType";
			...
			columns.Add("3", "Name");
				...
				return View(new T_ClaimType());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_ClaimType", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_ClaimType", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_ClaimType()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_ClaimType/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_claimtypefacilityassociation,string t_typeofclaim ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_ClaimType  = searchRecords(lstT_ClaimType, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_ClaimType = searchRecords(lstT_ClaimType, search,true);
			}
			...
			lstT_ClaimType = lstT_ClaimType.Include(t=>t.t_claimtypefacilityassociation);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_ClaimType", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_ClaimType = lstT_ClaimType.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_ClaimType = Sorting.Sort<T_ClaimType>(lstT_ClaimType, DataOrdering);
            var _T_ClaimType = lstT_ClaimType;
			//if (lstT_ClaimType.Where(p => p.t_claimtypefacilityassociation != null).Count() <= 50)
		    //ViewBag.t_claimtypefacilityassociation = new SelectList(lstT_ClaimType.Where(p => p.t_claimtypefacilityassociation != null).Select(P => P.t_claimtypefacilityassociation).Distinct(), "ID", "DisplayValue");
            if (t_claimtypefacilityassociation != null)
            {
                var ids = t_claimtypefacilityassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_ClaimType = _T_ClaimType.Where(p => ids1.Contains(p.T_ClaimTypeFacilityAssociationID));
            }
				if (t_typeofclaim != null)
            {
                var ids = t_typeofclaim.Split(",".ToCharArray());
                ...
                ids1 = ids1.ToList();
                _T_ClaimType = _T_ClaimType.Where(p => ids1.Contains(p.Id));
            }
			if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ClaimType");
			...
			if (!Request.IsAjaxRequest())
			{
			  ...
			  if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_ClaimType", tagsSplit.ToArray());
                    }
		...
		var entity = "T_ClaimType";
           ...
           var entitylist = db.T_ClaimTypes;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_ClaimTypes.GroupBy(p => p.t_claimtypefacilityassociation.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Facility", "Claim Type"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_ClaimType", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_ClaimTypes.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_claimtype = jedb.T_ClaimTypes.Find(id);
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_ClaimTypes.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_ClaimTypes.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_ClaimType>(_Obj, "T_ClaimType", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_ClaimType> list = db.T_ClaimTypes;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_ClaimTypes;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_ClaimType), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_ClaimType, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_ClaimType>)q);
				   list = ((IQueryable<T_ClaimType>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_ClaimTypes;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_ClaimType>(User,list, "T_ClaimType",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_ClaimType>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_ClaimType), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_ClaimType).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_ClaimType), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_ClaimType), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_ClaimType), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_ClaimType), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_ClaimType), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_ClaimType), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_ClaimType), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_ClaimType), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_ClaimType", User) || !User.CanAdd("T_ClaimType"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_ClaimType")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_ClaimType").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_ClaimType");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_ClaimType" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Claim Types";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_ClaimTypeFacilityAssociationID":
										 var t_claimtypefacilityassociationId = db.T_Facilitys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_claimtypefacilityassociationId == null)
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_ClaimTypeFacilityAssociationID":
										 var strPropertyT_ClaimTypeFacilityAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_ClaimTypeFacilityAssociationID").Value;
										 ModelReflector.Property propT_ClaimTypeFacilityAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Facility").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_ClaimTypeFacilityAssociation);
										 //var elementTypeT_ClaimTypeFacilityAssociation = db.T_Facilitys.ElementType;
										 //var propertyInfoT_ClaimTypeFacilityAssociation = elementTypeT_ClaimTypeFacilityAssociation.GetProperty(propT_ClaimTypeFacilityAssociation.Name);
										 // var t_claimtypefacilityassociationId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_ClaimTypeFacilityAssociation.GetValue(p, null)) == assovalue);
										 var t_claimtypefacilityassociationId = db.T_Facilitys.Where(propT_ClaimTypeFacilityAssociation.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_claimtypefacilityassociationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_ClaimType model = new T_ClaimType();
	...
	break;
					 case "T_ClaimTypeFacilityAssociationID":
		 dynamic t_claimtypefacilityassociationId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_ClaimTypeFacilityAssociation = db.T_Facilitys.ElementType;
			 //var propertyInfoT_ClaimTypeFacilityAssociation = elementTypeT_ClaimTypeFacilityAssociation.GetProperty(propT_ClaimTypeFacilityAssociation.Name);			 
			 //t_claimtypefacilityassociationId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_ClaimTypeFacilityAssociation.GetValue(p, null)) == columnValue);
			 t_claimtypefacilityassociationId = db.T_Facilitys.Where(propT_ClaimTypeFacilityAssociation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_claimtypefacilityassociationId = db.T_Facilitys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_claimtypefacilityassociationId != null)
			model.T_ClaimTypeFacilityAssociationID = t_claimtypefacilityassociationId.Id;
		  else
			{
				if ((collection["T_ClaimTypeFacilityAssociationID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_ClaimTypeFacilityAssociationID = objT_Facility.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_ClaimType validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_ClaimTypes.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_ClaimType OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_ClaimType").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_ClaimType");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_ClaimType OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_ClaimType");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_ClaimType OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_ClaimType");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_ClaimType OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_ClaimType",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_ClaimType OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_ClaimType");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_ClaimType OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ClaimType").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_ClaimTypes.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_ClaimTypes;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_ClaimType, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_ClaimType, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_ClaimType>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_ClaimType t_claimtype)
        {
        ...
        public ActionResult GetCalculationValues(T_ClaimType t_claimtype)
        {
            t_claimtype.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_claimtype.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_ClaimType"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_ClaimType t_claimtype, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_ClaimType t_claimtype, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
