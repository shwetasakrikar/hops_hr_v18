...
{	
    ...
    {
		private void LoadViewDataForCount(T_LeaveProfileReason t_leaveprofilereason, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_LeaveProfileReason t_leaveprofilereason)
        {		
			 ViewBag.T_LeaveReasonID = new SelectList(db.T_LeaveReasons, "ID", "DisplayValue", t_leaveprofilereason.T_LeaveReasonID);
			 ViewBag.T_LeaveProfileID = new SelectList(db.T_LeaveProfiles, "ID", "DisplayValue", t_leaveprofilereason.T_LeaveProfileID);
			CustomLoadViewDataListAfterEdit(t_leaveprofilereason);
        }
        private void LoadViewDataBeforeOnEdit(T_LeaveProfileReason t_leaveprofilereason)
        {		
               var _objT_LeaveReason = new List<T_LeaveReason>();
               _objT_LeaveReason.Add(t_leaveprofilereason.t_leavereason);
			   			   ViewBag.T_LeaveReasonID = new SelectList(_objT_LeaveReason, "ID", "DisplayValue", t_leaveprofilereason.T_LeaveReasonID);
			               var _objT_LeaveProfile = new List<T_LeaveProfile>();
               _objT_LeaveProfile.Add(t_leaveprofilereason.t_leaveprofile);
			   			   ViewBag.T_LeaveProfileID = new SelectList(_objT_LeaveProfile, "ID", "DisplayValue", t_leaveprofilereason.T_LeaveProfileID);
					CustomLoadViewDataListBeforeEdit(t_leaveprofilereason);
        }
        private void LoadViewDataAfterOnCreate(T_LeaveProfileReason t_leaveprofilereason)
        {			
			 ViewBag.T_LeaveReasonID = new SelectList(db.T_LeaveReasons, "ID", "DisplayValue", t_leaveprofilereason.T_LeaveReasonID);
			 ViewBag.T_LeaveProfileID = new SelectList(db.T_LeaveProfiles, "ID", "DisplayValue", t_leaveprofilereason.T_LeaveProfileID);
		CustomLoadViewDataListAfterOnCreate(t_leaveprofilereason);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_LeaveReason" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_LeaveProfileReason_T_LeaveReason")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_LeaveReasonID = new SelectList(db.T_LeaveReasons.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_LeaveReasons.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
								//Manytomany associations
								var T_LeaveProfileReasonlist = db.T_LeaveProfiles.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
								ViewBag.t_leaveprofilereason = new SelectList(T_LeaveProfileReasonlist, "ID", "DisplayValue");
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_LeaveProfileReasons.Where(p => p.T_LeaveProfileID == hostid).Select(q => q.T_LeaveReasonID);
		var AvailableList = db.T_LeaveReasons.Where(p => !Selectedlist.Contains(p.Id));
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        AvailableList = _fad.FilterDropdown<T_LeaveReason>(User, AvailableList, "T_LeaveReason", "T_LeaveProfileReason.T_LeaveReasonID");
                ViewBag.IsTop50 = "none";
				if (AvailableList.Count() > 50)
                    ViewBag.IsTop50 = "block";
				ViewBag.T_LeaveReasonIDAvailable  = new SelectList(AvailableList.OrderBy(p => p.DisplayValue).Take(50).ToList(), "ID", "DisplayValue");
				var SelectedListComplete = db.T_LeaveReasons.Where(p => Selectedlist.Contains(p.Id));
				var SelectedListFiltered = db.T_LeaveReasons.Where(p => Selectedlist.Contains(p.Id));
				SelectedListFiltered = _fad.FilterDropdown<T_LeaveReason>(User, SelectedListFiltered, "T_LeaveReason", "T_LeaveProfileReason.T_LeaveReasonID");
				ViewBag.SelectedListRemoved= string.Join(",", SelectedListComplete.Except(SelectedListFiltered).Select(p=>p.Id).ToList());
                ViewBag.T_LeaveReasonIDSelected = new MultiSelectList(SelectedListComplete.OrderBy(p => p.DisplayValue).ToList(), "ID", "DisplayValue", Selectedlist);
		    }
			if(HostingEntityName == "T_LeaveProfile" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_LeaveProfileReason_T_LeaveProfile")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_LeaveProfileID = new SelectList(db.T_LeaveProfiles.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_LeaveProfiles.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
						//ManytoOne association
						var T_EmployeeLeaveProfilelist = db.T_Employees.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_employeeleaveprofile = new SelectList(T_EmployeeLeaveProfilelist, "ID", "DisplayValue");
						//ManytoOne association
						var T_InjuryRelatedLeavelist = db.T_EmployeeInjurys.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_injuryrelatedleave = new SelectList(T_InjuryRelatedLeavelist, "ID", "DisplayValue");
								//Manytomany associations
								var T_LeaveProfileReasonlist = db.T_LeaveReasons.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
								ViewBag.t_leaveprofilereason = new SelectList(T_LeaveProfileReasonlist, "ID", "DisplayValue");
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_LeaveProfileReasons.Where(p => p.T_LeaveReasonID == hostid).Select(q => q.T_LeaveProfileID);
		var AvailableList = db.T_LeaveProfiles.Where(p => !Selectedlist.Contains(p.Id));
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        AvailableList = _fad.FilterDropdown<T_LeaveProfile>(User, AvailableList, "T_LeaveProfile", "T_LeaveProfileReason.T_LeaveProfileID");
                ViewBag.IsTop50 = "none";
				if (AvailableList.Count() > 50)
                    ViewBag.IsTop50 = "block";
				ViewBag.T_LeaveProfileIDAvailable  = new SelectList(AvailableList.OrderBy(p => p.DisplayValue).Take(50).ToList(), "ID", "DisplayValue");
				var SelectedListComplete = db.T_LeaveProfiles.Where(p => Selectedlist.Contains(p.Id));
				var SelectedListFiltered = db.T_LeaveProfiles.Where(p => Selectedlist.Contains(p.Id));
				SelectedListFiltered = _fad.FilterDropdown<T_LeaveProfile>(User, SelectedListFiltered, "T_LeaveProfile", "T_LeaveProfileReason.T_LeaveProfileID");
				ViewBag.SelectedListRemoved= string.Join(",", SelectedListComplete.Except(SelectedListFiltered).Select(p=>p.Id).ToList());
                ViewBag.T_LeaveProfileIDSelected = new MultiSelectList(SelectedListComplete.OrderBy(p => p.DisplayValue).ToList(), "ID", "DisplayValue", Selectedlist);
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_LeaveProfileReason> searchRecords(IQueryable<T_LeaveProfileReason> lstT_LeaveProfileReason, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_LeaveProfileReason = lstT_LeaveProfileReason.Where(s => (s.t_leavereason!= null && (s.t_leavereason.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_leaveprofile!= null && (s.t_leaveprofile.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_LeaveProfileReason = lstT_LeaveProfileReason.Where(s => (s.t_leavereason!= null && (s.t_leavereason.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_leaveprofile!= null && (s.t_leaveprofile.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_LeaveProfileReason;
        }
		private IQueryable<T_LeaveProfileReason> sortRecords(IQueryable<T_LeaveProfileReason> lstT_LeaveProfileReason, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_LeaveReasonID")
                return isAsc.ToLower() == "asc" ? lstT_LeaveProfileReason.OrderBy(p => p.t_leavereason.DisplayValue) : lstT_LeaveProfileReason.OrderByDescending(p => p.t_leavereason.DisplayValue);
	 if(sortBy == "T_LeaveProfileID")
                return isAsc.ToLower() == "asc" ? lstT_LeaveProfileReason.OrderBy(p => p.t_leaveprofile.DisplayValue) : lstT_LeaveProfileReason.OrderByDescending(p => p.t_leaveprofile.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_LeaveProfileReason.Sort(sortBy,true) : lstT_LeaveProfileReason.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_LeaveProfileReason), "t_leaveprofilereason");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_LeaveProfileReason>)lstT_LeaveProfileReason.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_LeaveProfileReason.ElementType, lambda.Body.Type },
                    lstT_LeaveProfileReason.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Comment")
            {
                ...
                url += "?search=";
						var T_LeaveProfile  = _Object.T_LeaveCommentsID;
						url += "&t_leaveprofile=" + T_LeaveProfile;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_leavereason,string t_leaveprofile, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_LeaveReasonlist = db.T_LeaveReasons.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_leavereason != null)
            {
                var ids = t_leavereason.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Leave Reason= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_LeaveReasonlist.Union(db.T_LeaveReasons.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_LeaveReason>(User, list, "T_LeaveReason", null);
					ViewBag.t_leavereason = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_LeaveReasonlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_LeaveReason>(User, list, "T_LeaveReason", null);
				ViewBag.t_leavereason = new SelectList(list, "ID", "DisplayValue");
			}
			var T_LeaveProfilelist = db.T_LeaveProfiles.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_leaveprofile != null)
            {
                var ids = t_leaveprofile.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Leave= ";
				   foreach (var str in ids)
                    {
					...
					ids1 = ids1.ToList();
					var list = T_LeaveProfilelist.Union(db.T_LeaveProfiles.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_LeaveProfile>(User, list, "T_LeaveProfile", null);
					ViewBag.t_leaveprofile = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_LeaveProfilelist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_LeaveProfile>(User, list, "T_LeaveProfile", null);
				ViewBag.t_leaveprofile = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_LeaveReason = new List<T_LeaveReason>();
		    ViewBag.t_leavereason = new SelectList(objT_LeaveReason, "ID", "DisplayValue");
			var objT_LeaveProfile = new List<T_LeaveProfile>();
		    ViewBag.t_leaveprofile = new SelectList(objT_LeaveProfile, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_LeaveProfileReason";
			...
			columns.Add("2", "Leave Reason");
			columns.Add("3", "Leave");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				return View(new T_LeaveProfileReason());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_LeaveProfileReason", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_LeaveProfileReason", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_LeaveProfileReason()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_LeaveProfileReason/FSearch/
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_leavereason,string t_leaveprofile ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_LeaveProfileReason  = searchRecords(lstT_LeaveProfileReason, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_LeaveProfileReason = searchRecords(lstT_LeaveProfileReason, search,true);
			}
			...
			lstT_LeaveProfileReason = lstT_LeaveProfileReason.Include(t=>t.t_leavereason).Include(t=>t.t_leaveprofile);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_LeaveProfileReason", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_LeaveProfileReason = lstT_LeaveProfileReason.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_LeaveProfileReason = Sorting.Sort<T_LeaveProfileReason>(lstT_LeaveProfileReason, DataOrdering);
            var _T_LeaveProfileReason = lstT_LeaveProfileReason;
			//if (lstT_LeaveProfileReason.Where(p => p.t_leavereason != null).Count() <= 50)
		    //ViewBag.t_leavereason = new SelectList(lstT_LeaveProfileReason.Where(p => p.t_leavereason != null).Select(P => P.t_leavereason).Distinct(), "ID", "DisplayValue");
            if (t_leavereason != null)
            {
                var ids = t_leavereason.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_LeaveReasons.Find(Convert.ToInt64(str));
                ...
                ids1 = ids1.ToList();
                _T_LeaveProfileReason = _T_LeaveProfileReason.Where(p => ids1.Contains(p.T_LeaveReasonID));
            }
				//if (lstT_LeaveProfileReason.Where(p => p.t_leaveprofile != null).Count() <= 50)
		    //ViewBag.t_leaveprofile = new SelectList(lstT_LeaveProfileReason.Where(p => p.t_leaveprofile != null).Select(P => P.t_leaveprofile).Distinct(), "ID", "DisplayValue");
            if (t_leaveprofile != null)
            {
                var ids = t_leaveprofile.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_LeaveProfileReason = _T_LeaveProfileReason.Where(p => ids1.Contains(p.T_LeaveProfileID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_LeaveProfileReason");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_LeaveProfileReason", tagsSplit.ToArray());
                    }
        ...
        {
		string result = "";
		var entity = "T_LeaveProfileReason";
           ...
           var entitylist = db.T_LeaveProfileReasons;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_LeaveProfileReasons.GroupBy(p => p.t_leavereason.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Leave Reason (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Leave Reason"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Leave Reason", "Leave Profile Reason"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_LeaveProfileReason", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_LeaveProfileReasons.GroupBy(p => p.t_leaveprofile.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "2" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Leave (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Leave"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Leave", "Leave Profile Reason"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_LeaveProfileReason", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_LeaveProfileReasons.FirstOrDefault(p => p.Id == idvalue);
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_LeaveProfileReasons.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_LeaveProfileReasons.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_LeaveProfileReason>(_Obj, "T_LeaveProfileReason", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_LeaveProfileReason> list = db.T_LeaveProfileReasons;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_LeaveProfileReasons;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_LeaveProfileReason), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_LeaveProfileReason, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_LeaveProfileReason>)q);
				   list = ((IQueryable<T_LeaveProfileReason>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_LeaveProfileReasons;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_LeaveProfileReason>(User,list, "T_LeaveProfileReason",caller);
        ...
        {
			int pagesize = 50;
			if (HostingEntityName == "T_LeaveReason")
            {
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_LeaveProfileReasons.Where(p => p.T_LeaveReasonID == hostid).Select(q => q.T_LeaveProfileID);
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        var Finallist = _fad.FilterDropdown<T_LeaveProfile>(User, db.T_LeaveProfiles.Where(p => !Selectedlist.Contains(p.Id)), "T_LeaveProfile", "T_LeaveProfileReason.T_LeaveProfileID").ToList();
                if(!string.IsNullOrEmpty(SearchString))
            ...
            if (HostingEntityName == "T_LeaveProfile")
            {
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_LeaveProfileReasons.Where(p => p.T_LeaveProfileID == hostid).Select(q => q.T_LeaveReasonID);
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
         var Finallist = _fad.FilterDropdown<T_LeaveReason>(User, db.T_LeaveReasons.Where(p => !Selectedlist.Contains(p.Id)), "T_LeaveReason", "T_LeaveProfileReason.T_LeaveReasonID").ToList();
                if(!string.IsNullOrEmpty(SearchString))
                    Finallist = Finallist.Where(p=>p.DisplayValue.ToUpper().Contains(SearchString.ToUpper())).OrderBy(p => p.DisplayValue).ToList();
                else
                    Finallist = Finallist.OrderBy(p => p.DisplayValue).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValues1(string SearchString, string HostingEntityName, string HostingEntityID, string AssociatedType, string size , string t_leaveprofilereason,string t_employeeleaveprofile,string t_injuryrelatedleave,string T_Intermittent,string T_FullDays  )
        {
			int pagesize = 50;
			if (HostingEntityName == "T_LeaveReason")
            {
        ...
        var Finallist = _fad.FilterDropdown<T_LeaveProfile>(User, db.T_LeaveProfiles.Where(p => !Selectedlist.Contains(p.Id)), "T_LeaveProfile", "T_LeaveProfileReason.T_LeaveProfileID").ToList();
			if(T_Intermittent!=null)
            {
                try
                {
                    ...
                    Finallist =  Finallist.Where(o => o.T_Intermittent == boolvalue).ToList();
                }
                catch { }
            }
			if(T_FullDays!=null)
            {
                try
                {
                    ...
                    Finallist =  Finallist.Where(o => o.T_FullDays == boolvalue).ToList();
                }
                catch { }
            }
				if(!string.IsNullOrEmpty(SearchString))
			...
			if (t_leaveprofilereason != null)
            {
               ...
               Finallist = Finallist.Where(p => ids1.Contains(p.Id)).ToList();
				...
				if (t_employeeleaveprofile != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_EmployeeLeaveProfileID)).ToList();
                }
				if (t_injuryrelatedleave != null)
                {
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_InjuryRelatedLeaveID)).ToList();
                }
				var ListCount = Finallist.Count();
				var Results= false;
				 if (!string.IsNullOrEmpty(size))
			...
			if (t_leaveprofilereason != null)
            {
   			 ...
   			 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                        var idvalue= Convert.ToInt64(str);
                        ids1.AddRange(db.T_LeaveProfileReasons.Where(p=>p.T_LeaveProfileID ==idvalue).Select(p=>p.T_LeaveReasonID));
                        }
                    }
                    //
                }
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_LeaveProfileReason>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_LeaveProfileReason), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_LeaveProfileReason).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_LeaveProfileReason), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_LeaveProfileReason), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_LeaveProfileReason), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_LeaveProfileReason), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_LeaveProfileReason), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_LeaveProfileReason), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_LeaveProfileReason), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_LeaveProfileReason), "p"));
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_LeaveProfileReason>(User, list, "T_LeaveProfileReason", null);
			if (key != null && key.Length > 0)
            {
			   var data = from x in list.Where(p=>p.DisplayValue.Contains(key)).OrderBy(q=>q.DisplayValue).Take(10).ToList()
                           select new { Id = x.Id, Name = x.DisplayValue };
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_LeaveProfileReason", User) || !User.CanAdd("T_LeaveProfileReason"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_LeaveProfileReason")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_LeaveProfileReason").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_LeaveProfileReason");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_LeaveProfileReason" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Leave Profile Reasons";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_LeaveReasonID":
										 var t_leavereasonId = db.T_LeaveReasons.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_leavereasonId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_LeaveProfileID":
										 var t_leaveprofileId = db.T_LeaveProfiles.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_leaveprofileId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                        default:
                                            break;
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_LeaveProfileReason";
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_LeaveReasonID":
										 var strPropertyT_LeaveReason = lstEntityProp.FirstOrDefault(p => p.Key == "T_LeaveReasonID").Value;
										 ModelReflector.Property propT_LeaveReason = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_LeaveReason").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_LeaveReason);
										 //var elementTypeT_LeaveReason = db.T_LeaveReasons.ElementType;
										 //var propertyInfoT_LeaveReason = elementTypeT_LeaveReason.GetProperty(propT_LeaveReason.Name);
										 // var t_leavereasonId = db.T_LeaveReasons.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_LeaveReason.GetValue(p, null)) == assovalue);
										 var t_leavereasonId = db.T_LeaveReasons.Where(propT_LeaveReason.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_leavereasonId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_LeaveProfileID":
										 var strPropertyT_LeaveProfile = lstEntityProp.FirstOrDefault(p => p.Key == "T_LeaveProfileID").Value;
										 ModelReflector.Property propT_LeaveProfile = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_LeaveProfile").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_LeaveProfile);
										 //var elementTypeT_LeaveProfile = db.T_LeaveProfiles.ElementType;
										 //var propertyInfoT_LeaveProfile = elementTypeT_LeaveProfile.GetProperty(propT_LeaveProfile.Name);
										 // var t_leaveprofileId = db.T_LeaveProfiles.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_LeaveProfile.GetValue(p, null)) == assovalue);
										 var t_leaveprofileId = db.T_LeaveProfiles.Where(propT_LeaveProfile.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_leaveprofileId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
        ...
        public ActionResult ImportData(FormCollection collection)
        {
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_LeaveProfileReason model = new T_LeaveProfileReason();
		 ...
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_LeaveReason = db.T_LeaveReasons.ElementType;
			 //var propertyInfoT_LeaveReason = elementTypeT_LeaveReason.GetProperty(propT_LeaveReason.Name);			 
			 //t_leavereasonId = db.T_LeaveReasons.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_LeaveReason.GetValue(p, null)) == columnValue);
			 t_leavereasonId = db.T_LeaveReasons.Where(propT_LeaveReason.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_leavereasonId = db.T_LeaveReasons.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_leavereasonId != null)
			model.T_LeaveReasonID = t_leavereasonId.Id;
		  else
			{
				if ((collection["T_LeaveReasonID"].ToString() == "true,false"))
				{
					try
					{
						T_LeaveReason objT_LeaveReason = new T_LeaveReason();
						objT_LeaveReason.T_Name  = (columnValue);
						db.T_LeaveReasons.Add(objT_LeaveReason);
						db.SaveChanges();
						model.T_LeaveReasonID = objT_LeaveReason.Id;
					}
					catch
					{
						continue;
		 ...
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_LeaveProfile = db.T_LeaveProfiles.ElementType;
			 //var propertyInfoT_LeaveProfile = elementTypeT_LeaveProfile.GetProperty(propT_LeaveProfile.Name);			 
			 //t_leaveprofileId = db.T_LeaveProfiles.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_LeaveProfile.GetValue(p, null)) == columnValue);
			 t_leaveprofileId = db.T_LeaveProfiles.Where(propT_LeaveProfile.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_leaveprofileId = db.T_LeaveProfiles.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_leaveprofileId != null)
			model.T_LeaveProfileID = t_leaveprofileId.Id;
         break;
            default:
                break;
        }
					    ...
					    if (ValidateModel(model))
                        {
                            ...
                            if (result == null || result.Count == 0)
                            {
								...
								{
									db.T_LeaveProfileReasons.Add(model);
									db.SaveChanges();
								}
								error += customerror;
							}
		...
		public bool ValidateModel(T_LeaveProfileReason validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_LeaveProfileReasons.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_LeaveProfileReason OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_LeaveProfileReason").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_LeaveProfileReason");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_LeaveProfileReason OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_LeaveProfileReason");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_LeaveProfileReason OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_LeaveProfileReason");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_LeaveProfileReason OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_LeaveProfileReason",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_LeaveProfileReason OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_LeaveProfileReason");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_LeaveProfileReason OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_LeaveProfileReason").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_LeaveProfileReasons.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_LeaveProfileReasons;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_LeaveProfileReason, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_LeaveProfileReason, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_LeaveProfileReason>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_LeaveProfileReason t_leaveprofilereason)
        {
        ...
        public ActionResult GetCalculationValues(T_LeaveProfileReason t_leaveprofilereason)
        {
            t_leaveprofilereason.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_leaveprofilereason.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_LeaveProfileReason"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_LeaveProfileReason t_leaveprofilereason, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_LeaveProfileReason t_leaveprofilereason, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
