...
{	
    ...
    {
		private void LoadViewDataForCount(T_LanguageCertifiedIn t_languagecertifiedin, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_LanguageCertifiedIn t_languagecertifiedin)
        {		
			 ViewBag.T_EmployeeID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_languagecertifiedin.T_EmployeeID);
			 ViewBag.T_LangaugeID = new SelectList(db.T_Langauges, "ID", "DisplayValue", t_languagecertifiedin.T_LangaugeID);
			CustomLoadViewDataListAfterEdit(t_languagecertifiedin);
        }
        private void LoadViewDataBeforeOnEdit(T_LanguageCertifiedIn t_languagecertifiedin)
        {		
               var _objT_Employee = new List<T_Employee>();
               _objT_Employee.Add(t_languagecertifiedin.t_employee);
			   			   ViewBag.T_EmployeeID = new SelectList(_objT_Employee, "ID", "DisplayValue", t_languagecertifiedin.T_EmployeeID);
			               var _objT_Langauge = new List<T_Langauge>();
               _objT_Langauge.Add(t_languagecertifiedin.t_langauge);
			   			   ViewBag.T_LangaugeID = new SelectList(_objT_Langauge, "ID", "DisplayValue", t_languagecertifiedin.T_LangaugeID);
					CustomLoadViewDataListBeforeEdit(t_languagecertifiedin);
        }
        private void LoadViewDataAfterOnCreate(T_LanguageCertifiedIn t_languagecertifiedin)
        {			
			 ViewBag.T_EmployeeID = new SelectList(db.T_Employees, "ID", "DisplayValue", t_languagecertifiedin.T_EmployeeID);
			 ViewBag.T_LangaugeID = new SelectList(db.T_Langauges, "ID", "DisplayValue", t_languagecertifiedin.T_LangaugeID);
		CustomLoadViewDataListAfterOnCreate(t_languagecertifiedin);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Employee" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_LanguageCertifiedIn_T_Employee")
			{
            ...
            else
			{
				...
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_LanguageCertifiedIns.Where(p => p.T_LangaugeID == hostid).Select(q => q.T_EmployeeID);
		var AvailableList = db.T_Employees.Where(p => !Selectedlist.Contains(p.Id));
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        AvailableList = _fad.FilterDropdown<T_Employee>(User, AvailableList, "T_Employee", "T_LanguageCertifiedIn.T_EmployeeID");
				...
				SelectedListFiltered = _fad.FilterDropdown<T_Employee>(User, SelectedListFiltered, "T_Employee", "T_LanguageCertifiedIn.T_EmployeeID");
			...
			if(HostingEntityName == "T_Langauge" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_LanguageCertifiedIn_T_Langauge")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
            ...
            else
			{
								...
								ViewBag.t_languagecertifiedin = new SelectList(T_LanguageCertifiedInlist, "ID", "DisplayValue");
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_LanguageCertifiedIns.Where(p => p.T_EmployeeID == hostid).Select(q => q.T_LangaugeID);
		var AvailableList = db.T_Langauges.Where(p => !Selectedlist.Contains(p.Id));
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        AvailableList = _fad.FilterDropdown<T_Langauge>(User, AvailableList, "T_Langauge", "T_LanguageCertifiedIn.T_LangaugeID");
                ViewBag.IsTop50 = "none";
				if (AvailableList.Count() > 50)
                    ViewBag.IsTop50 = "block";
				...
				SelectedListFiltered = _fad.FilterDropdown<T_Langauge>(User, SelectedListFiltered, "T_Langauge", "T_LanguageCertifiedIn.T_LangaugeID");
				ViewBag.SelectedListRemoved= string.Join(",", SelectedListComplete.Except(SelectedListFiltered).Select(p=>p.Id).ToList());
		...
		private IQueryable<T_LanguageCertifiedIn> searchRecords(IQueryable<T_LanguageCertifiedIn> lstT_LanguageCertifiedIn, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_LanguageCertifiedIn = lstT_LanguageCertifiedIn.Where(s => (s.t_employee!= null && (s.t_employee.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_langauge!= null && (s.t_langauge.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_LanguageCertifiedIn = lstT_LanguageCertifiedIn.Where(s => (s.t_employee!= null && (s.t_employee.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_langauge!= null && (s.t_langauge.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_LanguageCertifiedIn;
        }
		private IQueryable<T_LanguageCertifiedIn> sortRecords(IQueryable<T_LanguageCertifiedIn> lstT_LanguageCertifiedIn, string sortBy, string isAsc)
        {
            string methodName = "";
		    ...
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_LanguageCertifiedIn.Sort(sortBy,true) : lstT_LanguageCertifiedIn.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_LanguageCertifiedIn), "t_languagecertifiedin");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_LanguageCertifiedIn>)lstT_LanguageCertifiedIn.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_LanguageCertifiedIn.ElementType, lambda.Body.Type },
                    lstT_LanguageCertifiedIn.Expression,
                    lambda));
        }
        ...
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_employee,string t_langauge, bool? RenderPartial)
        {
		    ...
		    ViewBag.t_langauge = new SelectList(objT_Langauge, "ID", "DisplayValue");
				...
				ViewBag.EntityName = "T_LanguageCertifiedIn";
               ...
               = ViewBag.SuggestedPropertyValue = ViewBag.AssociationPropertyList = ViewBag.SuggestedDynamicValueInCondition71 = new SelectList(Dumplist, "key", "value");
				...
				return View(new T_LanguageCertifiedIn());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_LanguageCertifiedIn", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_LanguageCertifiedIn", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_LanguageCertifiedIn()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_LanguageCertifiedIn/FSearch/
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_employee,string t_langauge ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_LanguageCertifiedIn  = searchRecords(lstT_LanguageCertifiedIn, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_LanguageCertifiedIn = searchRecords(lstT_LanguageCertifiedIn, search,true);
			}
			...
			lstT_LanguageCertifiedIn = lstT_LanguageCertifiedIn.Include(t=>t.t_employee).Include(t=>t.t_langauge);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_LanguageCertifiedIn", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_LanguageCertifiedIn = lstT_LanguageCertifiedIn.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_LanguageCertifiedIn = Sorting.Sort<T_LanguageCertifiedIn>(lstT_LanguageCertifiedIn, DataOrdering);
            var _T_LanguageCertifiedIn = lstT_LanguageCertifiedIn;
			//if (lstT_LanguageCertifiedIn.Where(p => p.t_employee != null).Count() <= 50)
		    //ViewBag.t_employee = new SelectList(lstT_LanguageCertifiedIn.Where(p => p.t_employee != null).Select(P => P.t_employee).Distinct(), "ID", "DisplayValue");
            if (t_employee != null)
            {
                var ids = t_employee.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_LanguageCertifiedIn = _T_LanguageCertifiedIn.Where(p => ids1.Contains(p.T_EmployeeID));
            }
				//if (lstT_LanguageCertifiedIn.Where(p => p.t_langauge != null).Count() <= 50)
		    //ViewBag.t_langauge = new SelectList(lstT_LanguageCertifiedIn.Where(p => p.t_langauge != null).Select(P => P.t_langauge).Distinct(), "ID", "DisplayValue");
            if (t_langauge != null)
            {
                var ids = t_langauge.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_LanguageCertifiedIn = _T_LanguageCertifiedIn.Where(p => ids1.Contains(p.T_LangaugeID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_LanguageCertifiedIn");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_LanguageCertifiedIn", tagsSplit.ToArray());
                    }
		...
		var entity = "T_LanguageCertifiedIn";
           ...
           var entitylist = db.T_LanguageCertifiedIns;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_LanguageCertifiedIns.GroupBy(p => p.t_employee.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Employee Certified in the Language (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Employee Certified in the Language"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Employee Certified in the Language", "Language Certified In"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_LanguageCertifiedIn", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_LanguageCertifiedIns.GroupBy(p => p.t_langauge.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "2" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Languages Certified In (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Languages Certified In"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Languages Certified In", "Language Certified In"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_LanguageCertifiedIn", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_LanguageCertifiedIns.FirstOrDefault(p => p.Id == idvalue);
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_LanguageCertifiedIns.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_LanguageCertifiedIns.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_LanguageCertifiedIn>(_Obj, "T_LanguageCertifiedIn", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_LanguageCertifiedIn> list = db.T_LanguageCertifiedIns;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_LanguageCertifiedIns;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_LanguageCertifiedIn), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_LanguageCertifiedIn, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_LanguageCertifiedIn>)q);
				   list = ((IQueryable<T_LanguageCertifiedIn>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_LanguageCertifiedIns;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_LanguageCertifiedIn>(User,list, "T_LanguageCertifiedIn",caller);
        ...
        public JsonResult GetAllValues(string SearchString, string HostingEntityName, string HostingEntityID, string AssociatedType, string size)
        {
			...
			if (HostingEntityName == "T_Employee")
            {
                ...
                var Selectedlist = db.T_LanguageCertifiedIns.Where(p => p.T_EmployeeID == hostid).Select(q => q.T_LangaugeID);
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        var Finallist = _fad.FilterDropdown<T_Langauge>(User, db.T_Langauges.Where(p => !Selectedlist.Contains(p.Id)), "T_Langauge", "T_LanguageCertifiedIn.T_LangaugeID").ToList();
                if(!string.IsNullOrEmpty(SearchString))
            ...
            if (HostingEntityName == "T_Langauge")
            {
                ...
                var Selectedlist = db.T_LanguageCertifiedIns.Where(p => p.T_LangaugeID == hostid).Select(q => q.T_EmployeeID);
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
         var Finallist = _fad.FilterDropdown<T_Employee>(User, db.T_Employees.Where(p => !Selectedlist.Contains(p.Id)), "T_Employee", "T_LanguageCertifiedIn.T_EmployeeID").ToList();
                if(!string.IsNullOrEmpty(SearchString))
                    Finallist = Finallist.Where(p=>p.DisplayValue.ToUpper().Contains(SearchString.ToUpper())).OrderBy(p => p.DisplayValue).ToList();
                else
                    Finallist = Finallist.OrderBy(p => p.DisplayValue).ToList();
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_LanguageCertifiedIn>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_LanguageCertifiedIn), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_LanguageCertifiedIn).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_LanguageCertifiedIn), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_LanguageCertifiedIn), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_LanguageCertifiedIn), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_LanguageCertifiedIn), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_LanguageCertifiedIn), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_LanguageCertifiedIn), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_LanguageCertifiedIn), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_LanguageCertifiedIn), "p"));
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_LanguageCertifiedIn>(User, list, "T_LanguageCertifiedIn", null);
			if (key != null && key.Length > 0)
            {
			   var data = from x in list.Where(p=>p.DisplayValue.Contains(key)).OrderBy(q=>q.DisplayValue).Take(10).ToList()
                           select new { Id = x.Id, Name = x.DisplayValue };
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_LanguageCertifiedIn", User) || !User.CanAdd("T_LanguageCertifiedIn"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_LanguageCertifiedIn")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_LanguageCertifiedIn").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_LanguageCertifiedIn");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        string colDisKey = "";
                        string colListInx = "";
                        typeName = "T_LanguageCertifiedIn";
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_LanguageCertifiedIn" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Language Certified Ins";
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_LanguageCertifiedIn";
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_LanguageCertifiedIn model = new T_LanguageCertifiedIn();
                ...
                break;
					    ...
					    if (ValidateModel(model))
                        {
                            ...
                            if (result == null || result.Count == 0)
                            {
								...
								{
									db.T_LanguageCertifiedIns.Add(model);
									db.SaveChanges();
								}
								error += customerror;
							}
		...
		public bool ValidateModel(T_LanguageCertifiedIn validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_LanguageCertifiedIns.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_LanguageCertifiedIn OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_LanguageCertifiedIn").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_LanguageCertifiedIn");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_LanguageCertifiedIn OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_LanguageCertifiedIn");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_LanguageCertifiedIn OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_LanguageCertifiedIn");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_LanguageCertifiedIn OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_LanguageCertifiedIn",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_LanguageCertifiedIn OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_LanguageCertifiedIn");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_LanguageCertifiedIn OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_LanguageCertifiedIn").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_LanguageCertifiedIns.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_LanguageCertifiedIns;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_LanguageCertifiedIn, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_LanguageCertifiedIn, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_LanguageCertifiedIn>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_LanguageCertifiedIn t_languagecertifiedin)
        {
        ...
        public ActionResult GetCalculationValues(T_LanguageCertifiedIn t_languagecertifiedin)
        {
            t_languagecertifiedin.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_languagecertifiedin.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_LanguageCertifiedIn"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_LanguageCertifiedIn t_languagecertifiedin, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_LanguageCertifiedIn t_languagecertifiedin, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
