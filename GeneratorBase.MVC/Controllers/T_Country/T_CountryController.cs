...
{	
    ...
    {
		private void LoadViewDataForCount(T_Country t_country, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_Country t_country)
        {		
			CustomLoadViewDataListAfterEdit(t_country);
        }
        private void LoadViewDataBeforeOnEdit(T_Country t_country)
        {		
	 ViewBag.T_StateCountryCount = t_country.t_statecountry.Count();
	 ViewBag.T_CityCountryCount = t_country.t_citycountry.Count();
	 ViewBag.T_AddressCountryCount = t_country.t_addresscountry.Count();
		CustomLoadViewDataListBeforeEdit(t_country);
        }
        private void LoadViewDataAfterOnCreate(T_Country t_country)
        {			
		CustomLoadViewDataListAfterOnCreate(t_country);
        }
		...
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_Country = lstT_Country.Where(s => (!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_Country = lstT_Country.Where(s => (!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_Country;
        }
		private IQueryable<T_Country> sortRecords(IQueryable<T_Country> lstT_Country, string sortBy, string isAsc)
        {
            string methodName = "";
		    ...
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_Country.Sort(sortBy,true) : lstT_Country.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_Country), "t_country");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_Country>)lstT_Country.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_Country.ElementType, lambda.Body.Type },
                    lstT_Country.Expression,
                    lambda));
        }
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_Countrys.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_country = jedb.T_Countrys.Find(id);
			var T_StateCountryIds = jedb.T_States.Where(p=>p.T_StateCountryID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_State" && T_StateCountryIds.Contains(d.RecordId) && d.Type != "View"));
			var T_CityCountryIds = jedb.T_Citys.Where(p=>p.T_CityCountryID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_City" && T_CityCountryIds.Contains(d.RecordId) && d.Type != "View"));
			var T_AddressCountryIds = jedb.T_Addresss.Where(p=>p.T_AddressCountryID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_Address" && T_AddressCountryIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_Countrys.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_Countrys.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_Country>(_Obj, "T_Country", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_Country> list = db.T_Countrys;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_Countrys;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_Country), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_Country, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_Country>)q);
				   list = ((IQueryable<T_Country>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_Countrys;
            ...
            list = _fad.FilterDropdown<T_Country>(User,list, "T_Country",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_Country>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_Country), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_Country).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_Country), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Country), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Country), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Country), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Country), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Country), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_Country), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_Country), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_Country", User) || !User.CanAdd("T_Country"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_Country")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_Country").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_Country");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_Country" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Countrys";
	...
	break;
		...
		public bool ValidateModel(T_Country validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_Countrys.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_Country OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_Country").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_Country");
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p => p.TypeNo).Contains(4))
                    {
                        ...
                        foreach (var parametersArgs in Args)
                        {
                            ...
                            var entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Country");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_Country OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_Country");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_Country OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_Country");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_Country OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_Country",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_Country OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_Country");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_Country OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Country").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_Countrys.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_Countrys;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_Country, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_Country, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_Country>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_Country t_country)
        {
        ...
        public ActionResult GetCalculationValues(T_Country t_country)
        {
            t_country.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_country.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_Country t_country, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_Country t_country, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
