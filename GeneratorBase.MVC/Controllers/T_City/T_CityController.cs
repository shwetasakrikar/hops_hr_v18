...
{	
    ...
    {
		private void LoadViewDataForCount(T_City t_city, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_City t_city)
        {		
			 ViewBag.T_CityCountryID = new SelectList(db.T_Countrys, "ID", "DisplayValue", t_city.T_CityCountryID);
			 ViewBag.T_CityStateID = new SelectList(db.T_States, "ID", "DisplayValue", t_city.T_CityStateID);
			CustomLoadViewDataListAfterEdit(t_city);
        }
        private void LoadViewDataBeforeOnEdit(T_City t_city)
        {		
               var _objT_CityCountry = new List<T_Country>();
               _objT_CityCountry.Add(t_city.t_citycountry);
			   			   ViewBag.T_CityCountryID = new SelectList(_objT_CityCountry, "ID", "DisplayValue", t_city.T_CityCountryID);
			               var _objT_CityState = new List<T_State>();
               _objT_CityState.Add(t_city.t_citystate);
			   			   ViewBag.T_CityStateID = new SelectList(_objT_CityState, "ID", "DisplayValue", t_city.T_CityStateID);
				 ViewBag.T_AddressCityCount = t_city.t_addresscity.Count();
		CustomLoadViewDataListBeforeEdit(t_city);
        }
        private void LoadViewDataAfterOnCreate(T_City t_city)
        {			
			 ViewBag.T_CityCountryID = new SelectList(db.T_Countrys, "ID", "DisplayValue", t_city.T_CityCountryID);
			 ViewBag.T_CityStateID = new SelectList(db.T_States, "ID", "DisplayValue", t_city.T_CityStateID);
		CustomLoadViewDataListAfterOnCreate(t_city);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Country" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_CityCountry")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_CityCountryID = new SelectList(db.T_Countrys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Countrys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_CityCountry = new List<T_Country>();
			 ViewBag.T_CityCountryID = new SelectList(objT_CityCountry , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_State" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_CityState")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_CityStateID = new SelectList(db.T_States.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_States.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_CityState = new List<T_State>();
			 ViewBag.T_CityStateID = new SelectList(objT_CityState , "ID", "DisplayValue");
		    }
		...
		private IQueryable<T_City> sortRecords(IQueryable<T_City> lstT_City, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_CityCountryID")
                return isAsc.ToLower() == "asc" ? lstT_City.OrderBy(p => p.t_citycountry.DisplayValue) : lstT_City.OrderByDescending(p => p.t_citycountry.DisplayValue);
	 if(sortBy == "T_CityStateID")
                return isAsc.ToLower() == "asc" ? lstT_City.OrderBy(p => p.t_citystate.DisplayValue) : lstT_City.OrderByDescending(p => p.t_citystate.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_City.Sort(sortBy,true) : lstT_City.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_City), "t_city");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_City>)lstT_City.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_City.ElementType, lambda.Body.Type },
                    lstT_City.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_State")
            {
                ...
                url += "?search=";
						var T_CityCountry  = _Object.T_StateCountryID;
						url += "&t_citycountry=" + T_CityCountry;									
            ...
            if (sourceEntity == "T_Address")
            {
                ...
                url += "?search=";
						var T_CityCountry  = _Object.T_AddressCountryID;
						url += "&t_citycountry=" + T_CityCountry;									
						var T_CityState  = _Object.T_AddressStateID;
						url += "&t_citystate=" + T_CityState;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_citycountry,string t_citystate, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_CityCountrylist = db.T_Countrys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_citycountry != null)
            {
                var ids = t_citycountry.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n City Country= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_CityCountrylist.Union(db.T_Countrys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Country>(User, list, "T_Country", null);
					ViewBag.t_citycountry = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_CityCountrylist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Country>(User, list, "T_Country", null);
				ViewBag.t_citycountry = new SelectList(list, "ID", "DisplayValue");
			}
			var T_CityStatelist = db.T_States.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_citystate != null)
            {
                var ids = t_citystate.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n City State= ";
				   foreach (var str in ids)
                    {
					...
					ids1 = ids1.ToList();
					var list = T_CityStatelist.Union(db.T_States.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_State>(User, list, "T_State", null);
					ViewBag.t_citystate = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_CityStatelist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_State>(User, list, "T_State", null);
				ViewBag.t_citystate = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_CityCountry = new List<T_Country>();
		    ViewBag.t_citycountry = new SelectList(objT_CityCountry, "ID", "DisplayValue");
			var objT_CityState = new List<T_State>();
		    ViewBag.t_citystate = new SelectList(objT_CityState, "ID", "DisplayValue");
			}
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_City()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_City/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_citycountry,string t_citystate ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_City  = searchRecords(lstT_City, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_City = searchRecords(lstT_City, search,true);
			}
			...
			lstT_City = CustomSorting(lstT_City);
			lstT_City = lstT_City.Include(t=>t.t_citycountry).Include(t=>t.t_citystate);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_City", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_City = lstT_City.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_City = Sorting.Sort<T_City>(lstT_City, DataOrdering);
            var _T_City = lstT_City;
			//if (lstT_City.Where(p => p.t_citycountry != null).Count() <= 50)
		    //ViewBag.t_citycountry = new SelectList(lstT_City.Where(p => p.t_citycountry != null).Select(P => P.t_citycountry).Distinct(), "ID", "DisplayValue");
            if (t_citycountry != null)
            {
                var ids = t_citycountry.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_City = _T_City.Where(p => ids1.Contains(p.T_CityCountryID));
            }
				//if (lstT_City.Where(p => p.t_citystate != null).Count() <= 50)
		    //ViewBag.t_citystate = new SelectList(lstT_City.Where(p => p.t_citystate != null).Select(P => P.t_citystate).Distinct(), "ID", "DisplayValue");
            if (t_citystate != null)
            {
                var ids = t_citystate.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_City = _T_City.Where(p => ids1.Contains(p.T_CityStateID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_City");
		   ...
		   if (itemsPerPage != null)
			...
			if (!Request.IsAjaxRequest())
			{
				...
				var list = _T_City.ToPagedList(pageNumber, pageSize);
				ViewBag.EntityT_CityDisplayValue = new SelectList(list.Select(z => new { ID = z.Id, DisplayValue = z.DisplayValue }), "ID", "DisplayValue");
				TempData["T_Citylist"] = list.Select(z => new { ID = z.Id, DisplayValue = z.DisplayValue });
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        var tagsSplit = GroupByColumn.Split(',').Select(tag => tag.Trim()).Where(tag => !string.IsNullOrEmpty(tag));
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_City", tagsSplit.ToArray());
                    }
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_Citys.GroupBy(p => p.t_citycountry.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by City Country (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by City Country"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "City Country", "City"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_City", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_Citys.GroupBy(p => p.t_citystate.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "2" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by City State (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by City State"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "City State", "City"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_City", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_Citys.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_city = jedb.T_Citys.Find(id);
			var T_AddressCityIds = jedb.T_Addresss.Where(p=>p.T_AddressCityID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_Address" && T_AddressCityIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_Citys.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_Citys.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_City>(_Obj, "T_City", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_City> list = db.T_Citys;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                if (AssoID != null && AssoID > 0)
                {
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_City), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_City, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_City>)q);
				   list = ((IQueryable<T_City>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_Citys;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_City>(User,list, "T_City",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_City>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_City), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_City).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_City), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_City), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_City), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_City), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_City), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_City), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_City), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_City), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_City", User) || !User.CanAdd("T_City"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_City")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_City").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_City");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_City" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Citys";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_CityCountryID":
										 var t_citycountryId = db.T_Countrys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_citycountryId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_CityStateID":
										 var t_citystateId = db.T_States.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_citystateId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                        default:
                                            break;
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_CityCountryID":
										 var strPropertyT_CityCountry = lstEntityProp.FirstOrDefault(p => p.Key == "T_CityCountryID").Value;
										 ModelReflector.Property propT_CityCountry = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Country").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_CityCountry);
										 //var elementTypeT_CityCountry = db.T_Countrys.ElementType;
										 //var propertyInfoT_CityCountry = elementTypeT_CityCountry.GetProperty(propT_CityCountry.Name);
										 // var t_citycountryId = db.T_Countrys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_CityCountry.GetValue(p, null)) == assovalue);
										 var t_citycountryId = db.T_Countrys.Where(propT_CityCountry.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_citycountryId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_CityStateID":
										 var strPropertyT_CityState = lstEntityProp.FirstOrDefault(p => p.Key == "T_CityStateID").Value;
										 ModelReflector.Property propT_CityState = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_State").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_CityState);
										 //var elementTypeT_CityState = db.T_States.ElementType;
										 //var propertyInfoT_CityState = elementTypeT_CityState.GetProperty(propT_CityState.Name);
										 // var t_citystateId = db.T_States.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_CityState.GetValue(p, null)) == assovalue);
										 var t_citystateId = db.T_States.Where(propT_CityState.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_citystateId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
	...
	break;
		 ...
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_CityCountry = db.T_Countrys.ElementType;
			 //var propertyInfoT_CityCountry = elementTypeT_CityCountry.GetProperty(propT_CityCountry.Name);			 
			 //t_citycountryId = db.T_Countrys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_CityCountry.GetValue(p, null)) == columnValue);
			 t_citycountryId = db.T_Countrys.Where(propT_CityCountry.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_citycountryId = db.T_Countrys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_citycountryId != null)
			model.T_CityCountryID = t_citycountryId.Id;
		  else
			{
				if ((collection["T_CityCountryID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_CityCountryID = objT_Country.Id;
					}
					catch
					{
						continue;
		 ...
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_CityState = db.T_States.ElementType;
			 //var propertyInfoT_CityState = elementTypeT_CityState.GetProperty(propT_CityState.Name);			 
			 //t_citystateId = db.T_States.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_CityState.GetValue(p, null)) == columnValue);
			 t_citystateId = db.T_States.Where(propT_CityState.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_citystateId = db.T_States.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_citystateId != null)
			model.T_CityStateID = t_citystateId.Id;
		  else
			{
				if ((collection["T_CityStateID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_CityStateID = objT_State.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_City validate)
        {
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_City OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_City").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_City");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_City OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_City");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_City OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_City");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_City OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_City",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_City OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_City");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_City OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_City").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_Citys.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
				...
				public long? GetIdFromPropertyValue(string PropName, string PropValue)
        {
            ...
            lambda = Expression.Lambda<Func<T_City, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_City, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_City>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_City t_city)
        {
        ...
        public ActionResult GetCalculationValues(T_City t_city)
        {
            t_city.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_city.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_City t_city, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_City t_city, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
