...
{	
    public partial class FileDocumentController : BaseController
    {
        ...
        {		
			 ViewBag.T_EmployeeDocumentsID = new SelectList(db.T_Employees, "ID", "DisplayValue", filedocument.T_EmployeeDocumentsID);
			CustomLoadViewDataListAfterEdit(filedocument);
        }
        private void LoadViewDataBeforeOnEdit(FileDocument filedocument)
        {		
               var _objT_EmployeeDocuments = new List<T_Employee>();
               _objT_EmployeeDocuments.Add(filedocument.t_employeedocuments);
			   			   ViewBag.T_EmployeeDocumentsID = new SelectList(_objT_EmployeeDocuments, "ID", "DisplayValue", filedocument.T_EmployeeDocumentsID);
					CustomLoadViewDataListBeforeEdit(filedocument);
        }
        ...
        {			
			if(HostingEntityName == "T_Employee" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_EmployeeDocuments")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_EmployeeDocumentsID = new SelectList(db.T_Employees.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Employees.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_EmployeeDocuments = new List<T_Employee>();
			 ViewBag.T_EmployeeDocumentsID = new SelectList(objT_EmployeeDocuments , "ID", "DisplayValue");
		    }
		...
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Licenses")
            {
                ...
                url += "?search=";
						var T_EmployeeDocuments  = _Object.T_LicenseRecordsID;
						url += "&t_employeedocuments=" + T_EmployeeDocuments;									
            ...
            if (sourceEntity == "T_ServiceRecord")
            {
                ...
                url += "?search=";
						var T_EmployeeDocuments  = _Object.T_EmployeeEmploymentProfileID;
						url += "&t_employeedocuments=" + T_EmployeeDocuments;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Comment")
            {
                ...
                url += "?search=";
						var T_EmployeeDocuments  = _Object.T_EmployeeCommentsID;
						url += "&t_employeedocuments=" + T_EmployeeDocuments;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DrugAlcoholTest")
            {
                ...
                url += "?search=";
						var T_EmployeeDocuments  = _Object.T_EmployeeDrugAlcoholTestID;
						url += "&t_employeedocuments=" + T_EmployeeDocuments;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_EmployeeDocuments  = _Object.T_EmployeeAdministratorID;
						url += "&t_employeedocuments=" + T_EmployeeDocuments;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_JobAssignment")
            {
                ...
                url += "?search=";
						var T_EmployeeDocuments  = _Object.T_EmployeeJobAssignmentID;
						url += "&t_employeedocuments=" + T_EmployeeDocuments;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_LeaveProfile")
            {
                ...
                url += "?search=";
						var T_EmployeeDocuments  = _Object.T_EmployeeLeaveProfileID;
						url += "&t_employeedocuments=" + T_EmployeeDocuments;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_EmployeeInjury")
            {
                ...
                url += "?search=";
						var T_EmployeeDocuments  = _Object.T_EmployeeEmployeeInjuryID;
						url += "&t_employeedocuments=" + T_EmployeeDocuments;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_BackgroundCheck")
            {
                ...
                url += "?search=";
						var T_EmployeeDocuments  = _Object.T_EmployeeCriminalBackgroundCheckID;
						url += "&t_employeedocuments=" + T_EmployeeDocuments;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Education")
            {
                ...
                url += "?search=";
						var T_EmployeeDocuments  = _Object.T_EmployeeEducationID;
						url += "&t_employeedocuments=" + T_EmployeeDocuments;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Accommodation")
            {
                ...
                url += "?search=";
						var T_EmployeeDocuments  = _Object.T_EmployeeAccomodationID;
						url += "&t_employeedocuments=" + T_EmployeeDocuments;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_PayDetails")
            {
                ...
                url += "?search=";
						var T_EmployeeDocuments  = _Object.T_EmployeePayDetailsID;
						url += "&t_employeedocuments=" + T_EmployeeDocuments;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_employeedocuments, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_EmployeeDocumentslist = db.T_Employees.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_employeedocuments != null)
            {
                var ids = t_employeedocuments.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Employee= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_EmployeeDocumentslist.Union(db.T_Employees.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
					ViewBag.t_employeedocuments = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_EmployeeDocumentslist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Employee>(User, list, "T_Employee", null);
				ViewBag.t_employeedocuments = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_EmployeeDocuments = new List<T_Employee>();
		    ViewBag.t_employeedocuments = new SelectList(objT_EmployeeDocuments, "ID", "DisplayValue");
			}
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new FileDocument()).m_Timezone)).Date);
						}
						else
						{
		...
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_employeedocuments ,string DateCreatedFrom,string DateCreatedTo,string DateLastUpdatedFrom,string DateLastUpdatedTo,string DateCreatedFromhdn,string DateCreatedTohdn,string DateLastUpdatedFromhdn,string DateLastUpdatedTohdn,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
			...
			lstFileDocument = lstFileDocument.Include(t=>t.t_employeedocuments);
            ...
            var _FileDocument = lstFileDocument;
				if(DateCreatedFrom!=null || DateCreatedTo !=null)
				{
                      ...
                      _FileDocument =  _FileDocument.Where(o => o.DateCreated >= from && o.DateCreated <= to);
                   							ViewBag.SearchResult += "\r\n Created= "+DateCreatedFromhdn+"-"+DateCreatedTohdn;
						}
						catch { }
				}
				if(DateLastUpdatedFrom!=null || DateLastUpdatedTo !=null)
				{
                      ...
                      _FileDocument =  _FileDocument.Where(o => o.DateLastUpdated >= from && o.DateLastUpdated <= to);
                   							ViewBag.SearchResult += "\r\n Last Updated= "+DateLastUpdatedFromhdn+"-"+DateLastUpdatedTohdn;
						}
						catch { }
				}
			//if (lstFileDocument.Where(p => p.t_employeedocuments != null).Count() <= 50)
		    //ViewBag.t_employeedocuments = new SelectList(lstFileDocument.Where(p => p.t_employeedocuments != null).Select(P => P.t_employeedocuments).Distinct(), "ID", "DisplayValue");
            if (t_employeedocuments != null)
            {
                var ids = t_employeedocuments.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _FileDocument = _FileDocument.Where(p => ids1.Contains(p.T_EmployeeDocumentsID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
		...
		var entity = "FileDocument";
           ...
           var entitylist = db.FileDocuments;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.FileDocuments.GroupBy(p => p.t_employeedocuments.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Employee", "Document"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "FileDocument", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
...
[AcceptVerbs(HttpVerbs.Get)]
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<FileDocument>(User, list, "FileDocument", null);
			if (key != null && key.Length > 0)
            {
			   var data = from x in list.Where(p=>p.DisplayValue.Contains(key)).OrderBy(q=>q.DisplayValue).Take(10).ToList()
                           select new { Id = x.Id, Name = x.DisplayValue };
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_EmployeeDocumentsID":
										 var t_employeedocumentsId = db.T_Employees.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_employeedocumentsId == null)
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_EmployeeDocumentsID":
										 var strPropertyT_EmployeeDocuments = lstEntityProp.FirstOrDefault(p => p.Key == "T_EmployeeDocumentsID").Value;
										 ModelReflector.Property propT_EmployeeDocuments = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Employee").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_EmployeeDocuments);
										 //var elementTypeT_EmployeeDocuments = db.T_Employees.ElementType;
										 //var propertyInfoT_EmployeeDocuments = elementTypeT_EmployeeDocuments.GetProperty(propT_EmployeeDocuments.Name);
										 // var t_employeedocumentsId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EmployeeDocuments.GetValue(p, null)) == assovalue);
										 var t_employeedocumentsId = db.T_Employees.Where(propT_EmployeeDocuments.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_employeedocumentsId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
	...
	break;
					 case "T_EmployeeDocumentsID":
		 dynamic t_employeedocumentsId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_EmployeeDocuments = db.T_Employees.ElementType;
			 //var propertyInfoT_EmployeeDocuments = elementTypeT_EmployeeDocuments.GetProperty(propT_EmployeeDocuments.Name);			 
			 //t_employeedocumentsId = db.T_Employees.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_EmployeeDocuments.GetValue(p, null)) == columnValue);
			 t_employeedocumentsId = db.T_Employees.Where(propT_EmployeeDocuments.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_employeedocumentsId = db.T_Employees.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_employeedocumentsId != null)
			model.T_EmployeeDocumentsID = t_employeedocumentsId.Id;
		  else
			{
				if ((collection["T_EmployeeDocumentsID"].ToString() == "true,false"))
				{
					try
					{
				 ...
				 try { objT_Employee.T_FirstName =(columnValue); }
						...
						db.SaveChanges();
						model.T_EmployeeDocumentsID = objT_Employee.Id;
					}
					catch
					{
						continue;
        ...
        public JsonResult GetValidateBeforeSaveProperties(FileDocument OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "FileDocument",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
