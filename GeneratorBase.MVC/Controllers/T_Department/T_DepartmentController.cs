...
{	
    ...
    {
		private void LoadViewDataForCount(T_Department t_department, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_Department t_department)
        {		
			 ViewBag.T_DepartmentFacilityAssociationID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_department.T_DepartmentFacilityAssociationID);
			CustomLoadViewDataListAfterEdit(t_department);
        }
        private void LoadViewDataBeforeOnEdit(T_Department t_department)
        {		
               var _objT_DepartmentFacilityAssociation = new List<T_Facility>();
               _objT_DepartmentFacilityAssociation.Add(t_department.t_departmentfacilityassociation);
			   			   ViewBag.T_DepartmentFacilityAssociationID = new SelectList(_objT_DepartmentFacilityAssociation, "ID", "DisplayValue", t_department.T_DepartmentFacilityAssociationID);
				 ViewBag.T_WardDepartmentCount = t_department.t_warddepartment.Count();
		CustomLoadViewDataListBeforeEdit(t_department);
        }
        private void LoadViewDataAfterOnCreate(T_Department t_department)
        {			
			 ViewBag.T_DepartmentFacilityAssociationID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_department.T_DepartmentFacilityAssociationID);
		CustomLoadViewDataListAfterOnCreate(t_department);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Facility" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_DepartmentFacilityAssociation")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_DepartmentFacilityAssociationID = new SelectList(db.T_Facilitys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Facilitys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_DepartmentFacilityAssociation = new List<T_Facility>();
			 ViewBag.T_DepartmentFacilityAssociationID = new SelectList(objT_DepartmentFacilityAssociation , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_Department> searchRecords(IQueryable<T_Department> lstT_Department, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_Department = lstT_Department.Where(s => (s.t_departmentfacilityassociation!= null && (s.t_departmentfacilityassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_DEPTshort) && s.T_DEPTshort.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_DepartmentTitle) && s.T_DepartmentTitle.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_Department = lstT_Department.Where(s => (s.t_departmentfacilityassociation!= null && (s.t_departmentfacilityassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_DEPTshort) && s.T_DEPTshort.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_DepartmentTitle) && s.T_DepartmentTitle.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_Department;
        }
		private IQueryable<T_Department> sortRecords(IQueryable<T_Department> lstT_Department, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_DepartmentFacilityAssociationID")
                return isAsc.ToLower() == "asc" ? lstT_Department.OrderBy(p => p.t_departmentfacilityassociation.DisplayValue) : lstT_Department.OrderByDescending(p => p.t_departmentfacilityassociation.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_Department.Sort(sortBy,true) : lstT_Department.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_Department), "t_department");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_Department>)lstT_Department.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_Department.ElementType, lambda.Body.Type },
                    lstT_Department.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Position")
            {
                ...
                url += "?search=";
						var T_DepartmentFacilityAssociation  = _Object.T_FacilityAssignedToID;
						url += "&t_departmentfacilityassociation=" + T_DepartmentFacilityAssociation;									
            ...
            if (sourceEntity == "T_Employee")
            {
                ...
                url += "?search=";
						var T_DepartmentFacilityAssociation  = _Object.T_EmployeeAtFacilityID;
						url += "&t_departmentfacilityassociation=" + T_DepartmentFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DepartmentArea")
            {
                ...
                url += "?search=";
						var T_DepartmentFacilityAssociation  = _Object.T_DepartmentAreaEmployeeTypeAssociationID;
						url += "&t_departmentfacilityassociation=" + T_DepartmentFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_ClaimType")
            {
                ...
                url += "?search=";
						var T_DepartmentFacilityAssociation  = _Object.T_ClaimTypeFacilityAssociationID;
						url += "&t_departmentfacilityassociation=" + T_DepartmentFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Restrictions")
            {
                ...
                url += "?search=";
						var T_DepartmentFacilityAssociation  = _Object.T_RestrictionsFacilityAssociationID;
						url += "&t_departmentfacilityassociation=" + T_DepartmentFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_DepartmentFacilityAssociation  = _Object.T_FacilityUnitXID;
						url += "&t_departmentfacilityassociation=" + T_DepartmentFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Unit")
            {
                ...
                url += "?search=";
						var T_DepartmentFacilityAssociation  = _Object.T_FacilityUnitID;
						url += "&t_departmentfacilityassociation=" + T_DepartmentFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_SalaryRange")
            {
                ...
                url += "?search=";
						var T_DepartmentFacilityAssociation  = _Object.T_FacilitySalaryRangeAssociationID;
						url += "&t_departmentfacilityassociation=" + T_DepartmentFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_FacilityConfiguration")
            {
                ...
                url += "?search=";
						var T_DepartmentFacilityAssociation  = _Object.T_TenantConfigurationAssociationID;
						url += "&t_departmentfacilityassociation=" + T_DepartmentFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_departmentfacilityassociation, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_DepartmentFacilityAssociationlist = db.T_Facilitys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_departmentfacilityassociation != null)
            {
                var ids = t_departmentfacilityassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Facility= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_DepartmentFacilityAssociationlist.Union(db.T_Facilitys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
					ViewBag.t_departmentfacilityassociation = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_DepartmentFacilityAssociationlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
				ViewBag.t_departmentfacilityassociation = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_DepartmentFacilityAssociation = new List<T_Facility>();
		    ViewBag.t_departmentfacilityassociation = new SelectList(objT_DepartmentFacilityAssociation, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_Department";
			...
			columns.Add("3", "DEPTshort");
			columns.Add("4", "Department Title");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				return View(new T_Department());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_Department", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_Department", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_Department()).m_Timezone)).Date);
						}
						else
						{
		...
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_departmentfacilityassociation ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_Department  = searchRecords(lstT_Department, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_Department = searchRecords(lstT_Department, search,true);
			}
			...
			lstT_Department = lstT_Department.Include(t=>t.t_departmentfacilityassociation);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_Department", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_Department = lstT_Department.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_Department = Sorting.Sort<T_Department>(lstT_Department, DataOrdering);
            var _T_Department = lstT_Department;
			//if (lstT_Department.Where(p => p.t_departmentfacilityassociation != null).Count() <= 50)
		    //ViewBag.t_departmentfacilityassociation = new SelectList(lstT_Department.Where(p => p.t_departmentfacilityassociation != null).Select(P => P.t_departmentfacilityassociation).Distinct(), "ID", "DisplayValue");
            if (t_departmentfacilityassociation != null)
            {
                var ids = t_departmentfacilityassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_Department = _T_Department.Where(p => ids1.Contains(p.T_DepartmentFacilityAssociationID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Department");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_Department", tagsSplit.ToArray());
                    }
		...
		var entity = "T_Department";
           ...
           var entitylist = db.T_Departments;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_Departments.GroupBy(p => p.t_departmentfacilityassociation.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Facility", "Department"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Department", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_Departments.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_department = jedb.T_Departments.Find(id);
			var T_WardDepartmentIds = jedb.T_UnitXs.Where(p=>p.T_WardDepartmentID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_UnitX" && T_WardDepartmentIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_Departments.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_Departments.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_Department>(_Obj, "T_Department", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_Department> list = db.T_Departments;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_Departments;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_Department), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_Department, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_Department>)q);
				   list = ((IQueryable<T_Department>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_Departments;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_Department>(User,list, "T_Department",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_Department>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_Department), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_Department).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_Department), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Department), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Department), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Department), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Department), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Department), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_Department), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_Department), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_Department", User) || !User.CanAdd("T_Department"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_Department")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_Department").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_Department");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_Department" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Departments";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_DepartmentFacilityAssociationID":
										 var t_departmentfacilityassociationId = db.T_Facilitys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_departmentfacilityassociationId == null)
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_DepartmentFacilityAssociationID":
										 var strPropertyT_DepartmentFacilityAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_DepartmentFacilityAssociationID").Value;
										 ModelReflector.Property propT_DepartmentFacilityAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Facility").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_DepartmentFacilityAssociation);
										 //var elementTypeT_DepartmentFacilityAssociation = db.T_Facilitys.ElementType;
										 //var propertyInfoT_DepartmentFacilityAssociation = elementTypeT_DepartmentFacilityAssociation.GetProperty(propT_DepartmentFacilityAssociation.Name);
										 // var t_departmentfacilityassociationId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_DepartmentFacilityAssociation.GetValue(p, null)) == assovalue);
										 var t_departmentfacilityassociationId = db.T_Facilitys.Where(propT_DepartmentFacilityAssociation.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_departmentfacilityassociationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        for (int j = 0; j < sheetColumns.Count; j++)
                        {
                            ...
                            {
    case "T_DEPTshort":
    model.T_DEPTshort = columnValue;
	break;
    case "T_DepartmentTitle":
    model.T_DepartmentTitle = columnValue;
	break;
					 case "T_DepartmentFacilityAssociationID":
		 dynamic t_departmentfacilityassociationId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_DepartmentFacilityAssociation = db.T_Facilitys.ElementType;
			 //var propertyInfoT_DepartmentFacilityAssociation = elementTypeT_DepartmentFacilityAssociation.GetProperty(propT_DepartmentFacilityAssociation.Name);			 
			 //t_departmentfacilityassociationId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_DepartmentFacilityAssociation.GetValue(p, null)) == columnValue);
			 t_departmentfacilityassociationId = db.T_Facilitys.Where(propT_DepartmentFacilityAssociation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_departmentfacilityassociationId = db.T_Facilitys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_departmentfacilityassociationId != null)
			model.T_DepartmentFacilityAssociationID = t_departmentfacilityassociationId.Id;
		  else
			{
				if ((collection["T_DepartmentFacilityAssociationID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_DepartmentFacilityAssociationID = objT_Facility.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_Department validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_Departments.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_Department OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_Department").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_Department");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_Department OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_Department");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_Department OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_Department");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_Department OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_Department",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_Department OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_Department");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_Department OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Department").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_Departments.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_Departments;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_Department, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_Department, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_Department>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_Department t_department)
        {
        ...
        {
            string[][] groupsarr = new string[][] { new string[] {"48907","T_Department48907"} };
            return groupsarr;
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_Department t_department)
        {
            t_department.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_department.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_Department"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_Department t_department, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_Department t_department, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
