...
{	
    ...
    {
		private void LoadViewDataForCount(T_Unit t_unit, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_Unit t_unit)
        {		
			 ViewBag.T_FacilityUnitID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_unit.T_FacilityUnitID);
			CustomLoadViewDataListAfterEdit(t_unit);
        }
        private void LoadViewDataBeforeOnEdit(T_Unit t_unit)
        {		
               var _objT_FacilityUnit = new List<T_Facility>();
               _objT_FacilityUnit.Add(t_unit.t_facilityunit);
			   			   ViewBag.T_FacilityUnitID = new SelectList(_objT_FacilityUnit, "ID", "DisplayValue", t_unit.T_FacilityUnitID);
				 ViewBag.T_UnitXUnitAssociationCount = t_unit.t_unitxunitassociation.Count();
		CustomLoadViewDataListBeforeEdit(t_unit);
        }
        private void LoadViewDataAfterOnCreate(T_Unit t_unit)
        {			
			 ViewBag.T_FacilityUnitID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_unit.T_FacilityUnitID);
		CustomLoadViewDataListAfterOnCreate(t_unit);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Facility" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_FacilityUnit")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_FacilityUnitID = new SelectList(db.T_Facilitys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Facilitys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_FacilityUnit = new List<T_Facility>();
			 ViewBag.T_FacilityUnitID = new SelectList(objT_FacilityUnit , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_Unit> searchRecords(IQueryable<T_Unit> lstT_Unit, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_Unit = lstT_Unit.Where(s => (s.t_facilityunit!= null && (s.t_facilityunit.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_Unit = lstT_Unit.Where(s => (s.t_facilityunit!= null && (s.t_facilityunit.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_Unit;
        }
		private IQueryable<T_Unit> sortRecords(IQueryable<T_Unit> lstT_Unit, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_FacilityUnitID")
                return isAsc.ToLower() == "asc" ? lstT_Unit.OrderBy(p => p.t_facilityunit.DisplayValue) : lstT_Unit.OrderByDescending(p => p.t_facilityunit.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_Unit.Sort(sortBy,true) : lstT_Unit.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_Unit), "t_unit");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_Unit>)lstT_Unit.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_Unit.ElementType, lambda.Body.Type },
                    lstT_Unit.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Department")
            {
                ...
                url += "?search=";
						var T_FacilityUnit  = _Object.T_DepartmentFacilityAssociationID;
						url += "&t_facilityunit=" + T_FacilityUnit;									
            ...
            if (sourceEntity == "T_Position")
            {
                ...
                url += "?search=";
						var T_FacilityUnit  = _Object.T_FacilityAssignedToID;
						url += "&t_facilityunit=" + T_FacilityUnit;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Employee")
            {
                ...
                url += "?search=";
						var T_FacilityUnit  = _Object.T_EmployeeAtFacilityID;
						url += "&t_facilityunit=" + T_FacilityUnit;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DepartmentArea")
            {
                ...
                url += "?search=";
						var T_FacilityUnit  = _Object.T_DepartmentAreaEmployeeTypeAssociationID;
						url += "&t_facilityunit=" + T_FacilityUnit;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_ClaimType")
            {
                ...
                url += "?search=";
						var T_FacilityUnit  = _Object.T_ClaimTypeFacilityAssociationID;
						url += "&t_facilityunit=" + T_FacilityUnit;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Restrictions")
            {
                ...
                url += "?search=";
						var T_FacilityUnit  = _Object.T_RestrictionsFacilityAssociationID;
						url += "&t_facilityunit=" + T_FacilityUnit;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_FacilityUnit  = _Object.T_FacilityUnitXID;
						url += "&t_facilityunit=" + T_FacilityUnit;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_SalaryRange")
            {
                ...
                url += "?search=";
						var T_FacilityUnit  = _Object.T_FacilitySalaryRangeAssociationID;
						url += "&t_facilityunit=" + T_FacilityUnit;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_FacilityConfiguration")
            {
                ...
                url += "?search=";
						var T_FacilityUnit  = _Object.T_TenantConfigurationAssociationID;
						url += "&t_facilityunit=" + T_FacilityUnit;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_facilityunit, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_FacilityUnitlist = db.T_Facilitys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_facilityunit != null)
            {
                var ids = t_facilityunit.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Facility= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_FacilityUnitlist.Union(db.T_Facilitys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
					ViewBag.t_facilityunit = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_FacilityUnitlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
				ViewBag.t_facilityunit = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_FacilityUnit = new List<T_Facility>();
		    ViewBag.t_facilityunit = new SelectList(objT_FacilityUnit, "ID", "DisplayValue");
			}
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_Unit()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_Unit/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_facilityunit ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_Unit  = searchRecords(lstT_Unit, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_Unit = searchRecords(lstT_Unit, search,true);
			}
			...
			lstT_Unit = lstT_Unit.Include(t=>t.t_facilityunit);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_Unit", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_Unit = lstT_Unit.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_Unit = Sorting.Sort<T_Unit>(lstT_Unit, DataOrdering);
            var _T_Unit = lstT_Unit;
			//if (lstT_Unit.Where(p => p.t_facilityunit != null).Count() <= 50)
		    //ViewBag.t_facilityunit = new SelectList(lstT_Unit.Where(p => p.t_facilityunit != null).Select(P => P.t_facilityunit).Distinct(), "ID", "DisplayValue");
            if (t_facilityunit != null)
            {
                var ids = t_facilityunit.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_Unit = _T_Unit.Where(p => ids1.Contains(p.T_FacilityUnitID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Unit");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_Unit", tagsSplit.ToArray());
                    }
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_Units.GroupBy(p => p.t_facilityunit.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Facility", "Unit"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Unit", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_Units.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_unit = jedb.T_Units.Find(id);
			var T_UnitXUnitAssociationIds = jedb.T_UnitXs.Where(p=>p.T_UnitXUnitAssociationID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_UnitX" && T_UnitXUnitAssociationIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_Units.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_Units.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_Unit>(_Obj, "T_Unit", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_Unit> list = db.T_Units;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                if (AssoID != null && AssoID > 0)
                {
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_Unit), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_Unit, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_Unit>)q);
				   list = ((IQueryable<T_Unit>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_Units;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_Unit>(User,list, "T_Unit",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_Unit>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_Unit), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_Unit).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_Unit), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Unit), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Unit), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Unit), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Unit), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Unit), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_Unit), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_Unit), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_Unit", User) || !User.CanAdd("T_Unit"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_Unit")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_Unit").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_Unit");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_Unit" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Units";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_FacilityUnitID":
										 var t_facilityunitId = db.T_Facilitys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_facilityunitId == null)
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_FacilityUnitID":
										 var strPropertyT_FacilityUnit = lstEntityProp.FirstOrDefault(p => p.Key == "T_FacilityUnitID").Value;
										 ModelReflector.Property propT_FacilityUnit = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Facility").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_FacilityUnit);
										 //var elementTypeT_FacilityUnit = db.T_Facilitys.ElementType;
										 //var propertyInfoT_FacilityUnit = elementTypeT_FacilityUnit.GetProperty(propT_FacilityUnit.Name);
										 // var t_facilityunitId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_FacilityUnit.GetValue(p, null)) == assovalue);
										 var t_facilityunitId = db.T_Facilitys.Where(propT_FacilityUnit.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_facilityunitId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
	...
	break;
		 ...
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_FacilityUnit = db.T_Facilitys.ElementType;
			 //var propertyInfoT_FacilityUnit = elementTypeT_FacilityUnit.GetProperty(propT_FacilityUnit.Name);			 
			 //t_facilityunitId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_FacilityUnit.GetValue(p, null)) == columnValue);
			 t_facilityunitId = db.T_Facilitys.Where(propT_FacilityUnit.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_facilityunitId = db.T_Facilitys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_facilityunitId != null)
			model.T_FacilityUnitID = t_facilityunitId.Id;
		  else
			{
				if ((collection["T_FacilityUnitID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_FacilityUnitID = objT_Facility.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_Unit validate)
        {
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_Unit OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_Unit").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_Unit");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_Unit OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_Unit");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_Unit OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_Unit");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_Unit OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_Unit",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_Unit OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_Unit");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_Unit OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Unit").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_Units.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
				...
				public long? GetIdFromPropertyValue(string PropName, string PropValue)
        {
            ...
            lambda = Expression.Lambda<Func<T_Unit, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_Unit, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_Unit>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_Unit t_unit)
        {
        ...
        {
            string[][] groupsarr = new string[][] { new string[] {"48907","T_Unit48907"} };
            return groupsarr;
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_Unit t_unit)
        {
            t_unit.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_unit.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_Unit t_unit, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_Unit t_unit, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
