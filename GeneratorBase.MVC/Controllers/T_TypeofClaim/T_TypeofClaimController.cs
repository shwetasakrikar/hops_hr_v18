...
{	
    ...
    {
		private void LoadViewDataForCount(T_TypeofClaim t_typeofclaim, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_TypeofClaim t_typeofclaim)
        {		
			 ViewBag.T_ClaimTypeID = new SelectList(db.T_ClaimTypes, "ID", "DisplayValue", t_typeofclaim.T_ClaimTypeID);
			 ViewBag.T_EmployeeInjuryID = new SelectList(db.T_EmployeeInjurys, "ID", "DisplayValue", t_typeofclaim.T_EmployeeInjuryID);
			CustomLoadViewDataListAfterEdit(t_typeofclaim);
        }
        private void LoadViewDataBeforeOnEdit(T_TypeofClaim t_typeofclaim)
        {		
               var _objT_ClaimType = new List<T_ClaimType>();
               _objT_ClaimType.Add(t_typeofclaim.t_claimtype);
			   			   ViewBag.T_ClaimTypeID = new SelectList(_objT_ClaimType, "ID", "DisplayValue", t_typeofclaim.T_ClaimTypeID);
			               var _objT_EmployeeInjury = new List<T_EmployeeInjury>();
               _objT_EmployeeInjury.Add(t_typeofclaim.t_employeeinjury);
			   			   ViewBag.T_EmployeeInjuryID = new SelectList(_objT_EmployeeInjury, "ID", "DisplayValue", t_typeofclaim.T_EmployeeInjuryID);
					CustomLoadViewDataListBeforeEdit(t_typeofclaim);
        }
        private void LoadViewDataAfterOnCreate(T_TypeofClaim t_typeofclaim)
        {			
			 ViewBag.T_ClaimTypeID = new SelectList(db.T_ClaimTypes, "ID", "DisplayValue", t_typeofclaim.T_ClaimTypeID);
			 ViewBag.T_EmployeeInjuryID = new SelectList(db.T_EmployeeInjurys, "ID", "DisplayValue", t_typeofclaim.T_EmployeeInjuryID);
		CustomLoadViewDataListAfterOnCreate(t_typeofclaim);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_ClaimType" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_TypeofClaim_T_ClaimType")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_ClaimTypeID = new SelectList(db.T_ClaimTypes.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_ClaimTypes.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
								//Manytomany associations
								var T_TypeofClaimlist = db.T_EmployeeInjurys.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
								ViewBag.t_typeofclaim = new SelectList(T_TypeofClaimlist, "ID", "DisplayValue");
						//ManytoOne association
						var T_ClaimTypeFacilityAssociationlist = db.T_Facilitys.OrderBy(p => p.DisplayValue).Take(10).Distinct().ToList();
						ViewBag.t_claimtypefacilityassociation = new SelectList(T_ClaimTypeFacilityAssociationlist, "ID", "DisplayValue");
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_TypeofClaims.Where(p => p.T_EmployeeInjuryID == hostid).Select(q => q.T_ClaimTypeID);
		var AvailableList = db.T_ClaimTypes.Where(p => !Selectedlist.Contains(p.Id));
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        AvailableList = _fad.FilterDropdown<T_ClaimType>(User, AvailableList, "T_ClaimType", "T_TypeofClaim.T_ClaimTypeID");
                ViewBag.IsTop50 = "none";
				if (AvailableList.Count() > 50)
                    ViewBag.IsTop50 = "block";
				ViewBag.T_ClaimTypeIDAvailable  = new SelectList(AvailableList.OrderBy(p => p.DisplayValue).Take(50).ToList(), "ID", "DisplayValue");
				var SelectedListComplete = db.T_ClaimTypes.Where(p => Selectedlist.Contains(p.Id));
				var SelectedListFiltered = db.T_ClaimTypes.Where(p => Selectedlist.Contains(p.Id));
				SelectedListFiltered = _fad.FilterDropdown<T_ClaimType>(User, SelectedListFiltered, "T_ClaimType", "T_TypeofClaim.T_ClaimTypeID");
				ViewBag.SelectedListRemoved= string.Join(",", SelectedListComplete.Except(SelectedListFiltered).Select(p=>p.Id).ToList());
                ViewBag.T_ClaimTypeIDSelected = new MultiSelectList(SelectedListComplete.OrderBy(p => p.DisplayValue).ToList(), "ID", "DisplayValue", Selectedlist);
		    }
			if(HostingEntityName == "T_EmployeeInjury" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_TypeofClaim_T_EmployeeInjury")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
            ...
            else
			{
				...
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_TypeofClaims.Where(p => p.T_ClaimTypeID == hostid).Select(q => q.T_EmployeeInjuryID);
		var AvailableList = db.T_EmployeeInjurys.Where(p => !Selectedlist.Contains(p.Id));
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        AvailableList = _fad.FilterDropdown<T_EmployeeInjury>(User, AvailableList, "T_EmployeeInjury", "T_TypeofClaim.T_EmployeeInjuryID");
                ViewBag.IsTop50 = "none";
				if (AvailableList.Count() > 50)
                    ViewBag.IsTop50 = "block";
				...
				SelectedListFiltered = _fad.FilterDropdown<T_EmployeeInjury>(User, SelectedListFiltered, "T_EmployeeInjury", "T_TypeofClaim.T_EmployeeInjuryID");
				ViewBag.SelectedListRemoved= string.Join(",", SelectedListComplete.Except(SelectedListFiltered).Select(p=>p.Id).ToList());
		...
		private IQueryable<T_TypeofClaim> searchRecords(IQueryable<T_TypeofClaim> lstT_TypeofClaim, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_TypeofClaim = lstT_TypeofClaim.Where(s => (s.t_claimtype!= null && (s.t_claimtype.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_employeeinjury!= null && (s.t_employeeinjury.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_TypeofClaim = lstT_TypeofClaim.Where(s => (s.t_claimtype!= null && (s.t_claimtype.DisplayValue.ToUpper().Contains(searchString) )) ||(s.t_employeeinjury!= null && (s.t_employeeinjury.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_TypeofClaim;
        }
		private IQueryable<T_TypeofClaim> sortRecords(IQueryable<T_TypeofClaim> lstT_TypeofClaim, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_ClaimTypeID")
                return isAsc.ToLower() == "asc" ? lstT_TypeofClaim.OrderBy(p => p.t_claimtype.DisplayValue) : lstT_TypeofClaim.OrderByDescending(p => p.t_claimtype.DisplayValue);
	 if(sortBy == "T_EmployeeInjuryID")
                return isAsc.ToLower() == "asc" ? lstT_TypeofClaim.OrderBy(p => p.t_employeeinjury.DisplayValue) : lstT_TypeofClaim.OrderByDescending(p => p.t_employeeinjury.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_TypeofClaim.Sort(sortBy,true) : lstT_TypeofClaim.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_TypeofClaim), "t_typeofclaim");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_TypeofClaim>)lstT_TypeofClaim.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_TypeofClaim.ElementType, lambda.Body.Type },
                    lstT_TypeofClaim.Expression,
                    lambda));
        }
        ...
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_claimtype,string t_employeeinjury, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_ClaimTypelist = db.T_ClaimTypes.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_claimtype != null)
            {
                var ids = t_claimtype.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Type of Claim Facility= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_ClaimTypelist.Union(db.T_ClaimTypes.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_ClaimType>(User, list, "T_ClaimType", null);
					ViewBag.t_claimtype = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_ClaimTypelist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_ClaimType>(User, list, "T_ClaimType", null);
				ViewBag.t_claimtype = new SelectList(list, "ID", "DisplayValue");
			}
		    ...
		    ViewBag.t_claimtype = new SelectList(objT_ClaimType, "ID", "DisplayValue");
		    ...
		    ViewBag.t_employeeinjury = new SelectList(objT_EmployeeInjury, "ID", "DisplayValue");
				...
				ViewBag.EntityName = "T_TypeofClaim";
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_TypeofClaim()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_TypeofClaim/FSearch/
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_claimtype,string t_employeeinjury ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_TypeofClaim  = searchRecords(lstT_TypeofClaim, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_TypeofClaim = searchRecords(lstT_TypeofClaim, search,true);
			}
			...
			lstT_TypeofClaim = lstT_TypeofClaim.Include(t=>t.t_claimtype).Include(t=>t.t_employeeinjury);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_TypeofClaim", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_TypeofClaim = lstT_TypeofClaim.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_TypeofClaim = Sorting.Sort<T_TypeofClaim>(lstT_TypeofClaim, DataOrdering);
            var _T_TypeofClaim = lstT_TypeofClaim;
			//if (lstT_TypeofClaim.Where(p => p.t_claimtype != null).Count() <= 50)
		    //ViewBag.t_claimtype = new SelectList(lstT_TypeofClaim.Where(p => p.t_claimtype != null).Select(P => P.t_claimtype).Distinct(), "ID", "DisplayValue");
            if (t_claimtype != null)
            {
                var ids = t_claimtype.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_ClaimTypes.Find(Convert.ToInt64(str));
                ...
                ids1 = ids1.ToList();
                _T_TypeofClaim = _T_TypeofClaim.Where(p => ids1.Contains(p.T_ClaimTypeID));
            }
				//if (lstT_TypeofClaim.Where(p => p.t_employeeinjury != null).Count() <= 50)
		    //ViewBag.t_employeeinjury = new SelectList(lstT_TypeofClaim.Where(p => p.t_employeeinjury != null).Select(P => P.t_employeeinjury).Distinct(), "ID", "DisplayValue");
            if (t_employeeinjury != null)
            {
                var ids = t_employeeinjury.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_TypeofClaim = _T_TypeofClaim.Where(p => ids1.Contains(p.T_EmployeeInjuryID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_TypeofClaim");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_TypeofClaim", tagsSplit.ToArray());
                    }
		...
		var entity = "T_TypeofClaim";
           ...
           var entitylist = db.T_TypeofClaims;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_TypeofClaims.GroupBy(p => p.t_claimtype.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Type of Claim Facility (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Type of Claim Facility"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Type of Claim Facility", "Type of Claim"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_TypeofClaim", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_TypeofClaims.GroupBy(p => p.t_employeeinjury.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "2" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Employee Injury", "Type of Claim"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_TypeofClaim", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_TypeofClaims.FirstOrDefault(p => p.Id == idvalue);
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_TypeofClaims.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_TypeofClaims.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_TypeofClaim>(_Obj, "T_TypeofClaim", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_TypeofClaim> list = db.T_TypeofClaims;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_TypeofClaims;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_TypeofClaim), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_TypeofClaim, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_TypeofClaim>)q);
				   list = ((IQueryable<T_TypeofClaim>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_TypeofClaims;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_TypeofClaim>(User,list, "T_TypeofClaim",caller);
        ...
        {
			int pagesize = 50;
			if (HostingEntityName == "T_ClaimType")
            {
				var hostid = Convert.ToInt64(HostingEntityID);
                var Selectedlist = db.T_TypeofClaims.Where(p => p.T_ClaimTypeID == hostid).Select(q => q.T_EmployeeInjuryID);
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
        var Finallist = _fad.FilterDropdown<T_EmployeeInjury>(User, db.T_EmployeeInjurys.Where(p => !Selectedlist.Contains(p.Id)), "T_EmployeeInjury", "T_TypeofClaim.T_EmployeeInjuryID").ToList();
                if(!string.IsNullOrEmpty(SearchString))
            ...
            if (HostingEntityName == "T_EmployeeInjury")
            {
                ...
                var Selectedlist = db.T_TypeofClaims.Where(p => p.T_EmployeeInjuryID == hostid).Select(q => q.T_ClaimTypeID);
		FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
         var Finallist = _fad.FilterDropdown<T_ClaimType>(User, db.T_ClaimTypes.Where(p => !Selectedlist.Contains(p.Id)), "T_ClaimType", "T_TypeofClaim.T_ClaimTypeID").ToList();
                if(!string.IsNullOrEmpty(SearchString))
                    Finallist = Finallist.Where(p=>p.DisplayValue.ToUpper().Contains(SearchString.ToUpper())).OrderBy(p => p.DisplayValue).ToList();
                else
                    Finallist = Finallist.OrderBy(p => p.DisplayValue).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValues1(string SearchString, string HostingEntityName, string HostingEntityID, string AssociatedType, string size , string t_claimtypefacilityassociation,string t_typeofclaim,string t_typeofrestrictions,string t_employeeemployeeinjury,string t_typeofclaimmci,string t_accidentshift,string t_facilityaccidentoccured,string t_unitwhereaccidentoccured,string t_employeinjuryfloor,string t_locationofaccident,string t_employeeinjurycause,string t_employeeinjurynature,string t_employeeinjurybodypartsaffected,string t_employeeinjurymachinetool,string t_initialtreatmentlist,string t_employeeinjurymedicalfacilityfortreatment,string t_employeeinjuryrefusal,string T_OSHA  )
        {
			int pagesize = 50;
			if (HostingEntityName == "T_ClaimType")
            {
...
//ManyToMany search in m.m
        ...
        var Finallist = _fad.FilterDropdown<T_ClaimType>(User, db.T_ClaimTypes.Where(p => !Selectedlist.Contains(p.Id)), "T_ClaimType", "T_TypeofClaim.T_ClaimTypeID").ToList();
                if(!string.IsNullOrEmpty(SearchString))
                    ...
                    Finallist = Finallist.OrderBy(p => p.DisplayValue).ToList();
				if (t_claimtypefacilityassociation != null)
                {
                    var ids = t_claimtypefacilityassociation.Split(",".ToCharArray());
                    List<long?> ids1 = new List<long?>();
                    foreach (var str in ids)
                    {
                        //Null Search
                    ...
                    ids1 = ids1.ToList();
                    Finallist = Finallist.Where(p => ids1.Contains(p.T_ClaimTypeFacilityAssociationID)).ToList();
                }
			...
			if (t_typeofclaim != null)
            {
   			 ...
   			 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                        var idvalue= Convert.ToInt64(str);
                        ids1.AddRange(db.T_TypeofClaims.Where(p=>p.T_EmployeeInjuryID ==idvalue).Select(p=>p.T_ClaimTypeID));
                        }
                    }
                    //
                }
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_TypeofClaim>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_TypeofClaim), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_TypeofClaim).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_TypeofClaim), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TypeofClaim), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TypeofClaim), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TypeofClaim), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TypeofClaim), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_TypeofClaim), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_TypeofClaim), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_TypeofClaim), "p"));
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_TypeofClaim>(User, list, "T_TypeofClaim", null);
			if (key != null && key.Length > 0)
            {
			   var data = from x in list.Where(p=>p.DisplayValue.Contains(key)).OrderBy(q=>q.DisplayValue).Take(10).ToList()
                           select new { Id = x.Id, Name = x.DisplayValue };
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_TypeofClaim", User) || !User.CanAdd("T_TypeofClaim"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_TypeofClaim")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_TypeofClaim").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_TypeofClaim");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_TypeofClaim" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Type of Claims";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_ClaimTypeID":
										 var t_claimtypeId = db.T_ClaimTypes.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_claimtypeId == null)
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_ClaimTypeID":
										 var strPropertyT_ClaimType = lstEntityProp.FirstOrDefault(p => p.Key == "T_ClaimTypeID").Value;
										 ModelReflector.Property propT_ClaimType = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_ClaimType").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_ClaimType);
										 //var elementTypeT_ClaimType = db.T_ClaimTypes.ElementType;
										 //var propertyInfoT_ClaimType = elementTypeT_ClaimType.GetProperty(propT_ClaimType.Name);
										 // var t_claimtypeId = db.T_ClaimTypes.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_ClaimType.GetValue(p, null)) == assovalue);
										 var t_claimtypeId = db.T_ClaimTypes.Where(propT_ClaimType.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_claimtypeId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_EmployeeInjuryID":
        ...
        public ActionResult ImportData(FormCollection collection)
        {
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_TypeofClaim model = new T_TypeofClaim();
		 ...
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_ClaimType = db.T_ClaimTypes.ElementType;
			 //var propertyInfoT_ClaimType = elementTypeT_ClaimType.GetProperty(propT_ClaimType.Name);			 
			 //t_claimtypeId = db.T_ClaimTypes.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_ClaimType.GetValue(p, null)) == columnValue);
			 t_claimtypeId = db.T_ClaimTypes.Where(propT_ClaimType.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_claimtypeId = db.T_ClaimTypes.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_claimtypeId != null)
			model.T_ClaimTypeID = t_claimtypeId.Id;
		  else
			{
				if ((collection["T_ClaimTypeID"].ToString() == "true,false"))
				{
					try
					{
						T_ClaimType objT_ClaimType = new T_ClaimType();
						objT_ClaimType.T_Name  = (columnValue);
						db.T_ClaimTypes.Add(objT_ClaimType);
						db.SaveChanges();
						model.T_ClaimTypeID = objT_ClaimType.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_TypeofClaim validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_TypeofClaims.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_TypeofClaim OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_TypeofClaim").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_TypeofClaim");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_TypeofClaim OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_TypeofClaim");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_TypeofClaim OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_TypeofClaim");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_TypeofClaim OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_TypeofClaim",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_TypeofClaim OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_TypeofClaim");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_TypeofClaim OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_TypeofClaim").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_TypeofClaims.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_TypeofClaims;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_TypeofClaim, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_TypeofClaim, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_TypeofClaim>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_TypeofClaim t_typeofclaim)
        {
        ...
        public ActionResult GetCalculationValues(T_TypeofClaim t_typeofclaim)
        {
            t_typeofclaim.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_typeofclaim.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_TypeofClaim"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_TypeofClaim t_typeofclaim, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_TypeofClaim t_typeofclaim, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
