...
{	
    ...
    {
		private void LoadViewDataForCount(T_Address t_address, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_Address t_address)
        {		
			 ViewBag.T_AddressCountryID = new SelectList(db.T_Countrys, "ID", "DisplayValue", t_address.T_AddressCountryID);
			 ViewBag.T_AddressStateID = new SelectList(db.T_States, "ID", "DisplayValue", t_address.T_AddressStateID);
			 ViewBag.T_AddressCityID = new SelectList(db.T_Citys, "ID", "DisplayValue", t_address.T_AddressCityID);
			CustomLoadViewDataListAfterEdit(t_address);
        }
        private void LoadViewDataBeforeOnEdit(T_Address t_address)
        {		
               var _objT_AddressCountry = new List<T_Country>();
               _objT_AddressCountry.Add(t_address.t_addresscountry);
			   			   ViewBag.T_AddressCountryID = new SelectList(_objT_AddressCountry, "ID", "DisplayValue", t_address.T_AddressCountryID);
			               var _objT_AddressState = new List<T_State>();
               _objT_AddressState.Add(t_address.t_addressstate);
			   			   ViewBag.T_AddressStateID = new SelectList(_objT_AddressState, "ID", "DisplayValue", t_address.T_AddressStateID);
			               var _objT_AddressCity = new List<T_City>();
               _objT_AddressCity.Add(t_address.t_addresscity);
			   			   ViewBag.T_AddressCityID = new SelectList(_objT_AddressCity, "ID", "DisplayValue", t_address.T_AddressCityID);
					CustomLoadViewDataListBeforeEdit(t_address);
        }
        private void LoadViewDataAfterOnCreate(T_Address t_address)
        {			
			 ViewBag.T_AddressCountryID = new SelectList(db.T_Countrys, "ID", "DisplayValue", t_address.T_AddressCountryID);
			 ViewBag.T_AddressStateID = new SelectList(db.T_States, "ID", "DisplayValue", t_address.T_AddressStateID);
			 ViewBag.T_AddressCityID = new SelectList(db.T_Citys, "ID", "DisplayValue", t_address.T_AddressCityID);
		CustomLoadViewDataListAfterOnCreate(t_address);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Country" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_AddressCountry")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_AddressCountryID = new SelectList(db.T_Countrys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Countrys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_AddressCountry = new List<T_Country>();
			 ViewBag.T_AddressCountryID = new SelectList(objT_AddressCountry , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_State" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_AddressState")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_AddressStateID = new SelectList(db.T_States.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_States.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_AddressState = new List<T_State>();
			 ViewBag.T_AddressStateID = new SelectList(objT_AddressState , "ID", "DisplayValue");
		    }
			if(HostingEntityName == "T_City" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_AddressCity")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_AddressCityID = new SelectList(db.T_Citys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Citys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_AddressCity = new List<T_City>();
			 ViewBag.T_AddressCityID = new SelectList(objT_AddressCity , "ID", "DisplayValue");
		    }
		...
		private IQueryable<T_Address> sortRecords(IQueryable<T_Address> lstT_Address, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_AddressCountryID")
                return isAsc.ToLower() == "asc" ? lstT_Address.OrderBy(p => p.t_addresscountry.DisplayValue) : lstT_Address.OrderByDescending(p => p.t_addresscountry.DisplayValue);
	 if(sortBy == "T_AddressStateID")
                return isAsc.ToLower() == "asc" ? lstT_Address.OrderBy(p => p.t_addressstate.DisplayValue) : lstT_Address.OrderByDescending(p => p.t_addressstate.DisplayValue);
	 if(sortBy == "T_AddressCityID")
                return isAsc.ToLower() == "asc" ? lstT_Address.OrderBy(p => p.t_addresscity.DisplayValue) : lstT_Address.OrderByDescending(p => p.t_addresscity.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_Address.Sort(sortBy,true) : lstT_Address.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_Address), "t_address");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_Address>)lstT_Address.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_Address.ElementType, lambda.Body.Type },
                    lstT_Address.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_State")
            {
                var url = (Url.Action("FSearch"));
				var _Object = db.T_States.Find(Convert.ToInt64(id));
                url += "?search=";
						var T_AddressCountry  = _Object.T_StateCountryID;
						url += "&t_addresscountry=" + T_AddressCountry;									
            ...
            if (sourceEntity == "T_City")
            {
                ...
                url += "?search=";
						var T_AddressCountry  = _Object.T_CityCountryID;
						url += "&t_addresscountry=" + T_AddressCountry;									
						var T_AddressState  = _Object.T_CityStateID;
						url += "&t_addressstate=" + T_AddressState;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_addresscountry,string t_addressstate,string t_addresscity, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_AddressCountrylist = db.T_Countrys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_addresscountry != null)
            {
                var ids = t_addresscountry.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Address Country= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_AddressCountrylist.Union(db.T_Countrys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Country>(User, list, "T_Country", null);
					ViewBag.t_addresscountry = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_AddressCountrylist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Country>(User, list, "T_Country", null);
				ViewBag.t_addresscountry = new SelectList(list, "ID", "DisplayValue");
			}
			var T_AddressStatelist = db.T_States.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_addressstate != null)
            {
                var ids = t_addressstate.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Address State= ";
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            {
                               ids1.Add(Convert.ToInt64(str));
							   ViewBag.SearchResult += db.T_States.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_AddressStatelist.Union(db.T_States.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_State>(User, list, "T_State", null);
					ViewBag.t_addressstate = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_AddressStatelist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_State>(User, list, "T_State", null);
				ViewBag.t_addressstate = new SelectList(list, "ID", "DisplayValue");
			}
			var T_AddressCitylist = db.T_Citys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_addresscity != null)
            {
                var ids = t_addresscity.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Address City= ";
				   foreach (var str in ids)
                    {
                        if (!string.IsNullOrEmpty(str))
                        {
                            ...
                            {
                               ids1.Add(Convert.ToInt64(str));
							   ViewBag.SearchResult += db.T_Citys.Find(Convert.ToInt64(str)).DisplayValue+", ";
                            }
                        }
                    }
					ids1 = ids1.ToList();
					var list = T_AddressCitylist.Union(db.T_Citys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_City>(User, list, "T_City", null);
					ViewBag.t_addresscity = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_AddressCitylist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_City>(User, list, "T_City", null);
				ViewBag.t_addresscity = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_AddressCountry = new List<T_Country>();
		    ViewBag.t_addresscountry = new SelectList(objT_AddressCountry, "ID", "DisplayValue");
			var objT_AddressState = new List<T_State>();
		    ViewBag.t_addressstate = new SelectList(objT_AddressState, "ID", "DisplayValue");
			var objT_AddressCity = new List<T_City>();
		    ViewBag.t_addresscity = new SelectList(objT_AddressCity, "ID", "DisplayValue");
			}
			...
			columns.Add("2", "AddressLine1");
			columns.Add("3", "AddressLine2");
			columns.Add("4", "ZipCode");
			columns.Add("5", "Address Country");
			columns.Add("6", "Address State");
			columns.Add("7", "Address City");
				ViewBag.HideColumns = new MultiSelectList(columns, "Key", "Value");
				return View(new T_Address());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_Address", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_Address", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_Address()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_Address/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_addresscountry,string t_addressstate,string t_addresscity ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_Address  = searchRecords(lstT_Address, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_Address = searchRecords(lstT_Address, search,true);
			}
			...
			lstT_Address = lstT_Address.Include(t=>t.t_addresscountry).Include(t=>t.t_addressstate).Include(t=>t.t_addresscity);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_Address", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_Address = lstT_Address.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_Address = Sorting.Sort<T_Address>(lstT_Address, DataOrdering);
            var _T_Address = lstT_Address;
			//if (lstT_Address.Where(p => p.t_addresscountry != null).Count() <= 50)
		    //ViewBag.t_addresscountry = new SelectList(lstT_Address.Where(p => p.t_addresscountry != null).Select(P => P.t_addresscountry).Distinct(), "ID", "DisplayValue");
            if (t_addresscountry != null)
            {
                var ids = t_addresscountry.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_Address = _T_Address.Where(p => ids1.Contains(p.T_AddressCountryID));
            }
				//if (lstT_Address.Where(p => p.t_addressstate != null).Count() <= 50)
		    //ViewBag.t_addressstate = new SelectList(lstT_Address.Where(p => p.t_addressstate != null).Select(P => P.t_addressstate).Distinct(), "ID", "DisplayValue");
            if (t_addressstate != null)
            {
                var ids = t_addressstate.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_States.Find(Convert.ToInt64(str));
                            ViewBag.SearchResult += obj != null ? obj.DisplayValue + ", " : "";
                        }
                    }
                    //
                ...
                ids1 = ids1.ToList();
                _T_Address = _T_Address.Where(p => ids1.Contains(p.T_AddressStateID));
            }
				//if (lstT_Address.Where(p => p.t_addresscity != null).Count() <= 50)
		    //ViewBag.t_addresscity = new SelectList(lstT_Address.Where(p => p.t_addresscity != null).Select(P => P.t_addresscity).Distinct(), "ID", "DisplayValue");
            if (t_addresscity != null)
            {
                var ids = t_addresscity.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ...
				 foreach (var str in ids)
                {
                    ...
                    if (!string.IsNullOrEmpty(str))
                    {
                        ...
                        {
                            ids1.Add(Convert.ToInt64(str));
                            var obj = db.T_Citys.Find(Convert.ToInt64(str));
                            ViewBag.SearchResult += obj != null ? obj.DisplayValue + ", " : "";
                        }
                    }
                    //
                ...
                ids1 = ids1.ToList();
                _T_Address = _T_Address.Where(p => ids1.Contains(p.T_AddressCityID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Address");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_Address", tagsSplit.ToArray());
                    }
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_Addresss.GroupBy(p => p.t_addresscountry.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Address Country (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Address Country"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Address Country", "Address"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Address", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_Addresss.GroupBy(p => p.t_addressstate.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "2" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Address State (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Address State"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Pie, "", "Address State", "Address"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Address", new { type = "2", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
           ...
           {
                var gd = db.T_Addresss.GroupBy(p => p.t_addresscity.DisplayValue).ToList();
                var _xval = gd.Select(k => Convert.ToString(k.Key)).ToList();
				if (type == "3" || type == "all")
                {
                    var cntgrt10 = false;
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.BorderSkin.SkinStyle = BorderSkinStyle.Emboss;
				if (cntgrt10)
                chart.Titles.Add(CreateTitle("Count by Address City (Top 10)"));
				else
				 chart.Titles.Add(CreateTitle("Count by Address City"));
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Address City", "Address"));
                chart.Series.Add(CreateSeries(SeriesChartType.Column, ""));
                chart.Series[0].Points.DataBindXY(_xval, _yval);
                chart.Legends.Add(CreateLegend(""));
				 if (inlarge != null)
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Address", new { type = "3", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_Addresss.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_address = jedb.T_Addresss.Find(id);
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_Addresss.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_Addresss.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_Address>(_Obj, "T_Address", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_Address> list = db.T_Addresss;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_Addresss;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_Address), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_Address, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_Address>)q);
				   list = ((IQueryable<T_Address>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_Addresss;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_Address>(User,list, "T_Address",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_Address>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_Address), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_Address).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_Address), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Address), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Address), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Address), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Address), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Address), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_Address), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_Address), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_Address", User) || !User.CanAdd("T_Address"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_Address")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_Address").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_Address");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_Address" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Addresss";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_AddressCountryID":
										 var t_addresscountryId = db.T_Countrys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_addresscountryId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_AddressStateID":
										 var t_addressstateId = db.T_States.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_addressstateId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_AddressCityID":
										 var t_addresscityId = db.T_Citys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_addresscityId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                        default:
                                            break;
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_AddressCountryID":
										 var strPropertyT_AddressCountry = lstEntityProp.FirstOrDefault(p => p.Key == "T_AddressCountryID").Value;
										 ModelReflector.Property propT_AddressCountry = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Country").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_AddressCountry);
										 //var elementTypeT_AddressCountry = db.T_Countrys.ElementType;
										 //var propertyInfoT_AddressCountry = elementTypeT_AddressCountry.GetProperty(propT_AddressCountry.Name);
										 // var t_addresscountryId = db.T_Countrys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_AddressCountry.GetValue(p, null)) == assovalue);
										 var t_addresscountryId = db.T_Countrys.Where(propT_AddressCountry.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_addresscountryId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_AddressStateID":
										 var strPropertyT_AddressState = lstEntityProp.FirstOrDefault(p => p.Key == "T_AddressStateID").Value;
										 ModelReflector.Property propT_AddressState = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_State").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_AddressState);
										 //var elementTypeT_AddressState = db.T_States.ElementType;
										 //var propertyInfoT_AddressState = elementTypeT_AddressState.GetProperty(propT_AddressState.Name);
										 // var t_addressstateId = db.T_States.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_AddressState.GetValue(p, null)) == assovalue);
										 var t_addressstateId = db.T_States.Where(propT_AddressState.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_addressstateId == null)
											uniqueassoValues.Add(assovalue);
										 break;
																		 case "T_AddressCityID":
										 var strPropertyT_AddressCity = lstEntityProp.FirstOrDefault(p => p.Key == "T_AddressCityID").Value;
										 ModelReflector.Property propT_AddressCity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_City").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_AddressCity);
										 //var elementTypeT_AddressCity = db.T_Citys.ElementType;
										 //var propertyInfoT_AddressCity = elementTypeT_AddressCity.GetProperty(propT_AddressCity.Name);
										 // var t_addresscityId = db.T_Citys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_AddressCity.GetValue(p, null)) == assovalue);
										 var t_addresscityId = db.T_Citys.Where(propT_AddressCity.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_addresscityId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        for (int j = 0; j < sheetColumns.Count; j++)
                        {
                            ...
                            {
    case "T_AddressLine1":
    model.T_AddressLine1 = columnValue;
	break;
    case "T_AddressLine2":
    model.T_AddressLine2 = columnValue;
	break;
    case "T_ZipCode":
    model.T_ZipCode = columnValue;
	break;
					 case "T_AddressCountryID":
		 dynamic t_addresscountryId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_AddressCountry = db.T_Countrys.ElementType;
			 //var propertyInfoT_AddressCountry = elementTypeT_AddressCountry.GetProperty(propT_AddressCountry.Name);			 
			 //t_addresscountryId = db.T_Countrys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_AddressCountry.GetValue(p, null)) == columnValue);
			 t_addresscountryId = db.T_Countrys.Where(propT_AddressCountry.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_addresscountryId = db.T_Countrys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_addresscountryId != null)
			model.T_AddressCountryID = t_addresscountryId.Id;
		  else
			{
				if ((collection["T_AddressCountryID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_AddressCountryID = objT_Country.Id;
					}
					catch
					{
						continue;
		 ...
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_AddressState = db.T_States.ElementType;
			 //var propertyInfoT_AddressState = elementTypeT_AddressState.GetProperty(propT_AddressState.Name);			 
			 //t_addressstateId = db.T_States.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_AddressState.GetValue(p, null)) == columnValue);
			 t_addressstateId = db.T_States.Where(propT_AddressState.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_addressstateId = db.T_States.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_addressstateId != null)
			model.T_AddressStateID = t_addressstateId.Id;
		  else
			{
				if ((collection["T_AddressStateID"].ToString() == "true,false"))
				{
					try
					{
						T_State objT_State = new T_State();
						objT_State.T_Name  = (columnValue);
						db.T_States.Add(objT_State);
						db.SaveChanges();
						model.T_AddressStateID = objT_State.Id;
					}
					catch
					{
						continue;
		 ...
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_AddressCity = db.T_Citys.ElementType;
			 //var propertyInfoT_AddressCity = elementTypeT_AddressCity.GetProperty(propT_AddressCity.Name);			 
			 //t_addresscityId = db.T_Citys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_AddressCity.GetValue(p, null)) == columnValue);
			 t_addresscityId = db.T_Citys.Where(propT_AddressCity.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_addresscityId = db.T_Citys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_addresscityId != null)
			model.T_AddressCityID = t_addresscityId.Id;
		  else
			{
				if ((collection["T_AddressCityID"].ToString() == "true,false"))
				{
					try
					{
						T_City objT_City = new T_City();
						objT_City.T_Name  = (columnValue);
						db.T_Citys.Add(objT_City);
						db.SaveChanges();
						model.T_AddressCityID = objT_City.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_Address validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_Addresss.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_Address OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_Address").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_Address");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_Address OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_Address");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_Address OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_Address");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_Address OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_Address",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_Address OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_Address");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_Address OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Address").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_Addresss.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_Addresss;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_Address, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_Address, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_Address>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_Address t_address)
        {
        ...
        public ActionResult GetCalculationValues(T_Address t_address)
        {
            t_address.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_address.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_Address t_address, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_Address t_address, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
