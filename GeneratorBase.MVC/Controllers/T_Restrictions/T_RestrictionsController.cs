...
{	
    ...
    {
		private void LoadViewDataForCount(T_Restrictions t_restrictions, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_Restrictions t_restrictions)
        {		
			 ViewBag.T_RestrictionsFacilityAssociationID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_restrictions.T_RestrictionsFacilityAssociationID);
			CustomLoadViewDataListAfterEdit(t_restrictions);
        }
        private void LoadViewDataBeforeOnEdit(T_Restrictions t_restrictions)
        {		
               var _objT_RestrictionsFacilityAssociation = new List<T_Facility>();
               _objT_RestrictionsFacilityAssociation.Add(t_restrictions.t_restrictionsfacilityassociation);
			   			   ViewBag.T_RestrictionsFacilityAssociationID = new SelectList(_objT_RestrictionsFacilityAssociation, "ID", "DisplayValue", t_restrictions.T_RestrictionsFacilityAssociationID);
					CustomLoadViewDataListBeforeEdit(t_restrictions);
        }
        private void LoadViewDataAfterOnCreate(T_Restrictions t_restrictions)
        {			
			 ViewBag.T_RestrictionsFacilityAssociationID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_restrictions.T_RestrictionsFacilityAssociationID);
		CustomLoadViewDataListAfterOnCreate(t_restrictions);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Facility" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_RestrictionsFacilityAssociation")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_RestrictionsFacilityAssociationID = new SelectList(db.T_Facilitys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Facilitys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_RestrictionsFacilityAssociation = new List<T_Facility>();
			 ViewBag.T_RestrictionsFacilityAssociationID = new SelectList(objT_RestrictionsFacilityAssociation , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_Restrictions> searchRecords(IQueryable<T_Restrictions> lstT_Restrictions, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_Restrictions = lstT_Restrictions.Where(s => (s.t_restrictionsfacilityassociation!= null && (s.t_restrictionsfacilityassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||s.T_TypeOfRestrictions_t_restrictions.Any(c => c.t_employeeinjury.DisplayValue.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_Restrictions = lstT_Restrictions.Where(s => (s.t_restrictionsfacilityassociation!= null && (s.t_restrictionsfacilityassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_Restrictions;
        }
		private IQueryable<T_Restrictions> sortRecords(IQueryable<T_Restrictions> lstT_Restrictions, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_RestrictionsFacilityAssociationID")
                return isAsc.ToLower() == "asc" ? lstT_Restrictions.OrderBy(p => p.t_restrictionsfacilityassociation.DisplayValue) : lstT_Restrictions.OrderByDescending(p => p.t_restrictionsfacilityassociation.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_Restrictions.Sort(sortBy,true) : lstT_Restrictions.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_Restrictions), "t_restrictions");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_Restrictions>)lstT_Restrictions.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_Restrictions.ElementType, lambda.Body.Type },
                    lstT_Restrictions.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Department")
            {
                ...
                url += "?search=";
						var T_RestrictionsFacilityAssociation  = _Object.T_DepartmentFacilityAssociationID;
						url += "&t_restrictionsfacilityassociation=" + T_RestrictionsFacilityAssociation;									
            ...
            if (sourceEntity == "T_Position")
            {
                ...
                url += "?search=";
						var T_RestrictionsFacilityAssociation  = _Object.T_FacilityAssignedToID;
						url += "&t_restrictionsfacilityassociation=" + T_RestrictionsFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Employee")
            {
                ...
                url += "?search=";
						var T_RestrictionsFacilityAssociation  = _Object.T_EmployeeAtFacilityID;
						url += "&t_restrictionsfacilityassociation=" + T_RestrictionsFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_DepartmentArea")
            {
                ...
                url += "?search=";
						var T_RestrictionsFacilityAssociation  = _Object.T_DepartmentAreaEmployeeTypeAssociationID;
						url += "&t_restrictionsfacilityassociation=" + T_RestrictionsFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_ClaimType")
            {
                ...
                url += "?search=";
						var T_RestrictionsFacilityAssociation  = _Object.T_ClaimTypeFacilityAssociationID;
						url += "&t_restrictionsfacilityassociation=" + T_RestrictionsFacilityAssociation;									
							var T_TypeofClaim_t_claimtype_t_typeofrestrictions  = _Object.T_TypeofClaim_t_claimtype.Select(p => p.T_EmployeeInjuryID);
							if (T_TypeofClaim_t_claimtype_t_typeofrestrictions != null)
							{
								var str = String.Join(",", T_TypeofClaim_t_claimtype_t_typeofrestrictions);
								url += "&t_typeofrestrictions=" + str;
							}
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_RestrictionsFacilityAssociation  = _Object.T_FacilityUnitXID;
						url += "&t_restrictionsfacilityassociation=" + T_RestrictionsFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Unit")
            {
                ...
                url += "?search=";
						var T_RestrictionsFacilityAssociation  = _Object.T_FacilityUnitID;
						url += "&t_restrictionsfacilityassociation=" + T_RestrictionsFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_SalaryRange")
            {
                ...
                url += "?search=";
						var T_RestrictionsFacilityAssociation  = _Object.T_FacilitySalaryRangeAssociationID;
						url += "&t_restrictionsfacilityassociation=" + T_RestrictionsFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_FacilityConfiguration")
            {
                ...
                url += "?search=";
						var T_RestrictionsFacilityAssociation  = _Object.T_TenantConfigurationAssociationID;
						url += "&t_restrictionsfacilityassociation=" + T_RestrictionsFacilityAssociation;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_restrictionsfacilityassociation,string t_typeofrestrictions, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_RestrictionsFacilityAssociationlist = db.T_Facilitys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
                ...
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Facility= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_RestrictionsFacilityAssociationlist.Union(db.T_Facilitys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
					ViewBag.t_restrictionsfacilityassociation = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_RestrictionsFacilityAssociationlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
				ViewBag.t_restrictionsfacilityassociation = new SelectList(list, "ID", "DisplayValue");
			}
			var T_TypeOfRestrictionslist = db.T_EmployeeInjurys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
                ...
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Employees Injuries= ";
				  foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_TypeOfRestrictionslist.Union(db.T_EmployeeInjurys.Where(p=>ids1.Contains(p.Id))).Distinct().Select(p=>new { ID=p.Id,DisplayValue=p.DisplayValue }).ToList();
			...
			{
			var objT_RestrictionsFacilityAssociation = new List<T_Facility>();
		    ViewBag.t_restrictionsfacilityassociation = new SelectList(objT_RestrictionsFacilityAssociation, "ID", "DisplayValue");
			var objT_TypeOfRestrictions = new List<T_EmployeeInjury>();
		    ViewBag.t_typeofrestrictions = new SelectList(objT_TypeOfRestrictions, "ID", "DisplayValue");
				...
				ViewBag.EntityName = "T_Restrictions";
			...
			columns.Add("4", "Description");
				...
				return View(new T_Restrictions());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_Restrictions", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_Restrictions", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_Restrictions()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_Restrictions/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_restrictionsfacilityassociation,string t_typeofrestrictions ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_Restrictions  = searchRecords(lstT_Restrictions, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_Restrictions = searchRecords(lstT_Restrictions, search,true);
			}
			...
			lstT_Restrictions = lstT_Restrictions.Include(t=>t.t_restrictionsfacilityassociation);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_Restrictions", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_Restrictions = lstT_Restrictions.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_Restrictions = Sorting.Sort<T_Restrictions>(lstT_Restrictions, DataOrdering);
            var _T_Restrictions = lstT_Restrictions;
			//if (lstT_Restrictions.Where(p => p.t_restrictionsfacilityassociation != null).Count() <= 50)
		    //ViewBag.t_restrictionsfacilityassociation = new SelectList(lstT_Restrictions.Where(p => p.t_restrictionsfacilityassociation != null).Select(P => P.t_restrictionsfacilityassociation).Distinct(), "ID", "DisplayValue");
            if (t_restrictionsfacilityassociation != null)
            {
                var ids = t_restrictionsfacilityassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_Restrictions = _T_Restrictions.Where(p => ids1.Contains(p.T_RestrictionsFacilityAssociationID));
            }
				if (t_typeofrestrictions != null)
            {
                var ids = t_typeofrestrictions.Split(",".ToCharArray());
                ...
                ids1 = ids1.ToList();
                _T_Restrictions = _T_Restrictions.Where(p => ids1.Contains(p.Id));
            }
			if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Restrictions");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_Restrictions", tagsSplit.ToArray());
                    }
		...
		var entity = "T_Restrictions";
           ...
           var entitylist = db.T_Restrictionss;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_Restrictionss.GroupBy(p => p.t_restrictionsfacilityassociation.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Facility", "Restrictions"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_Restrictions", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_Restrictionss.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_restrictions = jedb.T_Restrictionss.Find(id);
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_Restrictionss.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_Restrictionss.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_Restrictions>(_Obj, "T_Restrictions", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_Restrictions> list = db.T_Restrictionss;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_Restrictionss;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_Restrictions), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_Restrictions, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_Restrictions>)q);
				   list = ((IQueryable<T_Restrictions>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_Restrictionss;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_Restrictions>(User,list, "T_Restrictions",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_Restrictions>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_Restrictions), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_Restrictions).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_Restrictions), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Restrictions), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Restrictions), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Restrictions), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Restrictions), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_Restrictions), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_Restrictions), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_Restrictions), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_Restrictions", User) || !User.CanAdd("T_Restrictions"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_Restrictions")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_Restrictions").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_Restrictions");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_Restrictions" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Restrictionss";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_RestrictionsFacilityAssociationID":
										 var t_restrictionsfacilityassociationId = db.T_Facilitys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_restrictionsfacilityassociationId == null)
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_Restrictions";
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_RestrictionsFacilityAssociationID":
										 var strPropertyT_RestrictionsFacilityAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_RestrictionsFacilityAssociationID").Value;
										 ModelReflector.Property propT_RestrictionsFacilityAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Facility").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_RestrictionsFacilityAssociation);
										 //var elementTypeT_RestrictionsFacilityAssociation = db.T_Facilitys.ElementType;
										 //var propertyInfoT_RestrictionsFacilityAssociation = elementTypeT_RestrictionsFacilityAssociation.GetProperty(propT_RestrictionsFacilityAssociation.Name);
										 // var t_restrictionsfacilityassociationId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_RestrictionsFacilityAssociation.GetValue(p, null)) == assovalue);
										 var t_restrictionsfacilityassociationId = db.T_Facilitys.Where(propT_RestrictionsFacilityAssociation.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_restrictionsfacilityassociationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_Restrictions model = new T_Restrictions();
	...
	break;
					 case "T_RestrictionsFacilityAssociationID":
		 dynamic t_restrictionsfacilityassociationId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_RestrictionsFacilityAssociation = db.T_Facilitys.ElementType;
			 //var propertyInfoT_RestrictionsFacilityAssociation = elementTypeT_RestrictionsFacilityAssociation.GetProperty(propT_RestrictionsFacilityAssociation.Name);			 
			 //t_restrictionsfacilityassociationId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_RestrictionsFacilityAssociation.GetValue(p, null)) == columnValue);
			 t_restrictionsfacilityassociationId = db.T_Facilitys.Where(propT_RestrictionsFacilityAssociation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_restrictionsfacilityassociationId = db.T_Facilitys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_restrictionsfacilityassociationId != null)
			model.T_RestrictionsFacilityAssociationID = t_restrictionsfacilityassociationId.Id;
		  else
			{
				if ((collection["T_RestrictionsFacilityAssociationID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_RestrictionsFacilityAssociationID = objT_Facility.Id;
					}
					catch
					{
						continue;
		...
		public bool ValidateModel(T_Restrictions validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_Restrictionss.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_Restrictions OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_Restrictions").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_Restrictions");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_Restrictions OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_Restrictions");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_Restrictions OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_Restrictions");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_Restrictions OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_Restrictions",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_Restrictions OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_Restrictions");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_Restrictions OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Restrictions").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_Restrictionss.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_Restrictionss;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_Restrictions, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_Restrictions, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_Restrictions>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_Restrictions t_restrictions)
        {
        ...
        public ActionResult GetCalculationValues(T_Restrictions t_restrictions)
        {
            t_restrictions.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_restrictions.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_Restrictions"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_Restrictions t_restrictions, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_Restrictions t_restrictions, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
