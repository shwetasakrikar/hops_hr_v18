...
{	
    ...
    {
		private void LoadViewDataForCount(T_DepartmentArea t_departmentarea, string AssocType)
        {
        }
		private void LoadViewDataAfterOnEdit(T_DepartmentArea t_departmentarea)
        {		
			 ViewBag.T_DepartmentAreaEmployeeTypeAssociationID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_departmentarea.T_DepartmentAreaEmployeeTypeAssociationID);
			CustomLoadViewDataListAfterEdit(t_departmentarea);
        }
        private void LoadViewDataBeforeOnEdit(T_DepartmentArea t_departmentarea)
        {		
               var _objT_DepartmentAreaEmployeeTypeAssociation = new List<T_Facility>();
               _objT_DepartmentAreaEmployeeTypeAssociation.Add(t_departmentarea.t_departmentareaemployeetypeassociation);
			   			   ViewBag.T_DepartmentAreaEmployeeTypeAssociationID = new SelectList(_objT_DepartmentAreaEmployeeTypeAssociation, "ID", "DisplayValue", t_departmentarea.T_DepartmentAreaEmployeeTypeAssociationID);
				 ViewBag.T_UnitXDepartmentAreaCount = t_departmentarea.t_unitxdepartmentarea.Count();
		CustomLoadViewDataListBeforeEdit(t_departmentarea);
        }
        private void LoadViewDataAfterOnCreate(T_DepartmentArea t_departmentarea)
        {			
			 ViewBag.T_DepartmentAreaEmployeeTypeAssociationID = new SelectList(db.T_Facilitys, "ID", "DisplayValue", t_departmentarea.T_DepartmentAreaEmployeeTypeAssociationID);
		CustomLoadViewDataListAfterOnCreate(t_departmentarea);
        }
        private void LoadViewDataBeforeOnCreate(string HostingEntityName, string HostingEntityID, string AssociatedType)
        {			
			if(HostingEntityName == "T_Facility" && Convert.ToInt64(HostingEntityID) > 0 && AssociatedType=="T_DepartmentAreaEmployeeTypeAssociation")
			{
			    var hostid = Convert.ToInt64(HostingEntityID);
				ViewBag.T_DepartmentAreaEmployeeTypeAssociationID = new SelectList(db.T_Facilitys.Where(p=>p.Id==hostid).ToList(), "ID", "DisplayValue",HostingEntityID);
				ViewBag.DisplayVal = db.T_Facilitys.Where(p => p.Id == hostid).ToList().FirstOrDefault().DisplayValue;
		    }
            else
			{
			var objT_DepartmentAreaEmployeeTypeAssociation = new List<T_Facility>();
			 ViewBag.T_DepartmentAreaEmployeeTypeAssociationID = new SelectList(objT_DepartmentAreaEmployeeTypeAssociation , "ID", "DisplayValue");
		    }
			CustomLoadViewDataListBeforeOnCreate(HostingEntityName, HostingEntityID, AssociatedType);
        }
		private IQueryable<T_DepartmentArea> searchRecords(IQueryable<T_DepartmentArea> lstT_DepartmentArea, string searchString, bool? IsDeepSearch)
        {
		   searchString = searchString.Trim();
		if(Convert.ToBoolean(IsDeepSearch))
            lstT_DepartmentArea = lstT_DepartmentArea.Where(s => (s.t_departmentareaemployeetypeassociation!= null && (s.t_departmentareaemployeetypeassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
		else
			 lstT_DepartmentArea = lstT_DepartmentArea.Where(s => (s.t_departmentareaemployeetypeassociation!= null && (s.t_departmentareaemployeetypeassociation.DisplayValue.ToUpper().Contains(searchString) )) ||(!String.IsNullOrEmpty(s.T_Name) && s.T_Name.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.T_Description) && s.T_Description.ToUpper().Contains(searchString)) ||(!String.IsNullOrEmpty(s.DisplayValue) && s.DisplayValue.ToUpper().Contains(searchString)));
            return lstT_DepartmentArea;
        }
		private IQueryable<T_DepartmentArea> sortRecords(IQueryable<T_DepartmentArea> lstT_DepartmentArea, string sortBy, string isAsc)
        {
            string methodName = "";
	 ...
	 if(sortBy == "T_DepartmentAreaEmployeeTypeAssociationID")
                return isAsc.ToLower() == "asc" ? lstT_DepartmentArea.OrderBy(p => p.t_departmentareaemployeetypeassociation.DisplayValue) : lstT_DepartmentArea.OrderByDescending(p => p.t_departmentareaemployeetypeassociation.DisplayValue);
		    if (sortBy.Contains("."))
                return isAsc.ToLower() == "asc" ? lstT_DepartmentArea.Sort(sortBy,true) : lstT_DepartmentArea.Sort(sortBy,false);
            ParameterExpression paramExpression = Expression.Parameter(typeof(T_DepartmentArea), "t_departmentarea");
            MemberExpression memExp = Expression.PropertyOrField(paramExpression, sortBy);
            LambdaExpression lambda = Expression.Lambda(memExp, paramExpression);
            return (IQueryable<T_DepartmentArea>)lstT_DepartmentArea.Provider.CreateQuery(
                Expression.Call(
                    typeof(Queryable),
                    methodName,
                    new Type[] { lstT_DepartmentArea.ElementType, lambda.Body.Type },
                    lstT_DepartmentArea.Expression,
                    lambda));
        }
		public ActionResult FindFSearch(string id, string sourceEntity)
        {
            if (sourceEntity == "T_Department")
            {
                ...
                url += "?search=";
						var T_DepartmentAreaEmployeeTypeAssociation  = _Object.T_DepartmentFacilityAssociationID;
						url += "&t_departmentareaemployeetypeassociation=" + T_DepartmentAreaEmployeeTypeAssociation;									
            ...
            if (sourceEntity == "T_Position")
            {
                ...
                url += "?search=";
						var T_DepartmentAreaEmployeeTypeAssociation  = _Object.T_FacilityAssignedToID;
						url += "&t_departmentareaemployeetypeassociation=" + T_DepartmentAreaEmployeeTypeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Employee")
            {
                ...
                url += "?search=";
						var T_DepartmentAreaEmployeeTypeAssociation  = _Object.T_EmployeeAtFacilityID;
						url += "&t_departmentareaemployeetypeassociation=" + T_DepartmentAreaEmployeeTypeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_ClaimType")
            {
                ...
                url += "?search=";
						var T_DepartmentAreaEmployeeTypeAssociation  = _Object.T_ClaimTypeFacilityAssociationID;
						url += "&t_departmentareaemployeetypeassociation=" + T_DepartmentAreaEmployeeTypeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Restrictions")
            {
                ...
                url += "?search=";
						var T_DepartmentAreaEmployeeTypeAssociation  = _Object.T_RestrictionsFacilityAssociationID;
						url += "&t_departmentareaemployeetypeassociation=" + T_DepartmentAreaEmployeeTypeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_UnitX")
            {
                ...
                url += "?search=";
						var T_DepartmentAreaEmployeeTypeAssociation  = _Object.T_FacilityUnitXID;
						url += "&t_departmentareaemployeetypeassociation=" + T_DepartmentAreaEmployeeTypeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_Unit")
            {
                ...
                url += "?search=";
						var T_DepartmentAreaEmployeeTypeAssociation  = _Object.T_FacilityUnitID;
						url += "&t_departmentareaemployeetypeassociation=" + T_DepartmentAreaEmployeeTypeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_SalaryRange")
            {
                ...
                url += "?search=";
						var T_DepartmentAreaEmployeeTypeAssociation  = _Object.T_FacilitySalaryRangeAssociationID;
						url += "&t_departmentareaemployeetypeassociation=" + T_DepartmentAreaEmployeeTypeAssociation;									
                Response.Redirect(url.ToString());
            }
            if (sourceEntity == "T_FacilityConfiguration")
            {
                ...
                url += "?search=";
						var T_DepartmentAreaEmployeeTypeAssociation  = _Object.T_TenantConfigurationAssociationID;
						url += "&t_departmentareaemployeetypeassociation=" + T_DepartmentAreaEmployeeTypeAssociation;									
                Response.Redirect(url.ToString());
            }
            return null;
        }
        public ActionResult SetFSearch(string searchString, string HostingEntity ,string t_departmentareaemployeetypeassociation, bool? RenderPartial)
        {
			int Qcount = 0;
			...
			if(Qcount > 0)
			{
			var T_DepartmentAreaEmployeeTypeAssociationlist = db.T_Facilitys.OrderBy(p=>p.DisplayValue).Take(10).Distinct();
            if (t_departmentareaemployeetypeassociation != null)
            {
                var ids = t_departmentareaemployeetypeassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
				 ViewBag.SearchResult += "\r\n Facility= ";
				   foreach (var str in ids)
					...
					ids1 = ids1.ToList();
					var list = T_DepartmentAreaEmployeeTypeAssociationlist.Union(db.T_Facilitys.Where(p=>ids1.Contains(p.Id))).Distinct();
					FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
					list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
					ViewBag.t_departmentareaemployeetypeassociation = new SelectList(list, "ID", "DisplayValue");
			}
			else
			{
				var list = T_DepartmentAreaEmployeeTypeAssociationlist;
				FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
				list = _fad.FilterDropdown<T_Facility>(User, list, "T_Facility", null);
				ViewBag.t_departmentareaemployeetypeassociation = new SelectList(list, "ID", "DisplayValue");
			}
			...
			{
			var objT_DepartmentAreaEmployeeTypeAssociation = new List<T_Facility>();
		    ViewBag.t_departmentareaemployeetypeassociation = new SelectList(objT_DepartmentAreaEmployeeTypeAssociation, "ID", "DisplayValue");
			}
				...
				ViewBag.EntityName = "T_DepartmentArea";
			...
			columns.Add("4", "Description");
				...
				return View(new T_DepartmentArea());
            }
		public string conditionFSearch(string property, string operators, string value)
        {
            string expression = string.Empty;
            var PrpertyType = GetDataTypeOfProperty("T_DepartmentArea", property);
            var dataType = PrpertyType[0];
            property = PrpertyType[1];
            if (value.StartsWith("[") && value.EndsWith("]"))
            {
                var ValueType = GetDataTypeOfProperty("T_DepartmentArea", value, true);
                ...
                case "DateTime":
                    {
					   ...
					   {
							expression = string.Format(" DbFunctions.TruncateTime(" + property + ") " + operators + " (\"{0}\")", (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, (new T_DepartmentArea()).m_Timezone)).Date);
						}
						else
						{
		...
		// GET: /T_DepartmentArea/FSearch/
		[Audit("FacetedSearch")]
		public ActionResult FSearch(string currentFilter, string searchString, string FSFilter, string sortBy, string isAsc, int? page, int? itemsPerPage, string search, bool? IsExport ,string t_departmentareaemployeetypeassociation ,string FilterCondition, string HostingEntity, string AssociatedType,string HostingEntityID, string viewtype, string SortOrder, string HideColumns, string GroupByColumn,bool? IsReports, bool? IsdrivedTab)
        {
            ...
            {
                lstT_DepartmentArea  = searchRecords(lstT_DepartmentArea, searchString.ToUpper(),true);
            }
			  if(!string.IsNullOrEmpty(search))
                	search=search.Replace("?IsAddPop=true", "");
			if(!string.IsNullOrEmpty(search)){
				ViewBag.SearchResult += "\r\n General Criterial= "+search;
				lstT_DepartmentArea = searchRecords(lstT_DepartmentArea, search,true);
			}
			...
			lstT_DepartmentArea = lstT_DepartmentArea.Include(t=>t.t_departmentareaemployeetypeassociation);
			if (!string.IsNullOrEmpty(FilterCondition))
            {
                ...
                foreach (var cond in conditions)
                {
                    ...
                    if (iCnt == conditions.Count())
                        LogicalConnector = "";
					ViewBag.SearchResult += " " + GetPropertyDP("T_DepartmentArea", PropertyName) + " " + Operator + " " + Value + " " + LogicalConnector;
                    whereCondition.Append(conditionFSearch(PropertyName, Operator, Value) + LogicalConnector);
                    iCnt++;
                }
                if (!string.IsNullOrEmpty(whereCondition.ToString()))
                    lstT_DepartmentArea = lstT_DepartmentArea.Where(whereCondition.ToString());
            ...
            if (!string.IsNullOrEmpty(DataOrdering))
                lstT_DepartmentArea = Sorting.Sort<T_DepartmentArea>(lstT_DepartmentArea, DataOrdering);
            var _T_DepartmentArea = lstT_DepartmentArea;
			//if (lstT_DepartmentArea.Where(p => p.t_departmentareaemployeetypeassociation != null).Count() <= 50)
		    //ViewBag.t_departmentareaemployeetypeassociation = new SelectList(lstT_DepartmentArea.Where(p => p.t_departmentareaemployeetypeassociation != null).Select(P => P.t_departmentareaemployeetypeassociation).Distinct(), "ID", "DisplayValue");
            if (t_departmentareaemployeetypeassociation != null)
            {
                var ids = t_departmentareaemployeetypeassociation.Split(",".ToCharArray());
                List<long?> ids1 = new List<long?>();
                ...
                ids1 = ids1.ToList();
                _T_DepartmentArea = _T_DepartmentArea.Where(p => ids1.Contains(p.T_DepartmentAreaEmployeeTypeAssociationID));
            }
				if (!string.IsNullOrEmpty(SortOrder))
            {
                ViewBag.SearchResult += " \r\n Sort Order = ";
                ...
                var Entity = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_DepartmentArea");
			...
			if (!Request.IsAjaxRequest())
			{
				...
				if (!string.IsNullOrEmpty(GroupByColumn))
                    foreach (var item in list)
                    {
                        ...
                        item.m_DisplayValue = EntityComparer.GetGroupByDisplayValue(item, "T_DepartmentArea", tagsSplit.ToArray());
                    }
		...
		var entity = "T_DepartmentArea";
           ...
           var entitylist = db.T_DepartmentAreas;
		...
		public string ShowGraph1(string type, int? inlarge)
        {
            string result = "";
           {
                var gd = db.T_DepartmentAreas.GroupBy(p => p.t_departmentareaemployeetypeassociation.DisplayValue).ToList();
			   ...
			   var chart = new System.Web.UI.DataVisualization.Charting.Chart
                ...
                chart.ChartAreas.Add(CreateChartArea(SeriesChartType.Column, "", "Facility", "Department Area"));
                ...
                if (cntgrt10)
                    {
                        string img = "<div class='col-sm-4' style='padding:0px; margin:0px 0px 0px 0px;'><img src='data:image/png;base64,{0}' alt='' usemap='#ImageMap' style='width:100%; cursor: pointer;' " + (inlarge == null ? "data-toggle='modal' data-target='#dvPopupBulkOperation' onclick=\"OpenPopUpGraph('PopupBulkOperation','dvPopupBulkOperation','" + Url.Action("ShowGraph", "T_DepartmentArea", new { type = "1", inlarge = 1 }) + "')\"" : "") + "/></div>";
                        string encoded = Convert.ToBase64String(bytes.ToArray());
                        result += String.Format(img, encoded);
                    }
                    else
        ...
        public string GetDisplayValue(string id)
        {
            ...
            var _Obj = db1.T_DepartmentAreas.FirstOrDefault(p => p.Id == idvalue);
		...
		var t_departmentarea = jedb.T_DepartmentAreas.Find(id);
			var T_UnitXDepartmentAreaIds = jedb.T_UnitXs.Where(p=>p.T_UnitXDepartmentAreaID == id).Select(p => p.Id);
		    predicateJournalEntry = predicateJournalEntry.Or(d => (d.EntityName == "T_UnitX" && T_UnitXDepartmentAreaIds.Contains(d.RecordId) && d.Type != "View"));
		...
		public object GetRecordById(string id)
        {
		            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            if (string.IsNullOrEmpty(id)) return "";
            var _Obj = db1.T_DepartmentAreas.Find(Convert.ToInt64(id));
            return _Obj;
        }
		public string GetRecordById_Reflection(string id)
        {
			...
			if (string.IsNullOrEmpty(id)) return "";
			var _Obj = db1.T_DepartmentAreas.Find(Convert.ToInt64(id));
            return _Obj == null ? "" : EntityComparer.EnumeratePropertyValues<T_DepartmentArea>(_Obj, "T_DepartmentArea", new string[] { ""  }); ;
			        }
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetAllValueForFilter(string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
		            IQueryable<T_DepartmentArea> list = db.T_DepartmentAreas;
		...
		public JsonResult GetAllValue(string caller, string key, string AssoNameWithParent, string AssociationID, string ExtraVal)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                {
                    IQueryable query = db.T_DepartmentAreas;
					string lambda = "";
                   ...
                   // ParameterExpression p = Expression.Parameter(typeof(T_DepartmentArea), "p");
                   // MemberExpression member = Expression.PropertyOrField(p, propToWhere);
                   // LambdaExpression lambda = Expression.Lambda<Func<T_DepartmentArea, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(AssoID), member.Type)), p);
                   // MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
                   // IQueryable q = query.Provider.CreateQuery(methodCall);
                   //list = ((IQueryable<T_DepartmentArea>)q);
				   list = ((IQueryable<T_DepartmentArea>)query);
                }
				else if (AssoID == 0)
                {
                   IQueryable query = db.T_DepartmentAreas;
			...
			FilterApplicationDropdowns _fad = new FilterApplicationDropdowns();
            list = _fad.FilterDropdown<T_DepartmentArea>(User,list, "T_DepartmentArea",caller);
...
[AcceptVerbs(HttpVerbs.Get)]
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
				try
                {
					...
					if (AssoID != null && AssoID > 0)
					{
						...
						MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
						IQueryable q = query.Provider.CreateQuery(methodCall);
						list = ((IQueryable<T_DepartmentArea>)q).Take(20);
					}
				}
				catch
                {
        ...
        public JsonResult GetAllMultiSelectValueForBR(string propNameBR)
        {
            ...
            if (!string.IsNullOrEmpty(propNameBR))
            {
				...
				//
                ParameterExpression param = Expression.Parameter(typeof(T_DepartmentArea), "d");
                //bulid expression tree:data.Field1
                var Property = typeof(T_DepartmentArea).GetProperty(propNameBR);
                ...
                if (targetType.GetGenericArguments().Count() > 0)
                {
                    if (targetType.GetGenericArguments()[0].Name == "DateTime")
                    {
                        ...
                        new Type[] { typeof(T_DepartmentArea), typeof(DateTime?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_DepartmentArea), typeof(decimal?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_DepartmentArea), typeof(bool?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_DepartmentArea), typeof(Int32?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_DepartmentArea), typeof(Int64?) }, Expression.Constant(list.AsQueryable()), pred);
                    ...
                    {
                        Expression expr = Expression.Call(typeof(Queryable), "Select",
                        new Type[] { typeof(T_DepartmentArea), typeof(double?) }, Expression.Constant(list.AsQueryable()), pred);
                ...
                {
                    Expression expr = Expression.Call(typeof(Queryable), "Select",
                            new Type[] { typeof(T_DepartmentArea), typeof(string) }, Expression.Constant(list.AsQueryable()), pred);
        ...
        public JsonResult GetAllMultiSelectValue(string key, string AssoNameWithParent, string AssociationID)
        {
            ...
            if (AssoNameWithParent != null && !string.IsNullOrEmpty(AssociationID))
            {
                ...
                paramList.Add(Expression.Parameter(typeof(T_DepartmentArea), "p"));
        ...
        public ActionResult Upload()
        {
			...
			if (!((CustomPrincipal)User).CanUseVerb("ImportExcel", "T_DepartmentArea", User) || !User.CanAdd("T_DepartmentArea"))
            {
                return RedirectToAction("Index", "Error");
            }
            //ViewBag.IsMapping = (db.ImportConfigurations.Where(p => p.Name == "T_DepartmentArea")).Count() > 0 ? true : false;
            var lstMappings = db.ImportConfigurations.Where(p => p.Name == "T_DepartmentArea").ToList();
         ...
         public ActionResult Upload([Bind(Include = "FileUpload")] HttpPostedFileBase FileUpload, FormCollection collection)
        {
            if (FileUpload != null)
            {
                ...
                if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv" || fileExtension == ".all")
                {
                    ...
                    var entList = GeneratorBase.MVC.ModelReflector.Entities.FirstOrDefault(e => e.Name == "T_DepartmentArea");
                    if (entList != null)
                    {
					...
					if (!string.IsNullOrEmpty(collection["ListOfMappings"]))
                    {
                        ...
                        {
                            var lstMapping = db.ImportConfigurations.Where(p => p.Name == "T_DepartmentArea" && p.IsDefaultMapping);
                        ...
                        DetailMessage = "We have received " + objDataSet.Tables[0].Rows.Count + " new Department Areas";
                        ...
                        if (entList != null)
                        {
                            ...
                            foreach (var association in entList.Associations)
                            {
                                ...
                                for (int i = 0; i < uniqueCols.Rows.Count; i++)
                                {
                                    ...
                                    {
                                										 case "T_DepartmentAreaEmployeeTypeAssociationID":
										 var t_departmentareaemployeetypeassociationId = db.T_Facilitys.FirstOrDefault(p => p.DisplayValue == assovalue);
										 if (t_departmentareaemployeetypeassociationId == null)
        ...
        public ActionResult ConfirmImportData(FormCollection collection)
        {
            ...
            string excelConnectionString = string.Empty;
			string typename = "T_DepartmentArea";
                ...
                if (entList != null)
                {
                    ...
                    foreach (var association in entList.Associations)
                    {
                        ...
                        for (int i = 0; i < uniqueCols.Rows.Count; i++)
                        {
                            ...
                            {
                                										 case "T_DepartmentAreaEmployeeTypeAssociationID":
										 var strPropertyT_DepartmentAreaEmployeeTypeAssociation = lstEntityProp.FirstOrDefault(p => p.Key == "T_DepartmentAreaEmployeeTypeAssociationID").Value;
										 ModelReflector.Property propT_DepartmentAreaEmployeeTypeAssociation = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_Facility").Properties.FirstOrDefault(p => p.DisplayName == strPropertyT_DepartmentAreaEmployeeTypeAssociation);
										 //var elementTypeT_DepartmentAreaEmployeeTypeAssociation = db.T_Facilitys.ElementType;
										 //var propertyInfoT_DepartmentAreaEmployeeTypeAssociation = elementTypeT_DepartmentAreaEmployeeTypeAssociation.GetProperty(propT_DepartmentAreaEmployeeTypeAssociation.Name);
										 // var t_departmentareaemployeetypeassociationId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_DepartmentAreaEmployeeTypeAssociation.GetValue(p, null)) == assovalue);
										 var t_departmentareaemployeetypeassociationId = db.T_Facilitys.Where(propT_DepartmentAreaEmployeeTypeAssociation.Name+"=\""+assovalue+"\"").FirstOrDefault();
										 if (t_departmentareaemployeetypeassociationId == null)
											uniqueassoValues.Add(assovalue);
										 break;
								                                default:
            ...
            if (fileExtension == ".xls" || fileExtension == ".xlsx" || fileExtension == ".csv")
            {
                ...
                if (selectedlist != null && columnlist != null)
                {
                    for (int i = 0; i < objDataSet.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (AreAllColumnsEmpty(objDataSet.Tables[0].Rows[i], sheetColumns))
                            continue;
                        T_DepartmentArea model = new T_DepartmentArea();
	...
	break;
					 case "T_DepartmentAreaEmployeeTypeAssociationID":
		 dynamic t_departmentareaemployeetypeassociationId = null;
		 if(lstEntityProp.Count > 0)
		 {
			 ...
			 //var elementTypeT_DepartmentAreaEmployeeTypeAssociation = db.T_Facilitys.ElementType;
			 //var propertyInfoT_DepartmentAreaEmployeeTypeAssociation = elementTypeT_DepartmentAreaEmployeeTypeAssociation.GetProperty(propT_DepartmentAreaEmployeeTypeAssociation.Name);			 
			 //t_departmentareaemployeetypeassociationId = db.T_Facilitys.AsEnumerable().FirstOrDefault(p => Convert.ToString(propertyInfoT_DepartmentAreaEmployeeTypeAssociation.GetValue(p, null)) == columnValue);
			 t_departmentareaemployeetypeassociationId = db.T_Facilitys.Where(propT_DepartmentAreaEmployeeTypeAssociation.Name + "=\"" + columnValue + "\"").FirstOrDefault();
		 }
		 else
			t_departmentareaemployeetypeassociationId = db.T_Facilitys.FirstOrDefault(p=>p.DisplayValue == columnValue);
		 if (t_departmentareaemployeetypeassociationId != null)
			model.T_DepartmentAreaEmployeeTypeAssociationID = t_departmentareaemployeetypeassociationId.Id;
		  else
			{
				if ((collection["T_DepartmentAreaEmployeeTypeAssociationID"].ToString() == "true,false"))
				{
					try
					{
						...
						db.SaveChanges();
						model.T_DepartmentAreaEmployeeTypeAssociationID = objT_Facility.Id;
					}
					catch
					{
						continue;
                ...
                break;
					    ...
					    if (ValidateModel(model))
                        {
                            ...
                            if (result == null || result.Count == 0)
                            {
								...
								{
									db.T_DepartmentAreas.Add(model);
									db.SaveChanges();
								}
								error += customerror;
							}
		...
		public bool ValidateModel(T_DepartmentArea validate)
        {
		...
		public object GetFieldValueByEntityId(long Id, string PropName)
        {
            ...
            {
			                ApplicationContext db1 = new ApplicationContext(new SystemUser());
                var obj1 = db1.T_DepartmentAreas.Find(Id);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetReadOnlyProperties(T_DepartmentArea OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            if (User.businessrules != null)
            {
                var BR = User.businessrules.Where(p => p.EntityName == "T_DepartmentArea").ToList();
                var BRAll = BR;
                if (BR != null && BR.Count > 0)
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ReadOnlyPropertiesRule(OModel, BR, "T_DepartmentArea");
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetMandatoryProperties(T_DepartmentArea OModel, string ruleType)
        {
            ...
            if (User.businessrules != null)
            {
                ...
                if (BR != null && BR.Count > 0)
                {
					...
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.MandatoryPropertiesRule(OModel, BR, "T_DepartmentArea");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetUIAlertBusinessRules(T_DepartmentArea OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.UIAlertRule(OModel, BR, "T_DepartmentArea");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetValidateBeforeSaveProperties(T_DepartmentArea OModel, string ruleType)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            var conditions = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.ValidateBeforeSavePropertiesRule(OModel, BR, "T_DepartmentArea",state);
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public JsonResult GetLockBusinessRules(T_DepartmentArea OModel)
        {
            Dictionary<string, string> RulesApplied = new Dictionary<string, string>();
            //string RulesApplied = "";
            if (User.businessrules != null)
            {
                ...
                {
					OModel.setCalculation();
                    var ResultOfBusinessRules = db.LockEntityRule(OModel, BR, "T_DepartmentArea");
                    BR = BR.Where(p => ResultOfBusinessRules.Values.Select(x => x.BRID).ToList().Contains(p.Id)).ToList();
		...
		private List<string> CheckMandatoryProperties(T_DepartmentArea OModel)
        {
                ...
                if (BR != null && BR.Count > 0)
                {
                    ...
                    if (ResultOfBusinessRules.Keys.Select(p=>p.TypeNo).Contains(2))
                    {
                        ...
                        foreach (string paramName in Args.Select(p => p.ParameterName))
                        {
                            ...
                            {
                                var dispName = ModelReflector.Entities.FirstOrDefault(p => p.Name == "T_DepartmentArea").Properties.FirstOrDefault(p => p.Name == paramName).DisplayName;
                                result.Add(dispName);
                            }
                        }
                    }
		...
		public long? GetIdFromDisplayValue(string displayvalue)
        {
            ...
            var _Obj = db1.T_DepartmentAreas.FirstOrDefault(p => p.DisplayValue == displayvalue);
            long outValue;
            if (_Obj != null)
        ...
        {
            ApplicationContext db1 = new ApplicationContext(new SystemUser());
            IQueryable query = db1.T_DepartmentAreas;
            Type[] exprArgTypes = { query.ElementType };
            ...
            lambda = Expression.Lambda<Func<T_DepartmentArea, bool>>(Expression.Equal(member, Expression.Convert(Expression.Constant(PropValue), member.Type)), p);
            else
                lambda = Expression.Lambda<Func<T_DepartmentArea, bool>>(Expression.Equal(member, Expression.Constant(null, member.Type)), p);
            MethodCallExpression methodCall = Expression.Call(typeof(Queryable), "Where", exprArgTypes, query.Expression, lambda);
            IQueryable q = query.Provider.CreateQuery(methodCall);
            long outValue;
            var list1 = ((IQueryable<T_DepartmentArea>)q);
		...
		[AcceptVerbs(HttpVerbs.Get)]
        public JsonResult Check1MThresholdValue(T_DepartmentArea t_departmentarea)
        {
        ...
        {
            string[][] groupsarr = new string[][] { new string[] {"48907","T_DepartmentArea48907"} };
            return groupsarr;
        }
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetCalculationValues(T_DepartmentArea t_departmentarea)
        {
            t_departmentarea.setCalculation();
            Dictionary<string, string> Calculations = new Dictionary<string, string>();
            System.Reflection.PropertyInfo[] properties = (t_departmentarea.GetType()).GetProperties().Where(p => p.PropertyType.FullName.StartsWith("System")).ToArray();
        ...
        public void GetTemplatesForList(string defaultview)
        {
            ...
            var lstDefaultEntityPage = from s in db.DefaultEntityPages
                                       where s.EntityName == "T_DepartmentArea"
                                       select s;
	...
	[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetails(T_DepartmentArea t_departmentarea, string IgnoreEditable)
        {
		...
		[HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetDerivedDetailsInline(string host, string value, T_DepartmentArea t_departmentarea, string IgnoreEditable)
        {
            Dictionary<string, string> derivedlist = new Dictionary<string, string>();
            return Json(derivedlist, "application/json", System.Text.Encoding.UTF8, JsonRequestBehavior.AllowGet);
        }
