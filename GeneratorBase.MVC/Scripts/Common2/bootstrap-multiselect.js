...
!function ($) {
     ...
     * @returns {Multiselect}
     */
    function Multiselect(select, options, host) {
    ...
    Multiselect.prototype = {
        defaults: {
            ...
            onDropdownShow: function (event) {
                if (this.$select.find('option').length == 0 && this.$select.attr("HostingName") != undefined) {
                    ...
                    $.ajax({
                        type: "GET",
                        url: $('#' + hostingid).attr("dataurl") + "?" + associationParam,
        ...
        dataproviderOnShowDropDown: function (dataprovider, url) {
            var urlGetAll = $('#' + url).attr("dataurl").replace("GetAllMultiSelectValue", "Index") + "?BulkOperation=multiple";
            var optionDOM = "";
            for (var i = 0; i < dataprovider.length; i++) {
                optionDOM += '<option value="' + dataprovider[i].Id + '">' + dataprovider[i].Name + '</option>';
            }
            //dataprovider.forEach(function (option) {
            //    optionDOM += '<option value="' + option.Id + '">' + option.Name + '</option>';
            //});
            this.$select.html(optionDOM);
            //var limore = "<li class='disabled-result disabled-result' style='font-style:Italic;' >Search for more...</li>";
            this.rebuild();
            if ($(optionDOM).length >= 10) {
                //  this.$ul.append($(limore));
        ...
        //function call on enter value in searchable text box
        dataproviderOnFilter: function (dataprovider, obj) {
            var urlGetAll = obj.attr("dataurl").replace("GetAllMultiSelectValue", "Index") + "?BulkOperation=multiple";
            var optionDOM = "";
            for (var i = 0; i < dataprovider.length; i++) {
                if (this.$select.find("option[value='" + dataprovider[i].Id + "']").length == 0)
                    optionDOM += '<option value="' + dataprovider[i].Id + '">' + dataprovider[i].Name + '</option>';
            }
