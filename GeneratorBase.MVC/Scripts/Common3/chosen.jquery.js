    ...
    AbstractChosen = (function () {
            ...
            if (_ref.length >= 10) {
                ...
                if (parentDDid != null || parentDDid != undefined) {
                    ...
                    for (var i = 0; i < Parents.length; i++) {
                        if ($("option:selected", $("select#" + Parents[i])).val().length > 0) {
        ...
        AbstractChosen.prototype.container_width = function () {
            if (this.options.width != null) {
                ...
                return "" + (this.form_field.offsetWidth == 0 ? "190" : this.form_field.offsetWidth) + "px";
            }
        };
...
function select(hostingentity, searchtxt) {
    ...
    if (searchtxt.length > 0)
        finalUrl = addParameterToURL(finalUrl, 'key', searchtxt);
    if (selectedval.length > 0)
        finalUrl = addParameterToURL(finalUrl, 'ExtraVal', selectedval);
    $.ajax({
        type: "GET",
        url: finalUrl,
        ...
        success: function (jsonObj) {
            var stuffnone = {
                ...
                disabled: false,
                group_array_index: undefined,
                classes: selectedval == '' ? "result-selected highlighted" : "",
                style: ""
            };
            stufflist.push(stuffnone);
            var listItems = "";
            $("#" + hostingentity).empty();
            if (selectedval != '' || IsFillWithFirst)
            ...
            for (i in jsonObj) {
                if (jsonObj[i].Id != undefined && jsonObj[i].Name != undefined) {
                    ...
                    var stuff = {
                        ...
                        text: jsonObj[i].Name,
                        html: jsonObj[i].Name,
                        selected: (selectedval == jsonObj[i].Id || IsFillWithFirst),
                        disabled: false,
                        group_array_index: undefined,
...
$(document).ready(function () {
    ...
    if ($inptextarea.length > 0) {
        $inptextarea.focus(function () {
            var txtareastyle = "padding:2px 5px !important;font-size: 12px !important;z-index: 1000; height:90px";
